...
public class AdminUserModules extends HttpServlet
{
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Admin - Rights</title>");

    scoutln(out, bytesOut, "<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"" + directoryUtils.getCssGeneralDirectory(con, stmt, rs, unm, dnm, defnsDir) + "general.css\">");

    scoutln(out, bytesOut, "<script language=\"JavaScript\">");

    scoutln(out, bytesOut, "function same(){var p2=document.forms[0].users.value;");
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/AdminSetUser?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=" + userCode + "&p2=\"+p2+\"&bnm=" + bnm + "\";}");

    scoutln(out, bytesOut, "</script>");

    signOnAdministrator.heading(out, true, unm, sid, uty, bnm, dnm, men, den, imagesDir, bytesOut);

    dashboardUtils.drawTitle(out, "Administration - User Rights for: " + userCode, "7008", unm, sid, uty, men, den, dnm, bnm, bytesOut);
    ...
    userDDL(con, stmt, rs, out, userCode, dnm, bytesOut);
  ...
  // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void userDDL(Connection con, Statement stmt, ResultSet rs, PrintWriter out, String thisUserCode, String dnm, int[] bytesOut) throws Exception
  {
