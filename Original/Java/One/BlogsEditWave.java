// =======================================================================================================================================================================================================
// System: ZaraStar Blog: Edit page - HTML
// Module: BlogsEdit.java
// Author: C.K.Harvey
...
public class BlogsEditWave extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "BlogsEdit", bytesOut);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String p1, String p2, String p3, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreenW(out, req, 13, unm, sid, uty, men, den, dnm, bnm, "BlogsEdit", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreenW(out, req, 12, unm, sid, uty, men, den, dnm, bnm, "BlogsEdit", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    scoutln(out, bytesOut, "8102\001Blogs\001" + p1 + "\001javascript:getHTML('BlogsEditWave','" + p1 + "')\001\001\001\001\003");

    set(con, stmt, rs, out, req, p1, p2, p3, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void set(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String p1, String p2, String p3, String unm, String sid, String uty,  String men, String den, String dnm, String bnm,
                   String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(p1.length() == 0) // new page
      blogsUtils.drawTitleW(con, stmt, rs, req, out, "Create Blog Entry", "8102", p1, false, true, false, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
    else blogsUtils.drawTitleW(con, stmt, rs, req, out, "Edit Blog Entry", "8102", p1, false, true, false, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
    ...
    if(p1.length() == 0) // new page
    {
      ...
      code[0] = text[0] = image[0] = topicName[0] = shortTitle[0] = "";
      serviceCode[0] = p3;
      type[0] = "1";
      title[0] = p2;
      published[0] = "Y";
    ...
    scoutln(out, bytesOut, "<input type=hidden name=callingServlet value=\"BlogsEditWave\">");
    ...
    if(p1.length() == 0) // new page, no need to fetch anything
    {
      scoutln(out, bytesOut, "<input type=hidden name=typeForEditor value=\"C\">");
      scoutln(out, bytesOut, "<input type=hidden name=codeForEditor value=\"\">");
    }
    else
    {
      scoutln(out, bytesOut, "<input type=hidden name=typeForEditor value=\"B\">");
      scoutln(out, bytesOut, "<input type=hidden name=codeForEditor value=\"" + code[0] + "\">");
    }
    ...
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td><p>Title:</p></td><td colspan=3><p><input type=text size=40 maxlength=100 name=title value=\"" + title[0] + "\">");
    ...
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scout(out, bytesOut, "<tr><td><p>Type:</p></td><td colspan=3>");
    buildTypeList(out, type[0], bytesOut);
    scoutln(out, bytesOut, "</td></tr>");

    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td><p>Image:</p></td><td colspan=3><p><input type=text size=60 maxlength=100 name=image value=\"" + image[0] + "\">");
    scoutln(out, bytesOut, "&nbsp;&nbsp;&nbsp;<a href=\"javascript:lib()\">Image Library</a>");
    scoutln(out, bytesOut, "</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td colspan=4><textarea id=\"text1\" name=\"text1\" cols=\"100\" rows=\"20\">" + text[0] + "</textarea>");
    scoutln(out, bytesOut, "<script language=\"JavaScript\">drawEditor('text1', 'wikiview', '100%', '400', '#FF8888', '#F0F0F0', '#880000','" + unm + "','" + sid + "','" + uty + "','" + men + "','" + den + "','" + dnm + "','"
                         + bnm + "');</script>");
  ...
  private void buildTypeList(PrintWriter out, String currentType, int[] bytesOut) throws Exception
  {
    ...
    scout(out, bytesOut, "<option value='1'");
    if(currentType.equals("1"))
      scout(out, bytesOut, " selected");
    scoutln(out, bytesOut, ">No Image");

    scout(out, bytesOut, "<option value='2'");
    if(currentType.equals("2"))
      scout(out, bytesOut, " selected");
    scoutln(out, bytesOut, ">Image Top-Left");

    scout(out, bytesOut, "<option value='3'");
    if(currentType.equals("3"))
      scout(out, bytesOut, " selected");
    scoutln(out, bytesOut, ">Image Top-Right");
