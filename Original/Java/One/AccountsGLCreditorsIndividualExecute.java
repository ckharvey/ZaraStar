// =======================================================================================================================================================================================================
// System: ZaraStar: Accounts: GL Creditors (Individual)
// Module: AccountsGLCreditorsIndividualExecute.java
// Author: C.K.Harvey
...
public class AccountsGLCreditorsIndividualExecute extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6030a: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsGLCreditorsIndividualExecute", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6030, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, String p3, String p4, String p5, String p6, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6030, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsGLCreditorsIndividualExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6030, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsGLCreditorsIndividualExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6030, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    if(p5.length() == 0)
      p5 = supplier.getSupplierNameGivenCode(con, stmt, rs, p1);

    if(p6.length() == 0)
      p6 = supplier.getSupplierCurrencyGivenCode(con, stmt, rs, p1);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6030, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
                   ...
                   String sid, String uty, String men, String den, String dnm, String bnm, String workingDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 5049, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/PaymentPage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 5080, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/PurchaseInvoicePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
                           + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p3=&p4=&p1=\"+p1;}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 5036, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/PurchaseDebitNotePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
                           + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p3=&p4=&p1=\"+p1;}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 5026, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/PurchaseCreditNotePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
                           + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p3=&p4=&p1=\"+p1;}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 6023, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/AccountsBatchEntries?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6030", "", true, plain, companyCode, dateFrom, dateTo, companyName, currency, "AccountsGLCreditorsIndividualExecute", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    String year = generalUtils.getFromDefnFile(unm, "accounts.dfn", workingDir, "");

    if(! plain)
    {
      pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "GL Account: " + companyCode + " - " + companyName + " (" + currency + ")", "6030", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    }
    else
    {
    ...
    getOutstandingForASupplier(out, companyCode, dateTo, unm, sid, uty, men, den, dnm, bnm, bytesOut);
  ...
  {
    onPurchaseInvoice(con, stmt, stmt2, rs, companyCode, dateFrom, dateTo, tmpTable);
    onPurchaseDebitNote(con, stmt, stmt2, rs, companyCode, dateFrom, dateTo, tmpTable);
    onPurchaseCreditNote(con, stmt, stmt2, rs, companyCode, dateFrom, dateTo, tmpTable);
    onPayment(con, stmt, stmt2, rs, companyCode, dateFrom, dateTo, tmpTable);
    forPaymentExchangeGainLoss(con, stmt, rs, companyCode, dateFrom, dateTo, baseCurrency, tmpTable);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPurchaseInvoice(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String companyCode, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT InvoiceCode, CompanyName, TotalTotal, Date, Currency, BaseTotalTotal FROM pinvoice WHERE CompanyCode = '" + companyCode + "' AND Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo
                         + "'} AND Settled != 'Y'");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPurchaseDebitNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String companyCode, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT PDNCode, CompanyName, TotalTotal, Date, Currency, BaseTotalTotal FROM pdebit WHERE CompanyCode = '" + companyCode + "' AND Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPurchaseCreditNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String companyCode, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT PCNCode, CompanyName, TotalTotal, Date, Currency, BaseTotalTotal FROM pcredit WHERE CompanyCode = '" + companyCode + "' AND Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    double amount, baseAmount;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPayment(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String companyCode, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT PaymentCode, CompanyName, TotalAmount, DatePaid, Currency, BaseTotalAmount FROM payment WHERE CompanyCode = '" + companyCode + "' AND Status != 'C' AND DatePaid >= {d '" + dateFrom + "'} AND DatePaid <= {d '"
                         + dateTo + "'}");
    double amount, baseAmount;
      ...
      if(! paymentCode.equals(lastPaymentCode))
      {
        if(! first)
        {
          if(runningDiff > 0) // exch loss
            addToTmpTable(con, stmt2, "L", lastPaymentCode, companyName, 0, runningDiff, datePaid, baseCurrency, tmpTable);

          if(runningDiff < 0) // exch gain
            addToTmpTable(con, stmt2, "G", lastPaymentCode, companyName, 0, (runningDiff * -1), datePaid, baseCurrency, tmpTable);
  ...
  private void onBatchCr(Connection con, Statement stmt2, String year, String companyCode, String dateFrom, String dateTo, String tmpTable, String dnm) throws Exception
  {
    ...
    try
    {
      ...
      accRs = accStmt.executeQuery("SELECT t2.Code, t2.Remark, t2.Amount, t2.TransactionDate, t2.Currency, t2.BaseAmount FROM joubatchl AS t2 INNER JOIN joubatch AS t1 ON t1.Code = t2.Code WHERE t1.Type = 'C' AND t2.AccCode = '" + companyCode
                                 + "' AND t2.DrCr = 'C' AND t2.TransactionDate >= {d '" + dateFrom + "'} AND t2.TransactionDate <= {d '" + dateTo + "'}");
      double amount, baseAmount;
  ...
  private void onBatchDr(Connection con, Statement stmt2, String year, String companyCode, String dateFrom, String dateTo, String tmpTable, String dnm) throws Exception
  {
    ...
    try
    {
      ...
      accRs = accStmt.executeQuery("SELECT t2.Code, t2.Remark, t2.Amount, t2.TransactionDate, t2.Currency, t2.BaseAmount "
                                 + "FROM joubatchl AS t2 INNER JOIN joubatch AS t1 ON t1.Code = t2.Code WHERE t1.Type = 'C' AND t2.AccCode = '"
  ...
  private void processTmpTable(Connection con, Statement stmt, ResultSet rs, String tmpTable, PrintWriter out, String accountCurrency, String baseCurrency, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    while(rs.next())
    {
      ...
      if(type.charAt(0) == 'R')
      {
        scout(out, bytesOut, "viewPayment('");
        type = "Payment";
        drCr = "Dr";
        balance -= amount;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void getOutstandingForASupplier(PrintWriter out, String customerCode, String dateTo, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    URL url = new URL("http://" + men + "/central/servlet/SupplierPeriods?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=" + customerCode + "&p2=" + dateTo + "&bnm=" + bnm);
    ...
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td colspan=4><p><b>Total from creditors ageing: " + currency + " " + generalUtils.doubleDPs(total, '2') + "</p></td></tr>"); 
  }
