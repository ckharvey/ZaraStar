// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: GR create listing page
// Module: GoodsReceivedListing.java
// Author: C.K.Harvey
...
public class GoodsReceivedListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "GoodsReceivedListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3036, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    String firstRecNum, String lastRecNum, int maxRows, int numRecs, String companyCode, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "GoodsReceivedListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3036, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "GoodsReceivedListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3036, bytesOut[0], 0, "SID:" + codeArg);
      if(con != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3036, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
    if(con != null) con.close();
    if(out != null) out.flush(); 
  }
  ...
  private void createPage(String[] html, char operation, char callType, String arg, String arg2, char inx, String srchStr, int[] numRecs,
                          ...
                          String lastRecNum, String[] newFirstRecNum, String[] newLastRecNum, int maxRows, String companyCode, String imagesDir,
                          String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM gr WHERE date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM gr" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM gr WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM gr WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM gr" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM gr WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT GRCode) AS rowcount FROM gr WHERE GRCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM gr" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String grCode="", name="", supplierCode="", date="", stockProcessed="", status;
    int grCodePosn, datePosn, companyCodePosn, companyNamePosn, stockProcessedPosn, statusPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT GRCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE GRCode LIKE '" + srchStr + "%'"
                       + companyCodeANDWHERE + " ORDER BY GRCode";
                   }
                   else
                   {
                     q = "SELECT GRCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr " + companyCodeWHERE + "ORDER BY GRCode";
                   }

                   grCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; stockProcessedPosn = 6;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, GRCode, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     grCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; stockProcessedPosn = 6;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM gr ORDER BY Date" + companyCodeWHERE;
                     grCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;  stockProcessedPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, GRCode, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE Date = {d '"
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, GRCode, Date, CompanyName, Status, StockProcessed FROM gr WHERE CompanyCode LIKE '" + srchStr
                       + "%' ORDER BY CompanyCode, GRCode";
                     
                     grCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 4; statusPosn = 5;  stockProcessedPosn = 6;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM gr " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     grCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; stockProcessedPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, GRCode, Date, CompanyName, Status, StockProcessed FROM gr WHERE CompanyCode = '" + codeArg
                 ...
                 while(rs.next())                  
                 {
                   grCode         = getValue(grCodePosn,         ' ', rs, rsmd);
                   ...
                   stockProcessed = getValue(stockProcessedPosn, ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, grCode, date, supplierCode, name, stockProcessed, status, grCode, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, grCode, supplierCode, name, stockProcessed, status, grCode, imagesDir);
                   }   
                   else // inx == '3'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, supplierCode);
                     else appendBodyLine(html, inx, supplierCode, grCode, date, name, stockProcessed, status, grCode, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = grCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = grCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = grCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = grCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT GRCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE GRCode LIKE '" + srchStr 
                       + "%' AND GRCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY GRCode DESC";
                   }
                   else
                   {  
                     q = "SELECT GRCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE GRCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                         + companyCodeANDWHERE + " ORDER BY GRCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM gr WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, GRCode, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, GRCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM gr WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                     
                     grCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; stockProcessedPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, GRCode, Date, CompanyName, Status, StockProcessed FROM gr WHERE CompanyCode = '"
                       + codeArg + "' ORDER BY CompanyCode, GRCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, grCode, date, supplierCode, name, stockProcessed, status, grCode, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, grCode, supplierCode, name, stockProcessed, status, grCode, imagesDir);
                   }  
                   else // inx == '3'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, supplierCode);
                     else prependBodyLine(html, inx, supplierCode, grCode, date, name, stockProcessed, status, grCode, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT grCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE GRCode LIKE '" + srchStr 
                       + "%' AND GRCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY GRCode";
                     
                     grCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; stockProcessedPosn = 6;
                   }
                   else
                   {                   
                     q = "SELECT GRCode, Date, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE GRCode > '" + codeArg + "'"
                       + companyCodeANDWHERE + " ORDER BY GRCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM gr WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, GRCode, CompanyCode, CompanyName, Status, StockProcessed FROM gr WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND GRCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, GRCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM gr WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, GRCode, Date, CompanyName, Status, StockProcessed FROM gr WHERE CompanyCode = '" + codeArg
                       + "' AND GRCode > '" + codeArg2 + "' ORDER BY CompanyCode, GRCode";
      ...
      case 'P' : // prev page
                 ...
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM gr WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, GRCode, Date, CompanyName, Status, StockProcessed FROM gr WHERE CompanyCode = '" + codeArg
                       + "' AND GRCode < '" + codeArg2 + "' ORDER BY CompanyCode, GRCode DESC";
                      ...
                      throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Goods Received Note Search Results for ";
        switch(inx)
        {
          case '1' : title += "Goods Received Note Code";  break;
          case '2' : title += "Goods Received Note Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Goods Received Note Listing by ";
        switch(inx)
        {
          case '1' : title += "Goods Received Note Code";  break;
          case '2' : title += "Goods Received Note Date";  break;
      ...
      else title = "Goods Received Note Listing";
 
      drawingUtils.drawTitle(out, false, false, "GoodsReceivedListing", "", title, "3036", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Goods Received Note Code");  break;
        case '2' : scout(out, bytesOut, "Goods Received Note Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty, String men, String den,
                       String dnm, String bnm, String newFirstRecNum, String newLastRecNum, int numRecs, String companyCode, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/GoodsReceivedPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/GoodsReceivedListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/GoodsReceivedListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "3036", "", "GoodsReceivedListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>GRNote Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>GRNote Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>GRNote Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>GRNote Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Goods Received Note Date");  break;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String stockProcessed, String status,
                              String code, String imagesDir) throws Exception
  {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String stockProcessed, String status,
                               String code, String imagesDir) throws Exception
  {
    String s = checkCode(code);
