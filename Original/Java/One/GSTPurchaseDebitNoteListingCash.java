// =======================================================================================================================================================================================================
// System: ZaraStar Document: GST: Purchase DNs listing (cash)
// Module: GSTPurchaseDebitNoteListingCash.java
// Author: C.K.Harvey
...
public class GSTPurchaseDebitNoteListingCash extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "GSTPurchaseDebitNoteListingCash", bytesOut);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "GSTPurchaseDebitNoteListingCash", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
                        ...
                        String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String dnCode, date, companyCode, baseGSTTotal, baseTotalTotal, totalTotal, gstTotal, currency;
     
    rs = stmt.executeQuery("SELECT DISTINCT t1.DNCode, t1.Date, t1.CompanyCode, t1.BaseGSTTotal, t1.BaseTotalTotal, "
                                   + " t1.TotalTotal, t1.GSTTotal, t1.Currency FROM pdebitl AS t2 INNER JOIN pdebit AS t1 "
                                   + "ON t2.PDNCode = t1.PDNCode WHERE t1.Date >= {d '" + dateFrom
                                   + "'} AND t1.Date <= {d '" + dateTo + "'} AND t1.Status != 'C' AND t1.CashOrAccount = 'C' AND t2.GSTRate = '" + gstRate
                                   + "'");
    ...
    while(rs.next())                  
    {
      ...
      appendBodyLine(out, dnCode, date, companyCode, baseGSTTotal, baseTotalTotal, totalTotal, gstTotal, currency, cssFormat, bytesOut);
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, boolean plain, String dateFrom, String dateTo, String gstRate, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String localDefnsDir,
                       String defnsDir,
                       int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>GST Purchase Debit Note Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/PurchaseDebitNotePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p1=\"+code2;}");
    ...
    outputPageFrame(con, stmt, rs, out, req, plain, dateFrom, dateTo, gstRate, "", "GST Reconciliation: Purchase Debit Note Listing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "<tr id=\"pageColumn\"><td nowrap><p>Debit Note Code &nbsp;</td>");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(PrintWriter out, String dnCode, String date, String companyCode, String baseGSTTotal,
  ...
  private String buildSubMenuText(boolean plain, String dateFrom, String dateTo, String gstRate, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] hmenuCount) throws Exception
  {
    ...
    if(! plain)
    {
      ...
      s += "<a href=\"/central/servlet/GSTPurchaseDebitNoteListingCash?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + dateFrom + "&p2=" + dateTo + "&p3=" + generalUtils.sanitise(gstRate) + "&p4=P\">Friendly</a></dt></dl>";
    }
