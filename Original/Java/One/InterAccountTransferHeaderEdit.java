// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: IAT edit header
// Module: InterAccountTransferHeaderEdit.java
// Author: C.K.Harvey
...
public class InterAccountTransferHeaderEdit extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "InterAccountTransferHeaderEdit", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6079, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "6079", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6079, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6079", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6079, bytesOut[0], 0, "SID:" + p1);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    interAccountTransfer.getRecToHTML(con, stmt, stmt2, rs, rs2, out, 'E', codeB, unm, sid, uty, men, den, dnm, bnm, workingDir, localDefnsDir, defnsDir, p2.charAt(0), p3,
                       dataAlready, req, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6079, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
