// =======================================================================================================================================================================================================
// System: ZaraStar: Accounts: GL Recoverable Expenses
// Module: AccountsGLRecoverableExpenses.java
// Author: C.K.Harvey
...
public class AccountsGLRecoverableExpenses extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6007s: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsGLRecoverableExpenses", bytesOut);
                    ...
                    String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsGLRecoverableExpenses", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsGLRecoverableExpenses", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6007", "", true, plain, account, p2, p3, "", "", "AccountsGLRecoverableExpenses", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    String year = generalUtils.getFromDefnFile(unm, "accounts.dfn", workingDir, "");

    if(! plain)
    {
      pageFrameUtils.drawTitle(out, true, plain, "AccountsGLRecoverableExpenses", account, dateFrom, dateTo, "", "", "GL Account: " + account + " - " + accountsUtils.getAccountDescriptionGivenAccCode(account, year, dnm, localDefnsDir, defnsDir) + " ("
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  public void calc(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String account, String year, String dateFrom, String dateTo, String dnm, String localDefnsDir, String defnsDir, String tmpTable) throws Exception
  {    
    ...
    onRVoucher(con, stmt, stmt2, rs, account, year, dateFrom, dateTo, tmpTable);
    ...
    onPayment(con, stmt, stmt2, rs, rs2, account, year, dateFrom, dateTo, tmpTable, dnm, localDefnsDir, defnsDir); // for nonsense case
  }
 ...
 //     if(! itemExists(con, stmt2, rs2, itemCode))
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onVoucher(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String accCode, String year, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    stmt = con.createStatement();
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onRVoucher(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String accCode, String year, String dateFrom, String dateTo,
                          String tmpTable) throws Exception
  {
    stmt = con.createStatement();
                             ...
                             String dateTo, String tmpTable) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.InvoiceCode, t1.CompanyName, t2.Amount2, t2.Amount, t1.Date, t1.Currency, t1.GroupDiscount, "
                         ...
                         + "' AND t1.Status != 'C' AND t1.Date >= {d '" + dateFrom + "'} AND t1.Date <= {d '" + dateTo + "'}");
    double amt = 0.0, bAmt = 0.0, amount, baseAmount, groupDiscount = 1, totalTotal, lastGroupDiscount = 0;           
    String invoiceCode, lastCompanyName="", companyName="", date="", currency="", lastCurrency="", lastDate="", lastInvoiceCode = "",
           groupDiscountType;
    boolean first = true;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPayment(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String accCode, String year, String dateFrom, String dateTo, String tmpTable, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT PaymentCode, CompanyName, BankAmount, DatePaid, Currency, Charges, Rate, DiscountAllowed, AccCredited FROM payment WHERE AccCredited = '" + accCode + "' AND Status != 'C' AND DatePaid >= {d '" + dateFrom
