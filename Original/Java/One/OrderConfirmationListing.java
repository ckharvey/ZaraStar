// =======================================================================================================================================================================================================
// System: ZaraStar: DocumentEngine: OC create listing page
// Module: OrderConfirmationListing.java
// Author: C.K.Harvey
...
public class OrderConfirmationListing extends HttpServlet
{
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM oc WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM oc" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM oc WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM oc WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM oc" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM oc WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CustomerPOCode) AS rowcount FROM oc WHERE CustomerPOCode LIKE '"
                                           + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT CustomerPOCode) AS rowcount FROM oc"); 
                   else
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM oc WHERE CustomerPOCode = '" + codeArg + "'"
                                           + companyCodeANDWHERE);
                   }
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT OCCode) AS rowcount FROM oc WHERE OCCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM oc" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String ocCode="", name="", customerCode="", date="", customerPOCode="", status;
    int ocCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, customerPOCodePosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc WHERE OCCode LIKE '" + srchStr 
                       + "%'" + companyCodeANDWHERE + " ORDER BY OCCode";
                   }
                   else
                   {
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc" + companyCodeWHERE + " ORDER BY OCCode";
                   }

                   ocCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; statusPosn = 5; 
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";

                     ocCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; statusPosn = 5; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM oc" + companyCodeWHERE + " ORDER BY Date";
                     
                     ocCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; customerPOCodePosn = -1; 
                     statusPosn = -1; 
                   }
                   else
                   {
                     ...
                     ocCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; 
                     statusPosn = 5;
                   }
                 }  
                 else
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, OCCode, CompanyName, CustomerPOCode, Status FROM oc WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     ocCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; customerPOCodePosn = 4; 
                     statusPosn = 5;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM oc" + companyCodeWHERE + " ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, OCCode, CompanyName, CustomerPOCode, Status FROM oc WHERE CompanyCode = '" + codeArg
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT CustomerPOCode, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE CustomerPOCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, OCCode";
                   ...
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM oc" + companyCodeWHERE + " ORDER BY CustomerPOCode";
                     ocCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; customerPOCodePosn = 1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CustomerPOCode, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE CustomerPOCode = '" 
                 ...
                 while(rs.next())                  
                 {
                   ocCode         = getValue(ocCodePosn,         ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, ocCode, date, customerCode, name, ocCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, ocCode, customerCode, name, ocCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, ocCode, name, customerPOCode, ocCode, status, imagesDir);
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, customerPOCode);
                     else appendBodyLine(html, inx, customerPOCode, ocCode, customerCode, name, ocCode, status, imagesDir);
                   }
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = ocCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = ocCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = ocCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = ocCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc WHERE OCCode LIKE '" + srchStr 
                       + "%' AND OCCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY OCCode DESC";
                   }
                   else
                   {  
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc WHERE OCCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY OCCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM oc WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, OCCode, CompanyCode, CompanyName, Status FROM oc" 
                       + " WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE
                       + " ORDER BY Date, OCCode DESC";
                     ocCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; 
                     statusPosn = 5;
                   }  
                 }
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM oc WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, OCCode, CompanyName, CustomerPOCode, Status FROM oc WHERE CompanyCode = '" + codeArg 
                        + "' ORDER BY CompanyCode, OCCode DESC";
                     ocCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; customerPOCodePosn = 4; 
                     statusPosn = 5;
                   }
                 }
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM oc WHERE CustomerPOCode <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                   ...
                   {
                     q = "SELECT CustomerPOCode, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE CustomerPOCode = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, OCCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, ocCode, date, customerCode, name, ocCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, ocCode, customerCode, name, ocCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, ocCode, name, customerPOCode, ocCode, status, imagesDir);
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, customerPOCode);
                     else prependBodyLine(html, inx, customerPOCode, ocCode, customerCode, name, ocCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc WHERE OCCode LIKE '" + srchStr 
                       + "%' AND OCCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY OCCode";
                     
                     ocCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; 
                     statusPosn = 5;
                   }
                   else
                   {                   
                     q = "SELECT OCCode, Date, CompanyCode, CompanyName, Status FROM oc WHERE OCCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY OCCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM oc WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND OCCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, OCCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM oc WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, OCCode, CompanyName, CustomerPOCode, Status FROM oc WHERE CompanyCode = '" + codeArg
                       + "' AND OCCode > '" + codeArg2 + "' ORDER BY CompanyCode, OCCode";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CustomerPOCode FROM oc WHERE CustomerPOCode >= '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT CustomerPOCode, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE CustomerPOCode = '"
                       + codeArg + "' AND OCCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, OCCode";
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM oc WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, OCCode, CompanyName, CustomerPOCode, Status FROM oc WHERE CompanyCode = '" + codeArg
                       + "' AND OCCode < '" + codeArg2 + "' ORDER BY CompanyCode, OCCode DESC";
                 ...
                 else // inx == '4'
                 {
                   ...
                   {
                     q = "SELECT CustomerPOCode, OCCode, CompanyCode, CompanyName, Status FROM oc WHERE CustomerPOCode = '" 
                       + codeArg + "' AND OCCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY CustomerPOCode, OCCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Order Confirmation Search Results for ";
        switch(inx)
        {
          case '1' : title += "Order Confirmation Code";  break;
          case '2' : title += "Order Confirmation Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Order Confirmation Listing by ";
        switch(inx)
        {
          case '1' : title += "Order Confirmation Code";  break;
          case '2' : title += "Order Confirmation Date";  break;
      ...
      else title = "Order Confirmation Listing";

      drawingUtils.drawTitle(out, false, false, "OrderConfirmationListing", "", title, "4053", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Order Confirmation Code");  break;
        case '2' : scout(out, bytesOut, "Order Confirmation Date");  break;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty, String men,
                       String den, String dnm, String bnm, String newFirstRecNum, String newLastRecNum, int numRecs,
                       String companyCode, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Order Confirmation Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/OrderConfirmationPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/OrderConfirmationListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/OrderConfirmationListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4053", "", "OrderConfirmationListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Order Confirmation Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Order Confirmation Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Order Confirmation Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Order Confirmation Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Order Confirmation Date"); break;
