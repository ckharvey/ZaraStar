...
// Module: GoodsReceivedBuilderPage.java
// Author: C.K.Harvey
...
public class GoodsReceivedBuilderPage extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p2  = req.getParameter("p2"); // lpCode (maybe)
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "GoodsReceivedBuilderPage", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3034, bytesOut[0], 0, "ERR:" + p1 + ":" + p2);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3034", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3034, bytesOut[0], 0, "ACC:" + p1 + ":" + p2);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3034", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3034, bytesOut[0], 0, "SID:" + p1 + ":" + p2);
      if(con != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3034, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "GR Builder: " + p1 + ":" + p2);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void set(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req, String code, byte[] codeB, String lpCode, String unm, String sid, String uty, String men, String den,
                   String dnm, String bnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "3034", "", "GoodsReceivedBuilderPage", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "GoodsReceivedBuilderPage", "", "Goods Received Builder", "3034", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form action=\"GoodsReceivedBuilderCreate\" name=lines enctype=\"application/x-www-form-urlencoded\" method=POST>");
    ...
    scoutln(out, bytesOut, "<tr><td><p>Outstanding LP Lines for: " + code + " (" + name + ")</td></tr></table>");
    ...
    scoutln(out, bytesOut, "<td nowrap><p>LP Code &nbsp;</td>");
    ...
    String[] lpCodesArg = new String[1];  lpCodesArg[0] = "";
    String[] lpLinesArg = new String[1];  lpLinesArg[0] = "";

    String suppCode = "";
    if(lpCode.length() > 0)
      suppCode = doOneLP(con, stmt, stmt2, rs, rs2, out, lpCode, lpCodesArg, lpLinesArg, bytesOut);
    else detLPLines(con, stmt, stmt2, rs, rs2, out, code, lpCodesArg, lpLinesArg, bytesOut);

    scoutln(out, bytesOut, "<input type=hidden name=lpCodes value=" + lpCodesArg[0] + ">");
    scoutln(out, bytesOut, "<input type=hidden name=lpLines value=" + lpLinesArg[0] + ">");
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private String doOneLP(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String lpCode, String[] lpCodesArg, String[] lpLinesArg, int[] bytesOut) throws Exception
  {
    String suppCode = "";

    try
    {
      String lpCodes = "", lpDates = "", allReceived;

      stmt = con.createStatement();
      rs = stmt.executeQuery("SELECT Date, AllReceived, CompanyCode FROM lp WHERE LPCode = '" + lpCode + "'");
      ...
      processLPCodes(con, stmt, stmt2, rs, rs2, out, lpCodes, lpDates, lpCodesArg, lpLinesArg, bytesOut);
    }
    catch(Exception e)
    {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void detLPLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String code, String[] lpCodesArg, String[] lpLinesArg, int[] bytesOut) throws Exception
  {
    try
    {
      String lpCodes = "", lpDates = "", allReceived;
      ...
      stmt = con.createStatement();
      rs = stmt.executeQuery("SELECT LPCode, Date, AllReceived FROM lp WHERE CompanyCode = '" + code + "'");
      ...
      while(rs.next())
      {
        ...
        if(! allReceived.equals("R") && ! allReceived.equals("Y")) // not marked as received
        {
          lpCodes += (rs.getString(1) + "\001");
          lpDates += (rs.getString(2) + "\001");
        }
      }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void processLPCodes(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String lpCodes, String lpDates, String[] lpCodesArg, String[] lpLinesArg, int[] bytesOut)
  {
    try
    {
      char dpOnQuantities = miscDefinitions.dpOnQuantities(con, stmt, rs, "5");

      String thisCode, thisDate, lpLine, qty, unitPrice, itemCode, store, mfrCode, desc, actualQtyStr, supplierItemCode, entry;
      double actualQty, qtyD;
      int x=0, y=0, lpCodesLen = lpCodes.length(), lpDatesLen = lpDates.length(), count=0;
      byte[] b = new byte[20];

      while(x < lpCodesLen)
      {
        thisCode="";
        while(x < lpCodesLen && lpCodes.charAt(x) != '\001')
          thisCode += lpCodes.charAt(x++);
        ++x;

        thisDate="";
        while(y < lpDatesLen && lpDates.charAt(y) != '\001')
          thisDate += lpDates.charAt(y++);
        ++y;

        stmt = con.createStatement();
        rs = stmt.executeQuery("SELECT ItemCode, Description, UnitPrice, Quantity, Line, StoreCode, ManufacturerCode, SupplierItemCode, Entry FROM lpl WHERE LPCode = '" + thisCode + "'");
        ...
        while(rs.next())
        {
          ...
          actualQty = goodsReceivedNote.getTotalReceivedForAPOLine(con, stmt2, rs2, thisCode, lpLine);
          ...
          if(actualQty < qtyD) // incomplete order
          {
            ...
            scoutln(out, bytesOut, "<td align=center><p>"+ lpLine + "</td>");
            ...
            lpCodesArg[0] += (thisCode + "\001");
            lpLinesArg[0] += (lpLine + "\001");
