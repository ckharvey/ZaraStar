// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: GR edit header
// Module: GoodsReceivedEdit.java
// Author: C.K.Harvey
...
public class GoodsReceivedEdit extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "GoodsReceivedEdit", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3027, bytesOut[0], 0, "ERR:" + p1);
                      ...
                      throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3027", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3027, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3027", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3027, bytesOut[0], 0, "SID:" + p1);
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    goodsReceivedNote.getRecToHTML(con, stmt, rs, out, 'E', codeB, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, p2.charAt(0), p3,
                       dataAlready, req, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3027, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "Edit Header: " + p1);
    if(con  != null) con.close();
    if(out != null) out.flush(); 
  }
