// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: GR update header rec
// Module: GoodsReceivedUpdate.java
// Author: C.K.Harvey
...
public class GoodsReceivedUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "GoodsReceivedUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3028, bytesOut[0], 0, "ERR:" + p2);
                    ...
                    String bnm, String codeStr, char cad, String p3, String thisOp, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3028", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3028, bytesOut[0], 0, "ACC:" + codeStr);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3028", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3028, bytesOut[0], 0, "SID:" + codeStr);
    if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    else
    {
      ...
      if(errorValidation.validatePageReturningStr(con, stmt, rs, "Date", retStr, goodsReceivedNote.getFieldNamesGR(), goodsReceivedNote.getFieldTypesGR(), goodsReceivedNote.getFieldStylesGR(), "gr",
                                        "Update Goods Received Note Header", dnm, unm, recData, y, imagesDir, localDefnsDir, defnsDir)) // validated ok
      {
        switch(goodsReceivedNote.putHead(con, stmt, rs, code, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
        {
          case ' ' : // updated successfully
                     recData[0] = '\000';
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, generalUtils.sanitise(generalUtils.stringFromBytes(newCode, 0L)), "", recData, 'A',
                            localDefnsDir);
                     break;
          case 'X' : // already exists
                     ...
                     break;
          case 'I' : // illegal code
                     errorValidation.formatErrMsg("Invalid Goods Received Note Code", retStr, imagesDir);
                     reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                     break;
          case 'N' : // no code specified
                     errorValidation.formatErrMsg("No Goods Received Note Code Specified", retStr, imagesDir);
                     reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                     break;
          case 'F' : // not updated
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3028, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "Update Header: " + codeStr);
    if(con  != null) con.close();
    if(out != null) out.flush(); 
  }
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/GoodsReceivedEdit");
