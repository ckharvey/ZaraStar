// =======================================================================================================================================================================================================
// System: ZaraStar Info: Consolidations Sales by Manufacturer
// Module: ConsolidationsSalesManufacturer.java
// Author: C.K.Harvey
...
public class ConsolidationsSalesManufacturer extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ConsolidationsSalesManufacturer", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6910, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                    String bnm, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "6910", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6910, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6910", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6910, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6910, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Sales by Manufacturer</title>");

    scoutln(out, bytesOut, "<SCRIPT LANGUAGE=\"JavaScript\">");

    scoutln(out, bytesOut, "function generate(which){");
    scoutln(out, bytesOut, "var p2=sanitise(document.forms[0].mfr.value);");
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/ConsolidationsSalesManufacturerExecute?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=\"+p2+\"&p1=\"+which;}");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6910", "", "ConsolidationsSalesManufacturer", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Sales by Manufacturer", "6910",  unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
