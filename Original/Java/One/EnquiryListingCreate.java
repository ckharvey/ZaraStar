// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Enquiry create listing page
// Module: EnquiryListingCreate.java
// Author: C.K.Harvey
...
public class EnquiryListingCreate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "EnquiryListingCreate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4530, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4530, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "EnquiryListingCreate", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4530, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "EnquiryListingCreate", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4530, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4530, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM enquiry WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM enquiry" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM enquiry WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM enquiry WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM enquiry" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM enquiry WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT EnquiryCode) AS rowcount FROM enquiry WHERE EnquiryCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM enquiry" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String enquiryCode="", name="", customerCode="", date="", status;
    int enquiryCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT EnquiryCode, Date, CompanyCode, CompanyName, Status FROM enquiry WHERE EnquiryCode LIKE '" + srchStr
                       + "%'" + companyCodeANDWHERE + " ORDER BY EnquiryCode";
                   }
                   else
                   {
                     q = "SELECT EnquiryCode, Date, CompanyCode, CompanyName, Status FROM enquiry " + companyCodeWHERE
                       + "ORDER BY EnquiryCode";
                   }

                   enquiryCodePosn = 1; datePosn = 2; companyCodePosn = 3; statusPosn = 5; companyNamePosn = 4;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, EnquiryCode, CompanyCode, CompanyName, Status FROM enquiry WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     enquiryCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM enquiry" + companyCodeWHERE + " ORDER BY Date";
                     enquiryCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1;
                     statusPosn = -1; 
                   }
                   else
                   {
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, EnquiryCode, CompanyName, Date, Status FROM enquiry WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, EnquiryCode";
                     enquiryCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM enquiry " + companyCodeWHERE + " ORDER BY CompanyCode";    
                     enquiryCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; 
                     statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT CompanyCode, EnquiryCode, Date, CompanyName, Status FROM enquiry WHERE CompanyCode = '"
                       + codeArg + "' ORDER BY CompanyCode, EnquiryCode";
                     enquiryCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 4; statusPosn = 5;
                   }
                 }  
                 ...
                 while(rs.next())                  
                 {
                   enquiryCode      = getValue(enquiryCodePosn,      ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, enquiryCode, date, customerCode, name, enquiryCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, enquiryCode, customerCode, name, enquiryCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, enquiryCode, name, date, enquiryCode, status, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = enquiryCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = enquiryCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = enquiryCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = enquiryCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT EnquiryCode, Date, CompanyCode, EnquiryCode, CompanyName, Status FROM enquiry WHERE EnquiryCode LIKE '" + srchStr 
                       + "%' AND EnquiryCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY EnquiryCode DESC";
                   }
                   else
                   {  
                     q = "SELECT EnquiryCode, Date, CompanyCode, EnquiryCode, Status FROM enquiry WHERE EnquiryCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                         + companyCodeANDWHERE + " ORDER BY EnquiryCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM enquiry WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                     enquiryCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; 
                     statusPosn = -1;
                   }
                   ...
                   {
                     q = "SELECT Date, EnquiryCode, CompanyCode, CompanyName, Status FROM enquiry WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, EnquiryCode DESC";
                     enquiryCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   }  
                 }
                 else // inx == '3'
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM enquiry WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                     enquiryCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                      q = "SELECT CompanyCode, EnquiryCode, CompanyName, Date, Status FROM enquiry WHERE CompanyCode = '"
                        + codeArg + "' ORDER BY CompanyCode, EnquiryCode DESC";
                     enquiryCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, enquiryCode, date, customerCode, name, enquiryCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, enquiryCode, customerCode, name, enquiryCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, enquiryCode, name, date, enquiryCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT EnquiryCode, Date, CompanyCode, CompanyName, Status FROM enquiry WHERE EnquiryCode LIKE '" + srchStr 
                       + "%' AND EnquiryCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY EnquiryCode";
                     enquiryCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   }
                   else
                   {                   
                     q = "SELECT EnquiryCode, Date, CompanyCode, CompanyName, Status FROM enquiry WHERE EnquiryCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY EnquiryCode";
                     enquiryCodePosn = 1; datePosn = 2; companyCodePosn = 3; statusPosn = 5; companyNamePosn = 4;
                   }  
                 }
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM enquiry WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                        + "" + companyCodeANDWHERE + " ORDER BY Date";
                     enquiryCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, EnquiryCode, CompanyCode, CompanyName, Status FROM enquiry WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND EnquiryCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, EnquiryCode";
                     enquiryCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM enquiry WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                     enquiryCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, EnquiryCode, CompanyName, Date, Status FROM enquiry WHERE CompanyCode = '"
                       + codeArg + "' AND EnquiryCode > '" + codeArg2 + "' ORDER BY CompanyCode, EnquiryCode";
                     enquiryCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5;
                   }
                 }  
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '2')
                 {
                   ...
                   else
                   {
                     q = "SELECT Date, EnquiryCode, CompanyCode, CompanyName, Status FROM enquiry WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND EnquiryCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, EnquiryCode DESC";
                     enquiryCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM enquiry WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                     enquiryCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, EnquiryCode, CompanyName, Date, Status FROM enquiry WHERE CompanyCode = '"
                       + codeArg + "' AND EnquiryCode < '" + codeArg2 + "' ORDER BY CompanyCode, EnquiryCode DESC";
                     enquiryCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5;
                   }  
                 }  
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Enquiry Search Results for ";
        switch(inx)
        {
          case '1' : title += "Enquiry Code";  break;
          case '2' : title += "Enquiry Date";  break;
      ...
      else title = "Enquiry Listing";

      drawingUtils.drawTitle(out, false, false, "EnquiryListingCreate", "", title, "4530", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Enquiry Code");  break;
        case '2' : scout(out, bytesOut, "Enquiry Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                         ...
                         throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Enquiry Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/EnquiryPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/EnquiryListingCreate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/EnquiryListingCreate?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4530", "", "EnquiryListingCreate", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Enquiry Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Enquiry Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Enquiry Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Enquiry Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Date &nbsp;</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Enquiry Date");  break;
