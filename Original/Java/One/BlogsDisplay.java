...
public class BlogsDisplay extends HttpServlet
{
                    ...
                    String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "BlogsDisplay", imagesDir, localDefnsDir, defnsDir, bytesOut);
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Blog</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/BlogsDisplay?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + userCode
    ...
    blogsUtils.outputPageFrame(con, stmt, rs, out, req, "BlogsDisplay", "", "", "8101", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    blogsUtils.drawTitle(con, stmt, rs, req, out, "Blog Entries for " + authenticationUtils.getUserNameGivenUserCode(con, stmt, rs, userCode), "8101", "", false, false, false, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                         ...
                         String[] newLastCodeOnPage, String[] newFirstCodeOnPage, String[] newLastDateOnPage, String[] newFirstDateOnPage, String localDefnsDir) throws Exception
  {
    ...
    {
      case 'F' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " ORDER BY Date, Code");
                 break;
      case 'L' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " ORDER BY Date DESC, Code DESC");
                 break;
      case 'N' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " AND Date >= {d '" + lastDateOnPage + "'} AND Code > '" + lastCodeOnPage + "' ORDER BY Date, Code");
                 break;
      case 'P' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " AND Date <= {d '" + firstDateOnPage + "'} AND Code < '" + firstCodeOnPage + "' ORDER BY Code DESC");
                 break;
      case 'C' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " AND Code = '" + codeRequired + "'");
                 break;
      case 'D' : rsInfo = stmtInfo.executeQuery("SELECT * from blogs WHERE Owner = '" + userCode + "'" + s + " AND Date = {d '" + dateRequired + "'}");
                 break;
    }
