// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: OC update line rec
// Module: OrderConfirmationUpdateLine.java
// Author: C.K.Harvey
...
public class OrderConfirmationUpdateLine extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "OrderConfirmationUpdateLine", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4049, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4049", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4049, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4049", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4049, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' : if(orderConfirmation.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                   getRec(out, unm, sid, uty, men, den, dnm, bnm, code, defnsDir);
                 else 
                 {
                   messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "4049", imagesDir,
      ...
      default  : // amend or create
                 ...
                 if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, orderConfirmation.getFieldNamesOCL(), orderConfirmation.getFieldTypesOCL(),
                                                   orderConfirmation.getFieldStylesOCL(), "ocl", "Update Order Confirmation Record", dnm, unm, 
                                                   recData, y, imagesDir, localDefnsDir, defnsDir))
                 {
                   switch(orderConfirmation.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                   {
                     ...
                     case 'X' : // already exists
                                ...
                                break;
                     case 'I' : // illegal code
                                errorValidation.formatErrMsg("Illegal Order Confirmation Line", retStr, imagesDir);
                                reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                break;
                     case 'N' : // no code specified
                                errorValidation.formatErrMsg("No Order Confirmation Line Specified", retStr, imagesDir);
                                reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                break;
                     case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 4050; operation = " Line: " + line;  break;
      case 'C' : service = 4049; operation = " Line: " + line;  break;
      case 'X' : service = 4049; operation = " Cancel";         break;
      default  : service = 4049; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/OrderConfirmationPage");
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm
              + "&bnm=" + bnm + "&p1="  + generalUtils.sanitise(code) + "&p2=A2&p3=&p4=";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                          ...
                          throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/OrderConfirmationLine");
