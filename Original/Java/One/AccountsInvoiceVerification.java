// =================================================================================================================================================================================================================================================
// System: ZaraStar Accounts: Invoice Verification
// Module: AccountsInvoiceVerification.java
// Author: C.K.Harvey
...
public class AccountsInvoiceVerification extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsInvoiceVerification", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6046, bytesOut[0], 0, "ERR:" + p1);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String year, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6046, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsInvoiceVerification", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6046, bytesOut[0], 0, "ACC:" + year);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsInvoiceVerification", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6046, bytesOut[0], 0, "SID:" + year);
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6046, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), year);
    if(con != null) con.close();
    if(out != null) out.flush();
  }
                      ...
                      String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6046", "", "AccountsInvoiceVerification", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Verification - Invoices: " + year, "6046", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<td><p>Invoice Currency &nbsp;</td>");
  ...
  private void sales(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    try
    {
      ...
      rs = stmt.executeQuery("SELECT InvoiceCode, Date, CompanyCode, CompanyName, TotalTotal, Currency FROM invoice WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'} ORDER BY InvoiceCode");

      String invoiceCode, date, companyCode, companyName, totalTotal, currency, cssFormat= "", customerCurrency;
      ...
      while(rs.next())
      {
        ...
        if(customer.existsCompanyRecGivenCode(con, stmt2, rs2, companyCode, date, currency, date))
        {
  ...
  private void purchase(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    try
    {
      ...
      rs = stmt.executeQuery("SELECT InvoiceCode, Date, CompanyCode, CompanyName, TotalTotal, Currency FROM pinvoice WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'} ORDER BY InvoiceCode");

      String invoiceCode, date, companyCode, companyName, totalTotal, currency, cssFormat= "", supplierCurrency;
      ...
      while(rs.next())
      {
        ...
        if(supplier.existsSupplierRecGivenCode(con, stmt2, rs2, companyCode, date, currency, date))
        {
          ...
          if(! supplierCurrency.equals(currency))
          {
            ...
            scoutln(out, bytesOut, "<td><p>Purchase Invoice</td>");
