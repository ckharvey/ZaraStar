// =======================================================================================================================================================================================================
// System: ZaraStar Analytic: Generate CAA report  
// Module: CAAReportGenerate.java
// Author: C.K.Harvey
...
public class CAAReportGenerate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p9  = req.getParameter("p9"); // supplier
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "CAAReportGenerate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "ERR:" + p3);
                    ...
                    throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "1013b", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "ACC:" + p3);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "1013b", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "SID:" + p3);
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    switch(r071(con, stmt, stmt2, stmt3, stmt4, rs, rs2, rs3, rs4, oneOrAll, p1.charAt(0), p10.charAt(0), p9, p2, p3, p4, p5, p6, p7, p8, dnm, unm,
                reportsDir, localDefnsDir, defnsDir, workingDir))
    {
      case -1 : // Definition File Not Found
                messagePage.msgScreen(false, out, req, 17, unm, sid, uty, men, den, dnm, bnm, "1013", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
      case -2 : // cannot create report output file
                messagePage.msgScreen(false, out, req, 18, unm, sid, uty, men, den, dnm, bnm, "1013", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
      default : // generated ok
                messagePage.msgScreen(false, out, req, 16, unm, sid, uty, men, den, dnm, bnm, "1013", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
    }

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 1013, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p3);
    if(con  != null) con.close();
    if(out != null) out.flush(); 
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  public int r071(Connection con, Statement stmt, Statement stmt2, Statement stmt3, Statement stmt4, ResultSet rs, ResultSet rs2, ResultSet rs3,
                  ResultSet rs4, char whichReportVariant, char summaryOrDetailed, char orderedBy, String supplierCode, String dateFrom, String dateTo,
                  ...
                  String localDefnsDir, String defnsDir, String workingDir) throws Exception
  {
    ...
    if((reportGeneration.fhO = reportGeneration.createNewFile((short)71, workingDir, localDefnsDir, defnsDir, reportsDir, newName)) == null)
      return -2;

    if((reportGeneration.fhPPR = generalUtils.fileOpenD("071.ppr", localDefnsDir)) == null)
    {
      if((reportGeneration.fhPPR = generalUtils.fileOpenD("071.ppr", defnsDir)) == null)
        return -1;
    }
    ...
    if(whichReportVariant == 'O')
    {
      ...
      String supplierName = supplier.getASupplierFieldGivenCode(con, stmt, rs, "Name", supplierCode);
      String currency     = supplier.getASupplierFieldGivenCode(con, stmt, rs, "Currency", supplierCode);
 
      generate(con, stmt2, stmt3, stmt4, rs2, rs3, rs4, summaryOrDetailed, supplierCode, supplierName, currency, dateFrom, dateTo, dateToEncoded, ignoreZero, dateRange1, dateRange2, dateRange3, dateRange4, grandTotals, fldNames, fldData, oCount,
               dnm, unm, pageHeaderSection, pageFooterSection, localDefnsDir, defnsDir);
    }
    else
    {
      // for all suppliers
      ...
      if(orderedBy == 'N')
        rs = stmt.executeQuery("SELECT SupplierCode, Name, Currency FROM supplier WHERE NoStatements != 'Y' ORDER BY Name");
      else rs = stmt.executeQuery("SELECT SupplierCode, Name, Currency FROM supplier WHERE NoStatements != 'Y' ORDER BY SupplierCode");

      String supplierCode2="", supplierName, currency;

      while(rs.next())
      {    
        supplierCode2 = rs.getString(1);
        supplierName  = rs.getString(2);
        currency      = rs.getString(3);

        generate(con, stmt2, stmt3, stmt4, rs2, rs3, rs4, summaryOrDetailed, supplierCode2, supplierName, currency, dateFrom, dateTo, dateToEncoded, ignoreZero, dateRange1, dateRange2, dateRange3, dateRange4, grandTotals, fldNames, fldData,
                 oCount, dnm, unm, pageHeaderSection, pageFooterSection, localDefnsDir, defnsDir);
      }
  ...
  private void generate(Connection con, Statement stmt, Statement stmt2, Statement stmt3, ResultSet rs, ResultSet rs2, ResultSet rs3,
                        char summaryOrDetailed, String supplierCode, String supplierName, String currency, String dateFrom, String dateTo,
                        int dateToEncoded, String ignoreZero, int dateRange1, int dateRange2, int dateRange3, int dateRange4, double[] grandTotals,
                        ...
                        String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    int[] latestPaymentDateEncoded = new int[1];
    ...
    double[] amtPaidBase      = new double[1];
    double[] amtPaidNonBase   = new double[1];
    ...
    generalUtils.repAlpha(fldData, 10000, (short)14, supplierCode);
    generalUtils.repAlpha(fldData, 10000, (short)15, supplierName);
    ...
    rs = stmt.executeQuery("SELECT InvoiceCode, CompanyCode, CompanyName, Date, TotalTotal, BaseTotalTotal FROM pinvoice "
                          + "WHERE CompanyCode = '" + supplierCode + "' AND Status != 'C' AND Settled != 'Y' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    ...
    {    
      ...
      latestPaymentDateEncoded[0] = 0;
      amtPaidBase[0] = amtPaidNonBase[0] = 0.0;

      getPayments(con, stmt2, rs2, invoiceCode, dateToEncoded, dateTo, amtPaidBase, amtPaidNonBase, latestPaymentDateEncoded);
      
      amtRepaidBase[0] = amtRepaidNonBase[0] = 0.0;

      // Now, chk against the CNs file
      getPurchaseCreditNotes(con, stmt2, stmt3, rs2, rs3, invoiceCode, dateTo, amtRepaidBase, amtRepaidNonBase, dnm, localDefnsDir, defnsDir);
      
      if(generalUtils.doubleDPs((amtPaidNonBase[0] + amtRepaidNonBase[0]),'2') != totalAmountNonBase) // >=
      {
        totalAmountNonBase -= (amtPaidNonBase[0] + amtRepaidNonBase[0]);
        totalAmountBase    -= (amtPaidBase[0]    + amtRepaidBase[0]);
    ...
    rs = stmt.executeQuery("SELECT PDNCode, CompanyCode, CompanyName, Date, TotalTotal, BaseTotalTotal FROM pdebit "
    ...
    String pdnCode, pdnCompanyCode, pdnCompanyName;
    while(rs.next())
    {    
      pdnCode            = rs.getString(1);
      pdnCompanyCode     = rs.getString(2);
      pdnCompanyName     = rs.getString(3);
      date               = rs.getString(4);
      totalAmountNonBase = generalUtils.doubleFromStr(generalUtils.doubleDPs(rs.getString(5), '2'));
      totalAmountBase    = generalUtils.doubleFromStr(generalUtils.doubleDPs(rs.getString(6), '2'));
      ...
      generalUtils.repAlpha(fldData, 10000, (short)9,  pdnCode);
      generalUtils.repAlpha(fldData, 10000, (short)10, pdnCompanyCode);
      generalUtils.repAlpha(fldData, 10000, (short)11, pdnCompanyName);
      generalUtils.repAlpha(fldData, 10000, (short)12, generalUtils.convertFromYYYYMMDD(date));

      latestPaymentDateEncoded[0] = 0;
      amtPaidBase[0] = amtPaidNonBase[0] = 0.0;

      getPayments(con, stmt2, rs2, pdnCode, dateToEncoded, dateTo, amtPaidBase, amtPaidNonBase, latestPaymentDateEncoded);
      
      amtRepaidBase[0] = amtRepaidNonBase[0] = 0.0;

      // Now, chk against the CNs file
      getPurchaseCreditNotes(con, stmt2, stmt3, rs2, rs3, pdnCode, dateTo, amtRepaidBase, amtRepaidNonBase, dnm, localDefnsDir, defnsDir);
    ...
    supplier.updatedDateInvoicePaid(con, stmt, rs, generalUtils.decodeToYYYYMMDD(earliestDate), supplierCode, dnm, localDefnsDir, defnsDir);
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  // puts date of latest payment in latestPaymentDate
  private void getPayments(Connection con, Statement stmt, ResultSet rs, String invoiceCode, int dateToEncoded, String dateTo,
                           double[] amtPaidBase, double[] amtPaidNonBase, int[] latestPaymentDateEncoded) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t1.DatePaid, t2.AmountPaid, t2.BaseAmountPaid "
                         + "FROM paymentl AS t2 INNER JOIN payment AS t1 ON t1.PaymentCode = t2.PaymentCode " 
                         + "WHERE t2.InvoiceCode = '" + invoiceCode + "' AND t1.Status != 'C' AND DatePaid <= {d '"
                         + dateTo + "'}");
    ...
    while(rs.next())
    {    
      datePaid = rs.getString(1);
      amtPaidNonBase[0] += generalUtils.doubleFromStr(generalUtils.doubleDPs(rs.getString(2), '2'));
      amtPaidBase[0]    += generalUtils.doubleFromStr(generalUtils.doubleDPs(rs.getString(3), '2'));

      if(generalUtils.encodeFromYYYYMMDD(datePaid) > latestPaymentDateEncoded[0])
        latestPaymentDateEncoded[0] = generalUtils.encodeFromYYYYMMDD(datePaid);
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void getPurchaseCreditNotes(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String invoiceCode, String dateTo,
                                      double[] amtRepaidBase, double[] amtRepaidNonBase, String dnm, String localDefnsDir,
                                      String defnsDir) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t2.Amount2, t2.Amount, t2.GSTRate FROM pcreditl AS t2 INNER JOIN pcredit AS t1 "
                         + "ON t1.PCNCode = t2.PCNCode WHERE t2.InvoiceCode = '" + invoiceCode + "' AND t1.Status != 'C' "
