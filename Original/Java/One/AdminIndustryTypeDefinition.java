// =================================================================================================================================
// System: ZaraStar AdminEngine: IndustryType Definition
// Module: AdminIndustryTypeDefinition.java
// Author: C.K.Harvey
...
public class AdminIndustryTypeDefinition extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AdminIndustryTypeDefinition", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7059, bytesOut[0], 0, "ERR:");
                      ...
                      String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "7059", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7059, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "7059", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7059, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 7059, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush();
  }
  ...
  private void create(Connection con, Statement stmt, ResultSet rs, PrintWriter out, String unm, String sid, String uty, String men,
                        ...
                        throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Admin - Industry Type Definition</title>");
    ...
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/AdminIndustryTypeDefinitionUpdate?unm=\" + escape('" + unm
    ...
    scoutln(out, bytesOut, "{");
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/AdminIndustryTypeDefinitionDelete?unm=\" + escape('" + unm
                         + "') + \"&sid=\" + escape('" + sid + "') + \"&uty=\" + escape('" + uty
                         + "') + \"&men=\" + escape('" + men + "') + \"&den=\" + escape('" + den + "') + \"&bnm=\" + escape('" + bnm
    ...
    scoutln(out, bytesOut, "this.location.replace(\"/central/servlet/AdminIndustryTypeDefinition?unm=" + unm + "&sid=" + sid + "&uty="
    ...
    dashboardUtils.drawTitle(out, "Industry Types", "7059", unm, sid, uty, men, den, dnm, bnm, bytesOut);
    ...
    scoutln(out, bytesOut, "<td nowrap><p>&nbsp; Industry Type &nbsp;</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td colspan=3><span id=\"option\"><p><a href=\"javascript:updateN()\">Add the Industry Type</a></span></td></tr>");
  ...
  private void list(PrintWriter out, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT * FROM industrytype");
