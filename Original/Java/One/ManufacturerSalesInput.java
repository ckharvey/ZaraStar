// =======================================================================================================================================================================================================
// System: ZaraStar Analytic: Manufacturer Sales by Customer from Invoice Lines params
// Module: ManufacturerSalesInput.java
// Author: C.K.Harvey
...
public class ManufacturerSalesInput extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ManufacturerSalesInput", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1022, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                    String bnm, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "1022", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1022, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "1022", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1022, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 1022, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Manufacturer Sales by Customer from Invoice Lines</title>");

    scoutln(out, bytesOut, "<SCRIPT LANGUAGE=\"JavaScript\">");

    scoutln(out, bytesOut, "function generate(){");
    scoutln(out, bytesOut, "var p1=sanitise(document.forms[0].effectiveDate.value);");
    scoutln(out, bytesOut, "var p2=sanitise(document.forms[0].mfr.value);");
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/ManufacturerSalesGenerate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=\"+p2+\"&p1=\"+p1;}");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "1022", "", "ManufacturerSalesInput", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Manufacturer Sales from Invoice Lines", "1022",  unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
