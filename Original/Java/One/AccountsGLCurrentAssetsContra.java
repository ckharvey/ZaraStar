// =======================================================================================================================================================================================================
// System: ZaraStar: Accounts: GL Current Assets - Contra
// Module: AccountsGLCurrentAssetsContra.java
// Author: C.K.Harvey
...
public class AccountsGLCurrentAssetsContra extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6007n: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsGLCurrentAssetsContra", bytesOut);
                    ...
                    String bnm, String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsGLCurrentAssetsContra", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsGLCurrentAssetsContra", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6007", "", true, plain, account, p2, p3, "", "", "AccountsGLCurrentAssetsContra", unm, sid, uty, men, den, dnm, bnm,
                          localDefnsDir, defnsDir, hmenuCount, bytesOut);

    String year = generalUtils.getFromDefnFile(unm, "accounts.dfn", workingDir, "");
    if(! plain)
    {
      pageFrameUtils.drawTitle(out, true, plain, "AccountsGLCurrentAssetsContra", account, dateFrom, dateTo, "", "", "GL Account: " + account + " - " + accountsUtils.getAccountDescriptionGivenAccCode(account, year, dnm, localDefnsDir, defnsDir) + " ("
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPayment(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String accCode, String year, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT PaymentCode, CompanyName, TotalAmount, BaseTotalAmount, DatePaid, Currency "
                         + "FROM payment WHERE AccCredited = '" + accCode + "' AND Status != 'C' AND DatePaid >= {d '"
                         + dateFrom + "'} AND DatePaid <= {d '" + dateTo + "'}");
    double amount, baseAmount;           
    ...
    {    
      amount     = generalUtils.doubleFromStr(rs.getString(3));
      baseAmount = generalUtils.doubleFromStr(rs.getString(4));
      addToTmpTable(con, stmt2, "P", rs.getString(1), rs.getString(2), amount, baseAmount, rs.getString(5), rs.getString(6), tmpTable);
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onReceipt(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String accCode, String year, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT ReceiptCode, CompanyName, TotalAmount, BaseTotalAmount, DateReceived, Currency "
                         + "FROM receipt WHERE AccDebited = '" + accCode + "' AND Status != 'C' AND DateReceived >= {d '"
                         + dateFrom + "'} AND DateReceived <= {d '" + dateTo + "'}");
    double amount, baseAmount;           
    ...
    {    
      amount     = generalUtils.doubleFromStr(rs.getString(3));
      baseAmount = generalUtils.doubleFromStr(rs.getString(4));
      addToTmpTable(con, stmt2, "E", rs.getString(1), rs.getString(2), amount, baseAmount, rs.getString(5), rs.getString(6), tmpTable);
    }
