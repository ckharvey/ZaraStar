// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Inbox update line rec
// Module: InboxLineUpdate.java
// Author: C.K.Harvey
...
public class InboxLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "InboxLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3136, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3136", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3136, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3136", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3136, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' : if(inbox.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                   getRec(out, unm, sid, uty, men, den, dnm, bnm, code, defnsDir);
                 else 
                 {
                   messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "3136", imagesDir,
      ...
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, inbox.getFieldNamesInboxL(), inbox.getFieldTypesInboxL(),
                                                     inbox.getFieldStylesInboxL(), "inboxl", "Update Inbox Record", dnm, unm, 
                                                     recData, y, imagesDir, localDefnsDir, defnsDir))
                 {
                   switch(inbox.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                   {
                     case ' ' : // updated successfully
                                if(thisOp == 'S') // save & new
                                  newLine(out, code, unm, sid, uty, men, den, dnm, bnm, localDefnsDir);
                     ...
                     case 'X' : // already exists
                                ...
                                break;
                     case 'I' : // illegal code
                                errorValidation.formatErrMsg("Illegal Inbox Record Line", retStr, imagesDir);
                                reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                break;
                     case 'N' : // no code specified
                                errorValidation.formatErrMsg("No Inbox Record Line Specified", retStr, imagesDir);
                                reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                break;
                     case 'F' : // not updated
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3136, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), code);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/InboxPage");
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                          ...
                          throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/InboxLinePage");
                         ...
                         String dnm, String bnm, String localDefnsDir) throws Exception
  {
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm
              + "&bnm=" + bnm + "&p1="  + generalUtils.sanitise(code) + "&p2=&p3=&p4=";
