// =======================================================================================================================================================================================================
// System: ZaraStar Document: DO fetch rec page
// Module: DeliveryOrderPage.java
// Author: C.K.Harvey
...
public class DeliveryOrderPage extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p7  = req.getParameter("p7"); // friendly as DO or LoanNote
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "DeliveryOrderPage", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4054, bytesOut[0], 0, "ERR:" + p1);
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      while(x < len)
      {
        ...
        if(name.equals("p7")) // friendly as DO or LoanNote
          p7 = value;
      }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, String p3, String p4, String p6, String p7, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4054", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4054, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4054", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4054, bytesOut[0], 0, "SID:" + p1);
      if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    boolean basedOn = false, basedOn2 = false, cart = false;
    String serviceCode="", sourceDocName="", returnServlet=""; 

    if(p1.startsWith("CREATEBASEDONPL:"))
    {
      p1 = p1.substring(16);
      serviceCode = "4057";
      returnServlet = "GoodsReceivedPickingList";
      sourceDocName = "Picking List";
      basedOn = true;
    }
    else
    ...
    if(basedOn2)
      buildRec2(out, unm, sid, uty, men, den, dnm, bnm, p1, serviceCode, sourceDocName, returnServlet, localDefnsDir);
    else 
    {
      boolean plain = false;
      if(p6.equals("P"))
      ...
      deliveryOrder.getRecToHTML(con, stmt, stmt2, rs, rs2, out, plain, p7, dispOrEdit, codeB, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, p2.charAt(0), p3, dataAlready, req, bytesOut);
    }
    
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4054, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  private void buildRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String code, String serviceCode, String sourceDocName, String returnServlet, String localDefnsDir) throws Exception
  {
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1="
              + generalUtils.sanitise(code) + "&p2=" + serviceCode + "&p5=" + returnServlet + "&p4=" + generalUtils.sanitise(sourceDocName) + "&p3=Delivery%20Order";
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void buildRec2(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String code, String serviceCode, String sourceDocName, String returnServlet, String localDefnsDir) throws Exception
  {
//    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/DocumentBuild");
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/DocumentBuildType2");
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + generalUtils.sanitise(code) + "&p2=" + serviceCode + "&p5=" + returnServlet + "&p4="
              + generalUtils.sanitise(sourceDocName) + "&p3=Delivery%20Order";
                                ...
                                throws Exception
  {
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=D&p2=do";
