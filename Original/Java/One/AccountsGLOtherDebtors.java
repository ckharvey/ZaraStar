// =======================================================================================================================================================================================================
// System: ZaraStar: Accounts: GL Other Debtors
// Module: AccountsGLOtherDebtors.java
// Author: C.K.Harvey
...
public class AccountsGLOtherDebtors extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6007w: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsGLOtherDebtors", bytesOut);
                    ...
                    String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsGLOtherDebtors", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsGLOtherDebtors", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    set(con, stmt, stmt2, rs, rs2, out, req, dateFrom, dateTo, p1, p2, p3, plain, unm, sid, uty, men, den, dnm, bnm, workingDir, localDefnsDir, defnsDir,
        bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6007, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p2 + " " + p3);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  private void set(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req,
                   String dateFrom, String dateTo, String p1, String p2, String p3, boolean plain, String unm, String sid, String uty, String men,
                   String den, String dnm, String bnm, String workingDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>GL Account: Other Debtors</title>");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6007", "", true, plain, "", p2, p3, "", "", "AccountsGLOtherDebtors", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    if(! plain)
    {
      pageFrameUtils.drawTitle(out, true, plain, "AccountsGLOtherDebtors", otherDebtorsAccount, dateFrom, dateTo, "", "", "GL Account: " + otherDebtorsAccount + " - " + accountsUtils.getAccountDescriptionGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + " ("
                                                                                 + accountsUtils.getAccountCurrencyGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + " "
                                                                                 + accountsUtils.getAccountDrCrGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + ")", "6007", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    }
    else
    {
      pageFrameUtils.drawTitle(out, false, plain, "", "", "", "", "", "", "GL Account: " + otherDebtorsAccount + " - " + accountsUtils.getAccountDescriptionGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + " ("
                                                               + accountsUtils.getAccountCurrencyGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + " " + accountsUtils.getAccountDrCrGivenAccCode(otherDebtorsAccount, year, dnm, localDefnsDir, defnsDir) + ")", "",
    ...
    calc(con, stmt, stmt2, rs, rs2, p1, year, otherDebtorsAccount, dateFrom, dateTo, dnm, localDefnsDir, defnsDir, tmpTable);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  public void calc(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String account, String year,
                   String otherDebtorsAccount, String dateFrom, String dateTo, String dnm, String localDefnsDir, String defnsDir, String tmpTable)
                   throws Exception
  {    
    onVoucherLines(con, stmt, stmt2, rs, otherDebtorsAccount, dateFrom, dateTo, tmpTable);
    onRVoucherLines(con, stmt, stmt2, rs, otherDebtorsAccount, dateFrom, dateTo, tmpTable);

    onInvoice(con, stmt, stmt2, rs, rs2, dateFrom, dateTo, tmpTable);
    onDebitNote(con, stmt, stmt2, rs, rs2, dateFrom, dateTo, tmpTable);
    onSalesCreditNote(con, stmt, stmt2, rs, rs2, dateFrom, dateTo, tmpTable);
    onReceipt(con, stmt, stmt2, rs, rs2, dateFrom, dateTo, tmpTable);
    onIATCr(con, stmt, stmt2, rs, year, otherDebtorsAccount, dateFrom, dateTo, tmpTable);
    onIATDr(con, stmt, stmt2, rs, year, otherDebtorsAccount, dateFrom, dateTo, tmpTable);
    onBatchCr(con, stmt2, year, otherDebtorsAccount, dateFrom, dateTo, tmpTable, dnm, localDefnsDir, defnsDir);
    onBatchDr(con, stmt2, year, otherDebtorsAccount, dateFrom, dateTo, tmpTable, dnm, localDefnsDir, defnsDir);
  }
  
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onVoucherLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String otherDebtorsAccount, String dateFrom,
                              String dateTo, String tmpTable) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.VoucherCode, t1.CompanyName, t2.Amount2, t2.Amount, t1.Date, t1.Currency "
                         + "FROM voucherl AS t2 INNER JOIN voucher AS t1 ON t1.VoucherCode = t2.VoucherCode WHERE t2.AccountDr = '"
                         + otherDebtorsAccount + "' AND t1.Status != 'C' AND t1.Date >= {d '" + dateFrom + "'} AND t1.Date <= {d '" + dateTo + "'}");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onRVoucherLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String otherDebtorsAccount, String dateFrom,
                               String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onInvoice(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT InvoiceCode, CompanyCode, CompanyName, TotalTotal, BaseTotalTotal, Date, Currency "
                         + "FROM invoice WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    double amount, baseAmount;
    
    while(rs.next())
    {    
      if(! existsCompany(con, stmt2, rs2, rs.getString(2)))
      {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onDebitNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo,
                           String tmpTable) throws Exception
  {
    stmt = con.createStatement();
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onSalesCreditNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo,
                            String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT CNCode, CompanyCode, CompanyName, TotalTotal, BaseTotalTotal, Date, Currency "
                         + "FROM credit WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    double amount, baseAmount;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onReceipt(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    stmt = con.createStatement();
    ...
    {    
      if(! existsCompany(con, stmt2, rs2, rs.getString(2)))
      {
        ...
        if(baseAmount != 0.0)
        {
          addToTmpTable(con, stmt2, "R", rs.getString(1), rs.getString(3), amount, baseAmount, rs.getString(6), rs.getString(7), rs.getString(8),
                        tmpTable);
        }
      }
    }
                                        ...
                                        double[] across2Coord, byte[] oBuf, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    double baseAmount, baseBalance = 0.0, monthBalance = 0.0, amount;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private boolean existsCompany(Connection con, Statement stmt, ResultSet rs, String companyCode) throws Exception
  {
    int numRecs = 0;
