// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Change Stores for a Document
// Module: DocumentStores.java
// Author: C.K.Harvey
...
public class DocumentStores extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "DocumentStores", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6096, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "6096", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6096, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6096", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6096, bytesOut[0], 0, "SID:" + p1);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6096, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Change Stores for a Document</title></head>");
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "6096", "", "DocumentStores", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "DocumentStores", "", "Documents - Stores Change", "6096", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT Name FROM store"); 
      while(rs.next())
      {
        store = rs.getString(1);
        scoutln(out, bytesOut, "<tr><td width=100></td><td width=90%><p>&nbsp;<a href=\"/central/servlet/DocumentStoresExecute?unm="
                               + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&p1="
                               + generalUtils.sanitise(docCode) + "&p2=" + generalUtils.sanitise(docType) + "&p3=" + generalUtils.sanitise(store)
                               + "&dnm=" + dnm + "&bnm=" + bnm + "\">" + store + "</a></td></tr>");
      }
