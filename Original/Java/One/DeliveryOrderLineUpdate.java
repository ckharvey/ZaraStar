// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: DO update line rec
// Module: DeliveryOrderLineUpdate.java
// Author: C.K.Harvey
...
public class DeliveryOrderLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "DeliveryOrderLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4062, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4062", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4062, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4062", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4062, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' :  if(deliveryOrder.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "4062", imagesDir, localDefnsDir, defnsDir, bytesOut);
                  break;
      case 'X' :  // cancel
                  getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  break;
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, deliveryOrder.getFieldNamesDOL(), deliveryOrder.getFieldTypesDOL(), deliveryOrder.getFieldStylesDOL(), "dol", "Update Delivery Order Record", dnm, unm, recData, y, imagesDir,
                                                    localDefnsDir, defnsDir))
                  {
                    switch(deliveryOrder.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Delivery Order Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Delivery Order Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 4063; operation = " Line: " + line;  break;
      case 'C' : service = 4062; operation = " Line: " + line;  break;
      case 'X' : service = 4062; operation = " Cancel";         break;
      default  : service = 4062; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                          ...
                          throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/DeliveryOrderLine");
