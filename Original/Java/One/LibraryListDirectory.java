...
public class LibraryListDirectory extends HttpServlet
{
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "LibraryListDirectory", imagesDir, localDefnsDir, defnsDir, bytesOut);
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Library Listing</title>");
    ...
    if(canEditDetails)
    {
      scoutln(out, bytesOut, "function details(docCode){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/LibraryEditDocumentDetails?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\"+docCode;}");

      scoutln(out, bytesOut, "function manage(docCode){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/LibraryManageDocument?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\"+docCode;}");
    }
    ...
    if(canCheckout)
    {
      scoutln(out, bytesOut, "function checkout(docCode){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/LibraryCheckoutFile?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\"+docCode;}");
    }
    
    if(canCheckin)
    {
      scoutln(out, bytesOut, "function checkin(docCode){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/LibraryFileCheckIn?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\"+docCode;}");
    }
    ...
    libraryUtils.outputPageFrame(con, stmt, rs, out, req, "LibraryListDirectory", "12000", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
 
    libraryUtils.drawTitle(con, stmt, rs, req, out, "Document List", "12000", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
