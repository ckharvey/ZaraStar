...
// Module: ConsolidationsSalesCustomerExecute.java
// Author: C.K.Harvey
...
public class ConsolidationsSalesCustomerExecute extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ConsolidationsSalesCustomerExecute", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6909, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String bnm, String p1, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6909, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsSalesCustomer", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6909, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsSalesCustomer", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6909, bytesOut[0], 0, "SID:" + p1);
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6909, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Sales by Customer</title>");

    scoutln(out, bytesOut, "</script><link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"" + directoryUtils.getCssGeneralDirectory(con, stmt, rs, unm, dnm, defnsDir) + "general.css\">");
   
    outputPageFrame(con, stmt, rs, out, req, false, "", "", "", "Sales by Customer", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
    ...
    forEachCustomer(fh, out, false, con, stmt, rs, "2011-01-01", "2014-12-31", bytesOut);
  ...
  private void processCSV(RandomAccessFile fh, Connection con, Statement stmt, ResultSet rs, PrintWriter out, int[] bytesOut) throws Exception
  {
    ...
    forEachCustomer(fh, out, true, con, stmt, rs, "2011-01-01", "2014-12-31", bytesOut);
  }
  
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void forEachCustomer(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, ResultSet rs, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT CompanyCode, Name, Country, IndustryType FROM company ORDER BY Name");

    String code, name, country, industryType;
    ...
    while(rs.next())
    {
      ...
      industryType    = generalUtils.deNull(rs.getString(4));
      
      this.forEachInvoice(fh, out, isCSV, con, stmt, rs, code, dateFrom, dateTo, name, country, industryType, bytesOut);      
    }
  ...
  private double forEachInvoice(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, ResultSet rs, String custCode, String dateFrom, String dateTo, String name, String country, String industryType, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.Amount2, t2.ItemCode, t2.Description, t1.Currency, t2.Quantity, t1.InvoiceCode, t1.Date, "
            + "t2.UnitPrice, t2.Manufacturer FROM invoice AS t1 INNER JOIN invoicel AS t2 ON t1.InvoiceCode = t2.InvoiceCode WHERE "
                         + "t1.CompanyCode = '" + custCode + "' AND t1.Date >= {d '" + dateFrom + "'} AND t1.Date <= {d '" + dateTo + "'} AND t1.Status != 'C' ORDER BY t1.Date");
  ...
  private void outputLine(RandomAccessFile fh, PrintWriter out, boolean isCSV, String itemCode, String desc, String amt, String currency,
          String qty, String invoiceCode, String invoiceDate, String unitPrice, String mfr, String custCode,
                               ...
                               String den, String dnm, String bnm, String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsSalesCustomer", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
  }
