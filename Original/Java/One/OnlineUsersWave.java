...
public class OnlineUsersWave extends HttpServlet
{
  ...
  private void doIt(HttpServletRequest req, HttpServletResponse res, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    String rtn = "Unexpected System Error: 12601aw";
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void mergeData(HttpServletResponse res, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    String defnsDir      = directoryUtils.getSupportDirs('D');
    String localDefnsDir = directoryUtils.getLocalOverrideDir(dnm);
    ...
    try
    {
      ...
      while(rs.next())
      {
        ...
        rtnStr += check(con, stmt2, rs2, stmtZC, rsZC, "Internal", userCode, userName, fs2, status, sessionsDir, timeNow, assumedSignedOut, idleTime, unm, dnm, showInDirectory, cssFormat, localDefnsDir, defnsDir);
      }
    ...
    try
    {
      ...
      while(rs.next())
      {
        ...
        rtnStr += check(con, stmt2, rs2, stmtZC, rsZC, "Registered", externalCode, name + " (" + companyName + ")", fs2, "L", sessionsDir, timeNow, assumedSignedOut, idleTime, unm, dnm, "Y", cssFormat, localDefnsDir, defnsDir);
      }
    ...
    res.getWriter().write(rtnStr);
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private String check(Connection con, Statement stmt, ResultSet rs, Statement stmtZC, ResultSet rsZC, String userType, String userCode, String userName, String[] fs2, String status, String sessionsDir, long timeNow, long assumedSignedOut,
                       long idleTime, String unm, String dnm, String showInDirectory, String[] cssFormat, String localDefnsDir, String defnsDir) throws Exception
  {
    String rtnStr = "";
