// =======================================================================================================================================================================================================
// System: ZaraStar Utils: fetch blogs
// Module: AdminControlUtilsBlogs.java
// Author: C.K.Harvey
...
public class AdminControlUtilsBlogs extends HttpServlet
{
  ...
  private String mergeData(HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    int num = authenticationUtils.countBloggers(dnm);
    if(num == 0)
      ;
    else
    if(num == 1)
    {
      s += addOption("All Blog Entries", "BlogsDisplayWave", "&p1=" + getTheBlogger(dnm), count);
      s += addOption("-", "", "", count);
    }
    else
    {
      s += addOption("Blog Roll", "BlogsBlogRollListWave", "", count);
      s += addOption("-", "", "", count);
    }

    s += getBlogMenuItems(con, stmt, rs, req, unm, sid, uty, dnm, localDefnsDir, defnsDir, count);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String getBlogMenuItems(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, String unm, String sid, String uty, String dnm, String localDefnsDir, String defnsDir, int[] count) throws Exception
  {
    String s = "";
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String getTheBlogger(String dnm) throws Exception
  {
    String userCode = "";
