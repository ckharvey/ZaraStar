// ==================================================================================================================================================================================================================================================
// System: ZaraStar: Accounts: GL Trade Debtors
// Module: AccountsGLTradeDebtors.java
// Author: C.K.Harvey
...
public class AccountsGLTradeDebtors extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6007f: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AccountsGLTradeDebtors", bytesOut);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p2, String p3, String p4, String p5, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AccountsGLTradeDebtors", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AccountsGLTradeDebtors", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    set(con, stmt, stmt2, rs, rs2, out, req, dateFrom, dateTo, p2, p3, plain, p5, unm, sid, uty, men, den, dnm, bnm, workingDir, localDefnsDir, defnsDir, bytesOut);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void set(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req, String dateFrom,
                   String dateTo, String p2, String p3, boolean plain, String p5, String unm, String sid, String uty, String men, String den, String dnm,
                   String bnm, String workingDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>GL Account: Trade Debtors</title>");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6007", "", true, plain, "", p2, p3, "", "", "AccountsGLTradeCreditors", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    String year = generalUtils.getFromDefnFile(unm, "accounts.dfn", workingDir, "");

    String account = accountsUtils.getTradeDebtorsAccCode(year, dnm, localDefnsDir, defnsDir);
    
    if(! plain)
    {
      pageFrameUtils.drawTitle(out, true, plain, "AccountsGLTradeDebtors", account, dateFrom, dateTo, p5, "", "GL Account: " + account + " - " + accountsUtils.getAccountDescriptionGivenAccCode(account, year, dnm, localDefnsDir, defnsDir) + " ("
    ...
    calc(con, stmt, stmt2, rs, rs2, p5, baseCurrency, year, debtorsAccount, dateFrom, dateTo, dnm, localDefnsDir, defnsDir, tmpTable);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  public void calc(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String baseCurrency, String year, String debtorsAccount, String dateFrom, String dateTo, String dnm, String localDefnsDir,
                   String defnsDir, String tmpTable) throws Exception
  {    
    onVoucherLines(con, stmt, stmt2, rs, currency, debtorsAccount, dateFrom, dateTo, tmpTable);
    onRVoucherLines(con, stmt, stmt2, rs, currency, debtorsAccount, dateFrom, dateTo, tmpTable);

    onInvoice(con, stmt, stmt2, rs, rs2, currency, dateFrom, dateTo, tmpTable);
    onDebitNote(con, stmt, stmt2, rs, rs2, currency, dateFrom, dateTo, tmpTable);
    onSalesCreditNote(con, stmt, stmt2, rs, rs2, currency, dateFrom, dateTo, tmpTable);
    onReceipt(con, stmt, stmt2, rs, rs2, currency, dateFrom, dateTo, tmpTable);
    
    if(currency.equals(baseCurrency))
      onReceiptGainLoss(con, stmt, stmt2, rs, rs2, currency, dateFrom, dateTo, tmpTable);
    
    onBatchCr(con, stmt2, currency, year, debtorsAccount, dateFrom, dateTo, tmpTable, dnm);
    onBatchDr(con, stmt2, currency, year, debtorsAccount, dateFrom, dateTo, tmpTable, dnm);
  }
  
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onVoucherLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String currency, String debtorsAccount, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    String s;
    ...
    rs = stmt.executeQuery("SELECT t2.VoucherCode, t1.CompanyName, t2.Amount2, t2.Amount, t1.Date, t1.Currency FROM voucherl AS t2 INNER JOIN voucher AS t1 ON t1.VoucherCode = t2.VoucherCode WHERE t2.AccountDr = '" + debtorsAccount
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onRVoucherLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String currency, String debtorsAccount, String dateFrom, String dateTo,
                               String tmpTable) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.VoucherCode, t1.CompanyName, t2.Amount2, t2.Amount, t1.Date, t1.Currency "
                         + "FROM rvoucherl AS t2 INNER JOIN rvoucher AS t1 ON t1.VoucherCode = t2.VoucherCode WHERE t2.AccountCr = '" + debtorsAccount
                         + "' AND t1.Status != 'C' AND t1.Date >= {d '" + dateFrom + "'} AND t1.Date <= {d '" + dateTo + "'}" + s);
    double amount, baseAmount;           
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onInvoice(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String dateFrom, String dateTo,
                                 String tmpTable) throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    rs = stmt.executeQuery("SELECT InvoiceCode, CompanyCode, CompanyName, TotalTotal, BaseTotalTotal, Date, Currency "
                         + "FROM invoice WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'} AND Settled != 'Y'" + s);
    double amount, baseAmount;
    
    while(rs.next())
    {    
      if(existsCustomer(con, stmt2, rs2, rs.getString(2)))
      {
        amount     = generalUtils.doubleFromStr(rs.getString(4));
        baseAmount = generalUtils.doubleFromStr(rs.getString(5));

        if(baseAmount != 0.0)
          addToTmpTable(con, stmt2, "N", rs.getString(1), rs.getString(3), amount, baseAmount, rs.getString(6), rs.getString(7), "", tmpTable);
      }
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onDebitNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String dateFrom, String dateTo,
                                   String tmpTable) throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    rs = stmt.executeQuery("SELECT DNCode, CompanyCode, CompanyName, TotalTotal, BaseTotalTotal, Date, Currency "
                         + "FROM debit WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}" + s);
    double amount, baseAmount;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onSalesCreditNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String dateFrom, String dateTo,
                                    String tmpTable) throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    rs = stmt.executeQuery("SELECT CNCode, CompanyCode, CompanyName, TotalTotal, BaseTotalTotal, Date, Currency "
                         + "FROM credit WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}" + s);
    double amount, baseAmount;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void onReceipt(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String dateFrom, String dateTo,
                         String tmpTable) throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    rs = stmt.executeQuery("SELECT ReceiptCode, CompanyCode, CompanyName, TotalAmount, BaseTotalAmount, DateReceived, Currency, ReceiptReference "
                         + "FROM receipt WHERE Status != 'C' AND DateReceived >= {d '" + dateFrom + "'} AND DateReceived <= {d '" + dateTo + "'}" + s);
    double amount, baseAmount;
    ...
    {    
      if(existsCustomer(con, stmt2, rs2, rs.getString(2)))
      {
        ...
        if(baseAmount != 0.0)
        {
          addToTmpTable(con, stmt2, "P", rs.getString(1), rs.getString(3), amount, baseAmount, rs.getString(6), rs.getString(7), rs.getString(8),
                        tmpTable);
        }
      }
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onReceiptGainLoss(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String currency, String dateFrom, String dateTo,
                                 String tmpTable) throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    rs = stmt.executeQuery("SELECT t1.CompanyCode, t1.ReceiptCode, t1.CompanyName, t1.Rate, t2.OriginalRate, t2.AmountReceived, t1.DateReceived, "
                         + "t1.Currency, t1.ReceiptReference FROM receiptl AS t2 INNER JOIN receipt AS t1 ON t1.ReceiptCode = t2.ReceiptCode "
                         + "WHERE t1.Status != 'C' AND t1.DateReceived >= {d '" + dateFrom + "'} AND t1.DateReceived <= {d '" + dateTo + "'}" + s + " ORDER BY t2.ReceiptCode");
    
    double amount, rate, originalRate, diff, runningDiff = 0.0;
    String companyCode="", lastCompanyCode="", lastReceiptCode = "", receiptCode="", companyName="", dateReceived="", currency2="", reference = "";
    boolean first = true;
    
    while(rs.next())
    { 
      companyCode  = rs.getString(1);
      receiptCode  = rs.getString(2);
      
      if(! receiptCode.equals(lastReceiptCode))
      {
        if(! first)
        {
          if(existsCustomer(con, stmt2, rs2, lastCompanyCode))
          {
            if(runningDiff < 0) // overall exch loss
              addToTmpTable(con, stmt2, "S", lastReceiptCode, companyName, 0, -runningDiff, dateReceived, currency2, reference, tmpTable);
            else
            if(runningDiff > 0) // overall exch gain
              addToTmpTable(con, stmt2, "Q", lastReceiptCode, companyName, 0, runningDiff, dateReceived, currency2, reference, tmpTable);
    ...
    if(runningDiff != 0.0)
    {
      if(existsCustomer(con, stmt2, rs2, companyCode))
      {
        if(runningDiff < 0) // exch loss
          addToTmpTable(con, stmt2, "S", receiptCode, companyName, 0, -runningDiff, dateReceived, currency2, reference, tmpTable);
        else
        if(runningDiff > 0) // exch gain
          addToTmpTable(con, stmt2, "Q", receiptCode, companyName, 0, runningDiff, dateReceived, currency2, reference, tmpTable);
      }
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onBatchCr(Connection con, Statement stmt2, String currency, String year, String debtorsAccount, String dateFrom, String dateTo, String tmpTable, String dnm) throws Exception
  {
    String s;
    if(currency.length() == 0)
      s = "";
    ...
    try
    {
      ...
      accRs = accStmt.executeQuery("SELECT t2.Code, t2.Remark, t2.Amount, t2.BaseAmount, t2.TransactionDate, t2.Currency, t2.AccCode FROM joubatchl AS t2 INNER JOIN joubatch AS t1 ON t1.Code = t2.Code "
                                 + "WHERE (t1.Type = 'D' OR ((t1.Type != 'C' AND t1.Type != 'D') AND t2.AccCode = '" + debtorsAccount + "')) AND t2.DrCr = 'C' AND t2.TransactionDate >= {d '" + dateFrom + "'} AND t2.TransactionDate <= {d '"
                                 + dateTo + "'}" + s);
      double amount, baseAmount;           
      String desc;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onBatchDr(Connection con, Statement stmt2, String currency, String year, String debtorsAccount, String dateFrom, String dateTo, String tmpTable, String dnm)
                         throws Exception
  {
    String s;
    if(currency.length() == 0)
    ...
    try
    {
      ...
      accRs = accStmt.executeQuery("SELECT t2.Code, t2.Remark, t2.Amount, t2.BaseAmount, t2.TransactionDate, t2.Currency, t2.AccCode FROM joubatchl AS t2 INNER JOIN joubatch AS t1 ON t1.Code = t2.Code "
                                 + "WHERE (t1.Type = 'D' OR ((t1.Type != 'C' AND t1.Type != 'D') AND t2.AccCode = '" + debtorsAccount + "')) AND t2.DrCr = 'D' AND t2.TransactionDate >= {d '" + dateFrom + "'} AND t2.TransactionDate <= {d '"
                                 + dateTo + "'}" + s);
      double amount, baseAmount;
      String desc;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  public void processTmpTableForListing(Connection con, Statement stmt, ResultSet rs, String tmpTable, String unm, String dnm, byte[] data, byte[] fldNames,
