// =======================================================================================================================================================================================================
// System: ZaraStar: DocumentEngine: LP create listing page
// Module: LocalPurchaseListing.java
// Author: C.K.Harvey
...
public class LocalPurchaseListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "LocalPurchaseListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5025, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5025, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "LocalPurchaseListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5025, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "LocalPurchaseListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5025, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5025, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM lp WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM lp" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM lp WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM lp WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM lp" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM lp WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(SalesPerson) AS rowcount FROM lp WHERE SalesPerson LIKE '"
                                           + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT SalesPerson) AS rowcount FROM lp"); 
                   else
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM lp WHERE SalesPerson = '" + codeArg + "'"
                                           + companyCodeANDWHERE);
                   }
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT LPCode) AS rowcount FROM lp WHERE LPCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM lp" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String lpCode="", name="", supplierCode="", date="", status, salesPerson="";
    int lpCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, salesPersonPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE LPCode LIKE '" + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY LPCode";
                   }
                   else
                   {
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp" + companyCodeWHERE + " ORDER BY LPCode";
                   }

                   lpCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, LPCode, CompanyCode, CompanyName, Status FROM lp WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     lpCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM lp" + companyCodeWHERE + " ORDER BY Date";
                     
                     lpCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, LPCode, CompanyCode, CompanyName, Status FROM lp WHERE Date = {d '"
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, LPCode, CompanyName, Status FROM lp WHERE CompanyCode LIKE '" + srchStr
                       + "%' ORDER BY CompanyCode, CompanyName";
                     
                     lpCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 4; salesPersonPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM lp" + companyCodeWHERE + " ORDER BY CompanyCode";
                     
                     lpCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, LPCode, CompanyName, Status FROM lp WHERE CompanyCode = '" + codeArg
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT SalesPerson, LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE SalesPerson LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY SalesPerson, Date";
                     
                     lpCodePosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6; salesPersonPosn = 1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM lp" + companyCodeWHERE + " ORDER BY SalesPerson";
                     
                     lpCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = 1;
                   }
                   else
                   {
                     q = "SELECT SalesPerson, LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE SalesPerson = '" 
                 ...
                 while(rs.next())                  
                 {
                   lpCode       = getValue(lpCodePosn,      ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, lpCode, date, supplierCode, name, null, lpCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, lpCode, supplierCode, name, null, lpCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, supplierCode, lpCode, name, null, null, lpCode, status, imagesDir);
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, salesPerson);
                     else appendBodyLine(html, inx, salesPerson, lpCode, date, supplierCode, name, lpCode, status, imagesDir);
                   }
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = lpCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = lpCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = lpCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = lpCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE LPCode LIKE '" + srchStr 
                       + "%' AND LPCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY LPCode DESC";
                   }
                   else
                   {  
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE LPCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY LPCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM lp WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE + " ORDER BY Date DESC";
                     
                     lpCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, LPCode, CompanyCode, CompanyName, Status FROM lp" 
                       + " WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE
                       + " ORDER BY Date, LPCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM lp WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, LPCode, CompanyName, Status FROM lp WHERE CompanyCode = '" + codeArg 
                        + "' ORDER BY CompanyCode, LPCode DESC";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM lp WHERE SalesPerson <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                   ...
                   {
                     q = "SELECT SalesPerson, LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE SalesPerson = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, LPCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, lpCode, date, supplierCode, name, null, lpCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, lpCode, supplierCode, name, null, lpCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, supplierCode, lpCode, name, null, null, lpCode, status, imagesDir);
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, salesPerson);
                     else prependBodyLine(html, inx, salesPerson, lpCode, date, supplierCode, name, lpCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE LPCode LIKE '" + srchStr 
                       + "%' AND LPCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY LPCode";
                     
                     lpCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE LPCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY LPCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM lp WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, LPCode, CompanyCode, CompanyName, Status FROM lp WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND LPCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, LPCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM lp WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, LPCode, CompanyName, Status FROM lp WHERE CompanyCode = '" + codeArg
                       + "' AND LPCode > '" + codeArg2 + "' ORDER BY CompanyCode, LPCode";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT SalesPerson FROM lp WHERE SalesPerson >= '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT SalesPerson, LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE SalesPerson = '"
                       + codeArg + "' AND LPCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, LPCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT LPCode, Date, CompanyName, CompanyCode, Status FROM lp WHERE LPCode < '" + codeArg
                     + "'" + companyCodeANDWHERE + " ORDER BY LPCode DESC";
                   }
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM lp WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, LPCode, CompanyName, Status FROM lp WHERE CompanyCode = '" + codeArg
                       + "' AND LPCode < '" + codeArg2 + "' ORDER BY CompanyCode, LPCode DESC";
                 ...
                 else // inx == '4'
                 {
                   ...
                   {
                     q = "SELECT SalesPerson, LPCode, Date, CompanyCode, CompanyName, Status FROM lp WHERE SalesPerson = '" 
                       + codeArg + "' AND LPCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, LPCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Local Purchase Search Results for ";
        switch(inx)
        {
          case '1' : title += "Local Purchase Code";  break;
          case '2' : title += "Local Purchase Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Local Purchase Listing by ";        
        switch(inx)
        {
          case '1' : title += "Local Purchase Code";  break;
          case '2' : title += "Local Purchase Date";  break;
      ...
      else title = "Local Purchase Listing";

      drawingUtils.drawTitle(out, false, false, "LocalPurchaseListing", "", title, "5025", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Local Purchase Code");  break;
        case '2' : scout(out, bytesOut, "Local Purchase Date");  break;
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Local Purchase Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/LocalPurchasePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/LocalPurchaseListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/LocalPurchaseListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5025", "", "LocalPurchaseListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Code &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Code &nbsp;</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Sales Person &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Local Purchase Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Local Purchase Date");  break;
                              ...
                              String status, String imagesDir) throws Exception
  {
    ...
    { 
      ...
      case '3' : html[0] += "<td nowrap><p>" + one + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + three + "&nbsp;</td>";
                 html[0] += "<td nowrap><a href=\"javascript:fetch('" + s + "')\">" + two + "</a>&nbsp;</td></tr>\n";
                 break;
