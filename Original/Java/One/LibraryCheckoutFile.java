...
public class LibraryCheckoutFile extends HttpServlet
{
                      ...
                      String den, String dnm, String bnm, String p1, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "LibraryCheckoutFile", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "function checkout(docName,directory,userName){p2=sanitise(docName);p3=sanitise(directory);");
    scoutln(out, bytesOut, "var libDir=sanitise(document.forms[0].libDir.value);");
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/LibraryCheckoutFileExecute?unm=" + unm + "&sid=" + sid + "&uty="
                           + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           + "&p2=\"+p2+\"&p3=\"+p3+\"&p4=\"+userName+\"&libDir=\"+libDir+\"&p1=" + docCode + "\";");
    ...
    libraryUtils.outputPageFrame(con, stmt, rs, out, req, "LibraryCheckoutFile", "12004", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td><input type=hidden name=\"libDir\" value='" + libraryDir + directory[0].substring(1)
                         + "/'></td></tr>");

    scoutln(out, bytesOut, "<tr><td colspan=2><p><a href=\"javascript:checkout('" + docName[0] + "','" + directory[0] 
                         + "','" + userName[0] + "')\">Check-Out</a> the Document</td></tr>");
