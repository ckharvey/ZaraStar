// ==================================================================================================================================================================================================================================================
// System: ZaraStar: Analytic: Purchases
// Module: AnalyticsPurchases.java
// Author: C.K.Harvey
...
public class AnalyticsPurchases extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p5  = req.getParameter("p5"); // purchases account or not
    ...
    catch(Exception e)
    {
      ...
      System.out.println("6007d: " + e);
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AnalyticsPurchases", bytesOut);
                    ...
                    String p1, String p2, String p3, String p4, String p5, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "AnalyticsPurchases", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "AnalyticsPurchases", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    set(con, stmt, stmt2, rs, rs2, out, req, dateFrom, dateTo, p1, p2, p3, plain, p5, unm, sid, uty, men, den, dnm, bnm, workingDir, localDefnsDir, defnsDir,
        bytesOut);
  ...
  private void set(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req,
                   String dateFrom, String dateTo, String account, String p2, String p3, boolean plain, String p5, String unm, String sid, String uty,
                   String men, String den, String dnm, String bnm, String workingDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6007", "", true, plain, account, p2, p3, p5, "", "AnalyticsPurchases", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    String year = generalUtils.getFromDefnFile(unm, "accounts.dfn", workingDir, "");

    if(! plain)
    {
      pageFrameUtils.drawTitle(out, true, plain, "AnalyticsPurchases", account, dateFrom, dateTo, p5, "", "GL Account: " + account + " - " + accountsUtils.getAccountDescriptionGivenAccCode(account, year, dnm, localDefnsDir, defnsDir) + " ("
    ...
    calc(con, stmt, stmt2, rs, rs2, account, year, dateFrom, dateTo, p5, unm, dnm, workingDir, localDefnsDir, defnsDir, tmpTable);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  public void calc(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String account, String year, String dateFrom, String dateTo, String p5, String unm, String dnm, String workingDir, String localDefnsDir,
                   String defnsDir, String tmpTable) throws Exception
  {
    ...
    onPurchaseInvoiceLine(con, stmt, stmt2, rs, rs2, account, dateFrom, dateTo, tmpTable);

    onPurchaseCreditNoteLine(con, stmt, stmt2, rs, rs2, account, dateFrom, dateTo, tmpTable);

    if(p5.equals("P") && (account.equals(accountsUtils.getPurchasesAccCode(year, dnm, localDefnsDir, defnsDir))))
    {
      ...
      onSalesCreditNoteLine(useWAC, con, stmt, stmt2, rs, rs2, dateFrom, dateTo, previousFinancialYearStartDate, tmpTable, dnm);
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onInvoiceLine(boolean useWAC, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo, String financialYearStartDate, String salesAccount, String tmpTable, String dnm) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t2.InvoiceCode, t1.CompanyName, t2.Amount2, t1.Date, t1.Currency, t2.ItemCode, t2.Quantity, "
                         + "t1.GroupDiscount, t1.GroupDiscountType, t1.TotalTotal, t1.GSTTotal, t2.SOCode "
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String getSOCodeFromInvoiceL(Connection con, Statement stmt, ResultSet rs, String invoiceCode, String itemCode) throws Exception
  {
    String soCode = "";
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT SOCode FROM invoicel WHERE InvoiceCode = '" + invoiceCode + "' AND ItemCode = '" + itemCode + "'");
  ...
  private void onSalesCreditNoteLine(boolean useWAC, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo,
                                String previousFinancialYearStartDate, String tmpTable, String dnm) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t2.CNCode, t1.CompanyName, t2.Amount2, t2.Amount, t1.Date, t1.Currency, t2.ItemCode, t2.Quantity, t2.InvoiceCode FROM creditl AS t2 INNER JOIN credit AS t1 ON t1.CNCode = t2.CNCode "
                         + "WHERE t2.CostOfSaleAdjustment = 'Y' AND t1.Status != 'C' AND t1.Date >= {d '" + dateFrom + "'} AND t1.Date <= {d '" + dateTo + "'} ORDER BY t2.CNCode");

    double bAmt = 0.0, baseAmount, quantity;
    String itemCode, cnCode, companyName="", date="", currency="", lastCurrency="", lastDate="", lastCNCode = "", lastCompanyName = "", dateOfInvoice, invoiceCode;
    boolean first = true;
    ...
    {    
      ...
      {      
        ...
        if(baseAmount != 0.0)
        {
          ...
          {
            addToTmpTable(con, stmt2, "W", lastCNCode, lastCompanyName, 0, bAmt, lastDate, lastCurrency, tmpTable);
            bAmt = baseAmount;
            lastCNCode   = cnCode;
            lastCurrency = currency;
            lastDate     = date;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void onPurchaseDebitNote(Connection con, Statement stmt, Statement stmt2, ResultSet rs, String dateFrom, String dateTo, String tmpTable) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT PDNCode, CompanyName, TotalTotal, BaseTotalTotal, GSTTotal, BaseGSTTotal, Date, Currency FROM pdebit WHERE Status != 'C' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    double amount, baseAmount;           
            
    while(rs.next())
    {    
      amount     = generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(3)), '2') - generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(5)), '2');
      baseAmount = generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(4)), '2') - generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(6)), '2');

      addToTmpTable(con, stmt2, "E", rs.getString(1), rs.getString(2), amount, baseAmount, rs.getString(7), rs.getString(8), tmpTable);
    }
