...
public class LibraryEditDocumentDetails extends HttpServlet
{
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String p1, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "LibraryEditDocumentDetails", imagesDir, localDefnsDir, defnsDir, bytesOut);
  ...
  { 
    scoutln(out, bytesOut, "<html><head><title>Library - Edit Document Details</title>");
    ...
    libraryUtils.outputPageFrame(con, stmt, rs, out, req, "LibraryEditDocumentDetails", "12001", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
    
    scoutln(out, bytesOut, "<FORM ACTION=\"LibraryEditDocumentDetailsUpdate\" ENCTYPE=\"application/x-www-form-urlencoded\" METHOD=POST>");
    ...
    libraryUtils.drawTitle(con, stmt, rs, req, out, "Edit Document Details", "12001", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
   ...
   scoutln(out, bytesOut, "<td><input type=hidden name=docName value='" + docName[0] + "'></td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td><p>File Size:</td><td><p>" + generalUtils.formatNumeric(fileSize[0], '0')
                         + " bytes</td></tr>");
