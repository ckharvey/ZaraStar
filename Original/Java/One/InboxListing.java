// =======================================================================================================================================================================================================
// System: ZaraStar: DocumentEngine: Inbox create listing page
// Module: InboxListing.java
// Author: C.K.Harvey
...
public class InboxListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "InboxListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3139, bytesOut[0], 0, "ERR:" + codeArg);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den,
                    String dnm, String bnm, String inx,  String operation, String callType, String codeArg, String codeArg2,
                    String srchStr, String firstRecNum, String lastRecNum, int maxRows, int numRecs, String companyCode,
                    int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 3139, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "InboxListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3139, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "InboxListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3139, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3139, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM inbox" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM inbox WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM inbox" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(FAO) AS rowcount FROM inbox WHERE FAO LIKE '"
                                           + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT FAO) AS rowcount FROM inbox"); 
                   else
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox WHERE FAO = '" + codeArg + "'"
                                           + companyCodeANDWHERE);
                   }
                   break;
        case '5' : if(callType == 'S')
                     r = stmt.executeQuery("SELECT COUNT(CustomerReference) AS rowcount FROM inbox WHERE CustomerReference LIKE '" 
                                           + srchStr + "%'" + companyCodeANDWHERE);
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT CustomerReference) AS rowcount FROM inbox" + companyCodeWHERE);
                   else
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox WHERE CustomerReference = '" + codeArg + "'"
                                           + companyCodeANDWHERE);
                   }
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT InboxCode) AS rowcount FROM inbox WHERE InboxCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM inbox" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String inboxCode="", name="", customerCode="", date="", customerReference="", status, fao="";
    int inboxPosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, customerReferencePosn, faoPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE InboxCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY InboxCode";
                   }
                   else
                   {
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox" + companyCodeWHERE
                       + " ORDER BY InboxCode";
                   }

                   inboxPosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; customerReferencePosn = 4; statusPosn = 6; 
                   faoPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, InboxCode, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     inboxPosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 5; customerReferencePosn = 4; 
                     statusPosn = 6; faoPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM inbox" + companyCodeWHERE + " ORDER BY Date";
                     inboxPosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; customerReferencePosn = -1; 
                     statusPosn = -1; faoPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, InboxCode, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE Date = {d '"
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, InboxCode, CompanyName, CustomerReference, Status FROM inbox WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     inboxPosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; customerReferencePosn = 4; 
                     statusPosn = 5; faoPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM inbox" + companyCodeWHERE + " ORDER BY CompanyCode";    
                     inboxPosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; customerReferencePosn = -1; 
                     statusPosn = -1; faoPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, InboxCode, CompanyName, CustomerReference, Status FROM inbox WHERE CompanyCode = '" + codeArg
                 ...
                 if(inx == '4')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT FAO, InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE FAO LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY FAO, Date";
                     inboxPosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 6; customerReferencePosn = 5; 
                     statusPosn = 7; faoPosn = 1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT FAO FROM inbox" + companyCodeWHERE + " ORDER BY FAO";
                     inboxPosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; customerReferencePosn = -1; 
                     statusPosn = -1; faoPosn = 1;
                   }
                   else
                   {
                     q = "SELECT FAO, InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE FAO = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY FAO";
                     inboxPosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 6; customerReferencePosn = 5; 
                     statusPosn = 7; faoPosn = 1;
                   }                   
                 }  
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CustomerReference, InboxCode, CompanyCode, CompanyName, Status FROM inbox WHERE CustomerReference LIKE '" + srchStr 
                       + "%'" + companyCodeANDWHERE + " ORDER BY CustomerReference, InboxCode";
                     inboxPosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; customerReferencePosn = 1; 
                     statusPosn = 5; faoPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerReference FROM inbox" + companyCodeWHERE + " ORDER BY CustomerReference";
                     inboxPosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; customerReferencePosn = 1; 
                     statusPosn = -1; faoPosn = -1;
                   }  
                   else
                   {
                     q = "SELECT CustomerReference, InboxCode, CompanyCode, CompanyName, Status FROM inbox WHERE CustomerReference = '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY CustomerReference";
                     inboxPosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; customerReferencePosn = 1; 
                     statusPosn = 5; faoPosn = -1;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   inboxCode         = getValue(inboxPosn,             ' ', rs, rsmd);
                   date              = getValue(datePosn,              'D', rs, rsmd);
                   customerCode      = getValue(companyCodePosn,       ' ', rs, rsmd);
                   name              = getValue(companyNamePosn,       ' ', rs, rsmd);
                   customerReference = getValue(customerReferencePosn, ' ', rs, rsmd);
                   fao               = getValue(faoPosn,               ' ', rs, rsmd);
                   status            = getValue(statusPosn,            ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, inboxCode, date, customerCode, customerReference, name, null, inboxCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, inboxCode, customerCode, customerReference, name, null, inboxCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, inboxCode, name, customerReference, null, null, inboxCode, status, imagesDir);
                   }   
                   else
                   if(inx == '4')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, fao);
                     else appendBodyLine(html, inx, fao, inboxCode, date, customerCode, customerReference, name, inboxCode, status, imagesDir);
                   }
                   else // inx == '5'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, customerReference);
                     else appendBodyLine(html, inx, customerReference, inboxCode, customerCode, name, null, null, inboxCode, status, imagesDir);
                   }
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = inboxCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = inboxCode;
                     else
                     if(inx == '2')
                     ...
                     if(inx == '4')
                       firstEntryOnPage[0] = fao;
                     else // inx == '5'
                       firstEntryOnPage[0] = customerReference;
                   }
                 }
                 
                 rs.close();
                                 
                 lastCodeOnPage[0] = inboxCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = inboxCode;
                 else
                 if(inx == '2')
                 ...
                 if(inx == '4')
                   lastEntryOnPage[0] = fao;
                 else // inx == '5'
                   lastEntryOnPage[0] = customerReference;
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE InboxCode LIKE '" + srchStr 
                       + "%' AND InboxCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY InboxCode DESC";
                   }
                   else
                   {  
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE InboxCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY InboxCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM inbox WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE + " ORDER BY Date DESC";
                     inboxPosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; customerReferencePosn = -1; 
                     statusPosn = -1; faoPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, InboxCode, CompanyCode, CustomerReference, CompanyName, Status FROM inbox" 
                       + " WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE
                       + " ORDER BY Date, InboxCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM inbox WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, InboxCode, CompanyName, CustomerReference, Status FROM inbox WHERE CompanyCode = '" + codeArg 
                        + "' ORDER BY CompanyCode, InboxCode DESC";
                 ...
                 if(inx == '4')
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT FAO FROM inbox WHERE FAO <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                       + companyCodeANDWHERE + " ORDER BY FAO DESC";
                   ...
                   {
                     q = "SELECT FAO, InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE FAO = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY FAO, InboxCode DESC";
                 ...
                 else // inx == '5'  
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerReference FROM inbox WHERE CustomerReference <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                       + companyCodeANDWHERE + " ORDER BY CustomerReference DESC";
                   ...
                   {
                     q = "SELECT CustomerReference, InboxCode, CompanyCode, CompanyName, Status FROM inbox WHERE CustomerReference = '" + codeArg
                       + companyCodeANDWHERE + "' ORDER BY CustomerReference, InboxCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, inboxCode, date, customerCode, customerReference, name, null, inboxCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, inboxCode, customerCode, customerReference, name, null, inboxCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, inboxCode, name, customerReference, null, null, inboxCode, status, imagesDir);
                   }  
                   else
                   if(inx == '4')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, fao);
                     else prependBodyLine(html, inx, fao, inboxCode, date, customerCode, customerReference, name, inboxCode, status, imagesDir);
                   }  
                   else // inx == '5'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, customerReference);
                     else prependBodyLine(html, inx, customerReference, inboxCode, customerCode, name, null, null, inboxCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE InboxCode LIKE '" + srchStr 
                       + "%' AND InboxCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY InboxCode";
                     
                     inboxPosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; customerReferencePosn = 4; 
                     statusPosn = 6; faoPosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE InboxCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY InboxCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM inbox WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, InboxCode, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND InboxCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, InboxCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM inbox WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, InboxCode, CompanyName, CustomerReference, Status FROM inbox WHERE CompanyCode = '" + codeArg
                       + "' AND InboxCode > '" + codeArg2 + "' ORDER BY CompanyCode, InboxCode";
                 ...
                 if(inx == '4')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT FAO FROM inbox WHERE FAO >= '" + codeArg + "'" + companyCodeANDWHERE
                       + " ORDER BY FAO";
                   ...
                   {
                     q = "SELECT FAO, InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE FAO = '"
                       + codeArg + "' AND InboxCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY FAO, InboxCode";
                 ...
                 {  
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CustomerReference FROM inbox WHERE CustomerReference > '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT CustomerReference, InboxCode, CompanyCode, CompanyName, Status FROM inbox WHERE CustomerReference = '" + codeArg
                       + "' AND InboxCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY CustomerReference, InboxCode";
                     
                     inboxPosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 5; customerReferencePosn = 1; 
                     statusPosn = 5; faoPosn = -1;
                   }
                 }  
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT InboxCode, Date, CompanyCode, CustomerReference, Status FROM inbox WHERE InboxCode < '" + codeArg 
                     + "'" + companyCodeANDWHERE + " ORDER BY InboxCode DESC";
                   }
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM inbox WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, InboxCode, CompanyName, CustomerReference, Status FROM inbox WHERE CompanyCode = '" + codeArg
                       + "' AND InboxCode < '" + codeArg2 + "' ORDER BY CompanyCode, InboxCode DESC";
                 ...
                 if(inx == '4')
                 {
                   ...
                   {
                     q = "SELECT FAO, InboxCode, Date, CompanyCode, CustomerReference, CompanyName, Status FROM inbox WHERE FAO = '" 
                       + codeArg + "' AND InboxCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY FAO, InboxCode DESC";
                 ...
                 {  
                   ...
                   {  
                     q = "SELECT DISTINCT CustomerReference FROM inbox WHERE CustomerReference < '" + codeArg + "'" + companyCodeANDWHERE
                       + " ORDER BY CustomerReference DESC";
                   ...
                   {
                     q = "SELECT CustomerReference, InboxCode, CompanyCode, CompanyName, Status FROM inbox WHERE CustomerReference = '" + codeArg
                       + "' AND InboxCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY CustomerReference, InboxCode DESC";
                     inboxPosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; customerReferencePosn = 1;
                     statusPosn = 5; faoPosn = -1;
                   }
                 }
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Inbox Record Results for ";
        switch(inx)
        {
          case '1' : title += "Inbox Record Code";   break;
          case '2' : title += "Inbox Record Date";   break;
          case '3' : title += "Customer Code";       break;
          case '4' : title += "Attention";           break;
          case '5' : title += "Customer Reference";  break;
        }
      ...
      else title = "Inbox Record Listing";

      drawingUtils.drawTitle(out, false, false, "InboxListing", "", title, "3139", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Inbox Record Code");   break;
        case '2' : scout(out, bytesOut, "Inbox Record Date");   break;
        case '3' : scout(out, bytesOut, "Customer Code");       break;
        case '4' : scout(out, bytesOut, "Attention");           break;
        case '5' : scout(out, bytesOut, "Customer Reference");  break;
      }
    }
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Inbox Record Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/InboxPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/InboxListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/InboxListing?unm=" + unm + "&sid=" + sid
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Reference</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Reference</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Reference</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Attention &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Reference</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      case '5' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Reference &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Inbox Record Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Inbox Record Date");   break;
      case '3' : scoutln(out, bytesOut, "Customer Code");       break;
      case '4' : scoutln(out, bytesOut, "Attention");           break;
      case '5' : scoutln(out, bytesOut, "Customer Reference");  break;
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six,
                              String code, String status, String imagesDir) throws Exception
  {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six,
                               String code, String status, String imagesDir) throws Exception
  {
    String s = checkCode(code);
