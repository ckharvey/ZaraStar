// =======================================================================================================================================================================================================
// System: ZaraStar: Customer: Change occurrences of cust code
// Module: CustomerChangeOccurrences.java
// Author: C.K.Harvey
...
public class CustomerChangeOccurrences extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 4225, bytesOut[0], 0, "ERR:");
                    ...
                    String bnm, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4225, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 4225, bytesOut[0], 0, "ACC:");
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 4225, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4225, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Replace a Customer Code</title>");
    ...
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/CustomerChangeOccurrencesExecute?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "4225", "", "CustomerChangeOccurrences", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Replace a Customer Code", "4225", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<table id=\"page\" width=100% border=0>");

    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");

    scoutln(out, bytesOut, "<tr><td colspan=2><p>This service replaces one customer code by another. All documents are examined and the ");
    scoutln(out, bytesOut, "<i>existing code</i> is replaced by the <i>new code</i>.");
    scoutln(out, bytesOut, "<br><br>Notes:<br><br>1. The following documents are updated: Quotation, Sales Order, Works Order, Picking List, ");
    scoutln(out, bytesOut, "Delivery Order, Invoice, Order Confirmation, Proforma Invoice, Receipt, Receipt Voucher, Credit Note, Debit Note.");

    scoutln(out, bytesOut, "<br><br>2. Inbox records are <b><i>not</i></b> updated.");
    scoutln(out, bytesOut, "<br><br>3. If a customer record exists for the <i>existing code</i>, it is deleted.</td></tr>");

    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td nowrap><p>Existing Customer Code &nbsp;</td><td><input type=text maxlength=20 size=20 name=oldCode></td></tr>");
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
