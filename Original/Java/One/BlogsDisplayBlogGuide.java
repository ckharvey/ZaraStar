...
public class BlogsDisplayBlogGuide extends HttpServlet
{
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>BlogGuide</title>");
    ...
    scoutln(out, bytesOut, "function fetch(svcs){");
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/BlogsDisplayBlogGuideDisplay?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\" + escape(svcs);}");
    ...
    {
      scoutln(out, bytesOut, "function create(title,svcs){var p1=sanitise(title);");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/BlogsEdit?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=\"+p1+\"&p3=\" + escape(svcs);}");

      isSysAdminOrDBAdmin = true;
    }

    scoutln(out, bytesOut, "function pdf(){this.location.href=\"/central/servlet/BlogsBlogGuideCreatePDF?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "\";}");
    ...
    blogGuideUtils.outputPageFrame(con, stmt, rs, out, req, "", "BlogsDisplayBlogGuide", "8110", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    blogGuideUtils.drawTitle(con, stmt, rs, req, out, "BlogGuide: " + name, "8110", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
        ...
        if(infoRs.next()) // just-in-case
        {
          ...
          if(! anyBlogsForAService(conInfo, infoStmt2, infoRs2, services))
          {
            if(isSysAdminOrDBAdmin)
              scoutln(out, bytesOut, "<td nowrap>" + indent + "<a href=\"javascript:create('" + generalUtils.sanitise2(title) + "','" + generalUtils.sanitise2(services) + "')\">" + title + "</a></td>");
            else scoutln(out, bytesOut, "<td nowrap><p>" + indent+title + "</td>");
          }
          else scoutln(out, bytesOut, "<td nowrap>" + indent + "<a href=\"javascript:fetch('" + generalUtils.sanitise2(services) + "')\">" + title + "</a></td>");
