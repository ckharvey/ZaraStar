// =======================================================================================================================================================================================================
// System: ZaraStar Info: Consolidations Obsolete Stock List
// Module: ConsolidationsObsoleteStock.java
// Author: C.K.Harvey
...
public class ConsolidationsObsoleteStock extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ConsolidationsObsoleteStock", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6916, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                    String bnm, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "6916", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6916, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6916", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6916, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6916, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Obsolete Stock List</title>");

    scoutln(out, bytesOut, "<SCRIPT LANGUAGE=\"JavaScript\">");

    scoutln(out, bytesOut, "function generate(which){");
    scoutln(out, bytesOut, "var p2=sanitise(document.forms[0].mfr.value);");
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/ConsolidationsObsoleteStockExecute?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=\"+p2+\"&p1=\"+which;}");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6916", "", "ConsolidationsObsoleteStock", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Obsolete Stock List", "6916",  unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
