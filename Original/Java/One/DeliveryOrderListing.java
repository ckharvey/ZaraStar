// =======================================================================================================================================================================================================
// System: ZaraStar Document: DO create listing page
// Module: DeliveryOrderListing.java
// Author: C.K.Harvey
...
public class DeliveryOrderListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "DeliveryOrderListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4066, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4066, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "DeliveryOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4066, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "DeliveryOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4066, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4066, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                           ...
                           String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM do WHERE date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM do" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM do WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM do WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM do" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM do WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT DOCode) AS rowcount FROM do WHERE DOCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT DOCode) AS rowcount FROM do" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM do" + companyCodeWHERE);
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
    ...
    String doCode="", name="", customerCode="", date="", plCode="", invCode="", custPOCode="", status, returned="";
    int doCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, plCodePosn, invoiceCodePosn, custPOCodePosn,
        returnedPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, InvoiceCode, Returned, Status FROM do WHERE DOCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY DOCode";
                   }
                   else
                   {
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, invoiceCode, Returned, Status FROM do " + companyCodeWHERE
                       + "ORDER BY DOCode";
                   }

                   doCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = -1; plCodePosn = 4; invoiceCodePosn = 5;
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, DOCode, CompanyCode, CompanyName, Returned, Status FROM do WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
    
                     doCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; plCodePosn = -1; invoiceCodePosn = -1;
                     custPOCodePosn = -1; returnedPosn = 5; statusPosn = 6; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM do " + companyCodeWHERE + " ORDER BY Date";

                     doCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; plCodePosn = -1;
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, DOCode, Date, CompanyName, Returned, Status FROM do WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, DOCode";

                     doCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 4; plCodePosn = -1; invoiceCodePosn = -1;
                     custPOCodePosn = -1; returnedPosn = 5; statusPosn = 6; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM do " + companyCodeWHERE + "ORDER BY CompanyCode";    
                     doCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; plCodePosn = -1;
                     invoiceCodePosn = -1; custPOCodePosn = -1; returnedPosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT CompanyCode, DOCode, Date, CompanyName, Returned, Status FROM do WHERE CompanyCode = '" + codeArg
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT DOCode, CompanyCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE DOCode LIKE '"
                       + srchStr + "%' ORDER BY DOCode, PORefNum, CompanyCode";
                     
                     doCodePosn = 1; datePosn = 3; companyCodePosn = 2; companyNamePosn = 5; plCodePosn = -1; invoiceCodePosn = -1;
                     custPOCodePosn = 4; returnedPosn = 6; statusPosn = 7; 
                   }
                   else
                   {
                     q = "SELECT DOCode, CompanyCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE DOCode = '"
                       + codeArg + "' ORDER BY DOCode";
                 ...
                 while(rs.next())                  
                 {
                   doCode       = getValue(doCodePosn,      ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, doCode, date, customerCode, plCode, invCode, returned, status, doCode, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date, cssFormat);
                     else appendBodyLine(html, inx, date, doCode, customerCode, name, null, returned, status, doCode, imagesDir, cssFormat);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, doCode, date, name, null, returned, status, doCode, imagesDir, cssFormat);
                   }   
                   else // inx == '4'
                     appendBodyLine(html, inx, doCode, customerCode, date, custPOCode, name, returned, status, doCode, imagesDir, cssFormat);
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = doCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = doCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = doCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = doCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, InvoiceCode, Returned, Status FROM do WHERE DOCode LIKE '" + srchStr 
                       + "%' AND DOCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY DOCode DESC";
                   }
                   else
                   {  
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, InvoiceCode, Returned, Status FROM do WHERE DOCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                         + companyCodeANDWHERE + " ORDER BY DOCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM do WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, DOCode, CompanyCode, CompanyName, Returned, Status FROM do WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, DOCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM do WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, DOCode, Date, CompanyName, Returned, Status FROM do WHERE CompanyCode = '" + codeArg
                       + "' ORDER BY CompanyCode, DOCode DESC";
                 ...
                 {  
                   ...
                   {
                     q = "SELECT DOCode, CompanyCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE DOCode LIKE '"
                       + srchStr + "%' AND DOCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY DOCode DESC";
                   }
                   else
                   {  
                     q = "SELECT DOCode, CompanyCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE "
                       + "DOCode <= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY DOCode DESC";
                   }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, doCode, date, customerCode, plCode, invCode, returned, status, doCode, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date, cssFormat);
                     else prependBodyLine(html, inx, date, doCode, customerCode, name, null, returned, status, doCode, imagesDir, cssFormat);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, doCode, date, name, null, returned, status, doCode, imagesDir, cssFormat);
                   }  
                   else // inx == '4'
                     prependBodyLine(html, inx, doCode, customerCode, date, custPOCode, name, returned, status, doCode, imagesDir, cssFormat);
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, InvoiceCode, Returned, Status FROM do WHERE DOCode LIKE '" + srchStr 
                       + "%' AND DOCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY DOCode";
                   ...
                   {                   
                     q = "SELECT DOCode, Date, CompanyCode, PLCode, InvoiceCode, Returned, Status FROM do WHERE DOCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY DOCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM do WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, DOCode, CompanyCode, CompanyName, Returned, Status FROM do WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND DOCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, DOCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM do WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, DOCode, Date, CompanyName, Returned, Status FROM do WHERE CompanyCode = '" + codeArg
                       + "' AND DOCode > '" + codeArg2 + "' ORDER BY CompanyCode, DOCode";
                 ...
                 {  
                   q = "SELECT DOCode, CompanyCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE DOCode = '" + codeArg
                     + "' AND PORefNum > '" + codeArg2 + "' ORDER BY DOCode";
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM do WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, DOCode, Date, PORefNum, CompanyName, Returned, Status FROM do WHERE CompanyCode = '"
                       + codeArg + "' AND DOCode < '" + codeArg2 + "' ORDER BY CompanyCode, PLDOde DESC";
  
                     doCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 5; plCodePosn = -1; invoiceCodePosn = -1;
                     custPOCodePosn = -1; returnedPosn = 5; statusPosn = 6; 
                   }  
                 }  
                 else // inx == '4'
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Delivery Order List Search Results for ";
        switch(inx)
        {
          case '1' : title += "Delivery Order Code";  break;
          case '2' : title += "Delivery Order Date";  break;
          case '3' : title += "Customer Code";        break;
          case '4' : title += "Delivery Order Code";  break;
        }
      ...
      if(operation == 'X')
      {
        title = "Delivery Order Listing by ";
        switch(inx)
        {
          case '1' : title += "Delivery Order Code";  break;
          case '2' : title += "Delivery Order Date";  break;
      ...
      else title = "Delivery Order Listing";

      drawingUtils.drawTitle(out, false, false, "DeliveryOrderListing", "", title, "4066", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Delivery Order Code");  break;
        case '2' : scout(out, bytesOut, "Delivery Order Date");  break;
        case '3' : scout(out, bytesOut, "Customer Code");        break;
        case '4' : scout(out, bytesOut, "Delivery Order Code");  break;
      }
    }
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Delivery Order Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/DeliveryOrderPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/DeliveryOrderListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/DeliveryOrderListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4066", "", "DeliveryOrderListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
      ...
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Delivery Order Date &nbsp;</td>");
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Delivery Order Date"); break;
