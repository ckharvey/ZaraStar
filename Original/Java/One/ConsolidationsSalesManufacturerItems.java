// =======================================================================================================================================================================================================
// System: ZaraStar Info: Consolidations Sales for Manufacturer Items
// Module: ConsolidationsSalesManufacturerItems.java
// Author: C.K.Harvey
...
public class ConsolidationsSalesManufacturerItems extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ConsolidationsSalesManufacturerItems", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6908, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                    String bnm, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6908", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6908, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6908, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
                   ...
                   String bnm, String imagesDir, String reportsDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "var p2=sanitise(document.forms[0].salesPerson.value);");
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/ConsolidationsSalesManufacturerItemsExecute?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=\"+p1+\"&p2=\"+p2;}");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "6908", "", "ConsolidationsSalesManufacturerItems", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Sales for Manufacturer Items", "6908",  unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td colspan=4 nowrap><p><a href=\"javascript:generate()\">Generate</a></td></tr>");
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
  ...
  private void getSalesPeople(Connection con, Statement stmt, ResultSet rs, PrintWriter out, int[] bytesOut) throws Exception
  {
    try
    {
      scoutln(out, bytesOut, "<tr><td><p>SalesPerson</td><td><select name=\"salesPerson\">");
      ...
      while(rs.next())                  
      {
        ...
        scoutln(out, bytesOut, "<option name=" + generalUtils.replaceSpacesWith20(name) + ">" + name);
      }
