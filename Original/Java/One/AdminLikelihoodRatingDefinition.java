// =================================================================================================================================
// System: ZaraStar AdminEngine: Likelihood Rating Definition
// Module: AdminLikelihoodRatingDefinition.java
// Author: C.K.Harvey
...
public class AdminLikelihoodRatingDefinition extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "AdminLikelihoodRatingDefinition", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7063, bytesOut[0], 0, "ERR:");
                      ...
                      String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "7063", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7063, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "7063", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7063, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 7063, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush();
  }
  ...
  private void create(Connection con, Statement stmt, ResultSet rs, PrintWriter out, String unm, String sid, String uty, String men,
                        ...
                        throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Admin - Likelihood Rating Definition</title>");
    ...
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/AdminLikelihoodRatingDefinitionUpdate?unm=\" + escape('" + unm
    ...
    scoutln(out, bytesOut, "{");
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/AdminLikelihoodRatingDefinitionDelete?unm=\" + escape('" + unm
                         + "') + \"&sid=\" + escape('" + sid + "') + \"&uty=\" + escape('" + uty
                         + "') + \"&men=\" + escape('" + men + "') + \"&den=\" + escape('" + den + "') + \"&bnm=\" + escape('" + bnm
    ...
    scoutln(out, bytesOut, "this.location.replace(\"/central/servlet/AdminLikelihoodRatingDefinition?unm=" + unm + "&sid=" + sid + "&uty="
    ...
    dashboardUtils.drawTitle(out, "Quotation Likelihood Ratings", "7063", unm, sid, uty, men, den, dnm, bnm, bytesOut);
    ...
    scoutln(out, bytesOut, "<td nowrap><p>&nbsp; Likelihood Rating &nbsp;</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td><span id=\"namebox\"><input type=text name=name size=40 maxlength=40></span></td></tr>");
    
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td colspan=3><span id=\"option\"><p><a href=\"javascript:updateN()\">Add the Likelihood Rating</a></span></td></tr>");
  ...
  private void list(PrintWriter out, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT * FROM quolike");
