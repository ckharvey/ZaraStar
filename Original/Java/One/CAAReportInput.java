// =======================================================================================================================================================================================================
// System: ZaraStar Analytic: Get statements report params
// Module: CAAReportInput.java
// Author: C.K.Harvey
...
public class CAAReportInput extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "CAAReportInput", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den,
                    String dnm, String bnm, int[] bytesOut) throws Exception
   {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "1013a", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "1013a", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1013, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 1013, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush(); 
  }
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Creditors Ageing Report Parameters</title>");
    ...
    scoutln(out, bytesOut, "window.location.href='/central/servlet/CAAReportGenerate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
                         + "&dnm=" + dnm + "&bnm=" + bnm + "&p1='+type+'&p2='+dateFrom+'&p3='+dateTo+'&p4='+ignoreZero+'&p5='+range1+'&p6='+range2"
                         + "+'&p7='+range3+'&p8='+range4+'&p10='+order+'&p9='+supplier;}");
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "1013", "", "CAAReport", unm, sid, uty, men, den, dnm, bnm, localDefnsDir,
                          defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Creditors Ageing Generation", "1013",  unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<td nowrap><p>Supplier Code &nbsp;<input name=order type=radio checked>&nbsp; Supplier Name &nbsp;");
