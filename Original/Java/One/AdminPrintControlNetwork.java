...
public class AdminPrintControlNetwork extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    String unm="", sid="", uty="", men="", den="", dnm="", bnm="", p1="", p2="", p3="", p4="", p5="", p8="", p9="";
    ...
    try
    {
      ...
      p9  = req.getParameter("p9"); // docCode
      ...
      doIt(out, req, unm, sid, uty, men, den, dnm, bnm, p1, p2, p3, p4, p5, p8, p9, bytesOut);
    }
    catch(Exception e)
    {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, String ptr, String returnServletAndParams, String returnMsg, String docType,
                    String docCode, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "7400a", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 7400, bytesOut[0], 0, "SID:" + p1 + " " + ptr);
    ...
    sendJob(unm, dnm, p1, ptr);

    try
    {
      if(docType.equals("D"))
      {
        sendNotificationsDO(con, stmt, rs, unm, docCode, ptr);
      }
    }
    catch(Exception e)
    {
      System.out.println("7400a: " + e);
    }

    messagePage.msgScreen(false, out, req, 21, unm, sid, uty, men, den, dnm, bnm, "7400", returnServletAndParams, returnMsg, "", imagesDir, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 7400, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1 + " " + ptr);
...
//    fho.writeBytes("%%EndComments\n");
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void sendNotificationsDO(Connection con, Statement stmt, ResultSet rs, String unm, String doCode, String ptrName) throws Exception
  {
            if(1==1)return;
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT Option1, Option2 FROM notifypeople WHERE Type = '1'");

      String people, alertSender, userCode;
      int x, len;

      while(rs.next())
      {
        people      = rs.getString(1);
        alertSender = rs.getString(2);
        ...
        {
          userCode = "";
          while(x < len && people.charAt(x) != ' ' && people.charAt(x) != '\t' && people.charAt(x) != ',' && people.charAt(x) != '\n' && people.charAt(x) != '\r' && people.charAt(x) != ':' && people.charAt(x) != ';')
            userCode += people.charAt(x++);

          // directoryUtils.createNotification(con, stmt, rs, "DO: " + doCode + " has been printed to " + ptrName + " [" + unm + "]", userCode, "O", "DO:" + doCode);
