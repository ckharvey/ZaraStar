...
// Module: ConsolidationsGPProductsExecute.java
// Author: C.K.Harvey
...
public class ConsolidationsGPProductsExecute extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ConsolidationsGPProductsExecute", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6914, bytesOut[0], 0, "ERR:" + p1+":"+p2);
                    ...
                    String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6914, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsGPProducts", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6914, bytesOut[0], 0, "ACC:" + p1+":"+p2);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsGPProducts", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6914, bytesOut[0], 0, "SID:" + p1+":"+p2);
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    {
      String fName;
      if(p2.equals("-")) fName = "6914.csv"; else fName = p2 + "-6914.csv";
      
      fh = generalUtils.create(workingDir + fName);

      processCSV(fh, con, stmt, stmt2, rs, rs2, out, p2, bytesOut);
      
      generalUtils.fileClose(fh);

      download(res, workingDir, fName, bytesOut);
    }
    else processScreen(fh, con, stmt, stmt2, rs, rs2, out, req, p1, p2, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6914, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1+":"+p2);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void processScreen(RandomAccessFile fh, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req, String p1, String p2,
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>GP by Products</title>");

    scoutln(out, bytesOut, "</script><link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"" + directoryUtils.getCssGeneralDirectory(con, stmt, rs, unm, dnm, defnsDir) + "general.css\">");
   
    outputPageFrame(con, stmt, rs, out, req, false, p1, p2, "", "GP by Products", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
    ...
    if(p2.equals("-"))
      forEachMfr(fh, out, false, con, stmt, stmt2, rs, rs2, "2011-01-01", "2014-12-31", bytesOut);
    else forAMfr(fh, out, false, con, stmt, stmt2, rs, rs2, p2, "2011-01-01", "2014-12-31", bytesOut);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void processCSV(RandomAccessFile fh, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, String mfr, int[] bytesOut) throws Exception
  {
    outputTitleLine(fh, out, true, bytesOut);

    if(mfr.equals("-"))
      forEachMfr(fh, out, true, con, stmt, stmt2, rs, rs2, "2011-01-01", "2014-12-31", bytesOut);
    else forAMfr(fh, out, true, con, stmt, stmt2, rs, rs2, mfr, "2011-01-01", "2014-12-31", bytesOut);
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void forEachMfr(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    ...
    while(rs.next())
    {
      ...
      forAnItem(fh, out, isCSV, con, stmt, stmt2, rs, rs2, mfr, dateFrom, dateTo, bytesOut);
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void forAMfr(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String mfr, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    forAnItem(fh, out, isCSV, con, stmt, stmt2, rs, rs2, mfr, dateFrom, dateTo, bytesOut);
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void forAnItem(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String mfr, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    stmt = con.createStatement();
    ...
    while(rs.next())
    {
      ...
      forEachInvoice(fh, out, isCSV, con, stmt, stmt2, rs, rs2, itemCode, desc, dateFrom, dateTo, bytesOut);
    }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private double forEachInvoice(RandomAccessFile fh, PrintWriter out, boolean isCSV, Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String itemCode, String desc, String dateFrom, String dateTo, int[] bytesOut) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t2.Amount, t1.CompanyCode, t1.CompanyName, t1.Currency, t2.Quantity, t1.InvoiceCode, t1.Date, "
                         + "t2.UnitPrice, t2.Manufacturer, t2.SOCode FROM invoice AS t1 INNER JOIN invoicel AS t2 ON t1.InvoiceCode = t2.InvoiceCode WHERE "
    ...
    double amount = 0.0;
    String amt, currency, qty, invoiceCode, invoiceDate, amt2, mfr, unitPrice, custCode, custName, industryType, country, soCode;
    ...
    while(rs.next())
    {
      ...
      soCode = generalUtils.deNull(rs.getString(10));
      ...
      country      = customer.getACompanyFieldGivenCode(con, stmt2, rs2, "Country", custCode);
    
      getCOS(con, stmt2, rs2, soCode, itemCode, cosAmt, cosUnitPrice, cosQty);
      
      outputLine(fh, out, isCSV, itemCode, desc, amt, currency, qty, invoiceCode, invoiceDate, unitPrice, mfr, custCode, custName, industryType, country, cosAmt[0], cosUnitPrice[0], cosQty[0], bytesOut);
    }
                               ...
                               String den, String dnm, String bnm, String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, "ConsolidationsGPProducts", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
  }
  ...
  private void outputLine(RandomAccessFile fh, PrintWriter out, boolean isCSV, String itemCode, String desc, String amt, String currency, String qty, String invoiceCode, String invoiceDate, String unitPrice, String mfr, String custCode,
                          String custName, String industryType, String country, String cosAmt, String cosUnitPrice, String cosQty, int[] bytesOut) throws Exception
  {
    if(isCSV)
    {  
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void getCOS(Connection con, Statement stmt, ResultSet rs, String soCode, String itemCode, String[] cosAmt, String[] cosUnitPrice, String[] cosQty) throws Exception
  {
