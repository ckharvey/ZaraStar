// =======================================================================================================================================================================================================
// System: SteeClaws DocumentEngine: OA update header rec
// Module: OrderAcknowledgementHeaderUpdate.java
// Author: C.K.Harvey
...
public class OrderAcknowledgementHeaderUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "OrderAcknowledgementHeaderUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4134, bytesOut[0], 0, "ERR:" + p2);
                    ...
                    String codeStr, char cad, String p3, String thisOp, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4134", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4134, bytesOut[0], 0, "ACC:" + codeStr);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4134", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4134, bytesOut[0], 0, "SID:" + codeStr);
      if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    else
    {
      ...
      if(errorValidation.validatePageReturningStr(con, stmt, rs, "Date", retStr, orderAcknowledgement.getFieldNamesOA(), orderAcknowledgement.getFieldTypesOA(), orderAcknowledgement.getFieldStylesOA(), "oa",
                                        "Update Order Acknowledgement Header", dnm, unm, recData, y, imagesDir, localDefnsDir, defnsDir))
      { // validated ok
        switch(orderAcknowledgement.putHead(con, stmt, rs, code, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
        {
          ...
          case 'X' : // already exists
                      ...
                      break;
          case 'I' : // illegal code
                      errorValidation.formatErrMsg("Invalid Order Acknowledgement Code", retStr, imagesDir);
                      reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                      break;
          case 'N' : // no code specified
                      errorValidation.formatErrMsg("No Order Acknowledgement Code Specified", retStr, imagesDir);
                      reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                      break;
          case 'F' : // not updated
                      retStr[0]="";
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4134, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeStr);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/OrderAcknowledgementHeaderEdit");
