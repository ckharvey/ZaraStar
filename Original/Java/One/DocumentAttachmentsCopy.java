...
// Module: DocumentAttachmentsCopy.java
// Author: C.K.Harvey
...
public class DocumentAttachmentsCopy
{
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public void copyAttachments(Connection con, Statement stmt, ResultSet rs, String sourceFileName, byte[] sourceCodeB, String targetFileName, byte[] targetCodeB, String dnm,
                              String defnsDir, String localDefnsDir) throws Exception
  {
    ...
    try
    {
      ...
      if(numRecs > 0) // attachments exist
      {
        ...
        {
          stmt.executeUpdate("INSERT INTO " + targetFileName + " ( Code, LibraryDocCode ) VALUES ('" + targetCode + "','"
                             + libraryDocCodes[x] + "')");
        }
