// =======================================================================================================================================================================================================
// System: ZaraStar Analytic: Generate statements of outstanding report  
// Module: StatementsOutstandingGenerate.java
// Author: C.K.Harvey
...
public class StatementsOutstandingGenerate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "StatementsOutstandingGenerate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1011, bytesOut[0], 0, "ERR:" + p3);
                    ...
                    String bnm, String p1, String p2, String p3, String p4, String p5, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "1011b", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1011, bytesOut[0], 0, "ACC:" + p3);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "1011b", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 1011, bytesOut[0], 0, "SID:" + p3);
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    if(p3.length() > 0) // one customer
      rtn = r087(con, stmt, stmt2, stmt3, stmt4, rs, rs2, rs3, rs4, true, p3, p1, p2, liveDate, unm, dnm, bnm, p4, p5, reportsDir, workingDir, localDefnsDir, defnsDir);
    else rtn = r087(con, stmt, stmt2, stmt3, stmt4, rs, rs3, rs2, rs4, false, p3, p1, p2, liveDate, unm, dnm, bnm, p4, p5, reportsDir, workingDir, localDefnsDir, defnsDir);

    switch(rtn)
    {
      case -1 : // Definition File Not Found
                messagePage.msgScreen(false, out, req, 17, unm, sid, uty, men, den, dnm, bnm, "1011", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
      case -2 : // cannot create report output file
                messagePage.msgScreen(false, out, req, 18, unm, sid, uty, men, den, dnm, bnm, "1011", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
      default : // generated ok
                messagePage.msgScreen(false, out, req, 16, unm, sid, uty, men, den, dnm, bnm, "1011", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                break;
    }
    
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 1011, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p3);
    if(con != null) con.close();
    if(out != null) out.flush(); 
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private short r087(Connection con, Statement stmt, Statement stmt2, Statement stmt3, Statement stmt4, ResultSet rs, ResultSet rs2, ResultSet rs3, ResultSet rs4, boolean oneCustomer, String customerCode, String dateFrom, String dateTo,
                     String liveDate, String unm, String dnm, String bnm, String option, String order, String reportsDir, String workingDir, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if((reportGeneration.fhO = reportGeneration.createNewFile((short)87, workingDir, localDefnsDir, defnsDir, reportsDir, newName)) == null)
      return -2;

    if((reportGeneration.fhPPR = generalUtils.fileOpenD("087.ppr", localDefnsDir)) == null)
    {
      if((reportGeneration.fhPPR = generalUtils.fileOpenD("087.ppr", defnsDir)) == null)
        return -1;
    }
    ...
    else
    {
      ...
      {    
        ...
        reportGeneration.outputLine('F', null, fldData, fldNames, (short)35, ' ', dnm, unm, localDefnsDir, defnsDir);
      }
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void generate(Connection con, Statement stmt, Statement stmt2, Statement stmt3, ResultSet rs, ResultSet rs2, ResultSet rs3, String option,
                        String order, String customerCode, String customerName, String dateFrom, String dateTo, String liveDate, byte[] fldData,
                        byte[] fldNames, String unm, String dnm, String bnm, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    reportGeneration.outputLine('M', "", "", msg, fldData, fldNames, (short)35, ' ', dnm, unm, localDefnsDir, defnsDir);
    ...
    generalUtils.repAlpha(fldData, 10000, (short)3, customerCode.toUpperCase());
    ...
    reportGeneration.processSection("PH", fldData, fldNames, (short)35, dnm, unm, localDefnsDir, defnsDir);
    ...
    balance[0] = openBalance(con, stmt, stmt2, stmt3, rs, rs2, rs3, customerCode, currencyMismatch, dateFrom, dateTo, liveDate, customerCurrency, dateLess30,
                             dateLess60, dateLess90, dateLess120, dateLess150, dateLess180, dateLess210, dateLess240, dateLess270, dateLess300,
                             dateLess330, dateLess360, upto30, upto60, upto90, upto120, upto150, upto180, upto210, upto240, upto270, upto300, upto330,
                             upto360, over, earliestDate, dnm, localDefnsDir, defnsDir);
    
    writeLine(fldData, "O", "", "", "", "", balance);
    reportGeneration.processSection("BL1", fldData, fldNames, (short)35, dnm, unm, localDefnsDir, defnsDir);

    String tmpTable = unm + "_tmp";
      
    directoryUtils.createTmpTable(true, con, stmt, "DocumentDate date, DocumentCode char(20), DocumentType char(1), Reference char(50), Amount decimal(19,8), EffectiveDate date", "", tmpTable);

    stmt = con.createStatement();

    String invoiceCode, currency;
    double[] outstanding = new double[1];
    
    rs = stmt.executeQuery("SELECT InvoiceCode, Date, TotalTotal, Currency, PORefNum FROM invoice WHERE CompanyCode = '" + customerCode + "' AND Status != 'C' AND Settled != 'Y' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    ...
    {    
      ...
      if(generalUtils.doubleDPs(outstanding[0], '2') != 0.0)
        addToTmpTable(con, stmt, rs.getString(2), invoiceCode, "I", rs.getString(5), outstanding[0], rs.getString(2), tmpTable);
    ...
    rs = stmt.executeQuery("SELECT DNCode, Date, TotalTotal, Currency, PORefNum FROM debit WHERE CompanyCode = '" + customerCode + "' AND Status != 'C' AND Settled != 'Y' AND Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo + "'}");
    ...
    {    
      ...
      if(generalUtils.doubleDPs(outstanding[0], '2') != 0.0)
        addToTmpTable(con, stmt, rs.getString(2), rs.getString(1), "D", rs.getString(5), outstanding[0], rs.getString(2), tmpTable);
    ...
    int count = processTmpTable(con, stmt, stmt2, rs, rs2, tmpTable, order, customerCode, customerName, fldData, fldNames, balance, dateLess30, dateLess60,
                                dateLess90, dateLess120, dateLess150, dateLess180, dateLess210, dateLess240, dateLess270, dateLess300, dateLess330,
                                dateLess360, upto30, upto60, upto90, upto120, upto150, upto180, upto210, upto240, upto270, upto300, upto330, upto360,
                                over, earliestDate, unm, dnm, localDefnsDir, defnsDir); 
    ...
    {
      reportGeneration.processSection("RF3", fldData, fldNames, (short)35, dnm, unm, localDefnsDir, defnsDir);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private int processTmpTable(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String tmpTable, String order, String customerCode, String customerName,
                              byte[] fldData, byte[] fldNames, double[] balance, int dateLess30, int dateLess60, int dateLess90, int dateLess120,
                              int dateLess150, int dateLess180,
                              int dateLess210, int dateLess240, int dateLess270, int dateLess300, int dateLess330, int dateLess360, double[] upto30,
                              double[] upto60, double[] upto90, double[] upto120, double[] upto150, double[] upto180, double[] upto210,
                              double[] upto240, double[] upto270, double[] upto300, double[] upto330, double[] upto360, double[] over,
                              int[] earliestDate, String unm, String dnm, String localDefnsDir, String defnsDir) throws Exception 
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT DocumentDate, DocumentCode, DocumentType, Reference, Amount, EffectiveDate FROM " + tmpTable
                         + " ORDER BY DocumentDate, DocumentCode");
                               ...
                               throws Exception
  {
    ...
    {
      reportGeneration.outputLine('E', null, fldData, fldNames, (short)35, ' ', dnm, unm, localDefnsDir, defnsDir);
                           ...
                           byte[] fldData, String unm, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(option.charAt(0) == '1') // 30-60-90
      reportGeneration.processSection("RF1", fldData, fldNames, (short)35, dnm, unm, localDefnsDir, defnsDir);
    else reportGeneration.processSection("RF2", fldData, fldNames, (short)35, dnm, unm, localDefnsDir, defnsDir);
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private double openBalance(Connection con, Statement stmt, Statement stmt2, Statement stmt3, ResultSet rs, ResultSet rs2, ResultSet rs3, String customerCode,
                             boolean[] currencyMismatch, String dateFrom, String dateTo, String liveDate, String customerCurrency, int dateLess30,
                             int dateLess60, int dateLess90, int dateLess120, int dateLess150, int dateLess180, int dateLess210, int dateLess240,
                             int dateLess270, int dateLess300, int dateLess330, int dateLess360, double[] upto30, double[] upto60, double[] upto90,
                             double[] upto120, double[] upto150, double[] upto180, double[] upto210, double[] upto240, double[] upto270,
                             double[] upto300, double[] upto330, double[] upto360, double[] over, int[] earliestDate, String dnm,
  ...
  private void detReceiptLines(Connection con, Statement stmt, ResultSet rs, String invoiceCode, String dateFrom, double[] outstanding) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.AmountReceived FROM receiptl AS t2 INNER JOIN receipt AS t1 ON t1.ReceiptCode = t2.ReceiptCode "
                         + "WHERE t2.InvoiceCode = '" + invoiceCode + "' AND t1.Status != 'C' AND t1.DateReceived < {d '" + dateFrom + "'}");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void detSalesCreditNoteLines(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, String invoiceCode, String dateFrom, String dnm,
                                  String localDefnsDir, String defnsDir, double[] outstanding) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT t2.Amount2, t2.GSTRate FROM creditl AS t2 INNER JOIN credit AS t1 ON t1.CNCode = t2.CNCode "
                         + "WHERE t2.InvoiceCode = '" + invoiceCode + "' AND t1.Status != 'C' AND t1.Date < {d '" + dateFrom + "'}");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void addToTmpTable(Connection con, Statement stmt, String docDate, String docCode, String docType, String reference, double amount, String effectiveDate, String tmpTable) throws Exception
  {
    stmt = con.createStatement();
   
    String q = "INSERT INTO " + tmpTable + " ( DocumentDate, DocumentCode, DocumentType, Reference, Amount, EffectiveDate ) "
             + "VALUES ({d '" + docDate + "'}, '" + docCode + "', '" + docType + "', '" + generalUtils.sanitiseForSQL(reference) + "', '" + amount + "', {d '" + effectiveDate + "'} )";
