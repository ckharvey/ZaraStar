// =======================================================================================================================================================================================================
// System: ZaraStar: Document: PO create listing page
// Module: PurchaseOrderListing.java
// Author: C.K.Harvey
...
public class PurchaseOrderListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseOrderListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5015, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5015, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5015, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "PurchaseOrderListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5015, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5015, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM po WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM po" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM po WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM po WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM po" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM po WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(SalesPerson) AS rowcount FROM po WHERE SalesPerson LIKE '"
                                           + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT SalesPerson) AS rowcount FROM po"); 
                   else
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM po WHERE SalesPerson = '" + codeArg + "'"
                                           + companyCodeANDWHERE);
                   }
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT POCode) AS rowcount FROM po WHERE POCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE);
                   }
                   else
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM po" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String poCode="", name="", supplierCode="", date="", status, salesPerson="";
    int poCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, salesPersonPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE POCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY POCode";
                   }
                   else
                   {
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po" + companyCodeWHERE + " ORDER BY POCode";
                   }

                   poCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, POCode, CompanyCode, CompanyName, Status FROM po WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     poCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM po" + companyCodeWHERE + " ORDER BY Date";
                     
                     poCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, POCode, CompanyCode, CompanyName, Status FROM po WHERE Date = {d '"
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, POCode, CompanyName, Date, Status FROM po WHERE CompanyCode LIKE '" + srchStr
                       + "%' ORDER BY CompanyCode, POCode";
                     
                     poCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5; salesPersonPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM po" + companyCodeWHERE + " ORDER BY CompanyCode";
                     
                     poCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, POCode, CompanyName, Date, Status FROM po WHERE CompanyCode = '" + codeArg
                       + "' ORDER BY CompanyCode, POCode";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT SalesPerson, POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE SalesPerson LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY SalesPerson, Date";
                     
                     poCodePosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6; salesPersonPosn = 1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM po" + companyCodeWHERE + " ORDER BY SalesPerson";
                     
                     poCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = 1;
                   }
                   else
                   {
                     q = "SELECT SalesPerson, POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE SalesPerson = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, poCode, date, supplierCode, name, null, poCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date, cssFormat);
                     else appendBodyLine(html, inx, date, poCode, supplierCode, name, null, poCode, status, imagesDir, cssFormat);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, supplierCode, poCode, name, date, null, poCode, status, imagesDir, cssFormat);
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, salesPerson, cssFormat);
                     else appendBodyLine(html, inx, salesPerson, poCode, date, supplierCode, name, poCode, status, imagesDir, cssFormat);
                   }
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = poCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = poCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = poCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = poCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE POCode LIKE '" + srchStr 
                       + "%' AND POCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY POCode DESC";
                   }
                   else
                   {  
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE POCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY POCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM po WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE + " ORDER BY Date DESC";
                     
                     poCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1; salesPersonPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, POCode, CompanyCode, CompanyName, Status FROM po" 
                       + " WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE
                       + " ORDER BY Date, POCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM po WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, POCode, CompanyName, Date, Status FROM po WHERE CompanyCode = '" + codeArg 
                        + "' ORDER BY CompanyCode, POCode DESC";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM po WHERE SalesPerson <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                       + companyCodeANDWHERE + " ORDER BY SalesPerson DESC";
                   ...
                   {
                     q = "SELECT SalesPerson, POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE SalesPerson = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, POCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, poCode, date, supplierCode, name, null, poCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date, cssFormat);
                     else prependBodyLine(html, inx, date, poCode, supplierCode, name, null, poCode, status, imagesDir, cssFormat);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, supplierCode, poCode, name, date, null, poCode, status, imagesDir, cssFormat);
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, salesPerson, cssFormat);
                     else prependBodyLine(html, inx, salesPerson, poCode, date, supplierCode, name, poCode, status, imagesDir, cssFormat);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE POCode LIKE '" + srchStr 
                       + "%' AND POCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY POCode";
                     
                     poCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; salesPersonPosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE POCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY POCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM po WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, POCode, CompanyCode, CompanyName, Status FROM po WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND POCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, POCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM po WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, POCode, CompanyName, Date, Status FROM po WHERE CompanyCode = '" + codeArg
                       + "' AND POCode > '" + codeArg2 + "' ORDER BY CompanyCode, POCode";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT SalesPerson FROM po WHERE SalesPerson >= '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT SalesPerson, POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE SalesPerson = '"
                       + codeArg + "' AND POCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, POCode";
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM po WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, POCode, CompanyName, Date, Status FROM po WHERE CompanyCode = '" + codeArg
                       + "' AND POCode < '" + codeArg2 + "' ORDER BY CompanyCode, POCode DESC";
                 ...
                 else // inx == '4'
                 {
                   ...
                   {
                     q = "SELECT SalesPerson, POCode, Date, CompanyCode, CompanyName, Status FROM po WHERE SalesPerson = '" 
                       + codeArg + "' AND POCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, POCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Purchase Order Search Results for ";
        switch(inx)
        {
          case '1' : title += "Purchase Order Code";  break;
          case '2' : title += "Purchase Order Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Purchase Order Listing by ";        
        switch(inx)
        {
          case '1' : title += "Purchase Order Code";  break;
          case '2' : title += "Purchase Order Date";  break;
      ...
      else title = "Purchase Order Listing";

      drawingUtils.drawTitle(out, false, false, "PurchaseOrderListing", "", title, "5015", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Purchase Order Code");  break;
        case '2' : scout(out, bytesOut, "Purchase Order Date");  break;
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Order Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/PurchaseOrderPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PurchaseOrderListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PurchaseOrderListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5015", "", "PurchaseOrderListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Code &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Date &nbsp;</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Sales Person &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Order Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Purchase Order Date");  break;
