// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Proforma Invoice listing page
// Module: ProformaInvoiceListing.java
// Author: C.K.Harvey
...
public class ProformaInvoiceListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ProformaInvoiceListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4091, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4091, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ProformaInvoiceListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4091, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ProformaInvoiceListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4091, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4091, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM proforma WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM proforma" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM proforma WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM proforma WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM proforma" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM proforma WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT ProformaCode) AS rowcount FROM proforma WHERE ProformaCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM proforma" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String proformaCode="", name="", customerCode="", date="", custPOCode="", status;
    int proformaCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, custPOCodePosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY ProformaCode";
                   }
                   else
                   {
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma " + companyCodeWHERE
                       + "ORDER BY ProformaCode";
                   }

                   proformaCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; custPOCodePosn = -1; 
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, ProformaCode, CompanyCode, CompanyName, Status FROM proforma WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     proformaCodePosn = 2; datePosn = 1; companyCodePosn = 3; statusPosn = 5; custPOCodePosn = -1;
                     companyNamePosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM proforma ORDER BY Date" + companyCodeWHERE;
                     
                     proformaCodePosn = -1; datePosn = 1; companyCodePosn = -1; statusPosn = -1; custPOCodePosn = -1;
                     companyNamePosn = -1;
                   }
                   else
                   {
                     ...
                     proformaCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                     custPOCodePosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, ProformaCode, CompanyName, PORefNum, Status FROM proforma WHERE "
                      + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     proformaCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5; 
                     custPOCodePosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM proforma " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     proformaCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; 
                     custPOCodePosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, ProformaCode, CompanyName, Settled, PORefNum, Status FROM proforma WHERE "
                 ...
                 while(rs.next())                  
                 {
                   proformaCode = getValue(proformaCodePosn, ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, proformaCode, date, customerCode, name, proformaCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, proformaCode, customerCode, name, proformaCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, proformaCode, name, custPOCode, proformaCode, status, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = proformaCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = proformaCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = proformaCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = proformaCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode LIKE '"
                       + srchStr + "%' AND ProformaCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY ProformaCode DESC";
                   }
                   else
                   {  
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode "
                       + "<= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY ProformaCode DESC";
                   }
                   
                   proformaCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   custPOCodePosn = -1;
                 }  
                 else
                 if(inx == '2')
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM proforma WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, ProformaCode, CompanyCode, CompanyName, Status FROM proforma WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, ProformaCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM proforma WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, ProformaCode, CompanyName, Settled, PORefNum, Status FROM proforma WHERE "
                        + "CompanyCode = '" + codeArg + "' ORDER BY CompanyCode, ProformaCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, proformaCode, date, customerCode, name, proformaCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, proformaCode, customerCode, name, proformaCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, proformaCode, name, custPOCode, proformaCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode LIKE '"
                       + srchStr + "%' AND ProformaCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY ProformaCode";
                   ...
                   {                   
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY ProformaCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM proforma WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, ProformaCode, CompanyCode, CompanyName, Status FROM proforma WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND ProformaCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, ProformaCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM proforma WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, ProformaCode, CompanyName, Settled, PORefNum, Status FROM proforma WHERE "
                       + "CompanyCode = '" + codeArg + "' AND ProformaCode > '" + codeArg2 + "' ORDER BY CompanyCode, ProformaCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT ProformaCode, Date, CompanyCode, CompanyName, Status FROM proforma WHERE ProformaCode < '" + codeArg 
                     + "'" + companyCodeANDWHERE + " ORDER BY ProformaCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   ...
                   {  
                     ...
                     proformaCodePosn = 2; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     custPOCodePosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, ProformaCode, CompanyCode, CompanyName, Status FROM proforma WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND ProformaCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, ProformaCode DESC";
                     
                     proformaCodePosn = 1; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                     custPOCodePosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM proforma WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, ProformaCode, CompanyName, Settled, PORefNum, Status FROM proforma WHERE "
                       + "CompanyCode = '" + codeArg + "' AND ProformaCode < '" + codeArg2 + "' ORDER BY CompanyCode, ProformaCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Proforma Invoice Search Results for ";
        switch(inx)
        {
          case '1' : title += "Proforma Invoice Code";  break;
          case '2' : title += "Proforma Invoice Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Proforma Invoice Listing by ";
        switch(inx)
        {
          case '1' : title += "Proforma Invoice Code";  break;
          case '2' : title += "Proforma Invoice Date";  break;
      ...
      else title = "Proforma Invoice Listing";

      drawingUtils.drawTitle(out, false, false, "ProformaInvoiceListing", "", title, "4091", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Proforma Invoice Code");  break;
        case '2' : scout(out, bytesOut, "Proforma Invoice Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Proforma Invoice Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/ProformaInvoicePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/ProformaInvoiceListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/ProformaInvoiceListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4091", "", "ProformaInvoiceListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Proforma Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Proforma Invoice Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Proforma Invoice Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Proforma Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Proforma Invoice Date"); break;
