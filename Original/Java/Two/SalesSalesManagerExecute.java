// =======================================================================================================================================================================================================
// System: ZaraStar: Quotation & Sales Order Processing - Sales Manager
// Module: SalesSalesManagerExecute.java
// Author: C.K.Harvey
...
public class SalesSalesManagerExecute extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesSalesManagerExecute", bytesOut);
                    ...
                    String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesSalesManagerExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesSalesManagerExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4019, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/QuotationPage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "2030", "", "SalesSalesManagerExecute", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesSalesManagerExecute", "", "Sales Order Processing - Sales Manager", "2030", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form name=\"go\" action=\"SalesManagerUpdate\" enctype=\"application/x-www-form-urlencoded\" method=POST>");
    ...
    scoutln(out, bytesOut, "<td align=center><p><b> Engineering </b></td>");
    scoutln(out, bytesOut, "<td align=center><p><b> Procurement </b></td>");
    scoutln(out, bytesOut, "<td align=center><p><b> Scheduling </b></td>");

    scoutln(out, bytesOut, "<td align=center><p><b> Manager Approved </b></td>");
                                       ...
                                       String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT SalesPerson, SOType, SOCode, Date, CompanyCode, CustomerPOCode, TotalTotal, Currency2, CompanyName, "
                         + "EngineeringApprovedDate, EngineeringApprovedSignOn, ProcurementConfirmedDate, ProcurementConfirmedSignOn, "
                         + "SchedulingConfirmedDate, SchedulingConfirmedSignOn, ToManager, ToEngineeringDate, ToEngineeringSignOn, ToSchedulingDate, "
                         + "ToSchedulingSignOn, ToProcurementDate, ToProcurementSignOn, ToEngineering, ToScheduling, ToProcurement "
                         + "FROM so WHERE ManagerApproved != 'Y' AND Status != 'C' AND AllSupplied != 'Y' ORDER BY Date, SOCode");
      
    String salesPerson, soType, soCode, date, customerCode, custPOCode, currency, companyName, engineeringApprovedDate, engineeringApprovedSignOn,
           procurementConfirmedDate, procurementConfirmedSignOn, schedulingConfirmedDate, schedulingConfirmedSignOn, toManager, toEngineeringDate,
           toEngineeringSignOn, toSchedulingDate, toSchedulingSignOn, toProcurementDate, toProcurementSignOn, toEngineering, toScheduling,
           toProcurement;
    double totalTotal;
    byte[] b = new byte[20];
    ...
    {    
      ...
      {
        soType                     = generalUtils.deNull(rs.getString(2));
        soCode                     = generalUtils.deNull(rs.getString(3));
        date                       = generalUtils.deNull(rs.getString(4));
        customerCode               = generalUtils.deNull(rs.getString(5));
        custPOCode                 = generalUtils.deNull(rs.getString(6));
        totalTotal                 = generalUtils.doubleFromStr(generalUtils.deNull(rs.getString(7)));
        currency                   = generalUtils.deNull(rs.getString(8));
        companyName                = generalUtils.deNull(rs.getString(9));
        engineeringApprovedDate    = generalUtils.deNull(rs.getString(10));
        engineeringApprovedSignOn  = generalUtils.deNull(rs.getString(11));
        procurementConfirmedDate   = generalUtils.deNull(rs.getString(12));
        procurementConfirmedSignOn = generalUtils.deNull(rs.getString(13));
        schedulingConfirmedDate    = generalUtils.deNull(rs.getString(14));
        schedulingConfirmedSignOn  = generalUtils.deNull(rs.getString(15));
        toManager                  = generalUtils.deNull(rs.getString(16));
        toEngineeringDate          = generalUtils.deNull(rs.getString(17));
        toEngineeringSignOn        = generalUtils.deNull(rs.getString(18));
        toSchedulingDate           = generalUtils.deNull(rs.getString(19));
        toSchedulingSignOn         = generalUtils.deNull(rs.getString(20));
        toProcurementDate          = generalUtils.deNull(rs.getString(21));
        toProcurementSignOn        = generalUtils.deNull(rs.getString(22));
        toEngineering              = generalUtils.deNull(rs.getString(23));
        toScheduling               = generalUtils.deNull(rs.getString(24));
        toProcurement              = generalUtils.deNull(rs.getString(25));
        ...
        if(soType.equals("T"))
          scout(out, bytesOut, "<td><p>-</td>");
        else
        {
          if(engineeringApprovedSignOn == null || engineeringApprovedSignOn.length() == 0) // just-in-case
          {
            ...
            else scout(out, bytesOut, "<td><p><font color=red>Not Sent to Engineering</td>");
          }
        ...
        if(procurementConfirmedSignOn == null || procurementConfirmedSignOn.length() == 0) // just-in-case
        {
          ...
          else scout(out, bytesOut, "<td><p><font color=red>Not Sent to Procurement</td>");
        }
        else
        {
          scout(out, bytesOut, "<td><p><font color=green>" + generalUtils.convertFromYYYYMMDD(procurementConfirmedDate) + " by " + procurementConfirmedSignOn
                             + "</td>");
        }
        ...
        {
          if(schedulingConfirmedSignOn == null || schedulingConfirmedSignOn.length() == 0) // just-in-case
          {
            ...
            else scout(out, bytesOut, "<td><p><font color=red>Not Sent to Scheduling</td>");
          }
          else
          {
            scout(out, bytesOut, "<td><p><font color=green>" + generalUtils.convertFromYYYYMMDD(schedulingConfirmedDate)  + " by "
                                 + schedulingConfirmedSignOn  + "</td>");
          }
        }
        
        if(! toManager.equals("Y"))
          scout(out, bytesOut, "<td align=center><p><font color=red>Not Ready</td>");
        else scout(out, bytesOut, "<td align=center><p><input type=checkbox name=manApp" + generalUtils.sanitise(soCode) + "></td>");
