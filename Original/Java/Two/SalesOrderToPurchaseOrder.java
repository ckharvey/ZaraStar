// =======================================================================================================================================================================================================
// System: ZaraStar Document: SO to PO
// Module: SalesOrderToPurchaseOrder.java
// Author: C.K.Harvey
...
public class SalesOrderToPurchaseOrder
{
                        ...
                        byte[] newDocCode, int[] bytesOut) throws Exception
  {
    ...
    if(purchaseOrder.getRecGivenCode(con, stmt, rs, newDocCode, '\000', data, dnm, localDefnsDir, defnsDir, bytesOut) != -1) // just-in-case
    {
    ...
    soToPO(con, stmt, rs, 'H', newDocCode, data, dataTo, null, 0, "", rate, unm, dnm, localDefnsDir, defnsDir, b, itemCode);

    if(purchaseOrder.putRecHeadGivenCode(con, stmt, rs, newDocCode, 'N', dataTo, dnm, localDefnsDir, defnsDir) != ' ')
    {
    ...
    for(int xx=0;xx<linesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, linesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          ...
          soToPO(con, stmt, rs, 'L', newDocCode, data, dataTo, code, thisLine, qtyReqd, rate, unm, dnm, localDefnsDir, defnsDir, b, itemCode);
          purchaseOrder.putRecLine(con, stmt, rs, newDocCode, null, 'N', dataTo, dnm, localDefnsDir, defnsDir);
        }
      }
    }  
    ...
    for(int xx=0;xx<multipleLinesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, multipleLinesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          soToPO(con, stmt, rs, 'M', newDocCode, data, dataTo, null, 0, "", rate, unm, dnm, localDefnsDir, defnsDir, b, itemCode);
          purchaseOrder.putMultipleLine(con, stmt, rs, newDocCode, newLines[z], dataTo, dnm, localDefnsDir, defnsDir);
        }
      }
    }

    documentAttachmentsCopy.copyAttachments(con, stmt, rs, "soa", code, "poa", newDocCode, dnm, defnsDir, localDefnsDir);                 

    return true;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void soToPO(Connection con, Statement stmt, ResultSet rs, char which, byte[] newDocCode, byte[] fromBuf, byte[] toBuf, byte[] soCode, int soLine, String qtyReqd, double rate, String unm, String dnm, String localDefnsDir,
                      String defnsDir, byte[] b, byte[] itemCode) throws Exception
  {
    int x;
    ...
    switch(which)
    {
      case 'H' : // head
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)2, "-"); // custcode
      ...
      case 'L' : // line
                 ...
                 generalUtils.dfs(fromBuf, (short)28, b); // storecode
