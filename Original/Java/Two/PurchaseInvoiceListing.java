// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Purchase Invoice listing page
// Module: PurchaseInvoiceListing.java
// Author: C.K.Harvey
...
public class PurchaseInvoiceListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseInvoiceListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5088, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5088, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5088, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5088, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5088, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
...
if(con  != null) con.close();
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pinvoice WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM pinvoice" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pinvoice WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM pinvoice WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM pinvoice" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pinvoice WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(SupplierInvoiceCode) AS rowcount FROM pinvoice WHERE SupplierInvoiceCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT SupplierInvoiceCode) AS rowcount FROM pinvoice" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pinvoice WHERE SupplierInvoiceCode= '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT InvoiceCode) AS rowcount FROM pinvoice WHERE InvoiceCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pinvoice" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String invoiceCode="", name="", supplierCode="", date="", suppInvoiceCode="", status;
    int invoiceCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, suppInvoiceCodePosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE InvoiceCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY InvoiceCode";
                   }
                   else
                   {
                     q = "SELECT InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice " + companyCodeWHERE
                       + "ORDER BY InvoiceCode";
                   }

                   invoiceCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; 
                   suppInvoiceCodePosn = -1; 
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, InvoiceCode, CompanyCode, CompanyName, Status FROM pinvoice WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     invoiceCodePosn = 2; datePosn = 1; companyCodePosn = 3; statusPosn = 5; suppInvoiceCodePosn = -1;
                     companyNamePosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pinvoice ORDER BY Date" + companyCodeWHERE;
                     
                     invoiceCodePosn = -1; datePosn = 1; companyCodePosn = -1; statusPosn = -1; suppInvoiceCodePosn = -1;
                     companyNamePosn = -1; 
                   }
                   else
                   {
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, InvoiceCode, CompanyName, SupplierInvoiceCode, Status FROM pinvoice WHERE "
                      + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     invoiceCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5; 
                     suppInvoiceCodePosn = 4; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pinvoice " + companyCodeWHERE + "ORDER BY CompanyCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT SupplierInvoiceCode, InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE "
                      + "SupplierInvoiceCode LIKE '" + srchStr + "%' ORDER BY SupplierInvoiceCode, InvoiceCode";
                   
                     invoiceCodePosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6; 
                     suppInvoiceCodePosn = 1; 
                   }
                   else
                   if(callType == 'G')
                   ...
                   {
                     q = "SELECT SupplierInvoiceCode, InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE "
                       + "SupplierInvoiceCode = '" + codeArg + "' ORDER BY SupplierInvoiceCode";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   suppInvoiceCode = getValue(suppInvoiceCodePosn, ' ', rs, rsmd);
                   status          = getValue(statusPosn,          ' ', rs, rsmd);
                   
                   if(inx == '1')
                     appendBodyLine(html, inx, invoiceCode, date, supplierCode, name, null, invoiceCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, invoiceCode, supplierCode, name, null, invoiceCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       appendBodyLine(html, inx, supplierCode, invoiceCode, name, suppInvoiceCode, null, invoiceCode, status,
                                      imagesDir);
                     }
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, suppInvoiceCode);
                     else
                     {
                       appendBodyLine(html, inx, suppInvoiceCode, invoiceCode, date, supplierCode, name, invoiceCode, status,
                                      imagesDir);
                     }
                   }   
                   ...
                   {  
                     ...
                     if(inx == '3')
                       ...
                       firstEntryOnPage[0] = suppInvoiceCode;
                   }
                 }
                 ...
                 lastEntryOnPage[0] = suppInvoiceCode;
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {  
                     q = "SELECT InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE InvoiceCode "
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pinvoice WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pinvoice WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SupplierInvoiceCode FROM pinvoice WHERE SupplierInvoiceCode <= 'ZZZZZZZZZZZZZZZZZZZ' "
                       + "ORDER BY SupplierInvoiceCode DESC";
                   ...
                   {
                      q = "SELECT SupplierInvoiceCode, InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE "
                        + "SupplierInvoiceCode = '" + codeArg + "' ORDER BY SupplierInvoiceCode, InvoiceCode DESC";
                    
                      invoiceCodePosn = 2; datePosn = 3; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 6;
                      suppInvoiceCodePosn = 1;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, invoiceCode, date, supplierCode, name, null, invoiceCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, invoiceCode, supplierCode, name, null, invoiceCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       prependBodyLine(html, inx, supplierCode, invoiceCode, name, suppInvoiceCode, null, invoiceCode, status, imagesDir);
                     }
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, suppInvoiceCode);
                     else 
                     {
                       prependBodyLine(html, inx, suppInvoiceCode, invoiceCode, date, supplierCode, name, invoiceCode, status,
                                       imagesDir);
                     }
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {                   
                     q = "SELECT InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE InvoiceCode > '" + codeArg 
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM pinvoice WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pinvoice WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SupplierInvoiceCode FROM pinvoice WHERE SupplierInvoiceCode >= '" + codeArg
                       + "' ORDER BY SupplierInvoiceCode";
                   ...
                   {
                     q = "SELECT SupplierInvoiceCode, InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE "
                       + "SupplierInvoiceCode = '" + codeArg + "' AND InvoiceCode > '" + codeArg2
                       + "' ORDER BY SupplierInvoiceCode, InvoiceCode";
                   
                     invoiceCodePosn = 2; datePosn = 3; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                     suppInvoiceCodePosn = 4;
                   }
                 }  
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM pinvoice WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                 ...
                 else // inx == '4'
                 {
                   ...
                   else
                   {
                     q = "SELECT SupplierInvoiceCode, InvoiceCode, Date, CompanyCode, CompanyName, Status FROM pinvoice WHERE "
                       + "SupplierInvoiceCode = '" + codeArg + "' AND InvoiceCode < '" + codeArg2
                       + "' ORDER BY SupplierInvoiceCode, InvoiceCode DESC";
                   
                     invoiceCodePosn = 2; datePosn = -1; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6;
                     suppInvoiceCodePosn = 1;
                   }  
                 }  
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '3')
                   {
                     ...
                     {
                       prependBodyLine(html, inx, supplierCode, invoiceCode, name, suppInvoiceCode, null, invoiceCode, status,
                                       imagesDir);
                     }
                   }  
                   else // inx == '4'
                 ...
                 if(inx == '3')
                   firstEntryOnPage[0] = supplierCode;
                 else firstEntryOnPage[0] = suppInvoiceCode;
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Purchase Invoice Search Results for ";
        switch(inx)
        {
          case '1' : title += "Purchase Invoice Code";  break;
          case '2' : title += "Purchase Invoice Date";  break;
          case '3' : title += "Supplier Code";          break;
          case '4' : title += "Supplier Invoice Code";  break;
        }
      ...
      if(operation == 'X')
      {
        title = "Purchase Invoice Listing by ";
        switch(inx)
        {
          case '1' : title += "Purchase Invoice Code";  break;
          case '2' : title += "Purchase Invoice Date";  break;
      ...
      else title = "Purchase Invoice Listing";

      drawingUtils.drawTitle(out, false, false, "PurchaseInvoiceListing", "", title, "5088", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Purchase Invoice Code");   break;
        case '2' : scout(out, bytesOut, "Purchase Invoice Date");   break;
        case '3' : scout(out, bytesOut, "Supplier Code");           break;
        case '4' : scout(out, bytesOut, "Supplier Invoice Code");   break;
      }
    }
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Invoice Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/PurchaseInvoicePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PurchaseInvoiceListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PurchaseInvoiceListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5088", "", "PurchaseInvoiceListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Invoice Code</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Invoice Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Purchase Invoice Date");  break;
