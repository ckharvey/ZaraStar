// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: SO to invoice
...
public class SalesOrderToSalesInvoice
{
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public boolean create(Connection con, Statement stmt, ResultSet rs, byte[] code, char renumberLines, char renumberEntries, int[] lines, int numLines, String cashOrNot, String unm, String dnm, String localDefnsDir, String defnsDir,
                        byte[] newDocCode, int[] bytesOut) throws Exception
  {
    ...
    else documentUtils.getNextCode(con, stmt, rs, "invoice", true, newDocCode);
    ...
    soToInvoice(con, stmt, rs, 'H', cashOrNot, newDocCode, data, dataTo, null, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    for(int xx=0;xx<linesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, linesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          ...
          soToInvoice(con, stmt, rs, 'L', cashOrNot, newDocCode, data, dataTo, code, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    for(int xx=0;xx<multipleLinesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, multipleLinesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          soToInvoice(con, stmt, rs, 'M', cashOrNot, newDocCode, data, dataTo, null, unm, dnm, localDefnsDir, defnsDir, b);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void soToInvoice(Connection con, Statement stmt, ResultSet rs, char which, String cashOrNot, byte[] newDocCode, byte[] fromBuf, byte[] toBuf, byte[] code, String unm, String dnm, String localDefnsDir, String defnsDir, byte[] b)
                           throws Exception
  {
    int x;
    ...
    switch(which)
    {
      case 'H' : // head
                 ...
                 else generalUtils.repAlpha(toBuf, 3000, (short)27, "A"); // cashOrAccount
      ...
      case 'L' : // line
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)11, code); // soCode
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)21, accountsUtils.getSalesAccCode(accountsUtils.getAccountingYearForADate(con, stmt, rs, generalUtils.today(localDefnsDir, defnsDir), dnm, localDefnsDir, defnsDir), dnm, localDefnsDir, defnsDir));
