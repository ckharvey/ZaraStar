// =======================================================================================================================================================================================================
// System: ZaraStar RAT: RAT list
// Module: RATListWave.java
// Author: C.K.Harvey
...
public class RATListWave extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p1  = req.getParameter("p1"); // category
      p2  = req.getParameter("p2"); // status
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "RATList", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5906, bytesOut[0], 0, "ERR:" + p1);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String p1, String p2, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "RATList", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5906, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "RATList", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5906, bytesOut[0], 0, "SID:" + p1);
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }

    scoutln(out, bytesOut, "5906\001Issues\001Issues Main\001javascript:getHTML('RATListWave','')\001\001Y\001\001\003");

    set(con, stmt, rs, out, req, p1, p2, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5906, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con != null) con.close();
    if(out != null) out.flush();
  }
  ...
  {
//    scoutln(out, bytesOut, "<html><head><title>Forum</title>");
    ...
    issuesUtils.drawTitleW(con, stmt, rs, req, out, "Issues: " + p1, "5906", "", true, false, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
    ...
    issuesUtils.listRATs(out, p1, p2, unm, dnm, uty, localDefnsDir, defnsDir, bytesOut);
