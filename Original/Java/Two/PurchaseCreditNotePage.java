// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: PCN fetch rec page
// Module: PurchaseCreditNotePage.java
// Author: C.K.Harvey
...
public class PurchaseCreditNotePage extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseCreditNotePage", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5026, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String dnm, String bnm, String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "5026", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5026, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "5026", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5026, bytesOut[0], 0, "SID:" + p1);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    purchaseCreditNote.getRecToHTML(con, stmt, stmt2, rs, rs2, out, dispOrEdit, codeB, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, p2.charAt(0), p3,
                       dataAlready, req, bytesOut);
    
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5026, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
