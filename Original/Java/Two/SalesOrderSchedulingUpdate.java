// =======================================================================================================================================================================================================
// System: ZaraStar Scheduling: update SO
// Module: SalesOrderSchedulingUpdate.java
// Author: C.K.Harvey
...
public class SalesOrderSchedulingUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      byte[] schConCodes    = new byte[1000]; schConCodes[0]    = '\000';
      int[]  schConCodesLen = new int[1];    schConCodesLen[0] = 1000;
      ...
      while(en.hasMoreElements())
      {
        ...
        else // must be checkbox value
        {
          ...
          if(name.startsWith("schCon"))
          {          
            if((generalUtils.lengthBytes(schConCodes, 0) + thisEntryLen) >= schConCodesLen[0])
            {
              byte[] tmp = new byte[schConCodesLen[0]];
              System.arraycopy(schConCodes, 0, tmp, 0, schConCodesLen[0]);
              if(thisEntryLen > 1000)
                inc = thisEntryLen;
              else inc = 1000;
              schConCodesLen[0] += inc;
              schConCodes = new byte[schConCodesLen[0]];
              System.arraycopy(tmp, 0, schConCodes, 0, schConCodesLen[0] - inc);
            }
            
            generalUtils.catAsBytes(name + "\001", 0, schConCodes, false);
          }
          else // salesPeople
          {          
      ...
      doIt(out, req, schConCodes, salesPeople, unm, sid, uty, men, den, dnm, bnm, bytesOut);
    }
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrderSchedulingUpdate", bytesOut);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void doIt(PrintWriter out, HttpServletRequest req, byte[] schConCodes, byte[] salesPeople, String unm, String sid, String uty, String men,
                    String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrderSchedulingUpdate", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrderSchedulingUpdate", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    int x=0;
    int len = generalUtils.lengthBytes(schConCodes, 0);
    while(x < len)
    {
      code = "";
      while(schConCodes[x] != '\001' && schConCodes[x] != '\000')
        code += (char)schConCodes[x++];

      updateSO(con, stmt, code.substring(6), "SchedulingConfirmed", today, unm);
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/SalesOrderSchedulinga?unm=" + unm  + "&sid=" + sid + "&uty=" + uty
