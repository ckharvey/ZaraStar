// =======================================================================================================================================================================================================
// System: ZaraStar Document: Proforma Invoice to Invoice
// Module: ProformaInvoiceToSalesInvoice.java
// Author: C.K.Harvey
...
public class ProformaInvoiceToSalesInvoice
{
                        ...
                        String localDefnsDir, String defnsDir, byte[] newDocCode, int[] bytesOut) throws Exception
  {
    ...
    if(proformaInvoice.getRecGivenCode(con, stmt, rs, code, '\000', data, dnm, localDefnsDir, defnsDir, bytesOut) == -1)
    {
      generalUtils.strToBytes(newDocCode, "h0031.htm"); // not found
      return false;
    }

    proformaToInvoice(con, stmt, rs, 'H', newDocCode, data, dataTo, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    linesData = proformaInvoice.getLines(con, stmt, rs, code, '\000', linesData, listLen, linesCount, dnm, localDefnsDir, defnsDir);
    ...
    for(int xx=0;xx<linesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, linesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          ...
          proformaToInvoice(con, stmt, rs, 'L', newDocCode, data, dataTo, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    for(int xx=0;xx<multipleLinesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, multipleLinesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          proformaToInvoice(con, stmt, rs, 'M', newDocCode, data, dataTo, unm, dnm, localDefnsDir, defnsDir, b);
          salesInvoice.putMultipleLine(con, stmt, rs, newDocCode, newLines[z], dataTo);
        }
      }
    }

    documentAttachmentsCopy.copyAttachments(con, stmt, rs, "proformaa", code, "invoicea", newDocCode, dnm, defnsDir, localDefnsDir);                 

    return true;
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void proformaToInvoice(Connection con, Statement stmt, ResultSet rs, char which, byte[] newDocCode, byte[] fromBuf, byte[] toBuf, String unm, String dnm, String localDefnsDir,
                                 String defnsDir, byte[] b) throws Exception
  {
    ...
    switch(which)
    {
      ...
      case 'L' : // line
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)27, "0.0"); // costPrice

                 generalUtils.dfs(fromBuf, (short)28, b); // manufacturer
                 generalUtils.repAlpha(toBuf, 3000, (short)28, b);

                 generalUtils.dfs(fromBuf, (short)29, b); // manufacturerCode
                 generalUtils.repAlpha(toBuf, 3000, (short)29, b);
