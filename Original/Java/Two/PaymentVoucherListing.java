// =============================================================================================================================================================
// System: ZaraStar DocumentEngine: PV listing page
// Module: PaymentVoucherListing.java
// Author: C.K.Harvey
...
public class PaymentVoucherListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    String unm="", sid="", uty="", men="", den="", dnm="", bnm="", inx="", codeArg="", codeArg2="", srchStr="", operation="",  firstRecNum="", lastRecNum="",
           callType="", maxRows="", numRecs="", companyCode="";
    try
    {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PaymentVoucherListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6074, bytesOut[0], 0, "ERR:" + codeArg);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String inx,
                    ...
                    int numRecs, String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6074, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PaymentVoucherListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6074, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PaymentVoucherListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6074, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6074, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
  ...
  private void generate(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men,
                        ...
                        String firstRecNum, String lastRecNum, int maxRows, int numRecsIn, String companyCode, String imagesDir, String localDefnsDir,
                        String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    createPage(con, stmt, rs, html, operation2, callType, codeArg, codeArg2, inx, srchStr, numRecs, firstCodeOnPage, lastCodeOnPage, firstEntryOnPage,
               lastEntryOnPage, firstRecNum, lastRecNum, newFirstRecNum, newLastRecNum, maxRows, companyCode, imagesDir);
    ...
    setHead(con, stmt, rs, out, req, inx, unm, sid, uty, men, den, dnm, bnm, newFirstRecNum[0], newLastRecNum[0], numRecs[0], companyCode, localDefnsDir,
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void createPage(Connection con, Statement stmt, ResultSet rs, String[] html, char operation, char callType, String arg, String arg2, char inx,
                          String srchStr, int[] numRecs, String[] firstCodeOnPage, String[] lastCodeOnPage, String[] firstEntryOnPage, String[] lastEntryOnPage,
                          String firstRecNum, String lastRecNum, String[] newFirstRecNum, String[] newLastRecNum, int maxRows, String companyCode,
                          String imagesDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM voucher WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM voucher" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM voucher WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM voucher WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM voucher" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM voucher WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(ChequeNumber) AS rowcount FROM voucher WHERE ChequeNumber LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT ChequeNumber) AS rowcount FROM voucher" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM voucher WHERE ChequeNumber = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT VoucherCode) AS rowcount FROM voucher WHERE VoucherCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM voucher" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String voucherCode="", name="", supplierCode="", date="", chequeNumber="", type="", amount="", currency="", status;
    int voucherCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, chequeNumberPosn, typePosn, amountPosn,
        currencyPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY VoucherCode";
                   }
                   else
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher " + companyCodeWHERE
                       + "ORDER BY VoucherCode";
                   }

                   voucherCodePosn = 1; datePosn = 2; typePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6;
                   chequeNumberPosn = -1; amountPosn = -1; currencyPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, VoucherCode, Type, CompanyCode, CompanyName, Status FROM voucher WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     voucherCodePosn = 2; datePosn = 1; typePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6;
                     chequeNumberPosn = -1; amountPosn = -1; currencyPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM voucher ORDER BY Date" + companyCodeWHERE;
                     
                     voucherCodePosn = -1; datePosn = 1; typePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     chequeNumberPosn = -1; amountPosn = -1; currencyPosn = -1;
                   }
                   else
                   {
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, VoucherCode, Date, Type, CompanyName, Status FROM voucher WHERE "
                      + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, VoucherCode";
                     
                     voucherCodePosn = 2; datePosn = 3; typePosn = 4; companyCodePosn = 1; companyNamePosn = 5; statusPosn = 6;
                     chequeNumberPosn = -1; amountPosn = -1; currencyPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM voucher " + companyCodeWHERE + "ORDER BY CompanyCode";
               ...
               {  
                 if(callType == 'S')
                 {
                   q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM voucher WHERE "
                    + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, VoucherCode";
                   
                   voucherCodePosn = 2; datePosn = -1; typePosn = -1; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 7;
                   chequeNumberPosn = 1; amountPosn = 3; currencyPosn = 6;
                 }
                 else
                 if(callType == 'G')
                 {
                   q = "SELECT DISTINCT ChequeNumber FROM voucher " + companyCodeWHERE + "ORDER BY ChequeNumber";
                 ...
                 {
                   q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM voucher WHERE "
                     + "ChequeNumber = '" + codeArg + "' ORDER BY ChequeNumber";
                 ...
                 while(rs.next())                  
                 {
                   voucherCode  = getValue(voucherCodePosn,  ' ', rs, rsmd);
                   date         = getValue(datePosn,         'D', rs, rsmd);
                   supplierCode = getValue(companyCodePosn,  ' ', rs, rsmd);
                   name         = getValue(companyNamePosn,  ' ', rs, rsmd);
                   type         = getValue(typePosn,         ' ', rs, rsmd);
                   amount       = getValue(amountPosn,       ' ', rs, rsmd);
                   currency     = getValue(currencyPosn,     ' ', rs, rsmd);
                   chequeNumber = getValue(chequeNumberPosn, ' ', rs, rsmd);
                   status       = getValue(statusPosn,       ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, voucherCode, date, type, supplierCode, name, null, voucherCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date, cssFormat);
                     else appendBodyLine(html, inx, date, voucherCode, type, supplierCode, name, null, voucherCode, status, imagesDir, cssFormat);
                   }   
                   else
                   if(inx == '3')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, supplierCode, cssFormat);
                     else
                     {
                       appendBodyLine(html, inx, supplierCode, voucherCode, date, type, name, null, voucherCode, status, imagesDir, cssFormat);
                     }
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, chequeNumber, cssFormat);
                     else
                     {
                       appendBodyLine(html, inx, chequeNumber, voucherCode, amount, companyCode, name, currency, voucherCode, status, imagesDir, cssFormat);
                     }
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = voucherCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = voucherCode;
                     else
                     if(inx == '2')
                     ...
                     if(inx == '3')
                       ...
                       firstEntryOnPage[0] = chequeNumber;
                   }
                 }
                 
                 rs.close();
                                 
                 lastCodeOnPage[0] = voucherCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = voucherCode;
                 else
                 if(inx == '2')
                 ...
                 if(inx == '3')
                   ...
                   lastEntryOnPage[0] = chequeNumber;
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode LIKE '"
                       + srchStr + "%' AND VoucherCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY VoucherCode DESC";
                   }
                   else
                   {  
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode "
                       + "<= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY VoucherCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM voucher WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, VoucherCode, Type, CompanyCode, CompanyName, Status FROM voucher WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, VoucherCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM voucher WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, VoucherCode, Date, Type, CompanyName, Status FROM voucher WHERE "
                        + "CompanyCode = '" + codeArg + "' ORDER BY CompanyCode, VoucherCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM voucher WHERE ChequeNumber <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY ChequeNumber DESC";
                   ...
                   {
                      q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM voucher WHERE "
                        + "ChequeNumber = '" + codeArg + "' ORDER BY ChequeNumber, VoucherCode DESC";
                    
                      voucherCodePosn = 2; datePosn = -1; typePosn = -1; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 7;
                      chequeNumberPosn = -1; amountPosn = -1; currencyPosn = 6;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, voucherCode, date, type, supplierCode, name, null, voucherCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date, cssFormat);
                     else prependBodyLine(html, inx, date, voucherCode, type, supplierCode, name, null, voucherCode, status, imagesDir, cssFormat);
                   }  
                   else
                   if(inx == '3')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, supplierCode, cssFormat);
                     else 
                     {
                       prependBodyLine(html, inx, supplierCode, voucherCode, date, type, name, null, voucherCode, status, imagesDir, cssFormat);
                     }
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, chequeNumber, cssFormat);
                     else 
                     {
                       prependBodyLine(html, inx, chequeNumber, voucherCode, amount, companyCode, name, currency, voucherCode, status, imagesDir, cssFormat);
                     }
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode LIKE '"
                       + srchStr + "%' AND VoucherCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY VoucherCode";
                   ...
                   {                   
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode > '"
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY VoucherCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM voucher WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, VoucherCode, Type, CompanyCode, CompanyName, Status FROM voucher WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND VoucherCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM voucher WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, VoucherCode, Date, Type, CompanyName, Status FROM voucher WHERE "
                       + "CompanyCode = '" + codeArg + "' AND VoucherCode > '" + codeArg2 + "' ORDER BY CompanyCode, VoucherCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM voucher WHERE ChequeNumber >= '" + codeArg + "' ORDER BY ChequeNumber";
                   ...
                   {
                     q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM voucher WHERE "
                       + "ChequeNumber = '" + codeArg + "' AND VoucherCode > '" + codeArg2 + "' ORDER BY ChequeNumber, VoucherCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM voucher WHERE VoucherCode < '" + codeArg 
                     + "'" + companyCodeANDWHERE + " ORDER BY VoucherCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   ...
                   else
                   {
                     q = "SELECT Date, VoucherCode, Type, CompanyCode, CompanyName, Status FROM voucher WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND VoucherCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, VoucherCode DESC";
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM voucher WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, VoucherCode, Date, Type, CompanyName, Status FROM voucher WHERE "
                       + "CompanyCode = '" + codeArg + "' AND VoucherCode < '" + codeArg2 + "' ORDER BY CompanyCode, VoucherCode DESC";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT ChequeNumber FROM voucher WHERE ChequeNumber < '" + codeArg + "' ORDER BY ChequeNumber DESC";
                   ...
                   {
                     q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM voucher WHERE "
                       + "ChequeNumber = '" + codeArg + "' AND VoucherCode < '" + codeArg2 + "' ORDER BY ChequeNumber, VoucherCode DESC";
                      ...
                      int maxRows, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Voucher Search Results for ";
        switch(inx)
        {
          case '1' : title += "Voucher Code";  break;
          case '2' : title += "Voucher Date";  break;
          case '3' : title += "Supplier Code"; break;
          case '4' : title += "Cheque Number"; break;
        }
      ...
      else title = "Voucher Listing";

      drawingUtils.drawTitle(out, false, false, "PaymentVoucherListing", "", title, "6074", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Voucher Code");   break;
        case '2' : scout(out, bytesOut, "Voucher Date");   break;
        case '3' : scout(out, bytesOut, "Supplier Code");  break;
        case '4' : scout(out, bytesOut, "Cheque Numebr");  break;
      }
    }
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       String men, String den, String dnm, String bnm, String newFirstRecNum, String newLastRecNum, int numRecs, String companyCode,
                       String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Voucher Listing</title>");
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PaymentVoucherListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PaymentVoucherListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "6074", "", "PaymentVoucherListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Type &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Type &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Type &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Cheque Number &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Voucher Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Amount &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Voucher Date");   break;
      case '3' : scoutln(out, bytesOut, "Supplier Code");  break;
      case '4' : scoutln(out, bytesOut, "Cheque Number");  break;
    }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six, String code, String status,
                              String imagesDir, String[] cssFormat) throws Exception
  {
    ...
    { 
      ...
      case '4' : html[0] += "<td nowrap><p>" + one + "&nbsp;</td>";
                 ...
                 html[0] += "<td nowrap><p>" + three + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + four + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + five + " " + six + "&nbsp;</td></tr>\n";
                 break;
    }
  }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six, String code, String status,
                               String imagesDir, String[] cssFormat) throws Exception
  {
    if(cssFormat[0].equals("line1")) cssFormat[0] = "line2"; else cssFormat[0] = "line1";
