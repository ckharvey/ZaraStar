// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Quotes update line rec
// Module: QuotationLineUpdate.java
// Author: C.K.Harvey
...
public class QuotationLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "QuotationLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4026, bytesOut[0], 0, "ERR:" + p2);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String dnm, String men, String den, String bnm, String code, String line, char cad, char thisOp, String saveStr, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4026", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4026, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4026", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4026, bytesOut[0], 0, "SID:" + code);
      if(con  != null) con.close();
      if(conp[0] != null) conp[0].close();
      if(out != null) out.flush();
      return;
    ...
    switch(thisOp)
    {
      case 'D' :  if(quotation.deleteLine(con, stmt, rs, codeB, lineB, bytesOut))
                    getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "4026", imagesDir, localDefnsDir, defnsDir, bytesOut);
                  break;
      case 'X' :  // cancel
                  getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  break;
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, quotation.getFieldNamesQuoteL(), quotation.getFieldTypesQuoteL(), quotation.getFieldStylesQuoteL(), "quotel", "Update Quotation Record", dnm, unm, recData, y, imagesDir,
                                                    localDefnsDir, defnsDir))
                  {
                    switch(quotation.putLine(con, conp[0], stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Quotation Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Quotation Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 4027; operation = " Line: " + line;  break;
      case 'C' : service = 4026; operation = " Line: " + line;  break;
      case 'X' : service = 4026; operation = " Cancel";         break;
      default  : service = 4026; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                          ...
                          throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/QuotationLine");
