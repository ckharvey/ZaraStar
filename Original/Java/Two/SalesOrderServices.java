// =======================================================================================================================================================================================================
// System: ZaraStar Document: SO create focus page
// Module: SalesOrderServices.java
// Author: C.K.Harvey
...
public class SalesOrderServices extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrderServices", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 140, bytesOut[0], 0, "ERR:");
                      ...
                      String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "140", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 140, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "140", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 140, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 140, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush(); 
  }
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Sales Order Focus</title>");
    scoutln(out, bytesOut, "<script language=\"JavaScript\">");

    boolean DocLibEdit2 = authenticationUtils.verifyAccess(con, stmt, rs, req, 4032, unm, uty, dnm, localDefnsDir, defnsDir);
    ...
    if(DocLibEdit2)
    {
      ...
      scoutln(out, bytesOut, "var p1;if(type=='Q')p1='CREATEBASEDONQUOTE:'+sanitise(document.forms[0].doccode.value);");
      scoutln(out, bytesOut, "else if(type=='S')p1='CREATEBASEDONSO:'+sanitise(document.forms[0].doccode2.value);");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/SalesOrderPage?unm=" + unm + "&sid=" + sid + "&uty="
                             + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p3=&p4=&p1=\"+p1;}");
    }
    ...
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/SalesOrderListing?unm=" + unm + "&sid=" + sid + "&uty="
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "140", "", "SalesOrderServices", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesOrderServices", "", "Sales Order Focus", "140", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form><table id=\"page\" border=0 cellspacing=2 cellpadding=0 width=100%>");

    scoutln(out, bytesOut, "<tr><td colspan=5 nowrap><h1>Sales Order Fetch</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td colspan=5 nowrap><h1>Sales Order Access Listings</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td></td><td nowrap><p><a href=\"javascript:list(1)\")>List</a> by Sales Order Code</td></tr>");

    scoutln(out, bytesOut, "<tr><td></td><td nowrap><p><a href=\"javascript:list(2)\")>List</a> by Sales Order Date</td></tr>");
