// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: SO to OA
// Module: SalesOrderToOrderAcknowledgement.java
// Author: C.K.Harvey
...
public class SalesOrderToOrderAcknowledgement
{
  ...
  public boolean create(Connection con, Statement stmt, ResultSet rs, byte[] code, char renumberLines, char renumberEntries, int[] lines,
                        int numLines, String quantitiesReqd, String unm, String dnm, String localDefnsDir, String defnsDir, byte[] newDocCode,
                        int[] bytesOut) throws Exception
  {
    documentUtils.getNextCode(con, stmt, rs, "oa", true, newDocCode);
    ...
    if(orderAcknowledgement.getRecGivenCode(con, stmt, rs, newDocCode, '\000', data, dnm, localDefnsDir, defnsDir, bytesOut) != -1) // just-in-case
    {
    ...
    soToOA('H', newDocCode, data, dataTo, code, unm, localDefnsDir, defnsDir, b);

    if(orderAcknowledgement.putRecHeadGivenCode(con, stmt, rs, newDocCode, 'N', dataTo, dnm, localDefnsDir, defnsDir) != ' ')
    {
    ...
    for(int xx=0;xx<linesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, linesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          ...
          soToOA('L', newDocCode, data, dataTo, code, unm, localDefnsDir, defnsDir, b);
          orderAcknowledgement.putRecLine(con, stmt, rs, newDocCode, null, 'N', dataTo, dnm, localDefnsDir, defnsDir);
        }
      }
    }  
    ...
    for(int xx=0;xx<multipleLinesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, multipleLinesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          soToOA('M', newDocCode, data, dataTo, code, unm, localDefnsDir, defnsDir, b);
          orderAcknowledgement.putMultipleLine(con, stmt, rs, newDocCode, newLines[z], dataTo);
        }
      }
    }

    documentAttachmentsCopy.copyAttachments(con, stmt, rs, "soa", code, "oaa", newDocCode, dnm, defnsDir, localDefnsDir);                 

    return true;
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void soToOA(char which, byte[] newDocCode, byte[] fromBuf, byte[] toBuf, byte[] soCode, String unm, String localDefnsDir,
