// =======================================================================================================================================================================================================
// System: ZaraStar: Quotation & Sales Order Processing - Engineering
// Module: SalesOrderEngineeringExecute.java
// Author: C.K.Harvey
...
public class SalesOrderEngineeringExecute extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrderEngineeringExecute", bytesOut);
                    ...
                    String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrderEngineeringExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrderEngineeringExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "2031", "", "SalesOrderEngineeringExecute", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesOrderEngineeringExecute", "", "Sales Order Processing - Engineering", "2031", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form name=\"go\" action=\"SalesOrderEngineerUpdate\" enctype=\"application/x-www-form-urlencoded\" method=POST>");
    ...
    scoutln(out, bytesOut, "<td><p><b> Drawings Submitted </b></td>");
    scoutln(out, bytesOut, "<td><p><b> Engineering Approved </b></td>");
                                       ...
                                       String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT SalesPerson, SOCode, Date, CompanyCode, CustomerPOCode, TotalTotal, Currency2, CompanyName, DrawingsFiled, "
                         + "DrawingsFiledDate, DrawingsFiledSignOn, ToProcurement, ProcurementConfirmed, ToScheduling, SchedulingConfirmed, "
                         + "ProcurementConfirmedDate, ProcurementConfirmedSignOn, SchedulingConfirmedDate, SchedulingConfirmedSignOn, "
                         + "ToProcurementDate, ToProcurementSignOn, ToSchedulingDate, ToSchedulingSignOn, SOType "
                         + "FROM so WHERE ToEngineering = 'Y' AND EngineeringApproved != 'Y' AND Status != 'C' AND AllSupplied != 'Y' AND SOType != 'T' ORDER BY Date, SOCode");
      
    String salesPerson, soCode, date, customerCode, custPOCode, currency, companyName, drawingsFiled, drawingsFiledDate, drawingsFiledSignOn,
           toScheduling, schedulingConfirmed, toProcurement, procurementConfirmed, procurementConfirmedDate, procurementConfirmedSignOn,
           schedulingConfirmedDate, schedulingConfirmedSignOn, toProcurementDate, toProcurementSignOn, toSchedulingDate, toSchedulingSignOn, soType;
    ...
    {    
      ...
      if(salesPersonIsRequired(salesPerson, checkBoxes))
      {
        ...
        drawingsFiled        = generalUtils.deNull(rs.getString(9));
        drawingsFiledDate    = generalUtils.deNull(rs.getString(10));
        drawingsFiledSignOn  = generalUtils.deNull(rs.getString(11));
        toProcurement        = generalUtils.deNull(rs.getString(12));
        procurementConfirmed = generalUtils.deNull(rs.getString(13));
        toScheduling         = generalUtils.deNull(rs.getString(14));
        schedulingConfirmed  = generalUtils.deNull(rs.getString(15));
        procurementConfirmedDate   = generalUtils.deNull(rs.getString(16));
        procurementConfirmedSignOn = generalUtils.deNull(rs.getString(17));
        schedulingConfirmedDate    = generalUtils.deNull(rs.getString(18));
        schedulingConfirmedSignOn  = generalUtils.deNull(rs.getString(19));
        toProcurementDate          = generalUtils.deNull(rs.getString(20));
        toProcurementSignOn        = generalUtils.deNull(rs.getString(21));
        toSchedulingDate           = generalUtils.deNull(rs.getString(22));
        toSchedulingSignOn         = generalUtils.deNull(rs.getString(23));
        soType                     = generalUtils.deNull(rs.getString(24));
        ...
        if(drawingsFiled.equals("Y"))
          scoutln(out, bytesOut, "<td><p>" + generalUtils.convertFromYYYYMMDD(drawingsFiledDate) + " by " + drawingsFiledSignOn + "</td>");
        else scout(out, bytesOut, "<td align=center><p><input type=checkbox name=draw" + generalUtils.sanitise(soCode) + "></td>");

        scout(out, bytesOut, "<td align=center><p><input type=checkbox name=engApp" + generalUtils.sanitise(soCode) + "></td>");
                    
        if(! toProcurement.equals("Y"))
         scout(out, bytesOut, "<td align=center><p><font color=red>Not Confirmed</td>");
        else
        {
          ...
          {
            scout(out, bytesOut, "<td><p><font color=green>" + generalUtils.convertFromYYYYMMDD(procurementConfirmedDate)  + " by "
        ...
        scoutln(out, bytesOut, "<td><p><a href=\"javascript:viewCust('" + customerCode + "')\">" + customerCode + "</a></td>");
