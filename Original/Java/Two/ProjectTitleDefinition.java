// =======================================================================================================================================================================================================
// System: ZaraStar Project: Title Definition
// Module: ProjectTitleDefinition.java
// Author: C.K.Harvey
...
public class ProjectTitleDefinition extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ProjectTitleDefinition", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6804, bytesOut[0], 0, "ERR:");
                    ...
                    String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ProjectTitleDefinition", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6804, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "ProjectTitleDefinition", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6804, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6804, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush();
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void create(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                      String bnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Project - Title Definition</title>");
    ...
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/ProjectTitleDefinitionUpdate?unm=\" + escape('" + unm
    ...
    scoutln(out, bytesOut, "{");
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/ProjectTitleDefinitionDelete?unm=\" + escape('" + unm+ "') + \"&sid=\" + escape('" + sid + "') + \"&uty=\" + escape('" + uty + "') + \"&men=\" + escape('" + men
                         + "') + \"&den=\" + escape('" + den + "') + \"&bnm=\" + escape('" + bnm
    ...
    scoutln(out, bytesOut, "this.location.replace(\"/central/servlet/ProjectTitleDefinition?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
                           + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "\");}");
    
    scoutln(out, bytesOut, "</script>");

    int[] hmenuCount = new int[1];
    projectUtils.outputPageFrame(con, stmt, rs, out, req, "6804", "ProjectTitleDefinition", "", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    projectUtils.drawTitle(con, stmt, rs, req, out, "ProjectTitleDefinition", "Project Title Definition", "6804", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<td nowrap><p>&nbsp; Title &nbsp;</td></tr>");
    ...
    scoutln(out, bytesOut, "<tr><td><span id=\"namebox\"><input type=text name=name size=60 maxlength=60></span></td></tr>");
    
    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");
    scoutln(out, bytesOut, "<tr><td colspan=3><span id=\"option\"><p><a href=\"javascript:updateN()\">Add the Title</a></span></td></tr>");
  ...
  private void list(PrintWriter out, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    try
    {
      ...
      while(rs.next())                  
      {
        ...
        scoutln(out, bytesOut, "<td nowrap align=center>" + name + "</td>");
