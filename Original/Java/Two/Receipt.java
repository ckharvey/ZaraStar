// =======================================================================================================================================================================================================
// System: ZaraStar Document: Receipt Record Access
// Module: receipt.java
// Author: C.K.Harvey
...
public class Receipt
{
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringReceipt() throws Exception
  {
    return "receipt ( ReceiptCode char(20) not null, Date date not null,               CompanyCode char(20) not null, "
                   ...
                   + "TotalAmount decimal(19,8),     DateReceived date,                AccCredited char(20), "
                   ...
                   + "ARCode char(20),               Currency char(3),                 Rate decimal(16,8), "
                   + "BaseTotalAmount decimal(19,8), Status char(1),                   BaseGSTComponent decimal(19,8), "
                   + "CashOrNot char(1),             ReceiptReference char(20),        Reconciled char(1), "
                   + "BankAccount char(40),          Charges decimal(19,8),            DiscountAllowed decimal(19,8), "
                   + "Processed char(1),             DateProcessed date,               BankedAmount decimal(19,8), "
                   + "RateBankToBase decimal(16,8),  ExchangeAdjustment decimal(16,8), "
                   + "unique(ReceiptCode))";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsReceipt(String[] s) throws Exception
  {
    s[0] = "receiptDateInx on receipt (Date)";
    s[1] = "receiptCompanyCodeInx on receipt (CompanyCode)";

    return 2;
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesReceipt() throws Exception
  {
    return "ReceiptCode, Date, CompanyCode, CompanyName, Address1, Address2, Address3, Address4, Address5, PostCode, FAO, Notes, "
         + "Attention, Misc1, Misc2, TotalAmount, DateReceived, AccCredited, AccDebited, unused1, DateLastModified, SignOn, "
         + "unused2, ProjectCode, GSTComponent, ChequeNumber, PrintCount, ARCode, Currency, Rate, BaseTotalAmount, Status, "
         + "BaseGSTComponent, CashOrNot, ReceiptReference, Reconciled, BankAccount, Charges, DiscountAllowed, Processed, "
         + "DateProcessed, BankedAmount, RateBankToBase, ExchangeAdjustment";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesReceipt() throws Exception
  {
    return "CDCCCCCCCCCCCCCFDCCCSCCCFCICCFFCFCCCCFFCDFFF";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesReceipt(short[] sizes) throws Exception
  {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesReceipt() throws Exception
  {
    return "MMMOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringReceiptL() throws Exception
  {
    return "receiptl ( ReceiptCode char(20) not null,  InvoiceCode char(20) not null,  InvoiceDate date, "
                    + "InvoiceAmount decimal(19,8),    Description char(80),           AmountReceived decimal(19,8), "
                    + "DateLastModified timestamp,     SignOn char(20),                unused1 char(20), "
                    + "unused2 char(20),               OriginalRate decimal(16,8),     unused3 decimal(19,8), "
                    + "GSTComponent decimal(19,8),     AccountDr char(20),             InvoiceLine integer, "
                    + "Line integer not null,          Entry char(6),                  BaseAmountReceived decimal(19,8), "
                    + "BaseGSTComponent decimal(19,8), "
                    + "unique(ReceiptCode, Line))";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsReceiptL(String[] s) throws Exception
  {
    s[0] = "receiptlInvoiceCodeInx on receiptl(InvoiceCode)";

    return 1;
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesReceiptL() throws Exception
  {
    return "ReceiptCode, InvoiceCode, InvoiceDate, InvoiceAmount, Description, AmountReceived, DateLastModified, SignOn, unused1, "
         + "unused2, OriginalRate, unused3, GSTComponent, AccountDr, InvoiceLine, Line, Entry, BaseAmountReceived, "
         + "BaseGSTComponent";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesReceiptL() throws Exception
  {
    return "CCDFCFSCCCFFFCIICFF";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesReceiptL(short[] sizes) throws Exception
  {
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesReceiptL() throws Exception
  {
    return "OMOOOOOOOOOOOOOOMOO";
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  public boolean getRecToHTML(Connection con, Statement stmt, ResultSet rs, PrintWriter out, char dispOrEdit, byte[] code, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String workingDir,
                              String localDefnsDir, String defnsDir, char cad, String errStr, String cashOrNot, byte[] dataAlready, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    if(cad == 'A' && code[0] == '\000')
    {
      messagePage.msgScreen(false, out, req, 6, unm, sid, uty, men, den, dnm, bnm, "Receipt", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      return true;
    }
    ...
    {                            // in the correct order
      dispOrEdit = 'E';
      sortFields(dataAlready, headData, "receipt");
    }
    ...
    short[] fieldSizesReceipt = new short[70]; // plenty
    getFieldSizesReceipt(fieldSizesReceipt);

    if(cad == 'A') // not a new one
    {
      short[] fieldSizesReceiptL = new short[30]; // plenty
      getFieldSizesReceiptL(fieldSizesReceiptL);
      ...
      if(dispOrEdit == 'D') // display head *and* lines
      {
        ...
        stmt.executeUpdate("UPDATE receipt SET TotalAmount = " + totalAmtLines + ", BaseTotalAmount = " + totalBaseAmtLines + " WHERE ReceiptCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
        stmt.close();
      }

      if(dispOrEdit == 'D')
      {
        numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "266.htm", 2, getFieldNamesReceipt(), fieldSizesReceipt, getFieldNamesReceiptL(), fieldSizesReceiptL, null, null);
      }
      else
      {
        numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "266a.htm", 1, getFieldNamesReceipt(), fieldSizesReceipt, null, null, null, null);
      }
      ...
      {
        prependEdit(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, "A", generalUtils.stringFromBytes(code, 0L), date, "266a.htm", localDefnsDir, defnsDir, prependCode, req, bytesOut);
      }
      ...
      if(dispOrEdit == 'D') // display head *and* lines
      {
        ...
        for(int x=0;x<linesCount[0];++x)
        {
          ...
          generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 5000, 6); // amountReceived
          ...
          generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 5000, 18); // baseAmountReceived
      ...
      scoutln(headData, bytesOut, "receipt.basecurrency=" + accountsUtils.getBaseCurrency(con, stmt, rs, dnm, localDefnsDir, defnsDir) + "\001");

      generalUtils.bytesDPsGivenSeparator(true, '\001', '2', headData, 5000, 29); // rate
      scoutln(headData, bytesOut, "receipt.rateNoTrailing=" + generalUtils.dfsAsStrGivenBinary1(true, headData, (short) 29) + "\001");    
    }
    else // edit
    {
      ddlData = accountsUtils.getCurrencyNamesDDLData(con, stmt, rs, "receipt.Currency", dnm, ddlData, ddlDataUpto, ddlDataLen, localDefnsDir, defnsDir);
      ddlData = accountsUtils.getAROrAPAccountsDDLData('R', "receipt.AccDebited", unm, dnm, ddlData, ddlDataUpto, ddlDataLen, workingDir);
      
      generalUtils.dfsGivenSeparator(true, '\001', headData, (short)18, b); // accountDr
      if(b[0] != '\000')
      {
        generalUtils.repAlphaGivenSeparator('\001', headData, 2000, (short)18, accountsUtils.getAccountPlusDescriptionGivenAccCode(b, unm, dnm, workingDir, localDefnsDir, defnsDir));
      }
    ...
    if(dataAlready[0] == '\000') // NOT coming with an err msg
    {
      ...
      generalUtils.bytesDPsGivenSeparator(true, '\001', '2', headData, 5000, 41); // bankedAmount
      ...
      generalUtils.dfsGivenSeparator(true, '\001', headData, (short) 16, b); // dateReceived
      generalUtils.convertFromYYYYMMDD(b);
    ...
    // a new layout
    {
      numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "266a.htm", 1,
                                      getFieldNamesReceipt(), fieldSizesReceipt, null, null, null, null);

      if(dataAlready[0] != '\000') // coming with an err msg - passing-in data already entered but the data's fields may (will) not
      {                            // be in the correct order
        sortFields(dataAlready, data, "receipt");
      }
      else
      {
        screenLayout.buildNewDetails(fieldNames, numFields, "receipt", data);

        documentUtils.getNextCode(con, stmt, rs, "receipt", true, code);

        generalUtils.repAlphaUsingOnes(data, 5000, "ReceiptCode", generalUtils.stringFromBytes(code, 0L));
      ...
      prependEdit(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, "C", generalUtils.stringFromBytes(code, 0L), date, "266a.htm", localDefnsDir,
  ...
  {
    b[0] = '\000';
    scoutln(b, bytesOut, "_.lineFile=receiptl.line\001");
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4211, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "receipt", date, unm))
  ...
  {
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4209, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      b[0] = '\000';
      scoutln(b, bytesOut, "<script language=\"JavaScript\">");
      scoutln(b, bytesOut, "function affect(line){");
      scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/ReceiptLine?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den
                       ...
                       String code, String date, byte[] b, HttpServletRequest req, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4207, unm, uty, dnm, localDefnsDir, defnsDir))
    {
    scoutln(b, bytesOut, "function fetch(){");
    scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/ReceiptHeaderEdit?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den="
    ...
    scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/ReceiptLine?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den="
            + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2
            + "&p2=&p3=&p4=\");}");
    }
    ...
    scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/MailExternalUserCreate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den="
            + den + "&dnm=" + dnm + "&p1=ReceiptHeaderEdit&p2=" + code2
            + "&p3=Receipt&bnm=" + bnm + "\");}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 11800, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/TrailShow?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
                         + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=E\";}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4212, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/AdminPrintControl?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
                          + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=ReceiptPrint&p2=" + code2 + "&p3=\";}");
    }

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4189, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/FaxCreate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men
                         + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=E&p3=ReceiptHeaderEdit&p4=&p5=Receipt\";}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 11200, unm, uty, dnm, localDefnsDir, defnsDir) && authenticationUtils.verifyAccess(con, stmt, rs, req, 4051, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/PrintToPDFUtils?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
                         + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=" + code2 + "&p1=0.000&p5=Receipt&p2=ReceiptPrint\";}");
    }
    ...
    outputPageFrame(con, stmt, rs, b, req, ' ', ' ', "", "ReceiptPage", date, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
                           ...
                           HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    sourceBuf = htmlBuild.layoutToBuffer(sourceBuf, 5000, localDefnsDir,
        defnsDir, layoutFile);
    ...
    scoutln(b, bytesOut, "main.style.visibility='visible';second.style.visibility='hidden';submenu.style.visibility='visible';}");

    scoutln(b, bytesOut, "function save(thisOp){var saveStr=\"\";" + saveStr + "\n");
    scoutln(b, bytesOut, "var saveStr2=strip(saveStr);window.location.replace(\"/central/servlet/ReceiptHeaderUpdate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
    ...
    outputPageFrame(con, stmt, rs, b, req, ' ', cad.charAt(0), "", "ReceiptHeaderEdit", date, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
                      ...
                      byte[] recData, int recDataLen, byte[] rtnLineBytes, int[] bytesOut) throws Exception
  {
    ...
    if(searchDataString(recData, recDataLen, "receiptl", b, lineBytes) == -1)
      return 'N';
    ...
    sortFields(recData, buf, "receiptl"); // sorts the data field (using buf in the process); results are put back into recData
    recDataLen = generalUtils.lengthBytes(recData, 0);
    generalUtils.zeroize(buf, 2000);

    String fieldNames = getFieldNamesReceiptL();
    ...
    if(searchDataString(recData, recDataLen, "receiptl", fieldName, value) != -1) // entry exists
    {
    if(fieldCount == 2) // invoicedate
    {
    generalUtils.putAlpha(buf, 2000, (short) 2, generalUtils.convertDateToSQLFormat(generalUtils
        .stringFromBytes(value, 0L)));
    }
    else
      ...
      if(value[0] == '\000')
        ...
        generalUtils.repAlpha(buf, 2000, (short) 10, value);
      }
      else
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void updateDLM(Connection con, Statement stmt, ResultSet rs, byte[] code, String unm, String dnm,
      String localDefnsDir, String defnsDir) throws Exception
  {
    stmt = con.createStatement();
    stmt.executeUpdate("UPDATE receipt SET SignOn = '" + unm
        + "', DateLastModified = NULL WHERE ReceiptCode = '"
        + generalUtils.stringFromBytes(code, 0L) + "'");
    if(stmt != null) stmt.close();
  }

  // ------------------------------------------------------------------------------------------------------------------------------------------------
  public String getAReceiptFieldGivenCode(Connection con, Statement stmt, ResultSet rs, String fieldName, String ocCode, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    byte[] value = new byte[300]; // plenty
    byte[] b     = new byte[21];
    generalUtils.strToBytes(b, ocCode);
    
    getAReceiptFieldGivenCode(con, stmt, rs, fieldName, b, value, dnm, localDefnsDir, defnsDir);
    
    return generalUtils.stringFromBytes(value, 0L);
  }
  public void getAReceiptFieldGivenCode(Connection con, Statement stmt, ResultSet rs, String fieldName, byte[] receiptCode, byte[] value, String dnm, String localDefnsDir, String defnsDir)
                                        throws Exception
  {
    if(receiptCode[0] == '\000') // just-in-case
      return;

    generalUtils.toUpper(receiptCode, 0);

    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT " + fieldName + " FROM receipt WHERE ReceiptCode = '" + generalUtils.stringFromBytes(receiptCode, 0L) + "'");
    if(!rs.next())
      value[0] = '\000';
    else
    {
  ...
  public int searchDataString(byte[] data, int lenData, String tableName,
                                        ...
                                        byte[] dataAlready, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, layoutFile, 1,
                                     getFieldNamesReceiptL(), fieldSizesReceiptL, null, null, null, null);
    ...
    if(line[0] != '\000') // existing line
    {
      if(code[0] != '\000') // just-in-case
      {
        if(dataAlready[0] != '\000') // coming with an err msg - passing-in data already entered but the data's fields may (will) not
        {                          // be in the correct order
          sortFields(dataAlready, data, "receiptl");
        }
        else
        {
          ...
          generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 2000, 5); // amountReceived
          ...
          generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 2000, 17); // baseAmountReceived
    ...
    else // new line
    {
      ...
      {
        data[0]= '\000';
        scoutln(data, bytesOut, "receiptl.entry=" + generalUtils.intToStr(nextLine) + "\001");

        String gstRateDefault = accountsUtils.getDefaultGSTRate(con, stmt, rs, dnm, localDefnsDir, defnsDir);
        scoutln(data, bytesOut, "receiptl.gstrate=" + gstRateDefault + "\001");
      }
      
      newOrEdit = 'N';
    }

    ddlData = accountsUtils.getGSTRatesDDLData(con, stmt, rs, "receiptl.gstrate", dnm, ddlData, ddlDataUpto, ddlDataLen, localDefnsDir, defnsDir);

    getAReceiptFieldGivenCode(con, stmt, rs, "Currency", code, b, dnm, localDefnsDir, defnsDir);
    scoutln(data, bytesOut, "receiptl.currency=" + generalUtils.stringFromBytes(b, 0L) + "\001");

    scoutln(data, bytesOut, "receiptl.basecurrency=" + accountsUtils.getBaseCurrency(con, stmt, rs, dnm, localDefnsDir, defnsDir) + "\001");
    ...
    byte[] prependCode = new byte[25000];
    prependEditLine(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, cad, generalUtils.stringFromBytes(code, 0L), generalUtils.stringFromBytes(line, 0L), "266b.htm",
                               ...
                               char newOrEdit, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    scoutln(b, bytesOut, "second.style.visibility='hidden';third.style.visibility='hidden';submenu.style.visibility='visible';}");

    scoutln(b, bytesOut, "function save(thisOp){var saveStr=\"\";" + saveStr + "\n");
    scoutln(b, bytesOut, "var saveStr2=strip(saveStr);window.location.replace(\"/central/servlet/ReceiptLineUpdate?unm=" + unm + "&sid=" + sid
    ...
    outputPageFrame(con, stmt, rs, b, req, newOrEdit, ' ', "", "ReceiptLine", "", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private int getNextLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] data, String dnm,
      String localDefnsDir, String defnsDir) throws Exception
  {
    byte[] linesData = new byte[5000];
    int[] listLen = new int[1];
    ...
    linesData = getLines(con, stmt, rs, code, '\001', linesData, listLen, linesCount, dnm,
                      ...
                      byte[] code, int[] bytesOut) throws Exception
  {
    ...
    str[0] = '\000';
    generalUtils.catAsBytes("ReceiptCode", 0, str, true);

    if(searchDataString(recData, recDataLen, "receipt", str, code) == 0)
      return 'N';
    ...
    String fieldNamesReceipt = getFieldNamesReceipt();
    ...
    {
      getFieldName(fieldNamesReceipt, x, fldName);
      if(searchDataString(recData, recDataLen, "receipt", fldName, value) != -1) // entry exists
      {
        if(value[0] != '\000')
    ...
    double rate = 1.0;
    len = fieldNamesReceipt.length();
    int x = 0, y, fieldCount = 0;
    while (x < len)
    {
    y = 0;
    while (x < len && fieldNamesReceipt.charAt(x) != ',')
      fldName[y++] = (byte) fieldNamesReceipt.charAt(x++);
    fldName[y] = '\000';
    ++x;
    ...
    if((generalUtils.dfsAsStr(buf, (short) 41)).length() == 0) // BankedAmount 
    ...
    if((generalUtils.dfsAsStr(buf, (short) 16)).length() == 0) // dateReceived 
  ...
  {
    double amt = generalUtils.dfsAsDouble(buf, (short)5); // amountReceived
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void getFieldName(String fieldNames, int fieldReqd, byte[] fieldName)
      throws Exception
  {
    int x = 0, y, fieldCount = 0;
    int len = fieldNames.length();
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public int getRecGivenCode(Connection con, Statement stmt, ResultSet rs, byte[] code, char separator, byte[] data,
                             String dnm, String localDefnsDir, String defnsDir, int[] bytesOut)
                             throws Exception
  {
    if(code[0] == '\000') // just-in-case
      return -1;
    ...
    rs = stmt.executeQuery("SELECT * FROM receipt WHERE ReceiptCode = '"
        + generalUtils.stringFromBytes(code, 0L) + "'");
    if(!rs.next())
    {
    rs.close();
    ...
    String fieldTypes = getFieldTypesReceipt();
    ...
    // separator == \001
    {
      String fieldNames = getFieldNamesReceipt();
      ...
      while (x < len)
      {
        ...
        scoutln(data, bytesOut, "receipt." + thisFieldName + "="
            + getValue(count, thisFieldType, rs, rsmd) + "\001");
  ...
  public char putRecHeadGivenCode(Connection con, Statement stmt, ResultSet rs, byte[] code, char newOrEdit, byte[] data, String dnm, String localDefnsDir, String defnsDir)
  {
    ...
    if(newOrEdit == 'N')
    {
      ...
      q = "INSERT INTO receipt (" + getFieldNamesReceipt() + ") VALUES (" + opStr + ")";
    }
    else
    // newOrEdit == 'E'
    {
    ...
    q = "UPDATE receipt SET " + opStr + " WHERE ReceiptCode = '"
        + generalUtils.stringFromBytes(code, 0L) + "'";
    }
                         ...
                         String defnsDir)
  {
    ...
    String opStr = "";
    boolean first = true;
    String fieldTypes = getFieldTypesReceiptL();
    int x = 0, count = 0;
    ...
    q = "INSERT INTO receiptl (" + getFieldNamesReceiptL() + ") VALUES ("
        + opStr + ")";
    }
    else
    // newOrEdit == 'E'
    ...
    while (x < len && fieldNames.charAt(x) == ' ')
      ...
      q = "UPDATE receiptl SET " + opStr + " WHERE ReceiptCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '"
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public boolean deleteLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] line, String dnm,
      String localDefnsDir, String defnsDir, int[] bytesOut)
  {
    ...
    if(getLine(con, stmt, rs, code, line, '\000', data, dnm, localDefnsDir, defnsDir,
        bytesOut) == -1) // just-in-case
      return false;
    ...
    stmt.executeUpdate("DELETE FROM receiptl WHERE ReceiptCode = '"
        + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '"
        + generalUtils.stringFromBytes(line, 0L) + "'");
    ...
    stmt = con.createStatement();
    stmt.executeUpdate("DELETE FROM receiptll WHERE ReceiptCode = '"
        + generalUtils.stringFromBytes(code, 0L) + "' AND Entry = '"
        + generalUtils.stringFromBytes(line, 0L) + "'");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public byte[] getLines(Connection con, Statement stmt, ResultSet rs, byte[] code, char separator, byte[] linesData,
      int[] listLen, int[] linesCount, String dnm, String localDefnsDir,
      String defnsDir) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT * FROM receiptl WHERE ReceiptCode = '"
            + generalUtils.stringFromBytes(code, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
    while (rs.next())
    {
  ...
  private int getLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] line, char separator, byte[] data,
      String dnm, String localDefnsDir, String defnsDir, int[] bytesOut)
      throws Exception
  {
    if(code[0] == '\000') // just-in-case
    ...
    rs = stmt
        .executeQuery("SELECT * FROM receiptl WHERE ReceiptCode = '"
            + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '"
            + generalUtils.stringFromBytes(line, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
    while (rs.next())
    ...
    {
    upto = generalUtils.stringIntoBytes(getValue((x + 1), fieldTypes.charAt(x), rs,
        rsmd), 0, data, upto);
    data[upto++] = '\000';
    }
    }
    ...
    while (x < len && fieldNames.charAt(x) != ',')
      thisFieldName += fieldNames.charAt(x++);
    thisFieldType = fieldTypes.charAt(count++);
    scoutln(data, bytesOut, "receiptl." + thisFieldName + "="
        + getValue(count, thisFieldType, rs, rsmd) + "\001");
    ++x;
    while (x < len && fieldNames.charAt(x) == ' ')
      ++x;
                                  ...
                                  int[] bytesOut) throws Exception
  {
    ...
    scoutln(b, bytesOut, "<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"" + cssDocumentDirectory + "document.css\">\n");
                                                                        // change VV
    outputPageFrame(con, stmt, rs, b, req, ' ', ' ', "", "OrderAcknowledgementLineUpdate", date, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void sortFields(byte[] ipBuf, byte[] opBuf, String which)
      throws Exception
  {
    String thisFieldName;
    String fieldNames;
    ...
    if(which.equals("receipt"))
      fieldNames = getFieldNamesReceipt();
    else
      // if(which.equals("receiptl"))
      fieldNames = getFieldNamesReceiptL();
    int len = fieldNames.length();
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  // uses layout 266d.htm
  public boolean getRecToHTMLDisplayOnly(Connection con, Statement stmt, ResultSet rs, PrintWriter out,
      HttpServletRequest req, byte[] code, String unm, String sid,
      String uty, String men, String den, String dnm, String bnm,
      String localDefnsDir, String defnsDir, int[] bytesOut)
      throws Exception
  {
    String imagesDir = directoryUtils.getSupportDirs('I');
    ...
    byte[] javaScriptCode = new byte[1000];
    javaScript(con, stmt, rs, req, generalUtils.stringFromBytes(code, 0L), unm, sid, uty, men,
        den, dnm, bnm, javaScriptCode, localDefnsDir, defnsDir, bytesOut);
    ...
    if(getRecGivenCode(con, stmt, rs, code, '\001', data, dnm, localDefnsDir, defnsDir,
        bytesOut) == -1) // just-in-case
    {
    messagePage.msgScreen(false, out, req, 6, unm, sid, uty, men, den,
        dnm, bnm, "Receipt", imagesDir, localDefnsDir, defnsDir, bytesOut);
    return false;
    }
    ...
    numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "266d.htm", 2,
        getFieldNamesReceipt(), fieldSizesReceipt, getFieldNamesReceiptL(),
        fieldSizesReceiptL, null, null);
    ...
    int x = 0, y, fieldCount = 0;
    int len = fieldNamesReceipt.length();
    while (x < len)
    {
    y = 0;
    while (x < len && fieldNamesReceipt.charAt(x) != ',')
      fieldName[y++] = (byte) fieldNamesReceipt.charAt(x++);
    fieldName[y] = '\000';
    ++x;
    ...
    if(searchDataString(data, recDataLen, "receipt", fieldName, value) != -1) // entry exists
    {
    ...
    for (x = 0; x < linesCount[0]; ++x)
    {
      if(generalUtils.getListEntryByNum(x, linesData, data)) // just-in-case
      {
        ...
        generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 2000, 6); // amountReceived
        ...
        generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 2000, 18); // baseAmountReceived
        ...
        screenLayout.bufferToBuffer(buf1, buf2, size1, size2, source, numFields, fieldNames, fieldTypes, fieldSizes, 'D', 'L', data, 2000, ddlData, ddlDataUpto[0],
                               ...
                               String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "4205", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    subMenuText += "<table id='title' width=100%><tr><td colspan=2 nowrap><p>Receipt" + directoryUtils.buildHelp(4205) + "</td></tr></table>";
                                  ...
                                  String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount) throws Exception
  {
    ...
    if(callingServlet.equals("ReceiptPage"))
      s += drawOptions4205(con, stmt, rs, req, hmenuCount, date, unm, uty, dnm, localDefnsDir, defnsDir);
    else
    if(callingServlet.equals("ReceiptHeaderEdit"))
      s += drawOptions4207(hmenuCount, cad, unm, sid, uty, men, den, dnm, bnm);
    else
    if(callingServlet.equals("ReceiptLine"))
      s += drawOptions4209(hmenuCount, newOrEdit);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions4205(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, int[] hmenuCount, String date, String unm, String uty, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    String s = "";

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4207, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "receipt", date, unm))
      s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0]++ + "');\"><a href=\"javascript:fetch()\">Edit Details</a></dt></dl>\n";
  
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4209, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "receipt", date, unm))
      s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0]++ + "');\"><a href=\"javascript:add()\">Add New Line</a></dt></dl>\n";
  
    boolean ReceiptPrint  = authenticationUtils.verifyAccess(con, stmt, rs, req, 4212,  unm, uty, dnm, localDefnsDir, defnsDir);
    boolean _4189  = authenticationUtils.verifyAccess(con, stmt, rs, req, 4189,  unm, uty, dnm, localDefnsDir, defnsDir);
    ...
    if(ReceiptPrint || _4189 || MailExternalUserCreate || PrintToPDFUtils)
    {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions4207(int[] hmenuCount, char cad, String unm, String sid, String uty, String men, String mainDNM, String dnm, String bnm)
                                 throws Exception
  {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions4209(int[] hmenuCount, char newOrEdit) throws Exception
  {
