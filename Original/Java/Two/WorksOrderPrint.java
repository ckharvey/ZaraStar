// =======================================================================================================================================================================================================
// System: ZaraStar: DocumentEngine: WO Print
// Module: WorksOrderPrint.java
// Author: C.K.Harvey
...
public class WorksOrderPrint extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "WorksOrderPrint", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4440, bytesOut[0], 0, "ERR:" + p1);
                      ...
                      String den, String dnm, String bnm, String p1, int[] bytesOut) throws Exception
  {
    ...
    try
    {
      ...
      {
        messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4440", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
        serverUtils.etotalBytes(req, unm, dnm, 4440, bytesOut[0], 0, "ACC:" + p1);
      ...
      {
        messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4440", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
        serverUtils.etotalBytes(req, unm, dnm, 4440, bytesOut[0], 0, "SID:" + p1);
        if(con  != null) con.close();
        if(out != null) out.flush();
        return;
      }
      ...
      switch(generate(con, stmt, rs, code, dnm, unm, workingDir, reportsDir, localDefnsDir, defnsDir, bytesOut))
      {
        case -1 : // Definition File Not Found
                  messagePage.msgScreen(false, out, req, 17, unm, sid, uty, men, den, dnm, bnm, "4440", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                  break;
        case -2 : // cannot create report output file
                  messagePage.msgScreen(false, out, req, 18, unm, sid, uty, men, den, dnm, bnm, "4440", "", "", "", imagesDir, localDefnsDir, defnsDir, bytesOut);
                  break;
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4440, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
        ...
        while(llCount != -1)
        {
          ...
          while(! quit1)
          {
            ...
            else
            {
              ...
              {
                ++lineCount;
                if(lineCount > miscDefinitions.docSizeMax(con, stmt, rs, "wo"))
                {
                  b[0] = '\000';
