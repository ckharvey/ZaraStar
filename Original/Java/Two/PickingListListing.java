// =======================================================================================================================================================================================================
// System: ZaraStar Document: PL create listing page
// Module: PickingListListing.java
// Author: C.K.Harvey
...
public class PickingListListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      doIt(out, req, unm, sid, uty, men, den, dnm, bnm, inx, operation, callType, codeArg, codeArg2, srchStr, firstRecNum, lastRecNum, generalUtils.strToInt(maxRows),
           generalUtils.strToInt(numRecs), companyCode, bytesOut);
    }
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PickingListListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3050, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    int numRecs, String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 3050, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PickingListListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3050, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PickingListListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3050, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3050, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pl WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(srchStr) + "'}"
                                         + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM pl" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pl WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}"
                       + companyCodeANDWHERE);
                   }
                   break;
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM pl WHERE CompanyCode LIKE '" + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM pl" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pl WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CustomerPOCode) AS rowcount FROM pl WHERE CustomerPOCode LIKE '" + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CustomerPOCode) AS rowcount FROM pl" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pl WHERE CustomerPOCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT PLCode) AS rowcount FROM pl WHERE PLCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pl" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String plCode="", name="", customerCode="", date="", custPOCode="", status, completed="";
    int plCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, custPOCodePosn, completedPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PLCode, Date, CompanyCode, CompanyName, CustomerPOCode, Completed, Status FROM pl WHERE PLCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY PLCode";
                   }
                   else
                   {
                     q = "SELECT PLCode, Date, CompanyCode, CompanyName, CustomerPOCode, Completed, Status FROM pl " + companyCodeWHERE + "ORDER BY PLCode";
                   }

                   plCodePosn = 1; datePosn = 2; companyCodePosn = 3; custPOCodePosn = 4; companyNamePosn = 5; completedPosn = 6; statusPosn = 7; 
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, PLCode, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     plCodePosn = 2; datePosn = 1; companyCodePosn = 3; custPOCodePosn = 4; companyNamePosn = 5; completedPosn = 6;  statusPosn = 7; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pl " + companyCodeWHERE + " ORDER BY Date";
                     plCodePosn = -1; datePosn = 1; companyCodePosn = -1; custPOCodePosn = -1; companyNamePosn = -1; completedPosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT Date, PLCode, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE Date = {d '"
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, PLCode";
                     
                     plCodePosn = 2; datePosn = 3; companyCodePosn = 1; custPOCodePosn = 4; companyNamePosn = 5; completedPosn = 6;
                     statusPosn = 7; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pl " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     plCodePosn = -1; datePosn = -1; companyCodePosn = 1; custPOCodePosn = -1; companyNamePosn = -1;
                     completedPosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CompanyCode = '"
                       + codeArg + "' ORDER BY CompanyCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CustomerPOCode, CompanyCode, PLCode, Date, CompanyName, Completed, Status FROM pl WHERE CustomerPOCode LIKE '" 
                       + srchStr + "%' ORDER BY CustomerPOCode, CompanyCode";
                     plCodePosn = 3; datePosn = 4; companyCodePosn = 2; custPOCodePosn = 1; companyNamePosn = 5; completedPosn = 6;
                     statusPosn = 7; 
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM pl " + companyCodeWHERE + "ORDER BY CustomerPOCode";    
                     plCodePosn = -1; datePosn = -1; companyCodePosn = -1; custPOCodePosn = 1; companyNamePosn = -1;
                     completedPosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT CustomerPOCode, CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CustomerPOCode = '"
                       + codeArg + "' ORDER BY CustomerPOCode";
                     plCodePosn = 3; datePosn = 4; companyCodePosn = 2; custPOCodePosn = 1; companyNamePosn = 5; completedPosn = 6;
                     statusPosn = 7; 
                   }
                 }  
                 ...
                 while(rs.next())                  
                 {
                   plCode       = getValue(plCodePosn,      ' ', rs, rsmd);
                   ...
                   completed    = getValue(completedPosn,   ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, plCode, date, customerCode, custPOCode, name, status, completed, plCode, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, plCode, customerCode, custPOCode, name, status, completed, plCode, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, plCode, date, custPOCode, name, status, completed, plCode, imagesDir);
                   }   
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, custPOCode);
                     else appendBodyLine(html, inx, custPOCode, customerCode, plCode, date, name, status, completed, plCode, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = plCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = plCode;
                     else
                     if(inx == '2')
                     ...
                     if(inx == '3')
                       ...
                       firstEntryOnPage[0] = custPOCode;
                   }
                 }
                 
                 rs.close();
                                 
                 lastCodeOnPage[0] = plCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = plCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PLCode, Date, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE PLCode LIKE '" + srchStr 
                       + "%' AND PLCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PLCode DESC";
                   }
                   else
                   {  
                     q = "SELECT PLCode, Date, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE PLCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                         + companyCodeANDWHERE + " ORDER BY PLCode DESC";
                   }
                   
                   plCodePosn = 1; datePosn = 2; companyCodePosn = 3; custPOCodePosn = 4; companyNamePosn = 5; completedPosn = 6;
                   statusPosn = 7; 
                 }  
                 else
                 if(inx == '2')
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pl WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                     plCodePosn = -1; datePosn = 1; companyCodePosn = -1; custPOCodePosn = -1; companyNamePosn = -1;
                     completedPosn = -1; statusPosn = -1; 
                   }
                   ...
                   {
                     q = "SELECT Date, PLCode, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PLCode DESC";
                     
                     plCodePosn = 2; datePosn = 1; companyCodePosn = 3; custPOCodePosn = 4; companyNamePosn = 5; completedPosn = 6;
                     statusPosn = 7; 
                   }  
                 }
                 else
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pl WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CompanyCode = '"
                       + codeArg + "' ORDER BY CompanyCode, PLCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM pl WHERE CustomerPOCode <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz' ORDER BY CustomerPOCode DESC";
                   ...
                   {
                     q = "SELECT CustomerPOCode, CompanyCode, PLCode, Date, CompanyName, Completed, Status FROM pl WHERE CustomerPOCode = '"
                        + codeArg + "' ORDER BY CustomerPOCode, CompanyCode DESC";
                     
                     plCodePosn = 3; datePosn = 4; companyCodePosn = 2; custPOCodePosn = 1; companyNamePosn = 5; statusPosn = 6;
                     completedPosn = 7;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, plCode, date, customerCode, custPOCode, name, status, completed, plCode, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, plCode, customerCode, custPOCode, name, status, completed, plCode, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, plCode, date, custPOCode, name, status, completed, plCode, imagesDir);
                   }  
                   else // inx == '4'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, custPOCode);
                     else prependBodyLine(html, inx, custPOCode, customerCode, plCode, date, name, status, completed, plCode, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT plCode, Date, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE PLCode LIKE '" + srchStr 
                       + "%' AND PLCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PLCode";
                   ...
                   {                   
                     q = "SELECT PLCode, Date, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE PLCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY PLCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM pl WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, PLCode, CompanyCode, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PLCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, PLCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pl WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CompanyCode = '" + codeArg
                       + "' AND PLCode > '" + codeArg2 + "' ORDER BY CompanyCode, PLCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM pl WHERE CustomerPOCode >= '" + codeArg + "' ORDER BY CustomerPOCode";
                   ...
                   {
                     q = "SELECT CustomerPOCode, CompanyCode, PLCode, Date, CompanyName, Completed, Status FROM pl WHERE CustomerPOCode = '" + codeArg
                       + "' AND PLCode > '" + codeArg2 + "' ORDER BY CustomerPOCode, CompanyCode";
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM pl WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, PLCode, Date, CustomerPOCode, CompanyName, Completed, Status FROM pl WHERE CompanyCode = '" + codeArg
                       + "' AND PLCode < '" + codeArg2 + "' ORDER BY CompanyCode, PLCode DESC";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CustomerPOCode FROM pl WHERE CustomerPOCode < '" + codeArg + "' ORDER BY CustomerPOCode DESC";
                   ...
                   {
                     q = "SELECT CustomerPOCode, CompanyCode, PLCode, Date, CompanyName, Completed, Status FROM pl WHERE CustomerPOCode = '" + codeArg
                       + "' AND PLCode < '" + codeArg2 + "' ORDER BY CustomerPOCode, CustomerCode DESC";
                 ...
                 if(inx == '3')
                   firstEntryOnPage[0] = customerCode;
                 else firstEntryOnPage[0] = custPOCode;
                      ...
                      throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Picking List Search Results for ";
        switch(inx)
        {
          case '1' : title += "Picking List Code";  break;
          case '2' : title += "Picking List Date";  break;
      ...
      else title = "Picking List Listing";

      drawingUtils.drawTitle(out, false, false, "PickingListListing", "", title, "3050", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Picking List Code");  break;
        case '2' : scout(out, bytesOut, "Picking List Date");  break;
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Picking List Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/GoodsReceivedPickingList?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PickingListListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PickingListListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "3050", "", "PickingListListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "<tr><td colspan=2 nowrap><p>Status &nbsp;</td>");

    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Picking List Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Picking List Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Picking List Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Picking List Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer PO Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Picking List Date");  break;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String five,
                              String status, String completed, String  code, String imagesDir) throws Exception
  {
    ...
    if(completed.equals("Y"))
      html[0] += "<td align=center><img src=\"" + imagesDir + "z0106.gif\" border=0></td>";
    else html[0] += "<td></td>";
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String five,
                               String status, String completed, String code, String imagesDir) throws Exception
  {
    String s = checkCode(code);
    ...
    if(completed.equals("Y"))
      line += "<td align=center><img src=\"" + imagesDir + "z0106.gif\" border=0></td>";
    else line += "<td></td>";
