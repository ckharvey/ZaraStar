// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: PCN listing page
// Module: PurchaseCreditNoteListing.java
// Author: C.K.Harvey
...
public class PurchaseCreditNoteListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseCreditNoteListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5035, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5035, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseCreditNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5035, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseCreditNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5035, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5035, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pcredit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM pcredit" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pcredit WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM pcredit WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM pcredit" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pcredit WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT PCNCode) AS rowcount FROM pcredit WHERE PCNCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pcredit" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String pcnCode="", name="", supplierCode="", date="", totalAmount="", status;
    int pcnCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, totalAmountPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode LIKE '" + srchStr + "%'" 
                       + companyCodeANDWHERE + " ORDER BY PCNCode";
                   }
                   else
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit " + companyCodeWHERE + "ORDER BY PCNCode";
                   }

                   pcnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, PCNCode, CompanyCode, CompanyName, Status FROM pcredit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     pcnCodePosn = 2; datePosn = 1; companyCodePosn = 3; statusPosn = 5; companyNamePosn = 4; totalAmountPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pcredit ORDER BY Date" + companyCodeWHERE;
                     
                     pcnCodePosn = -1; datePosn = 1; companyCodePosn = -1; statusPosn = -1; companyNamePosn = -1;
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     ...
                     pcnCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, PCNCode, CompanyName, TotalTotal, Status FROM pcredit WHERE CompanyCode LIKE '"
                       + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     pcnCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5; totalAmountPosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pcredit " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     pcnCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; 
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, PCNCode, CompanyName, TotalTotal, Status FROM pcredit WHERE CompanyCode = '" + codeArg
                 ...
                 while(rs.next())                  
                 {
                   pcnCode       = getValue(pcnCodePosn,      ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, pcnCode, date, supplierCode, name, pcnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, pcnCode, supplierCode, name, pcnCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, supplierCode, pcnCode, name, totalAmount, pcnCode, status, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = pcnCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = pcnCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = pcnCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = pcnCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode LIKE '" + srchStr
                       + "%' AND PCNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PCNCode DESC";
                   }
                   else
                   {  
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY PCNCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pcredit WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, PCNCode, CompanyCode, CompanyName, Status FROM pcredit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PCNCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pcredit WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, PCNCode, CompanyName, TotalTotal, Status FROM pcredit WHERE CompanyCode = '" + codeArg
                        + "' ORDER BY CompanyCode, PCNCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, pcnCode, date, supplierCode, name, pcnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, pcnCode, supplierCode, name, pcnCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, supplierCode, pcnCode, name, totalAmount, pcnCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode LIKE '"
                       + srchStr + "%' AND PCNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PCNCode";
                     
                     pcnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY PCNCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM pcredit WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, PCNCode, CompanyCode, CompanyName, Status FROM pcredit WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PCNCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, PCNCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pcredit WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, PCNCode, CompanyName, TotalTotal, Status FROM pcredit WHERE CompanyCode = '" + codeArg
                       + "' AND PCNCode > '" + codeArg2 + "' ORDER BY CompanyCode, PCNCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode LIKE '"
                       + srchStr + "%' AND PCNCode < '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PCNCode DESC";
                   }
                   else
                   {
                     q = "SELECT PCNCode, Date, CompanyCode, CompanyName, Status FROM pcredit WHERE PCNCode < '" + codeArg 
                 ...
                 if(inx == '2')
                 {
                   ...
                   {  
                     ...
                     pcnCodePosn = 2; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, PCNCode, CompanyCode, CompanyName, Status FROM pcredit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PCNCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, PCNCode DESC";
                     
                     pcnCodePosn = 1; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM pcredit WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, PCNCode, CompanyName, TotalTotal, Status FROM pcredit WHERE CompanyCode = '" + codeArg
                       + "' AND PCNCode < '" + codeArg2 + "' ORDER BY CompanyCode, PCNCode DESC";
                 ...
                 if(inx == '2')
                   firstEntryOnPage[0] = date;
                 else firstEntryOnPage[0] = supplierCode;
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Purchase Credit Note Search Results for ";
        switch(inx)
        {
          case '1' : title += "Purchase Credit Note Code";  break;
          case '2' : title += "Purchase Credit Note Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Purchase Credit Note Listing by ";
        switch(inx)
        {
          case '1' : title += "Purchase Credit Note Code";  break;
          case '2' : title += "Purchase Credit Note Date";  break;
      ...
      else title = "Purchase Credit Note Listing";

      drawingUtils.drawTitle(out, false, false, "PurchaseCreditNoteListing", "", title, "5035", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Purchase Credit Note Code");  break;
        case '2' : scout(out, bytesOut, "Purchase Credit Note Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Credit Note Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/PurchaseCreditNotePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PurchaseCreditNoteListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PurchaseCreditNoteListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5035", "", "PurchaseCreditNoteListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Credit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Credit Note Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Credit Note Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Credit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Total Amount</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Purchase Credit Note Date");  break;
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String code,
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String code,
                               String status, String imagesDir) throws Exception
  {
    String s = checkCode(code);
