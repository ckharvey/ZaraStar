// =======================================================================================================================================================================================================
// System: ZaraStar: Quotation & Sales Order Processing - Purchasing
// Module: SalesOrderPurchasingExecute.java
// Author: C.K.Harvey
...
public class SalesOrderPurchasingExecute extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrderPurchasinga", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2032, bytesOut[0], 0, "ERR:");
                    ...
                    String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 2032, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrderPurchasinga", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2032, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrderPurchasinga", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2032, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 2032, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
                   ...
                   String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "2032", "", "SalesOrderPurchasinga", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesOrderPurchasinga", "", "Sales Order Processing - Purchasing", "2032", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form name=\"go\" action=\"ProcurementUpdateSalesOrder\" enctype=\"application/x-www-form-urlencoded\" method=POST>");
    ...
    scoutln(out, bytesOut, "<td align=center><p><b> Purchasing Confirms </b></td>");
    scoutln(out, bytesOut, "<td align=center><p><b> Scheduling Confirmed </b></td>");
                                       ...
                                       String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT SalesPerson, SOCode, Date, CompanyCode, CustomerPOCode, TotalTotal, Currency2, CompanyName, EngineeringApproved, "
                         + "EngineeringApprovedDate, EngineeringApprovedSignOn, SchedulingConfirmed, SchedulingConfirmedDate, "
                         + "SchedulingConfirmedSignOn, SOType, ToEngineeringDate, ToEngineeringSignOn, ToSchedulingDate, ToSchedulingSignOn, "
                         + "ToEngineering, ToScheduling "
                         + "FROM so WHERE ToProcurement = 'Y' AND ProcurementConfirmed != 'Y' AND Status != 'C' AND AllSupplied != 'Y' ORDER BY Date, SOCode");
      
    String salesPerson, soCode, date, customerCode, custPOCode, currency, companyName, engineeringApproved, engineeringApprovedDate,
           engineeringApprovedSignOn, schedulingConfirmed, schedulingConfirmedDate, schedulingConfirmedSignOn, soType, toEngineeringDate,
           toEngineeringSignOn, toSchedulingDate, toSchedulingSignOn, toEngineering, toScheduling;
    ...
    {    
      ...
      if(salesPersonIsRequired(salesPerson, checkBoxes))
      {
        ...
        schedulingConfirmed        = generalUtils.deNull(rs.getString(12));
        schedulingConfirmedDate    = generalUtils.deNull(rs.getString(13));
        schedulingConfirmedSignOn  = generalUtils.deNull(rs.getString(14));
        ...
        toSchedulingDate           = generalUtils.deNull(rs.getString(18));
        toSchedulingSignOn         = generalUtils.deNull(rs.getString(19));
        toEngineering              = generalUtils.deNull(rs.getString(20));
        toScheduling               = generalUtils.deNull(rs.getString(21));
        ...
        else
        {
          if(! toEngineering.equals("Y"))
            scoutln(out, bytesOut, "<td align=center><p><font color=red>Not Sent to Engineering</td>");
          else
          if(engineeringApproved.equals("Y"))
          {
            scoutln(out, bytesOut, "<td align=center><p><font color=green>" + generalUtils.convertFromYYYYMMDD(engineeringApprovedDate) + " by "
                                   + engineeringApprovedSignOn + "</td>");
          }
          else
          {
            scoutln(out, bytesOut, "<td><p><font color=orange>Sent to Engineering " + generalUtils.convertFromYYYYMMDD(toEngineeringDate)  + " by "
                                   + toEngineeringSignOn  + "</td>");
          }
        }
        
        scout(out, bytesOut, "<td align=center><p><input type=checkbox name=proCon" + generalUtils.sanitise(soCode) + "></td>");
        ...
        else
        {
          if(! toScheduling.equals("Y"))
            scoutln(out, bytesOut, "<td align=center><p><font color=red>Not Sent to Scheduling</td>");
          else
          if(schedulingConfirmed.equals("Y"))
          {
            scoutln(out, bytesOut, "<td align=center><p><font color=green>" + generalUtils.convertFromYYYYMMDD(schedulingConfirmedDate) + " by "
                                   + schedulingConfirmedSignOn + "</td>");
          }
          else
          {
            scoutln(out, bytesOut, "<td><p><font color=orange>Sent to Scheduling " + generalUtils.convertFromYYYYMMDD(toSchedulingDate)  + " by "
                                   + toSchedulingSignOn  + "</td>");
          }
        }
