// =======================================================================================================================================================================================================
// System: ZaraStar Product: Stock Adjustment records for ANY item
// Module: StockAdjustmentAnyItem.java
// Author: C.K.Harvey
...
public class StockAdjustmentAnyItem extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "StockAdjustmentAnyItem", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3012, bytesOut[0], 0, "ERR:");
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 3012, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3012", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3012, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3012", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3012, bytesOut[0], 0, "SID:");
      if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3012, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con  != null) con.close();
    if(out != null) out.flush(); 
  }
  ...
  {
    scoutln(out, bytesOut, "<html><head><title>Stock Adjustment Record Entry</title>");
    ...
    scoutln(out, bytesOut, "var url = \"http://" + men + "/central/servlet/StockAdjustmentDataEntry?unm=\" + escape('" + unm + "') + \"&sid=\" + escape('" + sid + "') + \"&uty=\" + escape('" + uty + "') + \"&men=\" + escape('" + men + "') + \"&den=\" + escape('"
                         + den + "') + \"&bnm=\" + escape('" + bnm + "') + \"&p1=\" + itemCode + \"&p2=\" + storeFrom + \"&p3=\" + storeTo + \"&p4=\" + date + \"&p5=\" + qty + \"&p6=\" + remark + \"&p7=\" + mfr + \"&p8=\" + mfrCode+ \"&p9=\" + soCode + \"&p10=\" + poCode + \"&p11=\" + locationFrom + \"&p12=\" + locationTo + \"&dnm=\" + escape('" + dnm
                         + "');");
    ...
    outputPageFrame(con, stmt, rs, out, req, "", "Stock Adjustment Record Entry", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td nowrap><p>");
    scoutln(out, bytesOut, "<tr><td><input type=text name=mfrCode size=10 maxlength=20></td></td></tr>");
                               ...
                               String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "3012", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    subMenuText += "<table id='title' width=100%><tr><td colspan=2 nowrap><p>" + title + directoryUtils.buildHelp(3012) + "</td></tr></table>";
