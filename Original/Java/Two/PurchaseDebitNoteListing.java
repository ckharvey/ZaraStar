// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: PDN listing page
// Module: PurchaseDebitNoteListing.java
// Author: C.K.Harvey
...
public class PurchaseDebitNoteListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseDebitNoteListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5045, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5045, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5045, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5045, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5045, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pdebit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM pdebit" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pdebit WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM pdebit WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM pdebit" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pdebit WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT PDNCode) AS rowcount FROM pdebit WHERE PDNCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM pdebit" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String pdnCode="", name="", supplierCode="", date="", suppPOCode="", settled="", status;
    int pdnCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, suppPOCodePosn, settledPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY PDNCode";
                   }
                   else
                   {
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit " + companyCodeWHERE
                       + "ORDER BY PDNCode";
                   }

                   pdnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; suppPOCodePosn = -1; 
                   settledPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, PDNCode, CompanyCode, CompanyName, Status FROM pdebit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     pdnCodePosn = 2; datePosn = 1; companyCodePosn = 3; statusPosn = 5; suppPOCodePosn = -1;
                     companyNamePosn = 4; settledPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pdebit ORDER BY Date" + companyCodeWHERE;
                     
                     pdnCodePosn = -1; datePosn = 1; companyCodePosn = -1; statusPosn = -1; suppPOCodePosn = -1;
                   ...
                   else
                   {
                     ...
                     pdnCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                     suppPOCodePosn = -1; settledPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, PDNCode, CompanyName, Settled, PORefNum, Status FROM pdebit WHERE "
                      + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     pdnCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 6; 
                     suppPOCodePosn = 5;  settledPosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pdebit " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     pdnCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; 
                     suppPOCodePosn = -1; settledPosn = -1;
                   }
                   else
                   {
                 ...
                 while(rs.next())                  
                 {
                   pdnCode  = getValue(pdnCodePosn, ' ', rs, rsmd);
                   ...
                   suppPOCode   = getValue(suppPOCodePosn,  ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, pdnCode, date, supplierCode, name, null, pdnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, pdnCode, supplierCode, name, null, pdnCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       appendBodyLine(html, inx, supplierCode, pdnCode, name, settled, suppPOCode, pdnCode, status,
                                      imagesDir);
                     }
                   }   
                   ...
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = pdnCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = pdnCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = pdnCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = pdnCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode LIKE '"
                       + srchStr + "%' AND PDNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PDNCode DESC";
                   }
                   else
                   {  
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode "
                       + "<= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY PDNCode DESC";
                   }
                   
                   pdnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                   suppPOCodePosn = -1; settledPosn = -1;
                 }  
                 else
                 if(inx == '2')
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM pdebit WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, PDNCode, CompanyCode, CompanyName, Status FROM pdebit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PDNCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pdebit WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, PDNCode, CompanyName, Settled, PORefNum, Status FROM pdebit WHERE "
                        + "CompanyCode = '" + codeArg + "' ORDER BY CompanyCode, PDNCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, pdnCode, date, supplierCode, name, null, pdnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, pdnCode, supplierCode, name, null, pdnCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       prependBodyLine(html, inx, supplierCode, pdnCode, name, settled, suppPOCode, pdnCode, status,
                                       imagesDir);
                     }
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode LIKE '"
                       + srchStr + "%' AND PDNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PDNCode";
                   ...
                   {                   
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY PDNCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM pdebit WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, PDNCode, CompanyCode, CompanyName, Status FROM pdebit WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PDNCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, PDNCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM pdebit WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, PDNCode, CompanyName, Settled, PORefNum, Status FROM pdebit WHERE "
                       + "CompanyCode = '" + codeArg + "' AND PDNCode > '" + codeArg2 + "' ORDER BY CompanyCode, PDNCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PDNCode, Date, CompanyCode, CompanyName, Status FROM pdebit WHERE PDNCode < '" + codeArg 
                     + "'" + companyCodeANDWHERE + " ORDER BY PDNCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   ...
                   {  
                     ...
                     pdnCodePosn = 2; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     suppPOCodePosn = -1; settledPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, PDNCode, CompanyCode, CompanyName, Status FROM pdebit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PDNCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, PDNCode DESC";
                     
                     pdnCodePosn = 1; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5;
                     suppPOCodePosn = -1; settledPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM pdebit WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, PDNCode, CompanyName, Settled, PORefNum, Status FROM pdebit WHERE "
                       + "CompanyCode = '" + codeArg + "' AND PDNCode < '" + codeArg2 + "' ORDER BY CompanyCode, PDNCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Purchase Debit Note Search Results for ";
        switch(inx)
        {
          case '1' : title += "Purchase Debit Note Code";  break;
          case '2' : title += "Purchase Debit Note Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Purchase Debit Note Listing by ";
        switch(inx)
        {
          case '1' : title += "Purchase Debit Note Code";  break;
          case '2' : title += "Purchase Debit Note Date";  break;
      ...
      else title = "Purchase Debit Note Listing";

      drawingUtils.drawTitle(out, false, false, "PurchaseDebitNoteListing", "", title, "5045", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Purchase Debit Note Code");  break;
        case '2' : scout(out, bytesOut, "Purchase Debit Note Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Debit Note Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/PurchaseDebitNotePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PurchaseDebitNoteListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PurchaseDebitNoteListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5045", "", "PurchaseDebitNoteListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Debit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Debit Note Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Debit Note Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Purchase Debit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Supplier PO Code</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Purchase Debit Note Date"); break;
