// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Quotes create listing page
// Module: QuotationListing.java
// Author: C.K.Harvey
...
public class QuotationListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "QuotationListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4030, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4030, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "QuotationListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4030, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "QuotationListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4030, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4030, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM quote WHERE QuoteDate = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT QuoteDate) AS rowcount FROM quote" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM quote WHERE QuoteDate = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM quote WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM quote" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM quote WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT QuoteCode) AS rowcount FROM quote WHERE QuoteCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM quote" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String quoteCode="", name="", customerCode="", date="", reference="", status;
    int quoteCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, referencePosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 ...
                 {
                   if(callType == 'S')
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, DocumentStatus FROM quote WHERE QuoteCode LIKE '" + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY QuoteCode";
                   else q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, DocumentStatus FROM quote " + companyCodeWHERE + "ORDER BY QuoteCode";

                   quoteCodePosn = 1; datePosn = 2; companyCodePosn = 3; statusPosn = 5; referencePosn = -1; companyNamePosn = 4;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT QuoteDate, QuoteCode, CompanyCode, CompanyName, DocumentStatus FROM quote WHERE QuoteDate = {d '"  + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY QuoteDate";
                     quoteCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; referencePosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT QuoteDate FROM quote" + companyCodeWHERE + " ORDER BY QuoteDate";
                     quoteCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; referencePosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT QuoteDate, quoteCode, CompanyCode, CompanyName, DocumentStatus FROM quote WHERE QuoteDate = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY QuoteDate";
                     quoteCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; referencePosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, QuoteCode, CompanyName, QuoteDate, EnquiryCode, Status FROM quote WHERE CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, QuoteCode";
                     quoteCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 6; referencePosn = 5;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM quote " + companyCodeWHERE + " ORDER BY CompanyCode";    
                     quoteCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; referencePosn = -1; statusPosn = -1; 
                   }
                   else
                   {
                     q = "SELECT CompanyCode, QuoteCode, QuoteDate, CompanyName, EnquiryCode, Status FROM quote WHERE CompanyCode = '" + codeArg + "' ORDER BY CompanyCode, QuoteCode";
                     quoteCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 4; referencePosn = 5; statusPosn = 6;
                   }
                 }  
                 ...
                 while(rs.next())                  
                 {
                   quoteCode      = getValue(quoteCodePosn,      ' ', rs, rsmd);
                   date           = getValue(datePosn,           'D', rs, rsmd);
                   customerCode   = getValue(companyCodePosn,    ' ', rs, rsmd);
                   name           = getValue(companyNamePosn,    ' ', rs, rsmd);
                   reference      = getValue(referencePosn,      ' ', rs, rsmd);
                   status         = getValue(statusPosn,         ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, quoteCode, date, customerCode, name, null, quoteCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date, cssFormat);
                     else appendBodyLine(html, inx, date, quoteCode, customerCode, name, null, quoteCode, status, imagesDir, cssFormat);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, quoteCode, name, date, reference, quoteCode, status, imagesDir, cssFormat);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = quoteCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = quoteCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = quoteCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = quoteCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 ...
                 {
                   if(callType == 'S')
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, Status FROM quote WHERE QuoteCode LIKE '" + srchStr + "%' AND QuoteCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY QuoteCode DESC";
                   else q = "SELECT QuoteCode, QuoteDate, CompanyCode, EnquiryCode, Status FROM quote WHERE QuoteCode <= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY QuoteCode DESC";
                   
                   quoteCodePosn = 1; datePosn = 2; companyCodePosn = 3; referencePosn = -1; statusPosn = 5; companyNamePosn = 4;
                 }  
                 else
                 if(inx == '2')
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT QuoteDate FROM quote WHERE QuoteDate <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY QuoteDate DESC";
                     quoteCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; referencePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT QuoteDate, QuoteCode, CompanyCode, CompanyName, Status FROM quote WHERE QuoteDate = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY QuoteDate, QuoteCode DESC";
                     quoteCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; referencePosn = -1; statusPosn = 5;
                   }  
                 }
                 else // inx == '3'
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM quote WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                     quoteCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; referencePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                      q = "SELECT CompanyCode, QuoteCode, CompanyName, QuoteDate, EnquiryCode, Status FROM quote WHERE CompanyCode = '"
                        + codeArg + "' ORDER BY CompanyCode, QuoteCode DESC";
                     quoteCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; referencePosn = 5; statusPosn = 6;
                   }
                 }
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, quoteCode, date, customerCode, name, null, quoteCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date, cssFormat);
                     else prependBodyLine(html, inx, date, quoteCode, customerCode, name, null, quoteCode, status, imagesDir, cssFormat);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, quoteCode, name, date, reference, quoteCode, status, imagesDir, cssFormat);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, Status FROM quote WHERE QuoteCode LIKE '" + srchStr + "%' AND QuoteCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY QuoteCode";
                     quoteCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; referencePosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, Status FROM quote WHERE QuoteCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY QuoteCode";
                     quoteCodePosn = 1; datePosn = 2; companyCodePosn = 3; referencePosn = -1; statusPosn = 5; companyNamePosn = 4;
                   }  
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT QuoteDate FROM quote WHERE QuoteDate > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + "" + companyCodeANDWHERE + " ORDER BY QuoteDate";
                     quoteCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; referencePosn = -1; statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT QuoteDate, QuoteCode, CompanyCode, CompanyName, Status FROM quote WHERE QuoteDate = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND QuoteCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY QuoteDate, QuoteCode";
                     quoteCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; referencePosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM quote WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                     quoteCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; referencePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, QuoteCode, CompanyName, QuoteDate, EnquiryCode, Status FROM quote WHERE CompanyCode = '" + codeArg + "' AND QuoteCode > '" + codeArg2 + "' ORDER BY CompanyCode, QuoteCode";
                     quoteCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; referencePosn = 5; 
                     statusPosn = 6;
                   }
                 }  
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, Status FROM quote WHERE QuoteCode LIKE '" + srchStr 
                       + "%' AND QuoteCode < '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY QuoteCode DESC";
                   }
                   else
                   {
                     q = "SELECT QuoteCode, QuoteDate, CompanyCode, CompanyName, Status FROM quote WHERE QuoteCode < '" + codeArg 
                     + "'" + companyCodeANDWHERE + " ORDER BY QuoteCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT QuoteDate FROM quote WHERE QuoteDate < {d '" + generalUtils.convertDateToSQLFormat(codeArg) 
                       + "'}" + companyCodeANDWHERE + " ORDER BY QuoteDate DESC";
                     quoteCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; referencePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT QuoteDate, QuoteCode, CompanyCode, CompanyName, Status FROM quote WHERE QuoteDate = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND QuoteCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY QuoteDate, QuoteCode DESC";
                     quoteCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; referencePosn = -1; statusPosn = 5;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM quote WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                     quoteCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; referencePosn = -1; 
                     statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, QuoteCode, CompanyName, QuoteDate, EnquiryCode, Status FROM quote WHERE CompanyCode = '"
                       + codeArg + "' AND QuoteCode < '" + codeArg2 + "' ORDER BY CompanyCode, QuoteCode DESC";
                     quoteCodePosn = 2; datePosn = 4; companyCodePosn = 1; companyNamePosn = 3; referencePosn = 5; 
                     statusPosn = 6;
                   }  
                 }  
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Quotation Search Results for ";
        switch(inx)
        {
          case '1' : title += "Quotation Code";  break;
          case '2' : title += "Quotation Date";  break;
      ...
      else title = "Quotation Listing";

      drawingUtils.drawTitle(out, false, false, "QuotationListing", "", title, "4030", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Quotation Code");  break;
        case '2' : scout(out, bytesOut, "Quotation Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                         ...
                         throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Quotation Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/QuotationPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/QuotationListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/QuotationListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4030", "", "QuotationListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quotation Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quotation Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quotation Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quotation Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Enquiry Code</td></tr>");
                 break;
    }
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Quotation Date");  break;
