// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: CN listing page
// Module: SalesCreditNoteListing.java
// Author: C.K.Harvey
...
public class SalesCreditNoteListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesCreditNoteListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4110, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4110, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesCreditNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4110, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesCreditNoteListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4110, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4110, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM credit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM credit" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM credit WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM credit WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM credit" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM credit WHERE CompanyCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT CNCode) AS rowcount FROM credit WHERE CNCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM credit" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String cnCode="", name="", customerCode="", date="", totalAmount="", status;
    int cnCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, totalAmountPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode LIKE '" + srchStr + "%'" 
                       + companyCodeANDWHERE + " ORDER BY CNCode";
                   }
                   else
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit " + companyCodeWHERE + "ORDER BY CNCode";
                   }

                   cnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, CNCode, CompanyCode, CompanyName, Status FROM credit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date";
                     
                     cnCodePosn = 2; datePosn = 1; companyCodePosn = 3; statusPosn = 5; companyNamePosn = 4; totalAmountPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM credit ORDER BY Date" + companyCodeWHERE;
                     
                     cnCodePosn = -1; datePosn = 1; companyCodePosn = -1; statusPosn = -1; companyNamePosn = -1;
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     ...
                     cnCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, CNCode, CompanyName, TotalTotal, Status FROM credit WHERE CompanyCode LIKE '"
                       + srchStr + "%' ORDER BY CompanyCode, CompanyName";
                     
                     cnCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; statusPosn = 5; totalAmountPosn = 4;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM credit " + companyCodeWHERE + "ORDER BY CompanyCode";
                     
                     cnCodePosn = -1; datePosn = -1; companyCodePosn = 1; companyNamePosn = -1; statusPosn = -1; 
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     q = "SELECT CompanyCode, CNCode, CompanyName, TotalTotal, Status FROM credit WHERE CompanyCode = '" + codeArg
                 ...
                 while(rs.next())                  
                 {
                   cnCode       = getValue(cnCodePosn,      ' ', rs, rsmd);
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, cnCode, date, customerCode, name, cnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, cnCode, customerCode, name, cnCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     else appendBodyLine(html, inx, customerCode, cnCode, name, totalAmount, cnCode, status, imagesDir);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = cnCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = cnCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = cnCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = cnCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode LIKE '" + srchStr
                       + "%' AND CNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY CNCode DESC";
                   }
                   else
                   {  
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY CNCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM credit WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, CNCode, CompanyCode, CompanyName, Status FROM credit WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, CNCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM credit WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, CNCode, CompanyName, TotalTotal, Status FROM credit WHERE CompanyCode = '" + codeArg
                        + "' ORDER BY CompanyCode, CNCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, cnCode, date, customerCode, name, cnCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, cnCode, customerCode, name, cnCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     else prependBodyLine(html, inx, customerCode, cnCode, name, totalAmount, cnCode, status, imagesDir);
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode LIKE '"
                       + srchStr + "%' AND CNCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY CNCode";
                     
                     cnCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY CNCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM credit WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, CNCode, CompanyCode, CompanyName, Status FROM credit WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND CNCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, CNCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM credit WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, CNCode, CompanyName, TotalTotal, Status FROM credit WHERE CompanyCode = '" + codeArg
                       + "' AND CNCode > '" + codeArg2 + "' ORDER BY CompanyCode, CNCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode LIKE '"
                       + srchStr + "%' AND CNCode < '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY CNCode DESC";
                   }
                   else
                   {
                     q = "SELECT CNCode, Date, CompanyCode, CompanyName, Status FROM credit WHERE CNCode < '" + codeArg 
                 ...
                 if(inx == '2')
                 {
                   ...
                   {  
                     ...
                     cnCodePosn = 2; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     totalAmountPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, CNCode, CompanyCode, CompanyName, Status FROM credit WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND CNCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, CNCode DESC";
                     
                     cnCodePosn = 1; datePosn = 1; companyCodePosn = 3; companyNamePosn = 4; statusPosn = 5; totalAmountPosn = -1;
                   }
                 }  
                 else // inx == '3'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM credit WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, CNCode, CompanyName, TotalTotal, Status FROM credit WHERE CompanyCode = '" + codeArg
                       + "' AND CNCode < '" + codeArg2 + "' ORDER BY CompanyCode, CNCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount,int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Credit Note Search Results for ";
        switch(inx)
        {
          case '1' : title += "Credit Note Code";  break;
          case '2' : title += "Credit Note Date";  break;
      ...
      else title = "Credit Note Listing";

      drawingUtils.drawTitle(out, false, false, "SalesCreditNoteListing", "", title, "4110", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Credit Note Code");  break;
        case '2' : scout(out, bytesOut, "Credit Note Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Credit Note Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/SalesCreditNotePage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/SalesCreditNoteListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/SalesCreditNoteListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4110", "", "SalesCreditNoteListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Credit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Credit Note Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Credit Note Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Credit Note Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Credit Note Date");  break;
