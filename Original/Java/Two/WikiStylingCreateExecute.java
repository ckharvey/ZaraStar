...
public class WikiStylingCreateExecute extends HttpServlet
{
                   ...
                   String imagesDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(generalUtils.fileExists("/Zara/" + dnm + "/Css/" + newStyleName + "/general.css"))
    {
      ...
      dashboardUtils.drawTitle(out, "New Site Style", "7081", unm, sid, uty, men, den, dnm, bnm, bytesOut);
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/WikiStylingEdit?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=" + generalUtils.sanitise(newStyleName) + "&p2="
                    + source + "&bnm=" + bnm);
