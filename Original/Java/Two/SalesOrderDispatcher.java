// =======================================================================================================================================================================================================
// System: ZaraStar: Sales Orders Processing - Dispatcher
// Module: SalesOrderDispatcher.java
// Author: C.K.Harvey
...
public class SalesOrderDispatcher extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrderDispatcher", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2036, bytesOut[0], 0, "ERR:");
                    ...
                    String bnm, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 2036, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrderDispatcher", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2036, bytesOut[0], 0, "ACC:");
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrderDispatcher", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 2036, bytesOut[0], 0, "SID:");
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 2036, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
                   ...
                   String bnm, String imagesDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "2036", "", "SalesOrderDispatcher", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesOrderDispatcher", "", "Sales Order Processing - Dispatcher", "2036", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form><table id=\"page\" width=100% border=0>");

    scoutln(out, bytesOut, "<tr><td></td></tr><tr id=\"pageColumn\"><td></td>");

    scoutln(out, bytesOut, "<td><p><b> Document Manifest Verified </b></td>");
    scoutln(out, bytesOut, "<td><p><b> Dispatched </b></td>");
                                       ...
                                       String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT SOCode, Date, CompanyCode, CustomerPOCode, TotalTotal, Currency2, CompanyName, ToEngineering, "
                         ...
                         + "FROM so WHERE Status != 'C' AND AllSupplied != 'Y'     AND Date > {d '2007-07-01'} "); //////////// TODO
    ...
    {    
      ...
      else scout(out, bytesOut, "<td align=center><p>Verified</td>");
