// =======================================================================================================================================================================================================
// System: ZaraStar Document: Receipt monthly listing
// Module: ReceiptMonthlyListing.java
// Author: C.K.Harvey
...
public class ReceiptMonthlyListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ReceiptMonthlyListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4310, bytesOut[0], 0, "ERR:" + p1);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4310, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ReceiptMonthlyListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4310, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ReceiptMonthlyListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4310, bytesOut[0], 0, "SID:" + p1);
      if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4310, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
                        ...
                        int[] bytesOut) throws Exception
  {
    ...
    String receiptCode, name, customerCode, date, totalAmount, currency, baseTotalAmount, status, receiptReference, charges;
    int x, count = 0;
    ...
    rs = stmt.executeQuery("SELECT ReceiptCode, Date, CompanyCode, BaseTotalAmount, TotalAmount, Currency, CompanyName, Status, "
                                   + "ReceiptReference, Charges FROM receipt WHERE DateReceived >= {d '" + dateFrom + "'} AND DateReceived <= {d '"
                                   + dateTo + "'} ORDER BY ReceiptCode");
    ...
    while(rs.next())                  
    {
      ...
      receiptReference = rs.getString(9);
      charges          = rs.getString(10);
      
      appendBodyLine(out, receiptCode, date, customerCode, baseTotalAmount, totalAmount, currency, name, status, receiptReference, charges, imagesDir,
                     cssFormat, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td colspan=2><p>Number of Receipts:</td><td nowrap align=right><p>" + count + "</td></tr>");
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String p1, boolean plain, String unm, String sid,
                       ...
                       int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Receipt Monthly Listing</title>");
    ...
    scoutln(out, bytesOut, "function fetchAR(code){var code2=sanitise(code);");
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/_4214?unm=" + unm + "&sid="
                           + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           + "&p2=A&p1=\"+code2;}");
    ...
    outputPageFrame(con, stmt, rs, out, req, plain, p1, "", "Receipt Monthly Listing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, String p1, String imagesDir, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      scoutln(out, bytesOut, "<tr><td colspan=6><p>Receipt Listing for the month of " + s + "</td></tr>");
    ...
    scoutln(out, bytesOut, "<td id=\"pageColumn\">Receipt Code &nbsp;</td>");
    scoutln(out, bytesOut, "<td id=\"pageColumn\">Receipt Reference &nbsp;</td>");
    scoutln(out, bytesOut, "<td id=\"pageColumn\">Customer Code &nbsp;</td>");
    scoutln(out, bytesOut, "<td id=\"pageColumn\">Date Received &nbsp;</td>");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(PrintWriter out, String receiptCode, String date, String customerCode, String baseTotalAmount, String totalAmount,
                              String currency, String name, String status, String receiptReference, String charges, String imagesDir,
                              String[] cssFormat, int[] bytesOut) throws Exception
  {
    String s = checkCode(receiptCode);
    ...
    scoutln(out, bytesOut, "<td nowrap><p><a href=\"javascript:fetch('" + s + "')\">" + receiptCode + "</a>&nbsp;</td>");
    scoutln(out, bytesOut, "<td nowrap><p>" + receiptReference + "&nbsp;</td>");
                               ...
                               String dnm, String bnm, String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "4310", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);
    ...
    if(plain)
      scoutln(out, bytesOut, authenticationUtils.buildPlainScreen(con, stmt, rs, req, unm, sid, uty, men, den, dnm, bnm, "ReceiptMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
    else
    {
      scoutln(out, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, "ReceiptMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
  ...
  private String buildSubMenuText(boolean plain, String month, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] hmenuCount) throws Exception
  {
    ...
    if(! plain)
    {
      ...
      s += "<a href=\"/central/servlet/ReceiptMonthlyListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + month + "&p2=P\">Friendly</a></dt></dl>";
    }
