...
// Module: StockCheckValuation.java
// Author: C.K.Harvey
...
public class StockCheckValuation extends HttpServlet
{
                    ...
                    throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 3066, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 3066, bytesOut[0], 0, "ACC:");
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 3066, bytesOut[0], 0, "SID:");
      if(con != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3066, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
    if(con != null) con.close();
    if(out != null) out.flush(); 
  }
                   ...
                   throws Exception
  {
    ...
    scoutln(out, bytesOut, "var p2=document.forms[0].date.value;");
    scoutln(out, bytesOut, "window.location.href=\"/central/servlet/StockCheckValuationExecute?p1=\"+p1+\"&p2=\"+p2+\"&unm=" + unm + "&uty=" + uty + "&sid=" + sid + "&men=" + men
    ...
    scoutln(out, bytesOut, "Stock Check Records Dated &nbsp;&nbsp;</td><td><input type=text maxlength=10 size=10 name=date></td></tr>");
