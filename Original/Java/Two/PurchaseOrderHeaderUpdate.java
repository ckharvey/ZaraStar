// =======================================================================================================================================================================================================
// System: SteeClaws DocumentEngine: PO update header rec
// Module: PurchaseOrderHeaderUpdate.java
// Author: C.K.Harvey
...
public class PurchaseOrderHeaderUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseOrderHeaderUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5009, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "5009", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5009, bytesOut[0], 0, "ACC:" + codeStr);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "5009", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5009, bytesOut[0], 0, "SID:" + codeStr);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    else
    {
      ...
      if(errorValidation.validatePageReturningStr(con, stmt, rs, "Date", retStr, purchaseOrder.getFieldNamesPO(), purchaseOrder.getFieldTypesPO(), purchaseOrder.getFieldStylesPO(), "po",
                                        "Update Purchase Order Header", dnm, unm, recData, y, imagesDir, localDefnsDir, defnsDir)) // validated ok
      {
        switch(purchaseOrder.putHead(con, stmt, rs, code, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
        {
          ...
          case 'X' : // already exists
                      ...
                      break;
          case 'I' : // illegal code
                      errorValidation.formatErrMsg("Invalid Purchase Order Code", retStr, imagesDir);
                      reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                      break;
          case 'N' : // no code specified
                      errorValidation.formatErrMsg("No Purchase Order Code Specified", retStr, imagesDir);
                      reGetRec(out, unm, sid, uty, men, den, dnm, bnm, codeStr, retStr[0], p3, cad, localDefnsDir);
                      break;
          case 'F' : // not updated
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5009, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeStr);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/PurchaseOrderHeaderEdit");
