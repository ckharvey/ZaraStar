// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: PCN update line rec
// Module: PurchaseCreditNoteLineUpdate.java
// Author: C.K.Harvey
...
public class PurchaseCreditNoteLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseCreditNoteLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5031, bytesOut[0], 0, "ERR:" + p2);
                    ...
                    throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "5031", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5031, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "5031", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5031, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' :  if(purchaseCreditNote.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else 
                  {
                    messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "5031", imagesDir,
      ...
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, purchaseCreditNote.getFieldNamesPurchaseCreditNoteL(),
                                                    purchaseCreditNote.getFieldTypesPurchaseCreditNoteL(),
                                                    purchaseCreditNote.getFieldStylesPurchaseCreditNoteL(), "pcreditl", 
                                                    "Update Purchase Credit Note Record", dnm, unm,  recData, y, imagesDir,
                                                    localDefnsDir, defnsDir))
                  {
                    switch(purchaseCreditNote.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Purchase Credit Note Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Purchase Credit Note Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 5032; operation = " Line: " + line;  break;
      case 'C' : service = 5031; operation = " Line: " + line;  break;
      case 'X' : service = 5031; operation = " Cancel";         break;
      default  : service = 5031; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                        ...
                        throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/PurchaseCreditNoteLine");
