// =======================================================================================================================================================================================================
// System: ZaraStar: Stock - SC WIP Reconciliation - details
// Module: StockCheckWIPReconciliation.java
// Author: C.K.Harvey
...
public class StockCheckWIPReconciliation extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "StockCheckReconciliation", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3065, bytesOut[0], 0, "ERR:" + p1 + ":" + p2);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String p1, String p2, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "StockCheckWIPReconciliation", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3065, bytesOut[0], 0, "ACC:" + p1 + ":" + p2);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "StockCheckWIPReconciliation", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3065, bytesOut[0], 0, "SID:" + p1 + ":" + p2);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3065, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1 + ":" + p2);
  ...
  // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void set(Connection con, Statement stmt, Statement stmt2, ResultSet rs, ResultSet rs2, PrintWriter out, HttpServletRequest req, String itemCode, String dateTo, String unm, String sid, String uty, String men, String den,
                   String dnm, String bnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>WIP Reconciliation</title>");
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "3065", "", "StockCheckWIPReconciliation", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "StockCheckWIPReconciliation", "", "Work-in-Progress Reconciliation for " + itemCode, "3065", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<td align=center><p>SO Quantity</td>");
    scoutln(out, bytesOut, "<td align=center><p>PL Quantity</td>");
    scoutln(out, bytesOut, "<td align=center><p>DO Quantity</td>");
    scoutln(out, bytesOut, "<td align=center><p>SA Quantity</td></tr>");

    char dpOnQuantities = miscDefinitions.dpOnQuantities(con, stmt, rs, "5");
    
    double[] soTotalQty = new double[1];
    double[] plTotalQty = new double[1];
    double[] doTotalQty = new double[1];
    double[] saTotalQty = new double[1];
    
    allSOs(con, stmt, rs, out, itemCode, dateTo, dpOnQuantities, soTotalQty, plTotalQty, doTotalQty, saTotalQty, bytesOut);

    scoutln(out, bytesOut, "<tr><td>&nbsp;</td></tr>");

    scoutln(out, bytesOut, "<tr><td colspan=3><p>Totals</td>");
    scoutln(out, bytesOut, "<td align=center><p>SO: " + generalUtils.formatNumeric(soTotalQty[0], dpOnQuantities) + "</td>");
    scoutln(out, bytesOut, "<td align=center><p>PL: " + generalUtils.formatNumeric(plTotalQty[0], dpOnQuantities) + "</td>");
    scoutln(out, bytesOut, "<td align=center><p>DO: " + generalUtils.formatNumeric(doTotalQty[0], dpOnQuantities) + "</td>");
    scoutln(out, bytesOut, "<td align=center><p>SA: " + generalUtils.formatNumeric(saTotalQty[0], dpOnQuantities) + "</td></tr>");
  ...
  // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void allSOs(Connection con, Statement stmt, ResultSet rs, PrintWriter out, String itemCode, String dateTo, char dpOnQuantities, double[] soTotalQty, double[] plTotalQty, double[] doTotalQty, double[] saTotalQty, int[] bytesOut)
                      throws Exception
  {
    soTotalQty[0] = plTotalQty[0] = doTotalQty[0] = saTotalQty[0] = 0.0;

    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT DISTINCT t1.SOCode FROM sol AS t2 INNER JOIN so AS t1 ON t2.SOCode = t1.SOCode WHERE t1.Status != 'C' AND t1.Date <= {d '" + dateTo + "'} AND t2.ItemCode = '" + itemCode + "'");
      
    String[] cssFormat = new String[1];  cssFormat[0] = "";

    while(rs.next())
      allLines(con, stmt, rs, out, rs.getString(1), itemCode, dateTo, dpOnQuantities, soTotalQty, plTotalQty, doTotalQty, saTotalQty, bytesOut);
  ...
  // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void allLines(Connection con, Statement stmt, ResultSet rs, PrintWriter out, String soCode, String itemCode, String dateTo, char dpOnQuantities, double[] soTotalQty, double[] plTotalQty, double[] doTotalQty, double[] saTotalQty,
                        int[] bytesOut) throws Exception
  {
    ...
    while(rs.next())
    {
      ...
      soTotalQty[0] += generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(2)), '2');
      ...
      scoutln(out, bytesOut, "<td align=center><p>" + generalUtils.formatNumeric(qty, dpOnQuantities) + "</td><td></td><td></td><td></td></tr>");
    }
    ...
    rs = stmt.executeQuery("SELECT t1.PLCode, t1.Date, t2.QuantityPacked FROM pll AS t2 INNER JOIN pl AS t1 ON t2.PLCode = t1.PLCode WHERE t1.Status != 'C' AND t1.Date <= {d '" + dateTo + "'} AND t2.SOCode = '" + soCode + "' AND t2.ItemCode = '"
                         + itemCode + "'");
    ...
    while(rs.next())
    {
      ...
      plTotalQty[0] += generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(3)), '2');

      if(cssFormat[0].equals("line1")) cssFormat[0] = "line2"; else cssFormat[0] = "line1";

      scoutln(out, bytesOut, "<tr id='" + cssFormat[0] + "'><td><p>Picking List</td><td><p><a href=\"javascript:viewPL('" + code + "')\">" + code + "</a></td>");
      scoutln(out, bytesOut, "<td><p>" + generalUtils.convertFromYYYYMMDD(date) + "</td><td></td>");
      scoutln(out, bytesOut, "<td align=center><p>" + generalUtils.formatNumeric(qty, dpOnQuantities) + "</td><td></td><td></td></tr>");
    }
    ...
    rs = stmt.executeQuery("SELECT t1.DOCode, t1.Date, t2.Quantity FROM dol AS t2 INNER JOIN do AS t1 ON t2.DOCode = t1.DOCode WHERE t1.Status != 'C' AND t1.Date <= {d '" + dateTo + "'} AND t2.SOCode = '" + soCode + "' AND t2.ItemCode = '"
                         + itemCode + "'");
    ...
    while(rs.next())
    {
      ...
      doTotalQty[0] += generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(3)), '2');

      if(cssFormat[0].equals("line1")) cssFormat[0] = "line2"; else cssFormat[0] = "line1";

      scoutln(out, bytesOut, "<tr id='" + cssFormat[0] + "'><td><p>Delivery Order</td><td><p><a href=\"javascript:viewDO('" + code + "')\">" + code + "</a></td>");
      scoutln(out, bytesOut, "<td><p>" + generalUtils.convertFromYYYYMMDD(date) + "</td><td></td><td></td>");
      scoutln(out, bytesOut, "<td align=center><p>" + generalUtils.formatNumeric(qty, dpOnQuantities) + "</td><td></td></tr>");
    }
    ...
    rs = stmt.executeQuery("SELECT AdjustmentCode, Date, Quantity, Status FROM stocka WHERE SOCode = '" + soCode + "' AND StoreFrom = 'None' AND StoreTo != 'None' AND Date <= {d '" + dateTo + "'} AND ItemCode = '" + itemCode + "'");
    ...
    while(rs.next())
    {
      ...
      if(! status.equals("C"))
        saTotalQty[0] += generalUtils.doubleDPs(generalUtils.doubleFromStr(rs.getString(3)), '2');

      if(cssFormat[0].equals("line1")) cssFormat[0] = "line2"; else cssFormat[0] = "line1";

      scoutln(out, bytesOut, "<tr id='" + cssFormat[0] + "'><td><p>Stock Adjustment</td><td><p><a href=\"javascript:viewSA('" + code + "')\">" + code + "</a>");
      if(status.equals("C"))
        scoutln(out, bytesOut, "<font color=red>&nbsp;(Cancelled)</font>");
      scoutln(out, bytesOut, "</td>");

      
      scoutln(out, bytesOut, "<td><p>" + generalUtils.convertFromYYYYMMDD(date) + "</td><td></td><td></td><td></td>");
      scoutln(out, bytesOut, "<td align=center><p>" + generalUtils.formatNumeric(qty, dpOnQuantities) + "</td></tr>");
    }
...
//      total += (getWAC(con, stmt, rs, out, itemCode, desc, dateFrom, dateTo, dpOnQuantities, baseCurrency, cssFormat, bytesOut) * quantity);  
