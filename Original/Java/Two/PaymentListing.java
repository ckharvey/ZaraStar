// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Payment listing page
// Module: PaymentListing.java
// Author: C.K.Harvey
...
public class PaymentListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PaymentListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5057, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5057, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PaymentListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5057, bytesOut[0], 0, "ACC:" +codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PaymentListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5057, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5057, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM payment" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM payment WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM payment" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(ChequeNumber) AS rowcount FROM payment WHERE ChequeNumber LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT ChequeNumber) AS rowcount FROM payment" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment WHERE ChequeNumber = '" + codeArg + "'");
                   break;
        case '5' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(APCode) AS rowcount FROM payment WHERE APCode LIKE '" + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT APCode) AS rowcount FROM payment" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment WHERE APCode = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT PaymentCode) AS rowcount FROM payment WHERE PaymentCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM payment" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String paymentCode="", name="", supplierCode="", date="", totalAmount="", apCode="", chequeNumber="", status;
    int paymentCodePosn, datePosn, companyCodePosn, companyNamePosn, statusPosn, totalAmountPosn, apCodePosn,
        chequeNumberPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY PaymentCode";
                   }
                   else
                   {
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment " + companyCodeWHERE
                       + "ORDER BY PaymentCode";
                   }

                   paymentCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; statusPosn = 6; totalAmountPosn = 4; 
                   apCodePosn = -1; chequeNumberPosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, PaymentCode, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PaymentCode";
                     
                     paymentCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 5; statusPosn = 6; 
                     totalAmountPosn = 4; apCodePosn = -1; chequeNumberPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM payment ORDER BY Date" + companyCodeWHERE;
                     
                     paymentCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; statusPosn = -1;
                     totalAmountPosn = -1; apCodePosn = -1; chequeNumberPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, PaymentCode, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PaymentCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, PaymentCode, Date, TotalAmount, CompanyName, Status FROM payment WHERE "
                      + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, PaymentCode";
                     
                     paymentCodePosn = 2; datePosn = 3; companyCodePosn = 1; companyNamePosn = 5; statusPosn = 6;
                     totalAmountPosn = 4; apCodePosn = -1; chequeNumberPosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM payment " + companyCodeWHERE + "ORDER BY CompanyCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT ChequeNumber, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE "
                      + "ChequeNumber LIKE '" + srchStr + "%' ORDER BY ChequeNumber, PaymentCode";
                     
                     paymentCodePosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 5; statusPosn = 6;
                     totalAmountPosn = -1; apCodePosn = -1; chequeNumberPosn = 1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM payment " + companyCodeWHERE + "ORDER BY ChequeNumber";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT APCode, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE APCode LIKE '"
                       + srchStr + "%' ORDER BY APCode, PaymentCode";
                   ...
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT APCode FROM payment " + companyCodeWHERE + "ORDER BY APCode";
                   ...
                   {
                     q = "SELECT APCode, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE APCode = '"
                       + codeArg + "' ORDER BY APCode";
                 ...
                 while(rs.next())                  
                 {
                   paymentCode  = getValue(paymentCodePosn, ' ', rs, rsmd);
                   ...
                   apCode       = getValue(apCodePosn,      ' ', rs, rsmd);
                   totalAmount  = getValue(totalAmountPosn, ' ', rs, rsmd);
                   status       = getValue(statusPosn,      ' ', rs, rsmd);

                   if(inx == '1')
                     appendBodyLine(html, inx, paymentCode, date, supplierCode, totalAmount, name, paymentCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, paymentCode, supplierCode, totalAmount, name, paymentCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, supplierCode);
                     else
                     {
                       appendBodyLine(html, inx, supplierCode, paymentCode, date, totalAmount, name, paymentCode, status, imagesDir);
                     }
                   }   
                   else
                   if(inx == '4')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, chequeNumber);
                     else
                     {
                       appendBodyLine(html, inx, chequeNumber, paymentCode, date, supplierCode, name, paymentCode, status,
                                      imagesDir);
                     }
                   }   
                   else // inx == '5'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, apCode);
                     else
                     {
                       appendBodyLine(html, inx, apCode, paymentCode, date, supplierCode, name, paymentCode, status, imagesDir);
                     }
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = paymentCode;
                     
                     if(inx == '1')
                       firstEntryOnPage[0] = paymentCode;
                     else
                     if(inx == '2')
                     ...
                     firstEntryOnPage[0] = apCode;
                   }
                 }
                 
                 rs.close();
                                 
                 lastCodeOnPage[0] = paymentCode;

                 if(inx == '1')
                   lastEntryOnPage[0] = paymentCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode LIKE '"
                       + srchStr + "%' AND PaymentCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PaymentCode DESC";
                   }
                   else
                   {  
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode "
                       + "<= 'ZZZZZZZZZZZZZZZZZZZZ'" + companyCodeANDWHERE + " ORDER BY PaymentCode DESC";
                   }
                   
                   paymentCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; statusPosn = 6;
                   totalAmountPosn = 4; apCodePosn = -1; chequeNumberPosn = -1;
                 }  
                 else
                 if(inx == '2')
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM payment WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, PaymentCode, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, PaymentCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM payment WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                   ...
                   {
                      q = "SELECT CompanyCode, PaymentCode, Date, TotalAmount, CompanyName, Status FROM payment WHERE "
                        + "CompanyCode = '" + codeArg + "' ORDER BY CompanyCode, PaymentCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM payment WHERE ChequeNumber <= 'zzzzzzzzzzzzzzzzzzz' ORDER BY "
                     + "ChequeNumber DESC";
                   ...
                   {
                      q = "SELECT ChequeNumber, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE "
                        + "ChequeNumber = '" + codeArg + "' ORDER BY ChequeNumber, PaymentCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT APCode FROM payment WHERE APCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY APCode DESC";
                   ...
                   {
                      q = "SELECT APCode, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE APCode = '"
                        + codeArg + "' ORDER BY APCode, PaymentCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, paymentCode, date, supplierCode, totalAmount, name, paymentCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, paymentCode, supplierCode, totalAmount, name, paymentCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, supplierCode);
                     else 
                     {
                       prependBodyLine(html, inx, supplierCode, paymentCode, date, totalAmount, name, paymentCode, status, imagesDir);
                     }
                   }  
                   else
                   if(inx == '4')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, chequeNumber);
                     else 
                     {
                       prependBodyLine(html, inx, chequeNumber, paymentCode, date, supplierCode, name, paymentCode, status, imagesDir);
                     }
                   }  
                   else // inx == '5'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, apCode);
                     else 
                     {
                       prependBodyLine(html, inx, apCode, paymentCode, date, supplierCode, name, paymentCode, status, imagesDir);
                     }
                   }  
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode LIKE '"
                       + srchStr + "%' AND PaymentCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PaymentCode";
                   ...
                   {                   
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode > '"
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PaymentCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM payment WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, PaymentCode, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PaymentCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                        + " ORDER BY Date, PaymentCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM payment WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                   ...
                   {
                     q = "SELECT CompanyCode, PaymentCode, Date, TotalAmount, CompanyName, Status FROM payment WHERE "
                       + "CompanyCode = '" + codeArg + "' AND PaymentCode > '" + codeArg2 + "' ORDER BY CompanyCode, PaymentCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM payment WHERE ChequeNumber >= '" + codeArg
                   ...
                   {
                     q = "SELECT ChequeNumber, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE "
                       + "ChequeNumber = '" + codeArg + "' AND PaymentCode > '" + codeArg2 + "' ORDER BY ChequeNumber, PaymentCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT APCode FROM payment WHERE APCode >= '" + codeArg + "' ORDER BY APCode";
                   ...
                   {
                     q = "SELECT APCode, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE APCode = '" 
                       + codeArg + "' AND PaymentCode > '" + codeArg2 + "' ORDER BY APCode, PaymentCode";
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT PaymentCode, Date, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE PaymentCode < '"
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY PaymentCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   ...
                   else
                   {
                     q = "SELECT Date, PaymentCode, CompanyCode, TotalAmount, CompanyName, Status FROM payment WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND PaymentCode < '" + codeArg2 + "'" + companyCodeANDWHERE
                       + " ORDER BY Date, PaymentCode DESC";
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM payment WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                   ...
                   {
                     q = "SELECT CompanyCode, PaymentCode, Date, TotalAmount, CompanyName, Status FROM payment WHERE "
                       + "CompanyCode = '" + codeArg + "' AND PaymentCode < '" + codeArg2 + "' ORDER BY CompanyCode, PaymentCode DESC";
                 ...
                 if(inx == '4')
                 {
                   ...
                   {
                     q = "SELECT ChequeNumber, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE "
                       + "ChequeNumber = '" + codeArg + "' AND PaymentCode < '" + codeArg2
                       + "' ORDER BY ChequeNumber, PaymentCode DESC";
                 ...
                 else // inx == '5'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT APCode FROM payment WHERE APCode < '" + codeArg + "' ORDER BY APCode DESC";
                   ...
                   {
                     q = "SELECT APCode, PaymentCode, Date, CompanyCode, CompanyName, Status FROM payment WHERE APCode = '"
                       + codeArg + "' AND PaymentCode < '" + codeArg2 + "' ORDER BY APCode, PaymentCode DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Payment Search Results for ";
        switch(inx)
        {
          case '1' : title += "Payment Code";   break;
          case '2' : title += "Payment Date";   break;
          case '3' : title += "Customer Code";  break;
          case '4' : title += "Cheque Number";  break;
          case '5' : title += "AP Code";        break;
        }
      ...
      else title = "Payment Listing";

      drawingUtils.drawTitle(out, false, false, "PaymentListing", "", title, "5057", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '1' : scout(out, bytesOut, "Payment Code");  break;
        case '2' : scout(out, bytesOut, "Payment Date");  break;
        case '3' : scout(out, bytesOut, "Customer Code"); break;
        case '4' : scout(out, bytesOut, "Cheque Number"); break;
        case '5' : scout(out, bytesOut, "AP Code");       break;
      }
    }
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Payment Listing</title>");
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/PaymentListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/PaymentListing?unm=" + unm + "&sid=" + sid
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "5057", "", "PaymentListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Date &nbsp;</td>");
                 ...
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Total Amount &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name &nbsp;</td></tr>");
                 break;
      ...
      case '4' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Cheque Number &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name</td></tr>");
                 break;
      case '5' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>AP Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Payment Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Customer Name</td></tr>");
