// =======================================================================================================================================================================================================
// System: ZaraStar Document Engine: PL Record Access
// Module: PickingList.java
// Author: C.K.Harvey
...
public class PickingList
{
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringPL() throws Exception
  {  
    return "pl ( PLCode char(20) not null,      Date date not null,         CompanyCode char(20) not null, "
              ...
              + "Attention char(1),             DateOfStockUpdate date,     SignOnForUpdated char(20), "
              + "Misc1 char(80),                Misc2 char(80),             GSTTotal decimal(19,8), "
              + "TotalTotal decimal(19,8),      Completed char(1),          SignOn char(20), "
              + "GroupDiscount decimal(19,8),   DateLastModified timestamp, LocationCode char(20), "
              + "GroupDiscountType char(1),     Type char(1),               ShipAddressCode char(20), "
              + "PurchasedBy char(40),          CustomerPOCode char(40),    SalesPerson char(40), "
              + "ProjectCode char(20),          RevisionOf char(20),        ShipName char(60), "
              + "ShipAddress1 char(40),         ShipAddress2 char(40),      ShipAddress3 char(40), "
              + "ShipAddress4 char(40),         ShipAddress5 char(40),      Currency char(3), "
              + "NumCartons char(20),           Rate decimal(16,8),         StockProcessed char(1), "
              + "Time char(10),                 ExpectedDate date,          Terms char(20), "
              + "BaseTotalTotal decimal(19,8),  BaseGSTTotal decimal(19,8), Store char(20), "
              + "Status char(1),                TotalGrossWeight char(20),  Dimension char(40), "
              + "ExportStatement char(40),      InvoiceCode char(20),       OCCode char(20), "
              + "TotalNetWeight char(20),       BuyerEMail char(60),           AssignedStoreman char(40), "
              + "DateTimeAssigned timestamp,    AssignedBy char(20),        DateTimeReturned timestamp, "
              + "ReturnedBy char(20),           ReleaseToStore char(1),     TimesPrinted integer, "
              + "DateTimeLastPrinted timestamp, DateTimeQueued timestamp, "
              + "unique(PLCode))";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsPL(String[] s) throws Exception
  {
    s[0] = "plDateInx on pl (Date)";
    s[1] = "plCompanyCodeInx on pl (CompanyCode)";
    s[2] = "plCustomerPOCodeInx on pl (CustomerPOCode)";
    s[3] = "plInvoiceCodeInx on pl (InvoiceCode)";
    s[4] = "plProjectCodeInx on pl (ProjectCode)";
    
    return 5;
  }
      
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesPL() throws Exception
  {  
    return "PLCode, Date, CompanyCode, CompanyName, Address1, Address2, Address3, Address4, Address5, PostCode, FAO, Notes, Attention, DateOfStockUpdate, SignOnForUpdated, Misc1, Misc2, GSTTotal, TotalTotal, Completed, SignOn, GroupDiscount, "
          + "DateLastModified, LocationCode, GroupDiscountType, Type, ShipAddressCode, PurchasedBy, CustomerPOCode, SalesPerson, ProjectCode, RevisionOf, ShipName, ShipAddress1, ShipAddress2, ShipAddress3, ShipAddress4, ShipAddress5, Currency, "
          + "NumCartons, Rate, StockProcessed, Time, ExpectedDate, Terms, BaseTotalTotal, BaseGSTTotal, Store, Status, TotalGrossWeight, Dimension, ExportStatement, InvoiceCode, OCCode, TotalNetWeight, BuyerEMail, AssignedStoreman, "
          + "DateTimeAssigned, AssignedBy, DateTimeReturned, ReturnedBy, ReleaseToStore, TimesPrinted, DateTimeLastPrinted, DateTimeQueued";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesPL() throws Exception
  {
    return "CDCCCCCCCCCCCDCCCFFCCFSCCCCCCCCCCCCCCCCCFCCDCFFCCCCCCCCCCPCPCCIPP";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesPL(short[] sizes) throws Exception
  {
    sizes[0]  = 20;  sizes[1]  = 0;   sizes[2]  = 20;  sizes[3]  = 60;  sizes[4] = 40;   sizes[5] = 40;   sizes[6] = 40;  sizes[7]  = 40;  sizes[8]  = 40;  sizes[9]  = 20;  sizes[10] = 40;  sizes[11] = 250;  sizes[12] = 1;   sizes[13] = 0;
    sizes[14] = 20;  sizes[15] = 80;  sizes[16] = 80;  sizes[17] = 0;   sizes[18] = 0;   sizes[19] = 1;   sizes[20] = 20; sizes[21] = 0;   sizes[22] = -1;  sizes[23] = 20;  sizes[24] = 1;   sizes[25] = 1;    sizes[26] = 20;  sizes[27] = 40;
    sizes[28] = 40;  sizes[29] = 40;  sizes[30] = 20;  sizes[31] = 20;  sizes[32] = 60;  sizes[33] = 40;  sizes[34] = 40; sizes[35] = 40;  sizes[36] = 40;  sizes[37] = 40;  sizes[38] = 3;   sizes[39] = 20;   sizes[40] = 0;   sizes[41] = 1;
    sizes[42] = 10;  sizes[43] = 0;   sizes[44] = 20;  sizes[45] = 0;   sizes[46] = 0;   sizes[47] = 20;  sizes[48] = 1;  sizes[49] = 20;  sizes[50] = 40;  sizes[51] = 40;  sizes[52] = 20;  sizes[53] = 20;   sizes[54] = 20;  sizes[55] = 60;
    sizes[56] = 40;  sizes[57] = -1;  sizes[58] = 20;  sizes[59] = -1;  sizes[60] = 20;  sizes[61] = 1;   sizes[62] = 0;  sizes[63] = 0;   sizes[64] = 0;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesPL() throws Exception
  {
    return "MMMOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringPLL() throws Exception
  {  
    return "pll ( PLCode char(20) not null,  ItemCode char(20) not null,     Description char(80), "
               ...
               + "SignOn char(20),           Amount2 decimal(17,8),          CustomerPOCode char(40), "
               + "UoM char(20),              SerialNumber char(20),          Location char(20), "
               + "unused2 char(20),          QuantityRequired decimal(19,8), CustomerItemCode char(40), "
               + "Line integer not null,     Entry char(6),                  SOCode char(20), "
               + "GrossWeight decimal(16,8), WeightPer integer,              QuantityPacked decimal(19,8), "
               + "Instruction char(80),      LotNum char(40),                InvoiceCode char(20), "
               + "NetWeight decimal(16,8),   NumCartons integer,             Store char(20), "
               + "SOLine integer,            CostPrice decimal(17,8),        Manufacturer char(30), "
               + "ManufacturerCode char(60), "
               + "unique(PLCode, Line))";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsPLL(String[] s) throws Exception
  {
    s[0] = "pllItemCodeInx on pll(ItemCode)";
    s[1] = "pllSOCodeInx on pll(SOCode)";
    
    return 2;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesPLL() throws Exception
  {  
    return "PLCode, ItemCode, Description, UnitPrice, Quantity, Amount, GSTRate, Discount, DateLastModified, SignOn, Amount2, CustomerPOCode, UoM, SerialNumber, Location, unused2, QuantityRequired, CustomerItemCode, Line, Entry, SOCode, "
         + "GrossWeight, WeightPer, QuantityPacked, Instruction, LotNum, InvoiceCode, NetWeight, NumCartons, Store, SOLine, CostPrice, Manufacturer, ManufacturerCode";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesPLL() throws Exception
  {
    return "CCCFFFCFSCFCCCCCFCICCFIFCCCFICIFCC";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesPLL(short[] sizes) throws Exception
  {
    sizes[0] = 20;   sizes[1] = 20;    sizes[2] = 80;   sizes[3] = 0;     sizes[4] = 0;    sizes[5] = 0;    sizes[6] = 20;
    sizes[7] = 0;    sizes[8] = -1;    sizes[9] = 20;   sizes[10] = 0;    sizes[11] = 40;  sizes[12] = 20;  sizes[13] = 20;
    sizes[14] = 20;  sizes[15] = 20;   sizes[16] = 0;   sizes[17] = 40;   sizes[18] = 0;   sizes[19] = 6;   sizes[20] = 20;
    sizes[21] = 0;   sizes[22] = 0;    sizes[23] = 0;   sizes[24] = 80;   sizes[25] = 40;  sizes[26] = 20;  sizes[27] = 0;
    sizes[28] = 0;   sizes[29] = 20;   sizes[30] = 0;   sizes[31] = 0;    sizes[32] = 30;  sizes[33] = 60;  
  
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesPLL() throws Exception
  {
    return "OOOOOOOOOOOOOOOOOOMOOOOOOOOOOOOOOO";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringPLLL() throws Exception
  {  
    return "plll ( PLCode char(20) not null,   Entry char(6) not null,   Line integer not null,   Text char(80), "
                 + "unique(PLCode, Entry, Line))";
  }
  
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsPLLL(String[] s) throws Exception
  {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesPLLL() throws Exception
  {  
    return "PLCode, Entry, Line, Text";
  }
  
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesPLLL() throws Exception
  {
    return "CCIC";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesPLLL(short[] sizes) throws Exception
  {
    sizes[0] = 20;   sizes[1] = 6;   sizes[2] = 0;   sizes[3] = 80;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesPLLL() throws Exception
  {
    return "MMMO";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getTableCreateStringPLA() throws Exception
  {  
    return "pla ( Code char(20) not null, LibraryDocCode integer not null, unique(Code, LibraryDocCode))";
  }
  
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public int getIndexCreateStringsPLA(String[] s) throws Exception
  {
    return 0;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldNamesPLA() throws Exception
  {  
    return "Code, LibraryDocCode";
  }
  
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldTypesPLA() throws Exception
  {
    return "CI";
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public void getFieldSizesPLA(short[] sizes) throws Exception
  {
    sizes[0] = 20;   sizes[1] = 0;
  }

  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getFieldStylesPLA() throws Exception
  {
    return "MM";
  }
                              ...
                              String localDefnsDir, String defnsDir, char cad, String errStr, byte[] dataAlready, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    if(cad == 'A' && code[0] == '\000')
    {
      messagePage.msgScreen(false, out, req, 6, unm, sid, uty, men, den, dnm, bnm, "PickingList", imagesDir, localDefnsDir, defnsDir, bytesOut);
      return true;
    }
    ...
    {
      dispOrEdit = 'E';
      sortFields(dataAlready, headData, "pl");
    }
...
//    javaScriptCall(con, stmt, rs, javaScriptCallCode, req, date, unm, uty, dnm, localDefnsDir, defnsDir, bytesOut);
    ...
    // pickup the Completed status
    String completed = generalUtils.dfsAsStrGivenBinary1(true, headData, (short)19); // origin-0

    byte[] javaScriptCallCode = new byte[1000];
    javaScriptCall(con, stmt, rs, javaScriptCallCode, req, date, unm, uty, dnm, completed, localDefnsDir, defnsDir, bytesOut);

    short[] fieldSizesPL = new short[65]; // plenty
    getFieldSizesPL(fieldSizesPL);

    if(cad == 'A') // not a new one
    {
      short[] fieldSizesPLL = new short[40]; // plenty
      getFieldSizesPLL(fieldSizesPLL);

      double amt, amt2, totalAmtLines, totalGSTAmtLines, totalGSTAmt2Lines, totalAmt2Lines, groupDiscount, gstRate, totalNetWeight, totalGrossWeight, totalNumCartons;
      totalAmt2Lines = totalAmtLines = totalGSTAmtLines = totalGSTAmt2Lines = totalAmt2Lines = groupDiscount = totalNetWeight = totalGrossWeight = totalNumCartons = 0.0;
      char groupDiscountType;

      if(dispOrEdit == 'D')
        numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "202.htm", 2, getFieldNamesPL(), fieldSizesPL, getFieldNamesPLL(), fieldSizesPLL, null, null);
      else numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "202a.htm", 1, getFieldNamesPL(), fieldSizesPL, null, null, null, null);
      
      if(dispOrEdit == 'D')
        prepend(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, generalUtils.stringFromBytes(code, 0L), date, prependCode, req, completed, localDefnsDir, defnsDir, bytesOut);
      else prependEdit(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, "A", generalUtils.stringFromBytes(code, 0L), date, "202a.htm", localDefnsDir, defnsDir, prependCode, req, bytesOut);
      ...
      if(dispOrEdit == 'D') // display head *and* lines
      {
        ...
        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)21, b); // origin-0
        ...
        for(x=0;x<linesCount[0];++x)
        {
          if(generalUtils.getListEntryByNum(x, linesData, data)) // just-in-case
          {
            ...
            generalUtils.dfsGivenSeparator(true, '\001', data, (short)22, b); // origin-1
            totalGrossWeight += generalUtils.doubleFromChars(b);

            generalUtils.dfsGivenSeparator(true, '\001', data, (short)28, b);
            totalNetWeight += generalUtils.doubleFromChars(b);

            generalUtils.dfsGivenSeparator(true, '\001', data, (short)29, b);
            totalNumCartons += generalUtils.doubleFromChars(b);

            generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 8000,  6); // amt origin-1
            generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 8000, 11); // amt2
            generalUtils.bytesDPsGivenSeparator(true, '\001', '2', data, 8000,  8); // discount

            generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnQuantities, data, 8000,  5); // qty origin-1
            generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnQuantities, data, 8000, 24); // qty packed
            generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnQuantities, data, 8000, 17); // qty reqd
            generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnUnitPrice,  data, 8000,  4); // unitPrice

            generalUtils.dfsGivenSeparator(true, '\001', data, (short)19, line); // origin-1

            // add in the weight unit
            generalUtils.catAsBytes("pl.unitofweight=" + miscDefinitions.unitOfWeight(con, stmt, rs) + "\001", 0, data, false);
            ...
            screenLayout.bufferToBuffer(buf1, buf2, size1, size2, source, numFields, fieldNames, fieldTypes, fieldSizes, 'D', 'L', data, 8000, ddlData, ddlDataUpto[0],
                                        javaScriptCallCode, null);
          }
        }
        ...
        if(groupDiscountType != '%' && ! isAtLeastOneLineWithGST)
        {
          ...
          totalAmtLines  = totalAmt2Lines * generalUtils.dfsAsDoubleGivenSeparator(true, '\001', headData, (short)40);
        }
        ...
        stmt.executeUpdate("UPDATE pl SET TotalTotal = " + (totalAmt2Lines + totalGSTAmt2Lines) + ", GSTTotal = " + totalGSTAmt2Lines + ", BaseTotalTotal = " + (totalAmtLines + totalGSTAmtLines) + ", BaseGSTTotal = " + totalGSTAmtLines
                        + " WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
        if(stmt != null) stmt.close();

        generalUtils.repDoubleGivenSeparator('2', '\001', headData, 5000, (short)17, totalGSTAmt2Lines); // origin-0
        generalUtils.repDoubleGivenSeparator('2', '\001', headData, 5000, (short)18, (totalAmt2Lines + totalGSTAmt2Lines));

        scoutln(headData, bytesOut, "pl.subtotalbeforegst=" + generalUtils.doubleToStr(totalAmt2Lines) + "\001"); // subtotal on screen

        generalUtils.repDoubleGivenSeparator('2', '\001', headData, 5000, (short)45, (totalAmtLines + totalGSTAmtLines));
        generalUtils.repDoubleGivenSeparator('2', '\001', headData, 5000, (short)46, totalGSTAmtLines);

        generalUtils.bytesDPsGivenSeparator(true, '\001', '2', headData, 5000, 21); // group discount

        scoutln(headData, bytesOut, "pl.basecurrency=" + accountsUtils.getBaseCurrency(con, stmt, rs, dnm, localDefnsDir, defnsDir) + "\001");

        generalUtils.bytesDPsGivenSeparator(true, '\001', '2',  headData, 5000, 40); // rate
        scoutln(headData, bytesOut, "pl.rateNoTrailing=" + generalUtils.dfsAsStrGivenBinary1(true, headData, (short)40) + "\001");

        // display the customer's billing style
        generalUtils.dfsGivenBinary1(true, headData, (short)2, b); // companyCode
        String s="";
        
        switch(customer.getCompanyBillingStyleGivenCode(con, stmt, rs, b))
        {
          case 'I' : s = "Invoice";                         break;
          case 'D' : s = "Delivery Order";                  break;
          case 'B' : s = "Both Delivery Order and Invoice"; break;
          default  : s = "Unknown";                         break;
        }

        scoutln(headData, bytesOut, "pl.billingstyle=" + s + "\001");

        generalUtils.doubleToChars('8', totalGrossWeight, b, 0);
        generalUtils.formatNumeric(b, '0');
        updatePLField(con, stmt, rs, code, "TotalGrossWeight", generalUtils.stringFromBytes(b, 0L), dnm, localDefnsDir, defnsDir);

        generalUtils.doubleToChars('8', totalNetWeight, b, 0);
        generalUtils.formatNumeric(b, '0');
        updatePLField(con, stmt, rs, code, "TotalNetWeight", generalUtils.stringFromBytes(b, 0L), dnm, localDefnsDir, defnsDir);

        generalUtils.doubleToChars('8', totalNumCartons, b, 0);
        generalUtils.formatNumeric(b, '0');
        updatePLField(con, stmt, rs, code, "NumCartons", generalUtils.stringFromBytes(b, 0L), dnm, localDefnsDir, defnsDir);
        ...
        if(attachments.length() == 0)
          attachments= "none";
        scoutln(headData, bytesOut, "pl.Attachments=" + attachments + "\001");
      }
      else // edit
      {
        ddlData = accountsUtils.getCurrencyNamesDDLData(con, stmt, rs, "pl.Currency",  dnm, ddlData, ddlDataUpto, ddlDataLen, localDefnsDir, defnsDir);
        ddlData = documentUtils.getSalesPersonDDLData(con, stmt, rs, "pl.SalesPerson", ddlData, ddlDataUpto, ddlDataLen);
        ddlData = documentUtils.getStoreDDLData(con, stmt, rs, "pl.store",             ddlData, ddlDataUpto, ddlDataLen);
      ...
      if(dataAlready[0] == '\000') // NOT coming with an err msg
      {
        ...
        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)43, b); // origin-0
        generalUtils.convertFromYYYYMMDD(b); 
        generalUtils.repAlphaGivenSeparator('\001', headData, 5000, (short)43, b);

        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)57, b); // dateTimeAssigned
        generalUtils.convertFromTimestamp(b);
        generalUtils.repAlphaGivenSeparator('\001', headData, 5000, (short)57, b);

        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)59, b); // dateTimeReturned
        generalUtils.convertFromTimestamp(b);
        generalUtils.repAlphaGivenSeparator('\001', headData, 5000, (short)59, b);

        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)63, b); // dateTimeLastPrinted
        generalUtils.convertFromTimestamp(b);
        generalUtils.repAlphaGivenSeparator('\001', headData, 5000, (short)63, b);

        generalUtils.dfsGivenSeparator(true, '\001', headData, (short)64, b); // dateTimeQueued
        generalUtils.convertFromTimestamp(b);
        generalUtils.repAlphaGivenSeparator('\001', headData, 5000, (short)64, b);
      }

      generalUtils.bytesDPsGivenSeparator(true, '\001', '2',  headData, 5000, 21); // groupDiscount origin-0
    ...
    {
      numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "202a.htm", 1, getFieldNamesPL(), fieldSizesPL, null, null, null, null);

      if(dataAlready[0] != '\000') // coming with an err msg - passing-in data already entered but the data's fields may (will) not
      {                            // be in the correct order
        sortFields(dataAlready, data, "pl");
      }  
      else
      {
        screenLayout.buildNewDetails(fieldNames, numFields, "pl", data);
        
        documentUtils.getNextCode(con, stmt, rs, "pl", true, code);

        generalUtils.repAlphaUsingOnes(data, 8000, "PLCode", generalUtils.stringFromBytes(code, 0L));
      ...
      prependEdit(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, "C", generalUtils.stringFromBytes(code, 0L), date, "202a.htm", localDefnsDir, defnsDir, prependCode, req, bytesOut);

      screenLayout.bufferToBuffer(buf1, buf2, size1, size2, source, numFields, fieldNames, fieldTypes, fieldSizes, 'E', 'H', data, 8000, ddlData, ddlDataUpto[0], javaScriptCode, prependCode);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void javaScriptCall(Connection con, Statement stmt, ResultSet rs, byte[] b, HttpServletRequest req, String date, String unm, String uty, String dnm, String completed, String localDefnsDir, String defnsDir, int[] bytesOut)
                              throws Exception
  {
    b[0]= '\000';
    scoutln(b, bytesOut, "_.lineFile=pll.line\001");

    if(completed.equals("Y") && ! authenticationUtils.verifyAccess(con, stmt, rs, req, 3060, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
      scoutln(b, bytesOut, "_.permitted=n\001");
    else      
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3047, unm, uty, dnm, localDefnsDir, defnsDir))
  ...
  {
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3045, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      b[0]= '\000';
      scoutln(b, bytesOut, "<script language=\"JavaScript\">");
      scoutln(b, bytesOut, "function affect(line){");
      scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/PickingListLineFetch?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code + "&p2=\"+line+\"&p3=&p4=\");}</script>");
    }
    else b[0] = '\000';
  }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void prepend(Connection con, Statement stmt, ResultSet rs, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String code, String date, byte[] b, HttpServletRequest req, String completed,
                       String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3043, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function fetch(){");
      scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/PickingListEdit?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=A&p3=&p4=\");}\n");
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/GSTRatesChange?unm=" + unm + "&sid=" + sid + "&uty="
                           + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=PL\";}");
      ...
      if(authenticationUtils.verifyAccess(con, stmt, rs, req, 6098, unm, uty, dnm, localDefnsDir, defnsDir))
      {
        ...
        scoutln(b, bytesOut, "window.location.href=\"/central/servlet/DocumentChangeDiscounts?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den="
                           + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=PL\";}");
      }

      if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3043, unm, uty, dnm, localDefnsDir, defnsDir))
      {
        if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3054, unm, uty, dnm, localDefnsDir, defnsDir))
        {
          scoutln(b, bytesOut, "function updateQ(mode){");
          scoutln(b, bytesOut, "window.location.href=\"/central/servlet/PickingListSetQuantity?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den="
                               + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=\"+mode;}");
        }

        if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3056, unm, uty, dnm, localDefnsDir, defnsDir))
        {        
          scoutln(b, bytesOut, "function updateC(mode){");
          scoutln(b, bytesOut, "window.location.href=\"/central/servlet/PickingListSetQuantity?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=\"+mode;}");
        }
      }
    }   
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3045, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/PickingListLineFetch?unm=" + unm + "&sid=" + sid
                         + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2
                         + "&p2=&p3=&p4=\");}");
    }
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 1025, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/DocumentTraceExecute?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
                         + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=P\";}");
    } 
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 8016, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      scoutln(b, bytesOut, "window.location.replace(\"/central/servlet/MailExternalUserCreate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=QuotationPage&p2=" + code2 + "&p3=Picking%20List&bnm=" + bnm + "\");}");
    }    

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3048, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3049, unm, uty, dnm, localDefnsDir, defnsDir))
      {
        scoutln(b, bytesOut, "function queue(){");
        scoutln(b, bytesOut, "window.location.href=\"/central/servlet/PickingListQueueAdd?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "\";}");
      }
      else
      {
        scoutln(b, bytesOut, "function print(){");
        scoutln(b, bytesOut, "window.location.href=\"/central/servlet/AdminPrintControl?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=PickingListPrint&p6=" + code2 + "&p5=GoodsReceivedPickingList"
                             + generalUtils.sanitiseReplacingNewlines("?p1=" + code + "&p2=A&p3=&p4=&") + "&p2=" + code2 + "&p3=\";}");
      }
    }
    
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4183, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function fax(){");
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/FaxCreate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + code2 + "&p2=P&p3=PickingListPrint&p4=&p5=Picking%20List\";}");
    }
  
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 11200, unm, uty, dnm, localDefnsDir, defnsDir) && authenticationUtils.verifyAccess(con, stmt, rs, req, 3048, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function pdf(){");
      scoutln(b, bytesOut, "window.location.href=\"/central/servlet/PrintToPDFUtils?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=" + code2 + "&p1=0.000&p5=PickingList&p2=PickingListPrint\";}");
    }

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4057, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function dorder(){var p1='CREATEBASEDONPL:'+'" + code + "';");
    ...
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4068, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function invoicec(){var p1='CREATEBASEDONPLC:'+'" + code + "';");
      scoutln(b, bytesOut, "this.location.href=\"/central/servlet/SalesInvoicePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p3=&p4=&p1=\"+p1;}");
    }

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 4069, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      scoutln(b, bytesOut, "function invoice(){");
      scoutln(b, bytesOut, "var p1='CREATEBASEDONPL:'+'" + code + "';");
      scoutln(b, bytesOut, "this.location.href=\"/central/servlet/SalesInvoicePage?unm=" + unm + "&sid=" + sid + "&uty=" + uty
    ...
    outputPageFrame(con, stmt, rs, b, req, ' ', ' ', "GoodsReceivedPickingList", date, unm, sid, uty, men, den, dnm, bnm, completed, localDefnsDir, defnsDir, bytesOut);
                             ...
                             byte[] b, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    scoutln(b, bytesOut, "var saveStr2=strip(saveStr);window.location.replace(\"/central/servlet/PickingListHeaderUpdate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1="
    ...
    outputPageFrame(con, stmt, rs, b, req, ' ', cad.charAt(0), "PickingListEdit", date, unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
                      ...
                      throws Exception
  {
    ...
    if(searchDataString(recData, recDataLen, "pll", b, lineBytes) == -1)
      return 'N';
    ...
    sortFields(recData, buf, "pll"); // sorts the data field (using buf in the process); results are put back into recData
    recDataLen = generalUtils.lengthBytes(recData, 0);
    generalUtils.zeroize(buf, 2000);
        
    String fieldNames = getFieldNamesPLL();
    byte[] value      = new byte[3000]; // plenty - desc
    ...
    while(x < len)
    {
      ...
      if(searchDataString(recData, recDataLen, "pll", fieldName, value) != -1) // entry exists
      {
        if(fieldCount == 1) // itemCode
          itemCodeIn = generalUtils.stringFromBytes(value, 0L);
        else
        if(fieldCount == 32) // mfr
    ...
    generalUtils.repAlpha(buf, 2000, (short)32, mfrOut[0]);
    generalUtils.repAlpha(buf, 2000, (short)33, mfrCodeOut[0]);

    generalUtils.repAlpha(buf, 2000, (short)0,  code);
    generalUtils.repAlpha(buf, 2000, (short)18, lineBytes);
   
    if((generalUtils.dfsAsStr(buf, (short)4)).length() == 0) // quantity 
      generalUtils.putAlpha(buf, 2000, (short)4, "0");

    if((generalUtils.dfsAsStr(buf, (short)21)).length() == 0) // grossweight 
    ...
    if((generalUtils.dfsAsStr(buf, (short)23)).length() == 0) // qtypacked 
      generalUtils.putAlpha(buf, 2000, (short)23, "0");

    if((generalUtils.dfsAsStr(buf, (short)27)).length() == 0) // netweight 
      generalUtils.putAlpha(buf, 2000, (short)27, "0");

    if((generalUtils.dfsAsStr(buf, (short)28)).length() == 0) // numcartons 
      generalUtils.putAlpha(buf, 2000, (short)28, "0");

    if((generalUtils.dfsAsStr(buf, (short)31)).length() == 0) // costPrice 
      generalUtils.putAlpha(buf, 2000, (short)31, "0");

    if((generalUtils.dfsAsStr(buf, (short)30)).length() == 0) // soLine
      generalUtils.putAlpha(buf, 2000, (short)30, "0");

    if((generalUtils.dfsAsStr(buf, (short)14)).length() == 0) // Location
      generalUtils.putAlpha(buf, 2000, (short)14, "Unknown");

    generalUtils.repAlpha(buf, 2000, (short)9, unm);

    byte[] rateB = new byte[30];
    getAPLFieldGivenCode(con, stmt, rs, "Rate", code, rateB);
    ...
    if(   searchDataString(recData, recDataLen, "pll", fieldName, value) != -1 // exists
       && value[0] != '\000')
    {
      ...
      if(value[0] == '\000') // description fld is blank
      {
        ...
        if(inventory.getStockRecGivenCode(con, stmt, rs, itemCode, '\000', data) != -1) // exists
        {
          ...
          if(! unitPriceGiven)
          {
            ...
            getAPLFieldGivenCode(con, stmt, rs, "Currency", code, currency); 
          ...
          generalUtils.repAlpha(buf, 2000, (short)12, b); // origin-0
        }

        generalUtils.repDoubleGivenSeparator('8', '\000', buf, 2000, (short)3, nonBaseSellPrice);
      }
      else // strip only the first desc line for plL rec
      {
  ...
  private void updateDLM(Connection con, Statement stmt, byte[] code, String unm) throws Exception
  {
    ...
    stmt.executeUpdate("UPDATE pl SET SignOn = '" + unm + "', DateLastModified = NULL WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
    if(stmt != null) stmt.close();
  }
  
  // -------------------------------------------------------------------------------------------------------------------------------
  public void reCalculate(byte[] buf, int dataBufLen, double rate) throws Exception
  {
    String s = generalUtils.dfsAsStr(buf, (short)23); // qtyPacked
    ...
    s = generalUtils.dfsAsStr(buf, (short)3); // unitPrice
    ...
    s = generalUtils.dfsAsStr(buf, (short)7); // discount
    ...
    generalUtils.repDoubleGivenSeparator('2', '\000', buf, dataBufLen, (short)10, amt); // amount2

    amt = generalUtils.doubleDPs(amt, '2'); // added 5jul01
    generalUtils.repDoubleGivenSeparator('2', '\000', buf, dataBufLen, (short)5, (amt * rate)); // amount
  }
  ...
  private void updateMultipleLines(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] line, byte[] descData) throws Exception
  {
    try
    {
      ...
      rs = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM plll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) 
      ...
      while(lineCount <= numDescLines && lineCount <= numLinesAtPresent) // overwrite those lines which exist
      {
        ...
        stmt.executeUpdate("UPDATE plll SET Text = '" + sanitisedDesc + "' WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L)
      ...
      while(lineCount <= numDescLines) // add those lines which do not exist
      {
        ...
        stmt.executeUpdate("INSERT INTO plll (" + getFieldNamesPLLL() + ") VALUES ('" + generalUtils.stringFromBytes(code, 0L) + "','"
      ...
      {
        stmt.executeUpdate("DELETE FROM plll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Entry = '"
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getAPLFieldGivenCode(Connection con, Statement stmt, ResultSet rs, String fieldName, String plCode) throws Exception
  {
    byte[] value   = new byte[300];
    byte[] plCodeB = new byte[21];
    generalUtils.strToBytes(plCodeB, plCode);
    
    getAPLFieldGivenCode(con, stmt, rs, fieldName, plCodeB, value);
    
    return generalUtils.stringFromBytes(value, 0L);
  }
  public void getAPLFieldGivenCode(Connection con, Statement stmt, ResultSet rs, String fieldName, byte[] plCode, byte[] value) throws Exception
  {
    if(plCode[0] == '\000') // just-in-case
      return;

    generalUtils.toUpper(plCode, 0);

    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT " + fieldName + " FROM pl WHERE PLCode = '" + generalUtils.stringFromBytes(plCode, 0L) + "'");
    if(! rs.next())
      value[0] = '\000';
    else
    {    
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  public boolean buildHTMLLayoutForLine(Connection con, Statement stmt, ResultSet rs, PrintWriter out, byte[] code, byte[] line, String unm, String sid,
                                        String uty, String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, String errStr,
                                        byte[] dataAlready, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    String layoutFile="202b.htm";

    short[] fieldSizesPLL = new short[40]; // plenty
    getFieldSizesPLL(fieldSizesPLL);

    numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, layoutFile, 1, getFieldNamesPLL(),
                                     fieldSizesPLL, null, null, null, null);
    ...
    if(line[0] != '\000') // existing line
    {
      if(code[0] != '\000') // just-in-case
      {
        if(dataAlready[0] != '\000') // coming with an err msg - passing-in data already entered but the data's fields may (will) not
        {                            // be in the correct order
          sortFields(dataAlready, data, "pll");
        }
        else
        {
          ...
          generalUtils.bytesDPsGivenSeparator(true, '\001', '2',  data, 2000, 31); // costPrice

          generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnQuantities, data, 2000, 16); // qty reqd
          generalUtils.bytesDPsGivenSeparator(true, '\001', dpOnQuantities, data, 2000, 23); // qty packed
          generalUtils.bytesDPsGivenSeparator(true, '\001', '0', data, 2000, 21); // grossweight
          generalUtils.bytesDPsGivenSeparator(true, '\001', '0', data, 2000, 27); // weight per

          try
          {
            generalUtils.dfsGivenSeparator(true, '\001', data, (short)6, b);
            if(b[0] == '\000')
              generalUtils.repAlphaGivenSeparator(' ', data, 2000, (short)6, "<none>");
          }
          catch(Exception e) { }
    ...
    else // new line
    {
      ...
      {
        data[0]= '\000';
        scoutln(data, bytesOut, "pll.entry=" + generalUtils.intToStr(nextLine) + "\001");

        String gstRateDefault = accountsUtils.getDefaultGSTRate(con, stmt, rs, dnm, localDefnsDir, defnsDir);  
        scoutln(data, bytesOut, "pll.gstrate=" + gstRateDefault + "\001");

        String storeDefault = documentUtils.getDefaultStore(con, stmt, rs, dnm, localDefnsDir, defnsDir);
        scoutln(data, bytesOut, "pll.store=" + storeDefault + "\001");
      }

      newOrEdit = 'N';
    }

    ddlData = documentUtils.getStoreDDLData(con, stmt, rs, "pll.store",      ddlData, ddlDataUpto, ddlDataLen);
    ddlData = accountsUtils.getGSTRatesDDLData(con, stmt, rs, "pll.gstrate", dnm, ddlData, ddlDataUpto, ddlDataLen, localDefnsDir, defnsDir);

    ddlData = inventory.getMfrsDDLData(con, stmt, rs, "pll.manufacturer", ddlData, ddlDataUpto, ddlDataLen);

    getAPLFieldGivenCode(con, stmt, rs, "Currency", code, b);
    scoutln(data, bytesOut, "pll.currency=" + generalUtils.stringFromBytes(b, 0L) + "\001");

    scoutln(data, bytesOut, "pll.basecurrency=" + accountsUtils.getBaseCurrency(con, stmt, rs, dnm, localDefnsDir, defnsDir) + "\001");

    // add in the weight unit
    scoutln(data, bytesOut, "pl.unitofweight=" + miscDefinitions.unitOfWeight(con, stmt, rs) + "\001");
    ...
    byte[] prependCode = new byte[25000];
    prependEditLine(con, stmt, rs, unm, sid, uty, men, den, dnm, bnm, errStr, cad, generalUtils.stringFromBytes(code, 0L), generalUtils.stringFromBytes(line, 0L), "202b.htm",
                               ...
                               String defnsDir, byte[] b, char newOrEdit, HttpServletRequest req, int[] bytesOut) throws Exception
  {
    ...
    scoutln(b, bytesOut, "function save(thisOp){var saveStr=\"\";" + saveStr + "\n");
    scoutln(b, bytesOut, "var saveStr2=strip(saveStr);window.location.replace(\"/central/servlet/PickingListLineUpdate?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1="
    ...
    outputPageFrame(con, stmt, rs, b, req, newOrEdit, ' ', "PickingListLineFetch", "", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private int getNextLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] data, String dnm, String localDefnsDir, String defnsDir)
                          throws Exception
  {
  ...
  public char putHead(Connection con, Statement stmt, ResultSet rs, byte[] originalCode, String unm, String dnm, String localDefnsDir, String defnsDir, char cad, byte[] recData, int recDataLen, byte[] code, int[] bytesOut) throws Exception
  {
    ...
    generalUtils.catAsBytes("PLCode", 0, str, true);

    if(searchDataString(recData, recDataLen, "pl", str, code) == 0)
      return 'N';
    ...
    if(searchDataString(recData, recDataLen, "pl", fldName, value) != -1) // entry exists
    {
      if(value[0] == '\000' || generalUtils.doubleFromBytesCharFormat(value, 0) == 0.0)
      {
        ...
        searchDataString(recData, recDataLen, "pl", fldName, currency); // assumes exists

        generalUtils.strToBytes(fldName, "Date");
        searchDataString(recData, recDataLen, "pl", fldName, date); // assumes exists
    ...
    String fieldNamesPL = getFieldNamesPL();
    ...
    {
      getFieldName(fieldNamesPL, x, fldName);
      if(searchDataString(recData, recDataLen, "pl", fldName, value) != -1) // entry exists
      {
        if(value[0] != '\000')
    ...
    byte[] purchasedBy = new byte[50];
    len = fieldNamesPL.length();
    int x=0, y, fieldCount=0;
    while(x < len)
    {
      y=0;
      while(x < len && fieldNamesPL.charAt(x) != ',')
        fldName[y++] = (byte)fieldNamesPL.charAt(x++);
      fldName[y] = '\000';
      ++x;
      ...
      {  
        ...
        if(fieldCount == 10) // fao
        {
          // if PurchasedBy fld is blank then default to same as this FAO fld
          if(value[0] != '\000') // FAO itself is not blank
          {
            byte[] b = new byte[50];
            generalUtils.catAsBytes("PurchasedBy", 0, b, true);
            if(searchDataString(recData, recDataLen, "picklist", b, b) != -1) // entry exists
            {
              if(b[0] == '\000') // PurchasedBy does not already have a value
                generalUtils.bytesToBytes(purchasedBy, value, 0);
              else generalUtils.bytesToBytes(purchasedBy, b, 0);
            }
          }
          generalUtils.putAlpha(buf, 2000, (short)10, value);
        }
        else
        if(fieldCount == 13) // dateOfStockUpdate
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)13, "1970-01-01");
          else generalUtils.putAlpha(buf, 2000, (short)13, generalUtils.convertDateToSQLFormat(generalUtils.stringFromBytes(value, 0L)));  
        }
        else
        if(fieldCount == 27) // purchasedby
        {
          generalUtils.putAlpha(buf, 2000, (short)27, purchasedBy);
        }
        else
        if(fieldCount == 44) // terms
        {
          if(value[0] == '\000')
          {
            String days;
            if((days = customer.getACompanyFieldGivenCode(con, stmt, rs, "CreditDays", companyCode)).length() == 0) // unknown cust
              days = "30";            
            generalUtils.repAlpha(buf, 2000, (short)44, days + " days");
          }
          else generalUtils.putAlpha(buf, 2000, (short)44, value);
        }  
        else
        if(fieldCount == 40) // rate
        {
          if(rate == 0.0) rate = 1.0;

          generalUtils.repDoubleGivenSeparator('8', '\000', buf, 2000, (short)40, rate);
        }
        else
        if(fieldCount == 43) // expectedDate
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)43, "1970-01-01");
          else generalUtils.putAlpha(buf, 2000, (short)43, generalUtils.convertDateToSQLFormat(generalUtils.stringFromBytes(value, 0L)));
        }
        else
        if(fieldCount == 57) // dateTimeAssigned
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)57, "1970-01-01 12:15:00");
          else generalUtils.putAlpha(buf, 2000, (short)57, generalUtils.convertToTimestamp(generalUtils.stringFromBytes(value, 0L)));
        }
        else
        if(fieldCount == 59) // dateTimeReturned
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)59, "1970-01-01 12:15:00");
          else generalUtils.putAlpha(buf, 2000, (short)59, generalUtils.convertToTimestamp(generalUtils.stringFromBytes(value, 0L)));
        }
        else
        if(fieldCount == 63) // dateTimeLastPrinted
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)63, "1970-01-01 12:15:00");
          else generalUtils.putAlpha(buf, 2000, (short)63, generalUtils.convertToTimestamp(generalUtils.stringFromBytes(value, 0L)));
        }
        else
        if(fieldCount == 64) // dateTimeQueued
        {
          if(value[0] == '\000')
            generalUtils.putAlpha(buf, 2000, (short)64, "1970-01-01 12:15:00");
          else generalUtils.putAlpha(buf, 2000, (short)64, generalUtils.convertToTimestamp(generalUtils.stringFromBytes(value, 0L)));
        }
    ...
    if((generalUtils.dfsAsStr(buf, (short)45)).length() == 0) // baseTotalTotal 
      generalUtils.putAlpha(buf, 2000, (short)45, "0");
    
    if((generalUtils.dfsAsStr(buf, (short)46)).length() == 0) // BaseGSTTotal 
      generalUtils.putAlpha(buf, 2000, (short)46, "0");
    
    if((generalUtils.dfsAsStr(buf, (short)21)).length() == 0) // GroupDiscount 
      generalUtils.putAlpha(buf, 2000, (short)21, "0");

    if((generalUtils.dfsAsStr(buf, (short)13)).length() == 0) // dateofstockupdate 
      generalUtils.repAlpha(buf, 2000, (short)13, "1970-01-01"); 
    
    if((generalUtils.dfsAsStr(buf, (short)43)).length() == 0) // expecteddate
      generalUtils.repAlpha(buf, 2000, (short)43, "1970-01-01");

    if((generalUtils.dfsAsStr(buf, (short)57)).length() == 0) // dateTimeAssigned
      generalUtils.repAlpha(buf, 2000, (short)57, "1970-01-01 12:15:00");

    if((generalUtils.dfsAsStr(buf, (short)59)).length() == 0) // dateTimeReturned
      generalUtils.repAlpha(buf, 2000, (short)59, "1970-01-01 12:15:00");

    if((generalUtils.dfsAsStr(buf, (short)62)).length() == 0) // timesPrinted
      generalUtils.putAlpha(buf, 2000, (short)62, "0");

    if((generalUtils.dfsAsStr(buf, (short)63)).length() == 0) // dateTimeLastPrinted
      generalUtils.repAlpha(buf, 2000, (short)63, "1970-01-01 12:15:00");

    if((generalUtils.dfsAsStr(buf, (short)64)).length() == 0) // dateTimeQueued
      generalUtils.repAlpha(buf, 2000, (short)64, "1970-01-01 12:15:00");
    ...
    if(cad == 'A')
    {
      ...
      {
        if(rate != generalUtils.dfsAsDouble(data, (short)40)) // rate changed
          rateChanged = true;
      }
    }
      ...
      if(rateChanged)
      {
        ...
        for(x=0;x<linesCount[0];++x)
        {
          if(generalUtils.getListEntryByNum(x, linesData, line)) // just-in-case
          {
            ...
            generalUtils.repAlpha(buf, 2000, (short)9, unm); // origin-0

            generalUtils.dfs(buf, (short)18, str); // line - origin-0

            putRecLine(con, stmt, rs, code, str, 'E', buf, dnm, localDefnsDir, defnsDir);
          }
        }
      }

      if(newOrEdit == 'N')
        serverUtils.syncToIW(con, generalUtils.stringFromBytes(code, 0L), "", "", 'P', generalUtils.todaySQLFormat(localDefnsDir, defnsDir), unm, companyCode, dnm);
      else serverUtils.updateNewWavechannelsRec(con, stmt, rs, generalUtils.stringFromBytes(code, 0L), "P", companyCode);
    }
                              ...
                              int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT * FROM pl WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
    if(! rs.next())
    {
      rs.close();
      stmt.close();
    ...
    String fieldTypes = getFieldTypesPL();
    ...
    {    
      String fieldNames = getFieldNamesPL();
      ...
      while(x < len)
      {
        ...
        scoutln(data, bytesOut, "pl." + thisFieldName + "=" + getValue(count, thisFieldType, rs, rsmd) + "\001");
  ...
  public char putRecHeadGivenCode(Connection con, Statement stmt, ResultSet rs, byte[] code, char newOrEdit, byte[] data, String dnm, String localDefnsDir, String defnsDir)
  {
    try
    {
      ...
      if(newOrEdit == 'N')
      {
        ...
        q = "INSERT INTO pl (" + getFieldNamesPL() + ") VALUES (" + opStr + ")";
      }
      else // newOrEdit == 'E'
      {
        String thisFieldName;
        ...
        q = "UPDATE pl SET " + opStr + " WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'";
      }
                         ...
                         String defnsDir)
  {
    ...
    try
    {
      ...
      String opStr="";
      boolean first = true;
      String fieldTypes = getFieldTypesPLL();
      int x=0, count=0;
       ...
       if(newOrEdit == 'N')
      {
        ...
        q = "INSERT INTO pll (" + getFieldNamesPLL() + ") VALUES (" + opStr + ")";
      }
      else // newOrEdit == 'E'
      {
        String thisFieldName;
        ...
        q = "UPDATE pll SET " + opStr + " WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '" 
  ...
  public boolean deleteLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] line, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut)
  {
    try
    {
      ...
      stmt.executeUpdate("DELETE FROM pll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '"
                         + generalUtils.stringFromBytes(line, 0L) + "'");
                         ...
                         String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT * FROM pll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
    while(rs.next())
    {
      if(separator == '\000')
      ...
      else // separator == '\001'
      {
        ...
        while(x < lenFieldNames)
        {
          ...
          while(x < lenFieldNames && fieldNames.charAt(x) != ',')
            thisFieldName += fieldNames.charAt(x++);
          thisFieldType = fieldTypes.charAt(count++);
          s += "pll." + thisFieldName + "=" + getValue(count, thisFieldType, rs, rsmd) + "\001";
          ++x;
          while(x < lenFieldNames && fieldNames.charAt(x) == ' ')
            ++x;
        }
  ...
  private int getLine(Connection con, Statement stmt, ResultSet rs, byte[] code, byte[] line, char separator, byte[] data, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT * FROM pll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Line = '" + generalUtils.stringFromBytes(line, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
      ...
      else // separator == '\001'
      {
        ...
        while(x < len)
        {
          ...
          while(x < len && fieldNames.charAt(x) != ',')
            thisFieldName += fieldNames.charAt(x++);
          thisFieldType = fieldTypes.charAt(count++);
          scoutln(data, bytesOut, "pll." + thisFieldName + "=" + getValue(count, thisFieldType, rs, rsmd) + "\001");
          ++x;
          while(x < len && fieldNames.charAt(x) == ' ')
            ++x;
        }
                                   ...
                                   String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT * FROM plll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
    while(rs.next())
    {
                                  ...
                                  String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT Text FROM plll WHERE PLCode = '" + generalUtils.stringFromBytes(code, 0L) + "' AND Entry = '" + generalUtils.stringFromBytes(line, 0L) + "'");
    ResultSetMetaData rsmd = rs.getMetaData();
    while(rs.next())
    {    
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  public void updatePLField(Connection con, Statement stmt, ResultSet rs, byte[] code, String fieldName, String fieldValue, String dnm, String localDefnsDir,
                              String defnsDir) throws Exception
  {
    stmt = con.createStatement();
    stmt.executeUpdate("UPDATE pl SET " + fieldName + " = '" + fieldValue + "', DateLastModified = NULL WHERE PLCode = '"
                       + generalUtils.stringFromBytes(code, 0L) + "'");
    if(stmt != null) stmt.close();
  }

  // -------------------------------------------------------------------------------------------------------------------------------
  private void prependDisplayOnly(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, String code, String date, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String localDefnsDir,
                                  String defnsDir, byte[] b, int[] bytesOut) throws Exception
  {
    ...
    outputPageFrame(con, stmt, rs, b, req, ' ',  ' ', "ProformaInvoiceRegisteredUser", date, unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  }
  ...
  private void sortFields(byte[] ipBuf, byte[] opBuf, String which) throws Exception
  {
    ...
    if(which.equals("pl"))
      fieldNames = getFieldNamesPL();
    else // if(which.equals("pll"))
      fieldNames = getFieldNamesPLL();
    int len = fieldNames.length();
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public void setQuantities(Connection con, Statement stmt, ResultSet rs, byte[] code, String unm, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    updateDLM(con, stmt, code, unm);
    ...
    for(int x=0;x<linesCount[0];++x)
    {
      if(generalUtils.getListEntryByNum(x, linesData, line)) // just-in-case
      {
        ...
        if(generalUtils.dfsAsDouble(buf, (short)23) == 0) // qtyPacked
        {
          qtyReqd = generalUtils.dfsAsDouble(buf, (short)16);
          generalUtils.repAlpha(buf, 2000, (short)23, qtyReqd); // copy qtyReqd to qtyPacked
          generalUtils.repAlpha(buf, 2000, (short)9, unm);
          generalUtils.repAlpha(buf, 2000, (short)8, "");
                                           ...
                                           String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 6, unm, sid, uty, men, den, dnm, bnm, "PickingList", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      return false;
    }
    ...
    numFields = screenLayout.layoutToBuffer(buf1, buf2, size1, fieldNames, fieldTypes, fieldSizes, localDefnsDir, defnsDir, "202d.htm",
                                     2, getFieldNamesPL(), fieldSizesPL, getFieldNamesPLL(), fieldSizesPLL, null, null);
    ...
    int x=0, y, fieldCount=0;
    int len = fieldNamesPL.length();
    while(x < len)
    {
      y=0;
      while(x < len && fieldNamesPL.charAt(x) != ',')
        fieldName[y++] = (byte)fieldNamesPL.charAt(x++);
      fieldName[y] = '\000';
      ++x;
      ...
      if(searchDataString(data, recDataLen, "pl", fieldName, value) != -1) // entry exists
      {
    ...
    generalUtils.dfsGivenSeparator(true, '\001', data, (short)22, b);
    ...
    byte[] javaScriptCallCode = new byte[1000]; // lock
    javaScriptCall(con, stmt, rs, javaScriptCallCode, req, generalUtils.stringFromBytes(date, 0L), unm, uty, dnm, "N", localDefnsDir, defnsDir, bytesOut);

    String attachments = getAttachments(con, stmt, stmt2, rs, rs2, generalUtils.stringFromBytes(code, 0L), unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir);
    if(attachments.length() == 0)
      attachments= "none";
    scoutln(data, bytesOut, "pl.Attachments=" + attachments + "\001");
                               ...
                               String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT LibraryDocCode FROM pla WHERE Code = '" + code + "'");
      while(rs.next())           
      {    
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private void outputPageFrame(Connection con, Statement stmt, ResultSet rs, byte[] b, HttpServletRequest req, char newOrEdit, char cad, String callingServlet, String date, String unm, String sid, String uty, String men, String den, String dnm,
                               String bnm, String completed, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    int[] hmenuCount = new int[1];

    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "3038", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    subMenuText += "<table id='title' width=100%><tr><td colspan=2 nowrap><p>Picking List" + directoryUtils.buildHelp(3038) + "</td></tr></table>";

    subMenuText += buildSubMenuText(con, stmt, rs, req, cad, newOrEdit, callingServlet, completed, date, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    scoutln(b, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, callingServlet, subMenuText, hmenuCount[0], "", "", localDefnsDir, defnsDir));
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String buildSubMenuText(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, char cad, char newOrEdit, String callingServlet, String completed, String date, String unm, String sid, String uty, String men, String den,
                                  String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount) throws Exception
  {
    ...
    if(callingServlet.equals("GoodsReceivedPickingList"))
      s += drawOptions3038(con, stmt, rs, req, hmenuCount, date, unm, uty, dnm, completed, localDefnsDir, defnsDir);
    else
    if(callingServlet.equals("PickingListEdit"))
      s += drawOptions3043(hmenuCount, cad, unm, sid, uty, men, den, dnm, bnm);
    else 
    if(callingServlet.equals("PickingListLineFetch"))
      s += drawOptions3045(hmenuCount, newOrEdit, unm, sid, uty, men, den, dnm, bnm);
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions3038(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, int[] hmenuCount, String date, String unm, String uty, String dnm, String completed, String localDefnsDir, String defnsDir) throws Exception
  {
    boolean _3060 = authenticationUtils.verifyAccess(con, stmt, rs, req, 3060, unm, uty, dnm, localDefnsDir, defnsDir);

    String s = "";
    
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3043, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      if(! completed.equals("Y") || (completed.equals("Y") && _3060) && serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
        s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0]++ + "');\"><a href=\"javascript:fetch()\">Edit Details</a></dt></dl>\n";
    }
    
    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 3045, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
    {
      if(! completed.equals("Y") || (completed.equals("Y") && _3060))
        s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0]++ + "');\"><a href=\"javascript:add()\">Add New Line</a></dt></dl>\n";
    }    

    if(authenticationUtils.verifyAccess(con, stmt, rs, req, 6094, unm, uty, dnm, localDefnsDir, defnsDir) && serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
      s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0]++ + "');\"><a href=\"javascript:attach()\">Attachments</a></dt></dl>\n";

    boolean PickingListPrint  = authenticationUtils.verifyAccess(con, stmt, rs, req, 3048, unm, uty, dnm, localDefnsDir, defnsDir);
    boolean _3049  = authenticationUtils.verifyAccess(con, stmt, rs, req, 3049, unm, uty, dnm, localDefnsDir, defnsDir);
    boolean _4183  = authenticationUtils.verifyAccess(con, stmt, rs, req, 4183, unm, uty, dnm, localDefnsDir, defnsDir);
    ...
    if(PickingListPrint || _3049 || _4183 || MailExternalUserCreate)
    {
      ...
      {
        if(_3049)
          s += "<li><a href=\"javascript:queue()\">Queue</a></li>\n";
        else s += "<li><a href=\"javascript:print()\">Print</a></li>\n";
      }
    ...
    if(serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
    {
    ...
    if(serverUtils.passLockCheck(con, stmt, rs, "pl", date, unm))
    {
      boolean PickingListSetQuantity = authenticationUtils.verifyAccess(con, stmt, rs, req, 3054, unm, uty, dnm, localDefnsDir, defnsDir);
      boolean _3056 = authenticationUtils.verifyAccess(con, stmt, rs, req, 3056, unm, uty, dnm, localDefnsDir, defnsDir);
      if(PickingListSetQuantity || _3056)
      {
        s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0] + "');\">Update</dt><dd id='hmenu" + hmenuCount[0]++ + "'><ul>";

        if(PickingListSetQuantity)
        {
          if(! completed.equals("Y") || (completed.equals("Y") && _3060))
            s += "<li><a href=\"javascript:updateQ('Q')\">Set Quantities</a></li>";
        }
      
        if(_3056)
          s += "<li><a href=\"javascript:updateC('Y')\">Set Completed</a></li><li><a href=\"javascript:updateC('N')\">Set Not Completed</a></li>";

        s += "</ul></dd></dl>\n";
      }
    }
    
    boolean PickingListToDeliveryOrder = authenticationUtils.verifyAccess(con, stmt, rs, req, 4057, unm, uty, dnm, localDefnsDir, defnsDir);
    boolean PickingListToSalesInvoiceCash = authenticationUtils.verifyAccess(con, stmt, rs, req, 4068, unm, uty, dnm, localDefnsDir, defnsDir);
    boolean PickingListToSalesInvoiceTerms = authenticationUtils.verifyAccess(con, stmt, rs, req, 4069, unm, uty, dnm, localDefnsDir, defnsDir);
    if(PickingListToDeliveryOrder || PickingListToSalesInvoiceCash || PickingListToSalesInvoiceTerms)
    {
      s += "<dl><dt onmouseover=\"setup('hmenu" + hmenuCount[0] + "');\">Convert</dt><dd id='hmenu" + hmenuCount[0]++ + "'><ul>";
      if(PickingListToSalesInvoiceCash)
        s += "<li><a href=\"javascript:invoicec()\">Cash Invoice</a></li>\n";
      if(PickingListToSalesInvoiceTerms)
        s += "<li><a href=\"javascript:invoice()\">Terms Invoice</a></li>\n";
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions3043(int[] hmenuCount, char cad, String unm, String sid, String uty, String men, String mainDNM, String dnm, String bnm) throws Exception
  {      
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  private String drawOptions3045(int[] hmenuCount, char newOrEdit, String unm, String sid, String uty, String men, String mainDNM, String dnm, String bnm) throws Exception
  {      
  ...
  public void putMultipleLine(Connection con, Statement stmt, ResultSet rs, byte[] code, int line, byte[] descData, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    try
    {
      ...
      stmt.executeUpdate("INSERT INTO plll (" + getFieldNamesPLLL() + ") VALUES ('" + generalUtils.stringFromBytes(code, 0L) + "','" + line
  ...
  // --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public double getTotalPickedForASOLine(Connection con, Statement stmt, ResultSet rs, String soCode, String soLine) throws Exception
  {
    String[] plCodes = new String[1];  plCodes[0] = "";
    return getTotalPickedForASOLine(con, stmt, rs, soCode, soLine, plCodes);
  }
  public double getTotalPickedForASOLine(Connection con, Statement stmt, ResultSet rs, String soCode, String soLine, String[] plCodes) throws Exception
  {
    stmt = con.createStatement();

    rs = stmt.executeQuery("SELECT t2.PLCode, t2.QuantityPacked FROM pll AS t2 INNER JOIN pl AS t1 ON t2.PLCode = t1.PLCode WHERE t2.SOCode = '" + soCode + "' AND t2.SOLine = '" + soLine + "' AND t1.Status != 'C' AND t1.Completed = 'Y'");
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  public String getEarliestPLDateForAnSOCode(Connection con, Statement stmt, ResultSet rs, String soCode, String itemCode) throws Exception
  {
    String earliestPLDate = "";
    ...
    try
    {
      ...
      rs = stmt.executeQuery("SELECT t1.Date FROM pll AS t2 INNER JOIN pl AS t1 ON t1.PLCode = t2.PLCode WHERE t2.SOCode = '" + generalUtils.sanitiseForSQL(soCode) + "' AND t2.ItemCode = '" + generalUtils.sanitiseForSQL(itemCode) + "' ORDER BY t1.Date");
      if(rs.next())
        earliestPLDate = rs.getString(1);
    }
    catch(Exception e) { }
