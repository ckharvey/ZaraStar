// =======================================================================================================================================================================================================
// System: ZaraStar ProductEngine: Stock Adjustment listing page
// Module: StockAdjustmentListing.java
// Author: C.K.Harvey
...
public class StockAdjustmentListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "StockAdjustmentListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3013, bytesOut[0], 0, "ERR:" + codeArg);
                    ...
                    int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 3013, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "StockAdjustmentListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3013, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "StockAdjustmentListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3013, bytesOut[0], 0, "SID:" + codeArg);
    ...
    generate(con, stmt, rs, out, req, unm, sid, uty, men, den, dnm, bnm, inx.charAt(0), operation.charAt(0), callType.charAt(0), codeArg,
             codeArg2, srchStr, firstRecNum, lastRecNum, maxRows, numRecs, imagesDir, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3013, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                        ...
                        String imagesDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    createPage(con, stmt, rs, html, operation2, callType, codeArg, codeArg2, inx, srchStr, numRecs, firstCodeOnPage, lastCodeOnPage, 
               firstEntryOnPage, lastEntryOnPage, firstRecNum, lastRecNum, newFirstRecNum, newLastRecNum, maxRows, imagesDir);
    ...
    setHead(con, stmt, rs, out, req, inx, unm, sid, uty, men, den, dnm, bnm, newFirstRecNum[0], newLastRecNum[0], numRecs[0], imagesDir,
  ...
  private void createPage(Connection con, Statement stmt, ResultSet rs, String[] html, char operation, char callType, String arg, String arg2, char inx,
                          ...
                          String firstRecNum, String lastRecNum, String[] newFirstRecNum,  String[] newLastRecNum, int maxRows, String imagesDir)
                          throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '1' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM stocka WHERE ItemCode = '" + srchStr + "'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT ItemCode) AS rowcount FROM stocka");
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM stocka WHERE ItemCode = '" + codeArg + "'");
                   }
                   break;
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM stocka WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM stocka");
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM stocka WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}");
                   }
                   break;
        default  : // case '3'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT AdjustmentCode) AS rowcount FROM stocka WHERE AdjustmentCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM stocka");
                   break;
      }
      r.next();
    ...
    String adjustmentCode="", itemCode="", storeFrom="", storeTo="", date="", qty="", soCode="", poCode="", remark="", status;
    int adjustmentCodePosn, datePosn, itemCodePosn, storeFromPosn, storeToPosn, statusPosn, soCodePosn, poCodePosn, remarkPosn, qtyPosn;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '3')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE "
                       + "AdjustmentCode LIKE '" + srchStr + "%' ORDER BY AdjustmentCode";
                   }
                   else
                   {
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka ORDER BY AdjustmentCode";
                   }

                   adjustmentCodePosn = 1; datePosn = 3; itemCodePosn = 2; storeFromPosn = 4; storeToPosn = 5; qtyPosn = 6; soCodePosn = 7; poCodePosn = 8;
                   remarkPosn = 9; statusPosn = 10;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, AdjustmentCode, ItemCode, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'} ORDER BY Date";
                     
                     adjustmentCodePosn = 2; datePosn = 1; itemCodePosn = 3; storeFromPosn = 4; storeToPosn = 5; qtyPosn = 6; soCodePosn = 7; poCodePosn = 8;
                     remarkPosn = 9; statusPosn = 10;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM stocka ORDER BY Date";
                     
                     adjustmentCodePosn = -1; datePosn = 1; itemCodePosn = -1; storeFromPosn = -1; storeToPosn = -1; qtyPosn = -1; soCodePosn = -1;
                     poCodePosn = -1; remarkPosn = -1; statusPosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, AdjustmentCode, ItemCode, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} ORDER BY Date";
                 ...
                 else // inx == '1'
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT ItemCode, AdjustmentCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE ItemCode = '"
                       + srchStr + "' ORDER BY ItemCode";
                     
                     adjustmentCodePosn = 2; datePosn = 3; itemCodePosn = 1; storeFromPosn = 4; storeToPosn = 5; qtyPosn = 6; soCodePosn = 7; poCodePosn = 8;
                     remarkPosn = 9; statusPosn = 10;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ItemCode FROM stocka ORDER BY ItemCode";
                 ...
                 while(rs.next())                  
                 {
                   adjustmentCode = getValue(adjustmentCodePosn, ' ', rs, rsmd);
                   date           = getValue(datePosn,           'D', rs, rsmd);
                   itemCode       = getValue(itemCodePosn,       ' ', rs, rsmd);
                   storeFrom      = getValue(storeFromPosn,      ' ', rs, rsmd);
                   storeTo        = getValue(storeToPosn,        ' ', rs, rsmd);
                   qty            = getValue(qtyPosn,            ' ', rs, rsmd);
                   status         = getValue(statusPosn,         ' ', rs, rsmd);
                   soCode         = getValue(soCodePosn,         ' ', rs, rsmd);
                   poCode         = getValue(poCodePosn,         ' ', rs, rsmd);
                   remark         = getValue(remarkPosn,         ' ', rs, rsmd);

                   if(inx == '3')
                     appendBodyLine(html, inx, adjustmentCode, itemCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir,
                                    cssFormat);
                   else
                   if(inx == '2')
                   {
                     ...
                     else appendBodyLine(html, inx, date, adjustmentCode, itemCode, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status,
                                         imagesDir, cssFormat);
                   }   
                   else // inx == '1'
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, itemCode, cssFormat);
                     else appendBodyLine(html, inx, itemCode, adjustmentCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   }   
          
                   if(count++ == 1)
                   {  
                     firstCodeOnPage[0] = adjustmentCode;
                     
                     if(inx == '3')
                       firstEntryOnPage[0] = adjustmentCode;
                     else
                     if(inx == '2')
                 ...
                 lastCodeOnPage[0] = adjustmentCode;

                 if(inx == '3')
                   lastEntryOnPage[0] = adjustmentCode;
                 else
                 if(inx == '2')
      ...
      case 'L' : // last page
                 if(inx == '3')
                 {
                   ...
                   {
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE "
                       + "AdjustmentCode LIKE '" + srchStr + "%' AND AdjustmentCode > '" + codeArg + "' ORDER BY adjustmentCode DESC";
                   }
                   else
                   {  
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE AdjustmentCode "
                       + "<= 'ZZZZZZZZZZZZZZZZZZZZ' ORDER BY AdjustmentCode DESC";
                   }
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM stocka WHERE Date <= {d '9999-12-31'} ORDER BY Date DESC";
                   ...
                   {
                     q = "SELECT Date, AdjustmentCode, ItemCode, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} ORDER BY Date, AdjustmentCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ItemCode FROM stocka WHERE ItemCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY ItemCode DESC";
                   ...
                   {
                      q = "SELECT ItemCode, AdjustmentCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE ItemCode = '" 
                        + codeArg + "' ORDER BY ItemCode, AdjustmentCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '3')
                     prependBodyLine(html, inx, adjustmentCode, itemCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date, cssFormat);
                     else prependBodyLine(html, inx, date, adjustmentCode, itemCode, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   }  
                   else // inx == '1'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, itemCode, cssFormat);
                     else 
                     {
                       prependBodyLine(html, inx, itemCode, adjustmentCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                     }
                   }  
      ...
      case 'N' : // next page
                 if(inx == '3')
                 {
                   ...
                   {
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE "
                       + "AdjustmentCode LIKE '" + srchStr + "%' AND AdjustmentCode > '" + codeArg + "' ORDER BY AdjustmentCode";
                   ...
                   {                   
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE AdjustmentCode > '"
                       + codeArg + "' ORDER BY AdjustmentCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM stocka WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                        + " ORDER BY Date";
                   ...
                   {
                     q = "SELECT Date, AdjustmentCode, ItemCode, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND AdjustmentCode > '" + codeArg2 + "' ORDER BY Date, AdjustmentCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ItemCode FROM stocka WHERE ItemCode >= '" + codeArg + "' ORDER BY ItemCode";
                   ...
                   {
                     q = "SELECT ItemCode, AdjustmentCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE ItemCode = '"
                       + codeArg + "' AND AdjustmentCode > '" + codeArg2 + "' ORDER BY ItemCode, AdjustmentCode";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '3')
                     appendBodyLine(html, inx, adjustmentCode, itemCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date, cssFormat);
                     else appendBodyLine(html, inx, date, adjustmentCode, itemCode, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   }   
                   else // inx == '1'
                   { 
                     if(callType == 'G')
      ...
      case 'P' : // prev page
                 if(inx == '3')
                 {
                   ...
                   {
                     q = "SELECT AdjustmentCode, ItemCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE AdjustmentCode < '"
                       + codeArg + "' ORDER BY AdjustmentCode DESC";
                   }
                 ...
                 if(inx == '2')
                 {
                   ...
                   {
                     q = "SELECT Date, AdjustmentCode, ItemCode, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND AdjustmentCode < '" + codeArg2 + "' ORDER BY Date, AdjustmentCode DESC";
                 ...
                 else // (inx == '1'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT ItemCode FROM stocka WHERE ItemCode < '" + codeArg + "' ORDER BY ItemCode DESC";
                   ...
                   {
                     q = "SELECT ItemCode, AdjustmentCode, Date, StoreFrom, StoreTo, Quantity, SOCode, POCode, Remark, Status FROM stocka WHERE "
                       + "ItemCode = '" + codeArg + "' AND AdjustmentCode < '" + codeArg2 + "' ORDER BY ItemCode, AdjustmentCode DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   else // inx == '1'
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, itemCode, cssFormat);
                     else prependBodyLine(html, inx, itemCode, adjustmentCode, date, storeFrom, storeTo, qty, soCode, poCode, remark, itemCode, status, imagesDir, cssFormat);
                   }  
                      ...
                      int maxRows, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Stock Adjustment Search Results for ";
        switch(inx)
        {
          case '3' : title += "Stock Adjustment Code";  break;
          case '2' : title += "Stock Adjustment Date";  break;
      ...
      if(operation == 'X')
      {
        title = "Stock Adjustment Listing by ";
        switch(inx)
        {
          case '3' : title += "Stock Adjustment Code";  break;
          case '2' : title += "Stock Adjustment Date";  break;
      ...
      else title = "Stock Adjustment Listing";

    drawingUtils.drawTitle(out, false, false, "StockAdjustmentListing", "", title, "3013", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
    ...
    if(numRecs > 0 || first)
    {
      switch(inx)
      {
        case '3' : scout(out, bytesOut, "Stock Adjustment Code");  break;
        case '2' : scout(out, bytesOut, "Stock Adjustment Date");  break;
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       String imagesDir, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Stock Adjustment Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/StockAdjustmentItem?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=A&p1=\"+code2;}");
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/StockAdjustmentListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           + "&p1=" + inx + "&p2=\"+operation+\"&p3=\"+arg+\"&p4=\"+arg2+\"&p5=\"+srchStr+\"&p6=" + newFirstRecNum
                           + "&p10=" + numRecs + "&p8=\"+callType+\"&p9=\"+maxRows+\"&p7=" + newLastRecNum
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/StockAdjustmentListing?unm=" + unm + "&sid=" + sid
                         + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + inx
                         + "&p2=\"+operation+\"&p3=\"+arg+\"&p4=\"+arg2+\"&p5=\"+srchStr+\"&p6=" + newFirstRecNum + "&p10="
                         + numRecs + "&p8=\"+callType+\"&p9=\"+maxRows+\"&p7=" + newLastRecNum + "\");}");
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "3013", "", "StockAdjustmentListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '1' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Item Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Store From &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Store To &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quantity &nbsp;</td></tr>");
                 break;
      case '2' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Code &nbsp;</td>");
                 ...
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quantity &nbsp;</td></tr>");
                 break;
      case '3' : scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Item Code &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Adjustment Date &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Store From &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Store To &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>Quantity &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>SOCode &nbsp;</td>");
                 scoutln(out, bytesOut, "<td id=\"pageColumn\" nowrap>POCode &nbsp;</td>");
  ...
  private void startBody2(boolean first, PrintWriter out, char inx, String imagesDir, int[] bytesOut) throws Exception
  {
    ...
    switch(inx)
    {
      case '2' : scoutln(out, bytesOut, "Adjustment Date");  break;
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six, String seven, String eight,
                              String nine, String code, String status, String imagesDir, String[] cssFormat) throws Exception
  {
    ...
    { 
      ...
      case '3' : html[0] += "<td nowrap><p>" + one + "&nbsp;</td>";
                 ...
                 html[0] += "<td nowrap><p>" + four + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + five + "&nbsp;</td>";
                 html[0] += "<td nowrap align=right><p>" + six + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + seven + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + eight + "&nbsp;</td>";
                 html[0] += "<td nowrap><p>" + nine + "&nbsp;</td></tr>\n";
                 break;
    }
  }
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------------------------------
  private void prependBodyLine(String[] html, char inx, String one, String two, String three, String four, String five, String six, String seven, String eight,
                               String nine, String code, String status, String imagesDir, String[] cssFormat) throws Exception
  {
    if(cssFormat[0].equals("line1")) cssFormat[0] = "line2"; else cssFormat[0] = "line1";
    ...
    switch(inx)
    {
      ...
      case '3' : line += "<td nowrap><p>" + one + "&nbsp;</td>";
                 ...
                 line += "<td nowrap><p>" + eight + "&nbsp;</td>";
                 line += "<td nowrap><p>" + nine + "&nbsp;</td>";
                 html[0] = line + "</tr>\n" + html[0];
                 break;
    }
  }
