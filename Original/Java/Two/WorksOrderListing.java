...
public class WorksOrderListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "WorksOrderListing", bytesOut);
                    ...
                    String lastRecNum, int maxRows, int numRecs, String companyCode, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "WorksOrderListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "WorksOrderListing", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    generate(con, stmt, rs, out, req, unm, sid, uty, men, den, dnm, bnm, inx.charAt(0), operation.charAt(0), callType.charAt(0), codeArg, codeArg2, srchStr,
             firstRecNum, lastRecNum, maxRows, numRecs, companyCode, imagesDir, localDefnsDir, defnsDir, bytesOut);
  ...
  private void generate(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm,
                        ...
                        String lastRecNum, int maxRows, int numRecsIn, String companyCode, String imagesDir, String localDefnsDir, String defnsDir,
                        int[] bytesOut) throws Exception
  {
    ...
    createPage(html, operation2, callType, codeArg, codeArg2, inx, srchStr, numRecs, firstCodeOnPage, lastCodeOnPage, firstEntryOnPage,
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM wo WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(srchStr) + "'}"
                       + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                   ...
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM wo WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}"
                       + companyCodeANDWHERE);
                   }
                   break;
        ...
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(SalesPerson) AS rowcount FROM wo WHERE SalesPerson LIKE '" + srchStr + "%'"
                       + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                   ...
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM wo WHERE SalesPerson = '" + codeArg + "'" + companyCodeANDWHERE);
                   }
                   break;
        case '5' : if(callType == 'S')
                   ...
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM wo WHERE CustomerPOCode = '" + codeArg + "'" + companyCodeANDWHERE);
                   }
                   break;
        case '6' : if(callType == 'S')
                   ...
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM wo WHERE SOCode = '" + codeArg + "'" + companyCodeANDWHERE);
                   }
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT WOCode) AS rowcount FROM wo WHERE WOCode LIKE '" + srchStr + "%'"
                       + companyCodeANDWHERE);
                   }
                   else
                   ...
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT WOCode) AS rowcount FROM wo WHERE WOCode LIKE '" + srchStr + "%'"
                       + companyCodeANDWHERE); // always calc the total number even though we are on subsequent page
                   }
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE WOCode LIKE '" + srchStr + "%'"
                       + companyCodeANDWHERE + " ORDER BY WOCode";
                   }
                   else
                   {
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo" + companyCodeWHERE + " ORDER BY WOCode";
                   }

                   woCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; customerPOCodePosn = 4; statusPosn = 6; 
                   salesPersonPosn = -1; soCodePosn = -1;
                 }
                 else
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, WOCode, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE Date = {d '" 
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE + " ORDER BY Date, WOCode";
                     
                     woCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 5; customerPOCodePosn = 4; statusPosn = 6;
                     salesPersonPosn = -1; soCodePosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     ...
                     woCodePosn = -1; datePosn = 1; companyCodePosn = -1; companyNamePosn = -1; customerPOCodePosn = -1; statusPosn = -1;
                     salesPersonPosn = -1; soCodePosn = -1;
                   }
                   else
                   {
                     q = "SELECT Date, WOCode, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE + " ORDER BY Date, WOCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, WOCode, CompanyName, CustomerPOCode, Status FROM wo WHERE CompanyCode LIKE '" 
                       + srchStr + "%' ORDER BY CompanyCode, WOCode";
                     
                     woCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; customerPOCodePosn = 4; statusPosn = 5;
                     salesPersonPosn = -1; soCodePosn = -1;
                   }
                   else
                   if(callType == 'G')
                   ...
                   {
                     q = "SELECT CompanyCode, WOCode, CompanyName, CustomerPOCode, Status FROM wo WHERE CompanyCode = '" + codeArg
                       + "' ORDER BY CompanyCode";
                     woCodePosn = 2; datePosn = -1; companyCodePosn = 1; companyNamePosn = 3; customerPOCodePosn = 4; 
                     statusPosn = 5; salesPersonPosn = -1; soCodePosn = -1;
                   }
                 }  
                 else
                 if(inx == '4')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT SalesPerson, WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE SalesPerson LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY SalesPerson, Date";
                     woCodePosn = 2; datePosn = 3; companyCodePosn = 4; companyNamePosn = 6; customerPOCodePosn = 5; 
                     statusPosn = 7; salesPersonPosn = 1; soCodePosn = -1;
                   }
                   else
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM wo" + companyCodeWHERE + " ORDER BY SalesPerson";
                     woCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; customerPOCodePosn = -1; 
                     statusPosn = -1; salesPersonPosn = 1; soCodePosn = -1;
                   }
                   else
                   {
                     q = "SELECT SalesPerson, WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE SalesPerson = '" 
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CustomerPOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE CustomerPOCode LIKE '" + srchStr 
                       + "%'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, WOCode";
                     
                     woCodePosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = 1; 
                     statusPosn = 5; salesPersonPosn = -1; soCodePosn = -1;
                   }
                   else
                   if(callType == 'G')
                   ...
                   {
                     q = "SELECT CustomerPOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE CustomerPOCode = '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode";
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT SOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE SOCode LIKE '" + srchStr 
                       + "%'" + companyCodeANDWHERE + " ORDER BY SOCode, WOCode";
                     
                     woCodePosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; soCodePosn = 1; statusPosn = 5; salesPersonPosn = -1;
                     customerPOCodePosn = -1; 
                   }
                   else
                   if(callType == 'G')
                   {
                     ...
                     woCodePosn = -1; datePosn = -1; companyCodePosn = -1; companyNamePosn = -1; soCodePosn = 1; statusPosn = -1; salesPersonPosn = -1;
                     customerPOCodePosn = -1; 
                   }  
                   else
                   {
                     q = "SELECT SOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE SOCode = '" + codeArg + "'" + companyCodeANDWHERE
                       + " ORDER BY SOCode";
                 ...
                 while(rs.next())                  
                 {
                   woCode         = getValue(woCodePosn,         ' ', rs, rsmd);
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE WOCode LIKE '" + srchStr 
                       + "%' AND WOCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY WOCode DESC";
                   }
                   else
                   {  
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE WOCode <= 'ZZZZZZZZZZZZZZZZZZZZ'"
                       + companyCodeANDWHERE + " ORDER BY WOCode DESC";
                   }
                 ...
                 {              
                   ...
                   {
                     q = "SELECT Date, WOCode, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo" 
                       + " WHERE Date = {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" + companyCodeANDWHERE
                       + " ORDER BY Date, WOCode DESC";
                     
                     woCodePosn = 2; datePosn = 1; companyCodePosn = 3; companyNamePosn = 5; customerPOCodePosn = 4; 
                     statusPosn = 6; salesPersonPosn = -1; soCodePosn = -1;
                   }  
                 }
                 else
                 if(inx == '3')
                 {  
                   ...
                   {
                      q = "SELECT CompanyCode, WOCode, CompanyName, CustomerPOCode, Status FROM wo WHERE CompanyCode = '" + codeArg 
                        + "' ORDER BY CompanyCode, WOCode DESC";
                 ...
                 if(inx == '4')
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SalesPerson FROM wo WHERE SalesPerson <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                   ...
                   {
                     q = "SELECT SalesPerson, WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE SalesPerson = '" 
                       + codeArg + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, WOCode DESC";
                 ...
                 if(inx == '5')
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CustomerPOCode FROM wo WHERE CustomerPOCode <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'"
                   ...
                   {
                     q = "SELECT CustomerPOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE CustomerPOCode = '" + codeArg
                       + companyCodeANDWHERE + "' ORDER BY CustomerPOCode, WOCode DESC";
                 ...
                 else // inx == '6'
                 {
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT SOCode FROM wo WHERE SOCode <= 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'" + companyCodeANDWHERE
                       + " ORDER BY SOCode DESC";
                   ...
                   {
                     q = "SELECT SOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE SOCode = '" + codeArg + companyCodeANDWHERE
                       + "' ORDER BY CustomerPOCode, WOCode DESC";
      ...
      case 'N' : // next page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE WOCode LIKE '" + srchStr 
                       + "%' AND WOCode > '" + codeArg + "'" + companyCodeANDWHERE + " ORDER BY WOCode";
                     
                     woCodePosn = 1; datePosn = 2; companyCodePosn = 3; companyNamePosn = 5; customerPOCodePosn = 4; 
                     statusPosn = 6; salesPersonPosn = -1; soCodePosn = -1;
                   }
                   else
                   {                   
                     q = "SELECT WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE WOCode > '" + codeArg 
                       + "'" + companyCodeANDWHERE + " ORDER BY WOCode";
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM wo WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                   ...
                   {
                     q = "SELECT Date, WOCode, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE Date = {d '" 
                        + generalUtils.convertDateToSQLFormat(codeArg) + "'} AND WOCode > '" + codeArg2 + "'" + companyCodeANDWHERE
                 ...
                 {  
                   ...
                   {
                     q = "SELECT CompanyCode, WOCode, CompanyName, CustomerPOCode, Status FROM wo WHERE CompanyCode = '" + codeArg
                       + "' AND WOCode > '" + codeArg2 + "' ORDER BY CompanyCode, WOCode";
                 ...
                 if(inx == '4')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT SalesPerson FROM wo WHERE SalesPerson >= '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT SalesPerson, WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE SalesPerson = '"
                       + codeArg + "' AND WOCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, WOCode";
                 ...
                 {  
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CustomerPOCode FROM wo WHERE CustomerPOCode > '" + codeArg + "'" + companyCodeANDWHERE
                   ...
                   {
                     q = "SELECT CustomerPOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE CustomerPOCode = '" + codeArg
                       + "' AND WOCode > '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, WOCode";
      ...
      case 'P' : // prev page
                 ...
                 if(inx == '3')
                 {
                   ...
                   {
                     q = "SELECT CompanyCode, WOCode, CompanyName, CustomerPOCode, Status FROM wo WHERE CompanyCode = '" + codeArg
                       + "' AND WOCode < '" + codeArg2 + "' ORDER BY CompanyCode, WOCode DESC";
                 ...
                 if(inx == '4')
                 {
                   ...
                   {
                     q = "SELECT SalesPerson, WOCode, Date, CompanyCode, CustomerPOCode, CompanyName, Status FROM wo WHERE SalesPerson = '" 
                       + codeArg + "' AND WOCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SalesPerson, WOCode DESC";
                 ...
                 {  
                   ...
                   {
                     q = "SELECT CustomerPOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE CustomerPOCode = '" + codeArg
                       + "' AND WOCode < '" + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY CustomerPOCode, WOCode DESC";
                 ...
                 {  
                   ...
                   {
                     q = "SELECT SOCode, WOCode, CompanyCode, CompanyName, Status FROM wo WHERE SOCode = '" + codeArg + "' AND WOCode < '"
                       + codeArg2 + "'" + companyCodeANDWHERE + " ORDER BY SOCode, WOCode DESC";
                     
                     woCodePosn = 2; datePosn = -1; companyCodePosn = 3; companyNamePosn = 4; customerPOCodePosn = -1; statusPosn = 5;
                     salesPersonPosn = -1; soCodePosn = 1;
                   }
                 }
                      ...
                      String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      drawingUtils.drawTitle(out, false, false, "WorksOrderListing", "", title, "4442", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty, String men, String den,
                       String dnm, String bnm, String newFirstRecNum, String newLastRecNum, int numRecs, String companyCode, String localDefnsDir,
                       String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Works Order Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/WorksOrderPage?unm=" + unm + "&sid=" + sid + "&uty=" + uty
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/WorksOrderListing?unm=" + unm + "&sid=" + sid
    ...
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/WorksOrderListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men="
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "4442", "", "WorksOrderListing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);
