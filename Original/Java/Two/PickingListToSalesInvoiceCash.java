// =======================================================================================================================================================================================================
// System: ZaraStar Document: PL to CASH invoice
// Module: PickingListToSalesInvoiceCash.java
// Author: C.K.Harvey
...
public class PickingListToSalesInvoiceCash
{
  ...
  public boolean create(Connection con, Statement stmt, ResultSet rs, byte[] code, char renumberLines, char renumberEntries, int[] lines,
                        ...
                        throws Exception
  {
    documentUtils.getNextCode(con, stmt, rs, "cashinvoice", true, newDocCode);
    ...
    plToInvoice(con, stmt, rs, 'H', newDocCode, data, dataTo, code, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    for(int xx=0;xx<linesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, linesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          ...
          plToInvoice(con, stmt, rs, 'L', newDocCode, data, dataTo, null, unm, dnm, localDefnsDir, defnsDir, b);
    ...
    for(int xx=0;xx<multipleLinesCount[0];++xx)
    {
      if(generalUtils.getListEntryByNum(xx, multipleLinesData, data)) // just-in-case
      {
        ...
        if(found)
        {
          plToInvoice(con, stmt, rs, 'M', newDocCode, data, dataTo, null, unm, dnm, localDefnsDir, defnsDir, b);
          salesInvoice.putMultipleLine(con, stmt, rs, newDocCode, newLines[z], dataTo);
        }
      }
    }

    documentAttachmentsCopy.copyAttachments(con, stmt, rs, "pla", code, "invoicea", newDocCode, dnm, defnsDir, localDefnsDir);                 

    updatePL(con, stmt, rs, generalUtils.stringFromBytes(code, 0L), generalUtils.stringFromBytes(newDocCode, 0L), dnm, localDefnsDir, defnsDir);

    return true;
  }

  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void plToInvoice(Connection con, Statement stmt, ResultSet rs, char which, byte[] newDocCode, byte[] fromBuf, byte[] toBuf, byte[] code,
                           String unm, String dnm, String localDefnsDir, String defnsDir, byte[] b) throws Exception
  {
    int x;
    ...
    switch(which)
    {
      case 'H' : // head
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)57, code);
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)27, "C"); // cashOrAccount
      ...
      case 'L' : // line
                 ...
                 generalUtils.dfs(fromBuf, (short)24, b); // quantityPacked to quantity
                 ...
                 generalUtils.repAlpha(toBuf, 3000, (short)21, accountsUtils.getSalesAccCode(accountsUtils.getAccountingYearForADate(con, stmt, rs, 
  ...
  // -------------------------------------------------------------------------------------------------------------------------------  
  private void updatePL(Connection con, Statement stmt, ResultSet rs, String plCode, String newInvoiceCode, String dnm, String localDefnsDir, String defnsDir) throws Exception
  {
    stmt = con.createStatement();
    stmt.executeUpdate("UPDATE pl SET InvoiceCode = '" + newInvoiceCode + "' WHERE PLCode = '" + plCode + "'");
    
    if(stmt != null) stmt.close();
    
    stmt = con.createStatement();
    stmt.executeUpdate("UPDATE pll SET InvoiceCode = '" + newInvoiceCode + "' WHERE PLCode = '" + plCode + "'"); // all lines
