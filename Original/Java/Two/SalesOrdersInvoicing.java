// =======================================================================================================================================================================================================
// System: ZaraStar: Sales Orders Processing - Invoicing
// Module: SalesOrdersInvoicing.java
// Author: C.K.Harvey
...
public class SalesOrdersInvoicing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SalesOrdersInvoicing", bytesOut);
                    ...
                    String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SalesOrdersInvoicing", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SalesOrdersInvoicing", imagesDir, localDefnsDir, defnsDir, bytesOut);
                   ...
                   String bnm, String imagesDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    drawingUtils.outputPageFrame(con, stmt, rs, out, req, "2037", "", "SalesOrdersInvoicing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    drawingUtils.drawTitle(out, false, false, "SalesOrdersInvoicing", "", "Sales Order Processing - Invoicing", "2037", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    scoutln(out, bytesOut, "<form name=\"go\" action=\"SalesOrdersInvoicinga\" enctype=\"application/x-www-form-urlencoded\" method=POST>");
    ...
    scoutln(out, bytesOut, "<td><p><b> Invoiced </b></td>");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void forAllSalesOrderHeaders(Connection con, Statement stmt, ResultSet rs, PrintWriter out, char dpOnQuantities, String[] cssFormat,
                                       int[] oCount, String dnm, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    rs = stmt.executeQuery("SELECT SOCode, Date, CompanyCode, CustomerPOCode, TotalTotal, Currency2, CompanyName, ToEngineering, "
                         + "EngineeringApproved, ToManager, ManagerApproved "
                         + "FROM so WHERE Status != 'C' AND AllSupplied != 'Y' ORDER BY Date, SOCode");
      
    String soCode, date, customerCode, custPOCode, currency, companyName, toEngineering, engineeringApproved, toManager,
           managerApproved;
    double totalTotal;
    byte[] b = new byte[20];
