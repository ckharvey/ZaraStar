// =======================================================================================================================================================================================================
// System: ZaraStar CompanyEngine: Update supplier file
// Module: SupplierUpdate.java
// Author: C.K.Harvey
...
public class SupplierUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    boolean noStatementsFound=false, internalExternalOrOtherFound=false;
    ...
    try
    {
      ...
      while(en.hasMoreElements())
      {
        ...
        else 
        {
          ...
          {
            saveStr += ("supplier.NoStatements=Y\001");
            noStatementsFound = true;
          }
          else
          if(name.equalsIgnoreCase("InternalOrExternalOrOther"))
          {
            saveStr += ("supplier.InternalExternalOrOther=" + value[0] + "\001");
            internalExternalOrOtherFound = true;  
          }
          else saveStr += ("supplier." + name + "=" + value[0] + "\001");
        }

      }      
      
      if(! noStatementsFound)
        saveStr += ("supplier.NoStatements=N\001");

      if(! internalExternalOrOtherFound)
        saveStr += ("supplier.InternalOrExternalOrOther=I\001");
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SupplierUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5071, bytesOut[0], 0, "ERR:" + code);
                      ...
                      throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "SupplierUpdate", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5071, bytesOut[0], 0, "ACC:" + codeStr);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "SupplierUpdate", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5071, bytesOut[0], 0, "ERR:" + codeStr);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    else
    {
      ...
      if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, supplier.getFieldNames(), supplier.getFieldTypes(), supplier.getFieldStyles(), "supplier",
                                         "Update Supplier Record", dnm, unm, recData, y, imagesDir, localDefnsDir, defnsDir))
      { // validated ok
        switch(supplier.supplierPutRec(con, stmt, rs, code, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode))
        {
          ...
          case 'X' : // already exists
                      ...
                      break;
          case 'I' : // illegal code
                      errorValidation.formatErrMsg("Illegal Supplier Code", retStr, imagesDir);
                      reGetRec(out, codeStr, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, retStr[0], recData,
                               cad);
                      break;
          case 'N' : // no code specified
                      errorValidation.formatErrMsg("No Supplier Code Specified", retStr, imagesDir);
                      reGetRec(out, codeStr, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, retStr[0], recData,
                               cad);
                      break;
          case 'F' : // not updated
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5071, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeStr);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
  ...
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/SupplierPage?unm=" + unm  + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=" + code
                      + "&p2=A&bnm=" + bnm);
                          ...
                          String dnm, String bnm, String localDefnsDir, String errStr, byte[] dataAlready, char cad) throws Exception
  {
    ...
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/SupplierUpdate?unm=" + unm  + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&p1=" + code2 + "&p2="
