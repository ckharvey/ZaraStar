...
// Module: StockRecordsUploadRemoveExecute.java
// Author: C.K.Harvey
...
public class StockRecordsUploadRemoveExecute extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "StockRecordsUploadRemoveExecute", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3017, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String bnm, String p1, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "StockRecordsUploadRemoveExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3017, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "StockRecordsUploadRemoveExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3017, bytesOut[0], 0, "SID:" + p1);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 3017, (int)file.length(), 0, (new java.util.Date().getTime() - startTime), p1);
  ...
  {   
    ...
    while(getNextLine(fh, tmp, 5000))
    {
      try
      {
        ...
        itemCode = sanitise(fldEntry[0]);
        
        removeStockRecord(con, stmt, rs, itemCode, dnm, localDefnsDir, defnsDir);
      }  
      catch(Exception e)
      {
        System.out.println(e);
    ...
    messagePage.msgScreen(false, out, req, 4, unm, sid, uty, men, den, dnm, bnm, "StockRecordsUploadRemoveExecute", imagesDir, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(req, unm, dnm, 3017, bytesOut[0], 0, "");
  }
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------------------
  private void removeStockRecord(Connection con, Statement stmt, ResultSet rs, String itemCode, String dnm, String localDefnsDir, String defnsDir)
                                 throws Exception
  {
    ...
    // now remove rec from stock file (also removes any stockx recs)
