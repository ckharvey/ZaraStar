// =======================================================================================================================================================================================================
// System: ZaraStar Document: Purchase Invoice monthly listing
// Module: PurchaseInvoiceMonthlyListing.java
// Author: C.K.Harvey
...
public class PurchaseInvoiceMonthlyListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseInvoiceMonthlyListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4314, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4314, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceMonthlyListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4314, bytesOut[0], 0, "ACC:" + p1);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceMonthlyListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4314, bytesOut[0], 0, "SID:" + p1);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4314, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
                        ...
                        int[] bytesOut) throws Exception
  {
    ...
    String invoiceCode, name, supplierCode, date, totalTotal, currency, baseTotalTotal, status, gstTotal, suppInvoiceCode;
    int x, count = 0;
    ...
    rs = stmt.executeQuery("SELECT InvoiceCode, Date, CompanyCode, BaseTotalTotal, TotalTotal, Currency, CompanyName,"
                                   + " GSTTotal, Status, SupplierInvoiceCode FROM pinvoice WHERE Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo
    ...
    while(rs.next())                  
    {
      ...
      suppInvoiceCode = rs.getString(10);
      ...
      appendBodyLine(out, invoiceCode, date, supplierCode, baseTotalTotal, totalTotal, exclusiveTotal, currency, name, gstTotal, status, suppInvoiceCode, baseCurrency, imagesDir, cssFormat, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td colspan=2><p>Number of Purchase Invoices:</td><td nowrap align=right><p>" + count + "</td></tr>");
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String p1, boolean plain, String unm, String sid,
                       ...
                       int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Invoice Monthly Listing</title>");
    ...
    outputPageFrame(con, stmt, rs, out, req, plain, p1, "", "Purchase Invoice Monthly Listing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, String p1, String imagesDir, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      scoutln(out, bytesOut, "<tr><td colspan=6><p>Purchase Invoice Listing for the month of " + s + "</td></tr>");
    ...
    scoutln(out, bytesOut, "<td id=\"pageColumn\">Supplier Invoice Code &nbsp;</td>");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(PrintWriter out, String invoiceCode, String date, String supplierCode, String baseTotalTotal, String totalTotal, String exclusiveTotal,
                              String currency, String name, String gstTotal, String status, String suppInvoiceCode, String baseCurrency, String imagesDir, String[] cssFormat,
                              int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "<td nowrap><p>" + suppInvoiceCode + "&nbsp;</td>");
    ...
    if((! currency.equals(baseCurrency)) && (generalUtils.doubleFromStr(totalTotal) == generalUtils.doubleFromStr(baseTotalTotal)))
      scoutln(out, bytesOut, "<td nowrap align=right><p><font color=red size=3>" + generalUtils.doubleDPs(baseTotalTotal, '2') + "&nbsp;</font></td>");
    else scoutln(out, bytesOut, "<td nowrap align=right><p>" + generalUtils.doubleDPs(baseTotalTotal, '2') + "&nbsp;</td>");
                               ...
                               String dnm, String bnm, String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "4314", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    subMenuText += "<table id='title' width=100%><tr><td colspan=2 nowrap><p>" + title + directoryUtils.buildHelp(4314) + "</td></tr></table>";

    subMenuText += buildSubMenuText(plain, month, unm, sid, uty, men, den, dnm, bnm, hmenuCount);

    if(plain)
      scoutln(out, bytesOut, authenticationUtils.buildPlainScreen(con, stmt, rs, req, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
    else
    {
      scoutln(out, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, "PurchaseInvoiceMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
                                  ...
                                  int[] hmenuCount) throws Exception
  {
    ...
    if(! plain)
    {
      ...
      s += "<a href=\"/central/servlet/PurchaseInvoiceMonthlyListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=" + month + "&p2=P\">Friendly</a></dt></dl>";
    }
