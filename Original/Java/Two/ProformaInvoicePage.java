// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: ProInvoice fetch rec page
// Module: ProformaInvoicePage.java
// Author: C.K.Harvey
...
public class ProformaInvoicePage extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ProformaInvoicePage", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4080, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String dnm, String bnm, String p1, String p2, String p3, String p4, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4080", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4080, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4080", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4080, bytesOut[0], 0, "SID:" + p1);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    if(p1.startsWith("CREATEBASEDONINVOICE:"))
    {
      p1 = p1.substring(21);
      serviceCode = "4083";
      returnServlet = "SalesInvoicePage"; 
      sourceDocName = "Invoice";
      basedOn = true;
    }
    else
    ...
    {
      proformaInvoice.getRecToHTML(con, stmt, stmt2, rs, rs2, out, dispOrEdit, codeB, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, p2.charAt(0),
                         p3, dataAlready, req, bytesOut);
    }
    
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4080, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
                                ...
                                throws Exception
  {
    ...
    String s2 = "&unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p1=R&p2=proforma";
