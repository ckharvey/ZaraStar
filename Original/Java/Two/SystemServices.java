...
public class SystemServices extends HttpServlet
{
  ...
  private void set(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String unm, String sid, String uty, String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] bytesOut)
                   throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Admin - System Services Access</title>");
    ...
    adminUtils.outputPageFrame(con, stmt, rs, out, req, "SystemServices", "", "117", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, hmenuCount, bytesOut);

    adminUtils.drawTitle(con, stmt, rs, req, out, "Administration - User Services", "117", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    if(AdminSteelclawsSites || ExternalUserServices3 || ExternalUserServices1 || ExternalUserServices2 || ExternalUserServices4 || ZaracloudDataBase || _19500)
    {
      ...
      if(ZaracloudDataBase)
      {
        ...
        scoutln(out, bytesOut, "<a href=\"/central/servlet/ZaracloudDataBase?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm="
                             + dnm + "&bnm=" + bnm + "\">Create</a> UNSPSC Codes</td><td nowrap width=90% nowrap>" + directoryUtils.buildHelp(9015) + "</td></tr>");
