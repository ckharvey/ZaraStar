// =======================================================================================================================================================================================================
// System: ZaraStar: DocumentEngine: Purchase Invoices fetch line rec page
// Module: PurchaseInvoiceLine.java
// Author: C.K.Harvey
...
public class PurchaseInvoiceLine extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseInvoiceLine", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5084, bytesOut[0], 0, "ERR:" + p1);
                      ...
                      throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "5084", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5084, bytesOut[0], 0, "ACC:" + p1);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "5084", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 5084, bytesOut[0], 0, "SID:" + p1);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    purchaseInvoice.buildHTMLLayoutForLine(con, stmt, rs, out, codeB, lineB, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, p3,
                                 dataAlreadyB, req, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5084, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
