// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: RV update line rec
// Module: ReceiptVoucherLineUpdate.java
// Author: C.K.Harvey
...
public class ReceiptVoucherLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ReceiptVoucherLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6061, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "6061", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6061, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "6061", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6061, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' :  if(receiptVoucher.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else 
                  {
                    messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "6061", imagesDir, localDefnsDir, defnsDir,
                                       bytesOut);
                  }
                  break;
      case 'X' :  // cancel
      ...
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, receiptVoucher.getFieldNamesReceiptVoucherL(), receiptVoucher.getFieldTypesReceiptVoucherL(),
                                                    receiptVoucher.getFieldStylesReceiptVoucherL(), "rvoucherl", "Update Receipt Voucher Record", dnm, unm,
                                                    recData, y, imagesDir, localDefnsDir, defnsDir))
                  {
                    switch(receiptVoucher.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Receipt Voucher Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Receipt Voucher Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 6062; operation = " Line: " + line;  break;
      case 'C' : service = 6061; operation = " Line: " + line;  break;
      case 'X' : service = 6061; operation = " Cancel";         break;
      default  : service = 6061; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                        ...
                        throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/ReceiptVoucherLine");
