// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: PL update line rec
// Module: PickingListLineUpdate.java
// Author: C.K.Harvey
...
public class PickingListLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PickingListLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3046, bytesOut[0], 0, "ERR:" + p2);
                      ...
                      int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "3046", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3046, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "3046", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 3046, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush(); 
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' :  if(pickingList.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else 
                  {
                    messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "3046", imagesDir,
      ...
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, pickingList.getFieldNamesPLL(), pickingList.getFieldTypesPLL(),
                                                     pickingList.getFieldStylesPLL(), "pll", "Update Picking List Record", dnm, unm, 
                                                     recData, y, imagesDir, localDefnsDir, defnsDir))
                  {
                    switch(pickingList.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Picking List Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Picking List Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 3047; operation = " Line: " + line;  break;
      case 'C' : service = 3046; operation = " Line: " + line;  break;
      case 'X' : service = 3046; operation = " Cancel";         break;
      default  : service = 3046; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                          ...
                          throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/PickingListLineFetch");
  ...
  private void newLine(PrintWriter out, String code, String unm, String sid, String uty, String men, String den,
                         String dnm, String bnm, byte[] dataAlready, String defnsDir) throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", defnsDir) + "/central/servlet/PickingListLineFetch");
