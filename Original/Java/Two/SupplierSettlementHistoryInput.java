// =======================================================================================================================================================================================================
// System: ZaraStar: Supplier: Settlement History: Request params
// Module: SupplierSettlementHistoryInput.java
// Author: C.K.Harvey
...
public class SupplierSettlementHistoryInput extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      p1  = req.getParameter("p1"); // supplierCode
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "SupplierSettlementHistoryInput", bytesOut);
                    ...
                    String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 5002, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 5002, bytesOut[0], 0, "ACC:" + p1);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      ...
      serverUtils.etotalBytes(req, unm, dnm, 5002, bytesOut[0], 0, "SID:" + p1);
    ...
    if(p1.equalsIgnoreCase("null") || p1.length() == 0)
      supplierCode = "";
    else supplierCode = p1;

    set(con, stmt, rs, out, req, supplierCode, p2, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, bytesOut);

    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 5002, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), "");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void set(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String supplierCode, String cashTransactions, String unm,
                   ...
                   int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "this.location.href=\"/central/servlet/SupplierSettlementHistoryCreate?unm=" + unm + "&uty=" + uty + "&sid=" + sid
    ...
    pageFrameUtils.outputPageFrame(con, stmt, rs, out, req, "5002", "", "SupplierSettlementHistoryInput", unm, sid, uty, men, den, dnm, bnm, localDefnsDir,
                          defnsDir, hmenuCount, bytesOut);
                                
    scoutln(out, bytesOut, "<form>");
  
    if(cashTransactions.charAt(0) == 'C')
      pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Purchase Invoice Settlement History for CASH Transactions", "5002", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    else pageFrameUtils.drawTitle(out, false, false, "", "", "", "", "", "", "Purchase Invoice Settlement History", "5002", unm, sid, uty, men, den, dnm, bnm, hmenuCount, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td width=1% nowrap><p>Cut-off Date for Purchase Credit Notes and Payments &nbsp;</td><td>"
    ...
    scoutln(out, bytesOut, "<tr><td nowrap><p>Select the Purchase Invoice date range you require: &nbsp;</td></tr>");
    ...
    if(cashTransactions.charAt(0) != 'C')
    {
      scoutln(out, bytesOut, "<td><p>Enter a Supplier Code</td>");
      scoutln(out, bytesOut, "<td colspan=2><p><input type=text maxlength=20 size=20 name=companycode value=\"" + supplierCode + "\"></td></tr>");
    }
