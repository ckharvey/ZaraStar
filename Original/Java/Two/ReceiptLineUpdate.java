// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: Receipt update line rec
// Module: ReceiptLineUpdate.java
// Author: C.K.Harvey
...
public class ReceiptLineUpdate extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ReceiptLineUpdate", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4210, bytesOut[0], 0, "ERR:" + p2);
                    ...
                    throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "4210", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4210, bytesOut[0], 0, "ACC:" + code);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "4210", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4210, bytesOut[0], 0, "SID:" + code);
    if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    switch(thisOp)
    {
      case 'D' :  if(receipt.deleteLine(con, stmt, rs, codeB, lineB, dnm, localDefnsDir, defnsDir, bytesOut))
                     getRec(out, unm, sid, uty, men, den, dnm, bnm, code, localDefnsDir);
                  else 
                  {
                    messagePage.msgScreen(false, out, req, 5, unm, sid, uty, men, den, dnm, bnm, "4210", imagesDir,
      ...
      default  : // amend or create
                  ...
                  if(errorValidation.validatePageReturningStr(con, stmt, rs, "", retStr, receipt.getFieldNamesReceiptL(), receipt.getFieldTypesReceiptL(),
                                                    receipt.getFieldStylesReceiptL(), "receiptl", "Update Receipt Record", dnm, unm, 
                                                    recData, y, imagesDir, localDefnsDir, defnsDir))
                  {
                    switch(receipt.putLine(con, stmt, rs, lineB, codeB, unm, dnm, localDefnsDir, defnsDir, cad, recData, y, newCode, bytesOut))
                    {
                      ...
                      case 'X' : // already exists
                                 ...
                                 break;
                      case 'I' : // illegal code
                                 errorValidation.formatErrMsg("Illegal Receipt Line", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'N' : // no code specified
                                 errorValidation.formatErrMsg("No Receipt Line Specified", retStr, imagesDir);
                                 reGetRec(out, unm, sid, uty, men, den, dnm, bnm, code, line, retStr[0], recData, cad, localDefnsDir);
                                 break;
                      case 'F' : // not updated
    ...
    switch(thisOp)
    {
      case 'D' : service = 4211; operation = " Line: " + line;  break;
      case 'C' : service = 4210; operation = " Line: " + line;  break;
      case 'X' : service = 4210; operation = " Cancel";         break;
      default  : service = 4210; 
                 if(cad == 'A')
                   operation = " Amend";
  ...
  private void reGetRec(PrintWriter out, String unm, String sid, String uty, String men, String den, String dnm,
                        ...
                        throws Exception
  {
    URL url = new URL("http://" + serverUtils.serverToCall("OFSA", localDefnsDir) + "/central/servlet/ReceiptLine");
