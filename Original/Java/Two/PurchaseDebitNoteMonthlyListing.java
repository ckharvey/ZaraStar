// =======================================================================================================================================================================================================
// System: ZaraStar Document: Purchase Debit Note monthly listing
// Module: PurchaseDebitNoteMonthlyListing.java
// Author: C.K.Harvey
...
public class PurchaseDebitNoteMonthlyListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "PurchaseDebitNoteMonthlyListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4317, bytesOut[0], 0, "ERR:" + p1);
                    ...
                    String dnm, String bnm, String p1, String p2, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 4317, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteMonthlyListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4317, bytesOut[0], 0, "ACC:" + p1);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteMonthlyListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 4317, bytesOut[0], 0, "SID:" + p1);
      if(con  != null) con.close();
      if(out != null) out.flush();
      return;
    }
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 4317, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), p1);
    if(con  != null) con.close();
    if(out != null) out.flush();
  }
                        ...
                        int[] bytesOut) throws Exception
  {
    ...
    String pdnCode, name, supplierCode, date, totalTotal, currency, baseTotalTotal, status, gstTotal;
    int x, count = 0;
    ...
    rs = stmt.executeQuery("SELECT PDNCode, Date, CompanyCode, BaseTotalTotal, TotalTotal, Currency, CompanyName,"
                                   + " GSTTotal, Status FROM pdebit WHERE Date >= {d '" + dateFrom + "'} AND Date <= {d '" + dateTo
                                   + "'} ORDER BY PDNCode");
    ...
    while(rs.next())                  
    {
      ...
      appendBodyLine(out, pdnCode, date, supplierCode, baseTotalTotal, totalTotal, currency, name, gstTotal, status, imagesDir,
                     cssFormat, bytesOut);
    ...
    scoutln(out, bytesOut, "<tr><td colspan=2><p>Number of Purchase Debit Notes:</td><td nowrap align=right><p>" + count + "</td></tr>");
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, String p1, boolean plain, String unm, String sid,
                       ...
                       int[] bytesOut) throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Purchase Debit Note Monthly Listing</title>");
    ...
    outputPageFrame(con, stmt, rs, out, req, plain, p1, "", "Purchase Debit Note Monthly Listing", unm, sid, uty, men, den, dnm, bnm, "", localDefnsDir, defnsDir, bytesOut);
  ...
  private void startBody(boolean first, PrintWriter out, String p1, String imagesDir, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      scoutln(out, bytesOut, "<tr><td colspan=8><p>Purchase Debit Note Listing for the month of " + s + "</td></tr>");
  ...
  // -------------------------------------------------------------------------------------------------------------------------------
  private void appendBodyLine(PrintWriter out, String pdnCode, String date, String supplierCode, String baseTotalTotal, 
                              ...
                              String[] cssFormat, int[] bytesOut) throws Exception
  {
    ...
    scoutln(out, bytesOut, "<td nowrap><p><a href=\"javascript:fetch('" + s + "')\">" + pdnCode + "</a>&nbsp;</td>");
                               ...
                               String otherSetup, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    String subMenuText = dashboardUtils.buildSubMenuText(con, stmt, rs, req, "4317", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount);

    subMenuText += "<table id='title' width=100%><tr><td colspan=2 nowrap><p>" + title + directoryUtils.buildHelp(4317) + "</td></tr></table>";

    subMenuText += buildSubMenuText(plain, month, unm, sid, uty, men, den, dnm, bnm, hmenuCount);

    if(plain)
      scoutln(out, bytesOut, authenticationUtils.buildPlainScreen(con, stmt, rs, req, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
    else
    {
      scoutln(out, bytesOut, authenticationUtils.buildScreen(con, stmt, rs, req, true, unm, sid, uty, men, den, dnm, bnm, "PurchaseDebitNoteMonthlyListing", subMenuText, hmenuCount[0], bodyStr, otherSetup, localDefnsDir, defnsDir));
                                  ...
                                  int[] hmenuCount) throws Exception
  {
    ...
    if(! plain)
    {
      ...
      s += "<a href=\"/central/servlet/PurchaseDebitNoteMonthlyListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
