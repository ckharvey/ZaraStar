// =======================================================================================================================================================================================================
// System: ZaraStar RAT: Display RAT
...
public class RATDisplayIssue extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "RATDisplayIssue", bytesOut);
  ...
  private void doIt(PrintWriter out, HttpServletRequest req, String ratCode, String unm, String sid, String uty, String men, String den, String dnm, String bnm, int[] bytesOut) throws Exception
  {
    ...
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "RATDisplayIssue", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    {
      messagePage.msgScreen(false, out, req, 12, unm, sid, uty, men, den, dnm, bnm, "RATDisplayIssue", imagesDir, localDefnsDir, defnsDir, bytesOut);
    ...
    issuesUtils.getRATGivenRATCode(ratCode, dnm, title, desc, date, userCode, userCompany, release, category, type, status, fixRelease, dateClosed, dateTargeted, duplicateOf, priority, hoursSpent, closedByUser, isPrivate, service);
    ...
    set(con, stmt, rs, req, out, canEdit, ratCode, date[0], title[0], desc[0], release[0], category[0], type[0], userCode[0], userCompany[0], status[0], fixRelease[0], dateClosed[0], dateTargeted[0], duplicateOf[0], priority[0], hoursSpent[0],
        closedByUser[0], isPrivate[0], unm, sid, uty, men, den, dnm, bnm, imageLibraryDir, flashDir, localDefnsDir, defnsDir, bytesOut);
  ...
  private void set(Connection con, Statement stmt, ResultSet rs, HttpServletRequest req, PrintWriter out, boolean canEdit, String ratCode, String date, String title, String desc, String release, String category, String type, String userCode,
                   String userCompany, String status, String fixRelease, String dateClosed, String dateTargeted, String duplicateOf, String priority, String hoursSpent, String closedByUser, String isPrivate, String unm, String sid, String uty,
                   String men, String den, String dnm, String bnm, String imageLibraryDir, String flashDir, String localDefnsDir, String defnsDir, int[] bytesOut) throws Exception
  {
    ...
    if(canEdit)
    {
      ...
      scoutln(out, bytesOut, "function edit(){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/RATCreateEditIssue?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p2=" + category + "&p1=" + ratCode + "\";}");

      scoutln(out, bytesOut, "function editl(dateTime){var p2=sanitise(dateTime);");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/RATRespond?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=E&p2=\"+p2+\"&p1=" + ratCode + "\";}");

      scoutln(out, bytesOut, "function del(){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/RATDeleteIssue?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=" + category + "&p2=&p1=" + ratCode + "\";}");

      scoutln(out, bytesOut, "function dell(dateTime){var p2=sanitise(dateTime);");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/RATDeleteIssue?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=&p2=\"+p2+\"&p1=" + ratCode + "\";}");
    ...
    {
      scoutln(out, bytesOut, "<script javascript=\"JavaScript\">");
      scoutln(out, bytesOut, "function resp(){");
      scoutln(out, bytesOut, "this.location.href=\"/central/servlet/RATRespond?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm + "&p3=R&p1=" + ratCode + "\";}");
      scoutln(out, bytesOut, "</script>");
    }
    ...
    issuesUtils.outputPageFrame(con, stmt, rs, out, req, "RATDisplayIssue", "5901", "", "", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);

    if(title.length() == 0)
      title = "No title!";
    
    issuesUtils.drawTitle(con, stmt, rs, req, out, "Issue: " + ratCode + " (" + title +")", "5901", "", true, false, unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    ...
    else // Discussion
    {
      ...
      scoutln(out, bytesOut, "<tr><td><p><b>Category: </td><td colspan=3><p>" + category + "</td></tr>");
