// =======================================================================================================================================================================================================
// System: ZaraStar DocumentEngine: RV listing page
// Module: ReceiptVoucherListing.java
// Author: C.K.Harvey
...
public class ReceiptVoucherListing extends HttpServlet
{
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
    ...
    catch(Exception e)
    {
      ...
      messagePage.errorPage(out, req, e, "Communications Error", unm, sid, dnm, bnm, urlBit, men, den, uty, "ReceiptVoucherListing", bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6064, bytesOut[0], 0, "ERR:" + codeArg);
                      ...
                      String companyCode, int[] bytesOut) throws Exception
  {
    ...
    if(! authenticationUtils.verifyAccess(con, stmt, rs, req, 6064, unm, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ReceiptVoucherListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6064, bytesOut[0], 0, "ACC:" + codeArg);
    ...
    if(! serverUtils.checkSID(unm, sid, uty, dnm, localDefnsDir, defnsDir))
    {
      messagePage.msgScreen(false, out, req, 13, unm, sid, uty, men, den, dnm, bnm, "ReceiptVoucherListing", imagesDir, localDefnsDir,
                         defnsDir, bytesOut);
      serverUtils.etotalBytes(req, unm, dnm, 6064, bytesOut[0], 0, "SID:" + codeArg);
    ...
    serverUtils.totalBytes(con, stmt, rs, req, unm, dnm, 6054, bytesOut[0], 0, (new java.util.Date().getTime() - startTime), codeArg);
                          ...
                          String localDefnsDir, String defnsDir) throws Exception
  {
    ...
    if(numRecs[0] == -1 || operation == 'F')
    {
      ...
      switch(inx)
      {
        case '2' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM rvoucher WHERE Date = {d '"
                       + generalUtils.convertDateToSQLFormat(srchStr) + "'}" + companyCodeANDWHERE);
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT Date) AS rowcount FROM rvoucher" + companyCodeWHERE);
                   else 
                   {
                     r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM rvoucher WHERE Date = {d '"
        ...
        case '3' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(CompanyCode) AS rowcount FROM rvoucher WHERE CompanyCode LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                      r = stmt.executeQuery("SELECT COUNT(DISTINCT CompanyCode) AS rowcount FROM rvoucher" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM rvoucher WHERE CompanyCode = '" + codeArg + "'");
                   break;
        case '4' : if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(ChequeNumber) AS rowcount FROM rvoucher WHERE ChequeNumber LIKE '"
                       + srchStr + "%'");
                   }
                   else
                   if(callType == 'G')
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT ChequeNumber) AS rowcount FROM rvoucher" + companyCodeWHERE);  
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM rvoucher WHERE ChequeNumber = '" + codeArg + "'");
                   break;
        default  : // case '1'
                   if(callType == 'S')
                   {
                     r = stmt.executeQuery("SELECT COUNT(DISTINCT VoucherCode) AS rowcount FROM rvoucher WHERE VoucherCode LIKE '"
                   ...
                   else r = stmt.executeQuery("SELECT COUNT(*) AS rowcount FROM rvoucher" + companyCodeWHERE);
                   break;
      }
      r.next();
    ...
    String voucherCode="", name="", customerCode="", date="", chequeNumber="", type="", amount="", currency="", status;
    ...
    switch(operation)
    {
      case 'F' : // first page
                 if(inx == '1')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM rvoucher WHERE VoucherCode LIKE '"
                       + srchStr + "%'" + companyCodeANDWHERE + " ORDER BY VoucherCode";
                   }
                   else
                   {
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM rvoucher " + companyCodeWHERE
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'S')
                   {
                     q = "SELECT Date, VoucherCode, Type, CompanyCode, CompanyName, Status FROM rvoucher WHERE Date = {d '" 
                   ...
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM rvoucher ORDER BY Date" + companyCodeWHERE;
                 ...
                 {  
                   if(callType == 'S')
                   {
                     q = "SELECT CompanyCode, VoucherCode, Date, Type, CompanyName, Status FROM rvoucher WHERE "
                   ...
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM rvoucher " + companyCodeWHERE + "ORDER BY CompanyCode";
               ...
               {  
                 if(callType == 'S')
                 {
                   q = "SELECT ChequeNumber, VoucherCode, TotalTotal, CompanyCode, CompanyName, Currency, Status FROM rvoucher WHERE "
                    + "CompanyCode LIKE '" + srchStr + "%' ORDER BY CompanyCode, VoucherCode";
                 ...
                 if(callType == 'G')
                 {
                   q = "SELECT DISTINCT ChequeNumber FROM rvoucher " + companyCodeWHERE + "ORDER BY ChequeNumber";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     appendBodyLine(html, inx, voucherCode, date, type, customerCode, name, null, voucherCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       appendBodyLine2(html, callType, operation, imagesDir, date);
                     else appendBodyLine(html, inx, date, voucherCode, type, customerCode, name, null, voucherCode, status, imagesDir);
                   }   
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       appendBodyLine(html, inx, customerCode, voucherCode, date, type, name, null, voucherCode, status, imagesDir);
                     }
                   }   
                   else // inx == '4'
                   {
                     ...
                     {
                       appendBodyLine(html, inx, chequeNumber, voucherCode, amount, companyCode, name, currency, voucherCode, status, imagesDir);
                     }
                   }   
      ...
      case 'L' : // last page
                 if(inx == '1')
                 {
                   ...
                   {  
                     q = "SELECT VoucherCode, Date, Type, CompanyCode, CompanyName, Status FROM rvoucher WHERE VoucherCode "
                 ...
                 {              
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT Date FROM rvoucher WHERE Date <= {d '9999-12-31'}" + companyCodeANDWHERE
                       + " ORDER BY Date DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM rvoucher WHERE CompanyCode <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY CompanyCode DESC";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM rvoucher WHERE ChequeNumber <= 'ZZZZZZZZZZZZZZZZZZZ' ORDER BY ChequeNumber DESC";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   if(inx == '1')
                     prependBodyLine(html, inx, voucherCode, date, type, customerCode, name, null, voucherCode, status, imagesDir);
                   else
                   if(inx == '2')
                   {
                     if(callType == 'G')
                       prependBodyLine2(html, callType, date);
                     else prependBodyLine(html, inx, date, voucherCode, type, customerCode, name, null, voucherCode, status, imagesDir);
                   }  
                   else
                   if(inx == '3')
                   {
                     ...
                     {
                       prependBodyLine(html, inx, customerCode, voucherCode, date, type, name, null, voucherCode, status, imagesDir);
                     }
                   }  
                   else // inx == '4'
                   {
                     ...
                     {
                       prependBodyLine(html, inx, chequeNumber, voucherCode, amount, companyCode, name, currency, voucherCode, status,
                                       imagesDir);
                     }
                   }  
      ...
      case 'N' : // next page
                 ...
                 if(inx == '2')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT Date FROM rvoucher WHERE Date > {d '" + generalUtils.convertDateToSQLFormat(codeArg) + "'}" 
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT CompanyCode FROM rvoucher WHERE CompanyCode >= '" + codeArg + "' ORDER BY CompanyCode";
                 ...
                 {  
                   if(callType == 'G')
                   {
                     q = "SELECT DISTINCT ChequeNumber FROM rvoucher WHERE ChequeNumber >= '" + codeArg + "' ORDER BY ChequeNumber";
                 ...
                 while(rs.next())                  
                 {
                   ...
                   else // inx == '4'
                   {
                     ...
                     {
                       appendBodyLine(html, inx, chequeNumber, voucherCode, amount, companyCode, name, currency, voucherCode, status,
                                      imagesDir);
                     }
                   }   
      ...
      case 'P' : // prev page
                 if(inx == '1')
                 {
                   ...
                   {
                     q = "SELECT VoucherCode, Date, CompanyCode, CompanyName, Status FROM rvoucher WHERE VoucherCode < '" + codeArg 
                 ...
                 if(inx == '3')
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT CompanyCode FROM rvoucher WHERE CompanyCode < '" + codeArg + "' ORDER BY CompanyCode DESC";
                 ...
                 else // inx == '4'
                 {
                   if(callType == 'G')
                   {  
                     q = "SELECT DISTINCT ChequeNumber FROM rvoucher WHERE ChequeNumber < '" + codeArg + "' ORDER BY ChequeNumber DESC";
                      ...
                      String men, String den, String dnm, String bnm, String localDefnsDir, String defnsDir, int[] hmenuCount, int[] bytesOut) throws Exception
  {
    if(first)
    {
      ...
      if(callType == 'S')
      {
        title = "Receipt Voucher Search Results for ";
        switch(inx)
        {
      ...
      if(operation == 'X')
      {
        title = "Receipt Voucher Listing by ";
        switch(inx)
        {
          case '1' : title += "Voucher Code";  break;
          case '2' : title += "Voucher Date";  break;
      ...
      else title = "Receipt Voucher Listing";

      drawingUtils.drawTitle(out, false, false, "ReceiptVoucherListing", "", title, "6064", unm, sid, uty, men, den, dnm, bnm, localDefnsDir, defnsDir, hmenuCount, bytesOut);
    }
  ...
  private void setHead(Connection con, Statement stmt, ResultSet rs, PrintWriter out, HttpServletRequest req, char inx, String unm, String sid, String uty,
                       ...
                       throws Exception
  {
    scoutln(out, bytesOut, "<html><head><title>Receipt Voucher Listing</title>");
    ...
    scoutln(out, bytesOut, "this.location.href=\"http://" + men + "/central/servlet/ReceiptVoucherPage?unm=" + unm + "&sid="
    ...
    scoutln(out, bytesOut, "if(callType=='G'||callType=='S')this.location.replace(\"/central/servlet/ReceiptVoucherListing?unm=" + unm + "&sid=" + sid + "&uty=" + uty + "&men=" + men + "&den=" + den + "&dnm=" + dnm + "&bnm=" + bnm
                           ...
                           + "\");");
    scoutln(out, bytesOut, "else this.location.replace(\"/central/servlet/ReceiptVoucherListing?unm=" + unm + "&sid=" + sid
